...
fn bench_dequantization(c: &mut Criterion) {
    ...
    group.bench_function("int8_symmetric_dequant", |b| {
        b.iter(|| {
            let reconstructed = dequantize_int8_symmetric(black_box(&quantized), scale);
...
fn bench_per_channel_vs_per_tensor(c: &mut Criterion) {
    ...
    for n in &[16, 32, 64] {
        group.bench_with_input(BenchmarkId::new("per_tensor", n), n, |b, &n| {
        ...
        group.bench_with_input(BenchmarkId::new("per_channel", n), n, |b, &n| {
            let matrix = black_box(generate_neural_net_weights(n * n));
...
fn bench_quantization_accuracy(c: &mut Criterion) {
    ...
    group.bench_function("int4_error_rmse", |b| {
        ...
        let reconstructed: Vec<f32> = quantized.iter().map(|&q| (q as f32) * scale).collect();
