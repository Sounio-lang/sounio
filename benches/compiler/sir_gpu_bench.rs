...
use sounio::sir::gpu_attention::{
    GpuAttentionConfig, GpuAttentionScore, GpuLiveInterval, GpuMemorySpace, GpuResourceBudget,
    divergence_cost, divergence_probability, occupancy_for_registers, select_spill_victim,
};
use sounio::sir::kernels::monte_carlo::{
    EpistemicReduction, MonteCarloConfig, MonteCarloKernel, ParticleState, partition_by_attention,
...
fn main() {
    ...
    let particles: Vec<ParticleState> = (0..100_000)
        .map(|i| {
    ...
    println!(
        "  - 1K particle propagation:  {}",
        format_duration(propagation_1k)
    );
    println!(
        "  - 10K particle propagation: {}",
        format_duration(propagation_10k)
    );
    println!(
        "  - Attention scoring (100):  {} ({} intervals/sec)",
    ...
    println!(
        "  - 1K batch:  {:.2} M particles/sec",
        particles_per_sec_1k / 1_000_000.0
    );
    println!(
        "  - 10K batch: {:.2} M particles/sec",
        particles_per_sec_10k / 1_000_000.0
    );
    println!();
