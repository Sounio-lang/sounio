...
fn bench_sparse_csr_matmul(c: &mut Criterion) {
    ...
    group.bench_function("csr_64x64_50pct", |b| {
        let row_ptrs = (0..n + 1).map(|i| (i * nnz) / (n + 1)).collect::<Vec<_>>();
        let col_indices = (0..nnz).map(|i| (i * n) / nnz).collect::<Vec<_>>();
        let values = black_box(generate_matrix(nnz));
        let b = black_box(generate_matrix(n));

        b.iter(|| {
            let result = sparse_csr_matmul(&row_ptrs, &col_indices, &values, &b, n, nnz);
            black_box(result);
        });
    });
