    ...
    with open(path) as f:
        for line in f:
            ...
            if p[0] == "sub":
                ...
                # exsub <ont> <child> <role> <filler> (round 9 extraction)
                exsub_h.append((int(p[2]), int(p[3]), int(p[4])))
    return sub_h, disj_h, exsub_h
...
def main(argv=None):
    ...
    sub_h, disj_h, exsub_h = load_tbox(args.tbox)
    ...
    # Round 10: existential restrictions C <= ex role.F with BOTH endpoints
    # kept (the role-aware closure layer of stdlib/ontology/elplus.sio).
    # Anatomy human profile: role 0 is the single active role (part_of);
    # role 1 (ObsoleteProperty) never occurs in a restriction (verified in
    # round 9, see scale/gen_elplus_data.py) — assert it stays that way.
    exsub_kept = sorted({(new_id[c], r, new_id[fl])
                         for c, r, fl in exsub_h if c in seen and fl in seen})
    for _, r, _ in exsub_kept:
        if r != 0:
            sys.exit("single-active-role profile violated: "
                     "exsub with role != 0 in the capped set")
    ...
    # atomic closure edge count (reflexive; no top column, matching the
    # elplus_sparse_bfs return value over the capped class set)
    edges = sum(len(c) for c in clos)

    # ── round 10: role layer mirror (EL+ sparse reduction) ─────────────
    # Same reduction as scale/gen_elplus_data.py, valid for the Anatomy
    # profile (no conjunctions, single active role, no roleSub/roleComp):
    # E(c) = union over ancestors a of c of the ancestor sets of the
    # stated fillers of a.  anc[c] is a bitmask over atoms plus TOPBIT
    # (the top column the sparse module materialises at index W-1).
    TOPBIT = 1 << H
    anc = [(1 << c) | TOPBIT for c in range(H)]
    changed = True
    while changed:
        changed = False
        for a, b in sub_kept:
            ...
            if new != anc[a]:
                anc[a] = new
                changed = True
    # cross-check: bitmask ancestor sets == set-based closure (atoms only)
    for c in range(H):
        if bin(anc[c] & ~TOPBIT).count("1") != len(clos[c]):
            sys.exit(f"MIRROR CROSS-CHECK FAILED: ancestor set of {c} "
                     f"disagrees with set-based closure")
    stated = {}
    for c, _r, fl in exsub_kept:
    ...
    for c in range(H):
        ...
        while m:
            ...
            m >>= 1
            k += 1
        nR_atom += bin(em).count("1")

    # Role-aware vs atomic conflicts: the derived-conflict oracle ranges
    # over the interned universe, but every disjointness endpoint here is
    # an ATOM (asserted below), and no EL+ rule adds atomic subsumption
    # targets beyond the atomic closure (stoR/RtoS/Rmono/roleSub/roleComp
    # only touch existential targets; there are no conjunctions), so the
    # role-aware conflict relation over the asserted (atomic) classes is
    # EXACTLY the atomic one — n_conf, kept/dropped and drop5 carry over
    # from rounds 6-7 byte-identically.  The full-TBox instance of this
    # argument is machine-checked in round 9 (scale/gen_elplus_data.py
    # runs the general 8-rule fixpoint and compares, 736 == 736).
    for a, b in disj_kept:
        if a >= H or b >= H:
            sys.exit("non-atomic disjointness endpoint: role-aware "
                     "conflict reduction invalid")
    ...
    with open(args.out, "w") as f:
        ...
        f.write("// Round 10: includes the EL+ role layer (existential\n")
        f.write("// restrictions C <= ex part_of.F) for the role-aware closure\n")
        f.write("// of stdlib/ontology/elplus.sio.\n")
        f.write(f"// H (human classes kept)      = {H}\n")
        f.write(f"// sub axioms kept             = {NSUB}\n")
        f.write(f"// existential restrictions kept = {NEX}\n")
        f.write(f"// disjoint pairs kept         = {len(disj_kept)}\n")
        f.write(f"// candidate mappings M        = {M}\n")
        f.write(f"// mirror: closure passes      = {passes}\n")
        f.write(f"// mirror: closure edges (reflexive, atomic) = {edges}\n")
        f.write(f"// mirror: atom-source role edges = {nR_atom}\n")
        ...
        f.write(f"pub fn expected_exsub() -> i64 {{ return {NEX} }}\n")
        f.write(f"pub fn expected_disj() -> i64 {{ return {len(disj_kept)} }}\n")
        f.write(f"pub fn expected_mappings() -> i64 {{ return {M} }}\n")
        f.write(f"pub fn expected_closure_edges() -> i64 {{ return {edges} }}\n")
        f.write(f"pub fn expected_role_edges_atom() -> i64 {{ return {nR_atom} }}\n")
        ...
        f.write("// h_sub / ex_c / ex_f are padded to the 4096 sparse-module\n")
        f.write("// capacity (only the first expected_sub() / expected_exsub()\n")
        f.write("// cells are used) so they match the stdlib elplus.sio sparse\n")
        f.write("// function signatures.\n")
        f.write(f"pub var h_sub: [i64; 4096] = [0; 4096]  // child*10000+parent\n")
        f.write(f"pub var ex_c: [i64; 4096] = [0; 4096]  // existential child\n")
        f.write(f"pub var ex_f: [i64; 4096] = [0; 4096]  // existential filler\n")
        ...
        for k, (c, _r, fl) in enumerate(exsub_kept):
            assigns.append(f"ex_c[{k}] = {c}")
            assigns.append(f"ex_f[{k}] = {fl}")
        for a, b in disj_kept:
    ...
    print(f"existential restr kept = {NEX} (of {len(exsub_h)})")
    ...
    print(f"mirror closure edges (reflexive, atomic) = {edges}")
    print(f"mirror atom-source role edges = {nR_atom}")
