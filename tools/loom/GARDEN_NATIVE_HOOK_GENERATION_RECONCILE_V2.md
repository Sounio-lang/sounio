# Garden: Garden: Causal Hook-Generation Reconciliation V2 (entrypoint input robustness)

Stage: GARDEN. Producer: Sounio. Parity open: no. Claim ready: no.

`tools/loom/native_hook_generation_reconcile_authority_main.sio` reads its frame with the `read_line()` intrinsic, a single
...
`stdlib/coordination/loom_native_hook_generation_reconcile_authority.sio` does not change and the frozen decisions do not change. `tools/loom/native_hook_generation_reconcile.freeze.v1` and its first
