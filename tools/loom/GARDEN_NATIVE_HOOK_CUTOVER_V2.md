# Garden: Garden Seed: Native Hook Cutover V2 (entrypoint input robustness)

Stage: GARDEN. Producer: Sounio. Parity open: no. Claim ready: no.

`tools/loom/native_hook_cutover_authority_main.sio` reads its frame with the `read_line()` intrinsic, a single
...
`stdlib/coordination/loom_native_hook_cutover_authority.sio` does not change and the frozen decisions do not change. `tools/loom/native_hook_cutover.freeze.v1` and its first
