# Garden: Garden: Kernel Principal Extrusion V2 (entrypoint input robustness)

Stage: GARDEN. Producer: Sounio. Parity open: no. Claim ready: no.

`tools/loom/kernel_principal_authority_main.sio` reads its frame with the `read_line()` intrinsic, a single
...
`stdlib/coordination/loom_kernel_principal_authority.sio` does not change and the frozen decisions do not change. `tools/loom/kernel_principal_authority.freeze.v1` stays
