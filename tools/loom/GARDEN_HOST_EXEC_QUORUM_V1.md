...
The host experiment must use the existing broker and the frozen descriptor
barrier. It may add a host-only PrincipalCell entrypoint, but it must not add a
second broker, controller, resident, or semantic decision path.
...
and cannot promote `material_execution`, `launch_open`, `exec_attached`,
