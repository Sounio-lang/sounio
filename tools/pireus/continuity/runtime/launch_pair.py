...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("mode",choices=["qualify","qualify-model","inspect-runtime","qualify-marlin","qualify-overlay","profile-memory","qualify-repack","tokenize","serve-token-ids","serve"]);ap.add_argument("--minutes",type=int,default=30);ap.add_argument("--tokenizer-input",type=Path);args=ap.parse_args()
 if args.mode=="tokenize" and not args.tokenizer_input:raise SystemExit("tokenize requires --tokenizer-input")
 if args.tokenizer_input and args.mode!="tokenize":raise SystemExit("tokenizer input is only for tokenize")
 ...
 for pod in selected:
  ...
  for name in ["tokenizer_transport.py","run_in_container.sh","qualify_tp2.py","inspect_runtime.py","profile_model_memory.py","qualify_marlin_placeholders.py","patch_marlin_placeholders.py","patch_marlin_repack.py","install_marlin_overlay.py","qualify_model.py","serve_rank.sh","memory_guard.py","worker_prerequisites.py","inkling-files.json"]:
