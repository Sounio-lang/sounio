...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("mode",choices=["qualify-checkpoint-path","qualify-checkpoint-copy","qualify","qualify-model","inspect-runtime","qualify-marlin","qualify-overlay","profile-memory","profile-cuda-memory","qualify-repack","tokenize","qualify-offline-interface","offline-generate","serve-token-ids","serve"]);ap.add_argument("--minutes",type=int,default=30);ap.add_argument("--tokenizer-input","--input-bundle",dest="tokenizer_input",type=Path);args=ap.parse_args()
 ...
 for pod in selected:
  ...
  for name in ["profile_checkpoint_path.py","patch_checkpoint_copy.py","qualify_checkpoint_copy.py","offline_generate.py","evict_checkpoint_cache.py","patch_marlin_deinterleave.py","profile_cuda_memory.py","tokenizer_transport.py","run_in_container.sh","qualify_tp2.py","inspect_runtime.py","profile_model_memory.py","qualify_marlin_placeholders.py","patch_marlin_placeholders.py","patch_marlin_repack.py","install_marlin_overlay.py","qualify_model.py","serve_rank.sh","memory_guard.py","worker_prerequisites.py","inkling-files.json"]:
