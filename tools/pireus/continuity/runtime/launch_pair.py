...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("mode",choices=["qualify-embedding-offload","qualify-offline-preparation","qualify-bf16-path","qualify-vocab-path","qualify-vocab-copy","qualify-checkpoint-path","qualify-checkpoint-copy","qualify","qualify-model","inspect-runtime","qualify-marlin","qualify-overlay","profile-memory","profile-cuda-memory","qualify-repack","tokenize","qualify-offline-interface","offline-generate","serve-token-ids","serve"]);ap.add_argument("--minutes",type=int,default=30);ap.add_argument("--tokenizer-input","--input-bundle",dest="tokenizer_input",type=Path);args=ap.parse_args()
 if args.mode in ("tokenize","qualify-offline-interface","qualify-offline-preparation","offline-generate") and not args.tokenizer_input:raise SystemExit("tokenize requires --tokenizer-input")
 if args.tokenizer_input and args.mode not in ("tokenize","qualify-offline-interface","qualify-offline-preparation","offline-generate"):raise SystemExit("tokenizer input is only for tokenize")
