#!/usr/bin/env python3
"""Refusal controls for the native recovery observer migration."""
...
def main():
 ...
 with tempfile.TemporaryDirectory() as directory:
  ...
  for raw in ("", " ".join(map(str,base)), " ".join(map(str,base))+"\nextra",
              " ".join(map(str,base))+"\n\n", "-1 "+"1 "*17+"32768\n"):
   run(base,2,raw)
  values=base.copy();values[15]=256;run(values,2)
 print("PIREUS_RECOVERY_MIGRATION_CONTROLS_PASS; no cluster mutation")
