...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("mode",choices=["qualify-lm-head-tiling","qualify-kernel-warmup","qualify-embedding-offload","qualify-offline-preparation","qualify-bf16-path","qualify-vocab-path","qualify-vocab-copy","qualify-checkpoint-path","qualify-checkpoint-copy","qualify","qualify-model","inspect-runtime","qualify-marlin","qualify-overlay","profile-memory","profile-cuda-memory","qualify-repack","tokenize","qualify-offline-interface","offline-generate","serve-token-ids","serve"]);ap.add_argument("--minutes",type=int,default=30);ap.add_argument("--tokenizer-input","--input-bundle",dest="tokenizer_input",type=Path);args=ap.parse_args()
 if args.mode in ("tokenize","qualify-offline-interface","qualify-offline-preparation","qualify-kernel-warmup","offline-generate") and not args.tokenizer_input:raise SystemExit("tokenize requires --tokenizer-input")
 if args.tokenizer_input and args.mode not in ("tokenize","qualify-offline-interface","qualify-offline-preparation","qualify-kernel-warmup","offline-generate"):raise SystemExit("tokenizer input is only for tokenize")
 ...
 for pod in selected:
  ...
  for name in ["lm-head-offload-lock.json","offload_lm_head.py","qualify_lm_head_tiling.py","warmup_offline_kernels.py","embedding-offload-lock.json","offload_embedding.py","qualify_embedding_offload.py","qualify_offline_preparation.py","patch_vocab_copy.py","qualify_vocab_copy.py","profile_checkpoint_path.py","patch_checkpoint_copy.py","qualify_checkpoint_copy.py","offline_generate.py","evict_checkpoint_cache.py","patch_marlin_deinterleave.py","profile_cuda_memory.py","tokenizer_transport.py","run_in_container.sh","qualify_tp2.py","inspect_runtime.py","profile_model_memory.py","qualify_marlin_placeholders.py","patch_marlin_placeholders.py","patch_marlin_repack.py","install_marlin_overlay.py","qualify_model.py","serve_rank.sh","memory_guard.py","worker_prerequisites.py","inkling-files.json"]:
 ...
 if args.mode=="tokenize":
 ...
 elif args.mode=="qualify-lm-head-tiling":
  command="PIREUS_LM_HEAD_TILING_PROBE=1 PIREUS_OFFLINE_MODE=generate PIREUS_TOKEN_IDS=1 exec /scratch/pireus/runtime/serve_rank.sh"
 elif args.mode=="qualify-embedding-offload":
  command="PIREUS_EMBEDDING_OFFLOAD_PROBE=1 PIREUS_OFFLINE_MODE=generate PIREUS_TOKEN_IDS=1 exec /scratch/pireus/runtime/serve_rank.sh"
 elif args.mode=="qualify-kernel-warmup":
  command="PIREUS_KERNEL_WARMUP=1 PIREUS_OFFLINE_MODE=generate PIREUS_OFFLINE_INPUT="+shlex.quote(token_path)+" PIREUS_TOKEN_IDS=1 exec /scratch/pireus/runtime/serve_rank.sh"
 elif args.mode=="qualify-offline-preparation":
  command="PIREUS_PREPARATION_PROBE=1 PIREUS_OFFLINE_MODE=generate PIREUS_OFFLINE_INPUT="+shlex.quote(token_path)+" PIREUS_TOKEN_IDS=1 exec /scratch/pireus/runtime/serve_rank.sh"
  ...
  command="PIREUS_CHECKPOINT_PATH_PROBE=1 PIREUS_OFFLINE_MODE=generate PIREUS_TOKEN_IDS=1 exec /scratch/pireus/runtime/serve_rank.sh"
 elif args.mode=="qualify-vocab-copy":
  command="exec python3 /scratch/pireus/runtime/memory_guard.py -- /scratch/pireus/runtime/run_in_container.sh python3 /scratch/pireus/runtime/qualify_vocab_copy.py"
 elif args.mode=="qualify-checkpoint-copy":
  command="exec python3 /scratch/pireus/runtime/memory_guard.py -- /scratch/pireus/runtime/run_in_container.sh python3 /scratch/pireus/runtime/qualify_checkpoint_copy.py"
