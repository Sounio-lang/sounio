...
import subprocess
import sys
from freeze_external_overhead import verify,readiness,digest,HERE
from external_overhead_custody import arm_data
...
def launch(frozen,arm,stage,baseline=None,baseline_pin=None):
    ...
    if not (frozen/"readiness.json").exists():(frozen/"readiness.json").write_bytes(ready_raw)
