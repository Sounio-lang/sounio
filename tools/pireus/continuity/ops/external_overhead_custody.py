...
import sys
import tempfile
from freeze_external_overhead import verify, digest, HERE
from check_frozen_feedback import source_checks
from evaluate_external_overhead import require, evaluate, lifecycle_span
...
def collect(frozen,stage,output,arm,job,run=subprocess.check_output):
    ...
    raw=run(KUBE+["exec","slurm-pilot-controller-0","-c","slurmctld","--","sacct","-j",job,"--noheader","--parsable2","--format=JobID,JobName,State,ExitCode,NodeList,Start,End"])
    terminal=accounting(raw,job)
    output.mkdir(exist_ok=False)
    for name in ("start.json","launch.log","exit-code"):
...
def qualify(frozen,baseline,baseline_pin,observed,observed_pin):
    ...
    ready=json.loads((frozen/"readiness.json").read_bytes())
    require(ready["source_commit"]==spec["source_commit"] and ready["freeze_sha256"]==digest((frozen/"execution-freeze.json").read_bytes()),"readiness identity")
    source_checks(spec,list(ready["source_checks"].values()))
    result=evaluate(spec,left,right)
