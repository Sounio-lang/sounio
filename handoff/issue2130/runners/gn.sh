...
W=/tmp/sounio-gn-$$; mkdir -p "$W" && cd "$W" || exit 1
tar xzf - 2>/dev/null || exit 1
export SOUNIO_STDLIB_PATH="$W/stdlib"; export SOUNIO_BUILD_LOCK=/tmp/gn-$$.lock
bash scripts/ci/build_modular_madaros.sh "$W/m.elf" >"$W/b.log" 2>&1 </dev/null || { echo BUILDFAIL; tail -12 "$W/b.log"; exit 1; }
echo "BUILT"
printf "  native-v2: "; "$W/m.elf" --native-v2-compile x509/gn_probe.sio "$W/t.elf" >"$W/c.log" 2>&1 </dev/null
