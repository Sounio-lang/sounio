...
- **Dual-engine divergence is not a tilde curiosity.** Default Madaros and `SOUNIO_SOUC_ENGINE=lean_single` still disagree on `f128`/`f256` parse refusal: Madaros emits **E249** (V0-A); lean_single accepts annotated `f128` and computes in **binary128** (issue #2387, 113-halving probe) through locals, direct-call params/returns, IEEE compares and `as f64`; f128 struct fields, arrays, globals, tuples, methods, fn values, `println` and binary64-inexact literals (`0.1`) are refused fail-closed. `f256` is refused on lean_single (KL-9). Two further measured cases from 2026-08-17:
  ...
  - **#1792 (RE-MEASURED 2026-09-22, appears fixed — not the same status as when opened):** filed 2026-08-17 for Madaros printing `var(...)=0.000000` on dissertation surfaces (e.g. `rapamycin_epistemic_adaptive`) and an IEEE bit-pattern as ep28 confidence. Re-measured against a Madaros **rebuilt from current source** (`make build-madaros`, not the committed artifact — see operating principle 15): `bash scripts/ci/epistemic_fabrication_detect_gate.sh` now reports `PASS: F1 Madaros reports non-zero variance (engine healthy on this path)` and `PASS: F2 Madaros confidence print looks like a probability (engine healthy)` — not the fail-closed detection it was built to fall back to. Raw output confirms: `rapamycin_epistemic_adaptive` shows live variance-growth signals (`epistemic active sigs = 35`, distinct step-count from lean_single) instead of a flat zero; `epistemic_pbpk28` TEST 6 prints `AUC confidence: 0.671038`, the exact value the original issue cited as lean_single's correct reference. The full `dissertation_*` test family (18 files, `bash scripts/dev/run_sio_test_suite_v2.sh --filter dissertation`) passes under this same fresh build, spot-checked past the harness's own PASS/FAIL bookkeeping (e.g. `dissertation_pbpk28_confidence_gate.sio`'s printed `18.180000` is the documented sum of four prior central values, not a fabricated confidence figure; `dissertation_pbpk28_overclaim.sio` still genuinely refuses with `error[E215]`). **What is not established:** no commit was found explicitly fixing the ABI-level "Madaros variance slots + f64 return ABI" defect the original PR's own test plan left unchecked — the nearest candidate, `b2df0727` (2026-09-18, "accept correlated FO variance on the adaptive ODE"), reframes the test's acceptance criterion rather than declaring a compiler-side fix, so the improvement may be a side effect of other work in the intervening five weeks (candidates include #1669 and #2131, not verified as the cause). Re-verify with a from-source build before relying on this for the defense; do not assume the fix is permanent or root-caused.
