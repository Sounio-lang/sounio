...
int main(int argc, char **argv) {
    ...
    if (inputs.size() != 54 || expected.size() != 459) {
        std::fprintf(stderr, "frozen cardinality mismatch\n");
        return 2;
    }
    std::vector<q_t> output(expected.size());
    target23_scaled_taylor16(inputs.data(), output.data(), 3);
    size_t mismatches = 0;
    ...
    std::printf("CSIM_CASES=3\nCSIM_WORDS=%zu\nCSIM_MISMATCHES=%zu\n", output.size(), mismatches);
    std::printf("TARGET23_SCALED_TAYLOR16_CSIM_PASS=%s\n", mismatches == 0 ? "true" : "false");
