...
% --- Metadata ---
\title{Fano-Selective Invertibility: A Categorical Structure for\\Non-Associative Type Systems}
...
\begin{abstract}
We introduce \emph{Fano-selective invertibility}, a novel categorical
structure in which the invertibility of the associator morphism is
determined by a geometric predicate---specifically, by collinearity
in the Fano plane $\PG(2,2)$. This structure generalises both monoidal
categories (where every associator is an isomorphism) and skew monoidal
categories (where no associator is invertible), occupying a new point
in the design space.

We instantiate this structure for the Cayley--Dickson algebras, proving
that exactly 175 of 343 ordered triples of imaginary octonion basis
elements have invertible associators (42 from Fano-line collinearity
and 133 from repeated-index alternativity), while 168 have
sign-changing associators (non-associative triples), with
$168 = |\PSL(2,7)|$. The non-invertible associators decompose under
a dagger involution into $84 + 84$ forward/backward pairs with zero
fixed points.

We implement this structure as the \texttt{NonAssoc} algebraic effect
in Sounio, a self-hosted programming language: the compiler's
reassociation optimiser uses the Fano predicate to decide which
expressions can be safely rewritten, making $\PG(2,2)$ a compiler
optimisation guide. All results are formalised in Lean~4 (95 theorems,
no \texttt{sorry}, no Mathlib dependency) and cross-validated against
...
This paper introduces a categorical structure that captures this
phenomenon: \emph{Fano-selective invertibility}. In a standard
monoidal category, the associator
$\alpha: (A \otimes B) \otimes C \to A \otimes (B \otimes C)$
is a natural \emph{isomorphism}. In a skew monoidal category
\cite{szlachanyi2012,lackstreet2012}, it is merely a natural
transformation---a morphism with no inverse. We define a structure
where the associator is selectively invertible: it is an isomorphism
for Fano triples and a directed morphism for non-Fano triples, with
the selection determined by the Fano plane.

\paragraph{Contributions.}
\begin{enumerate}
\item \textbf{Fano-selective invertibility} (Definition~\ref{def:fsi},
  Theorem~\ref{thm:coherence}): a basis-indexed reassociation
  structure satisfying full skew monoidal coherence
  \cite{lackstreet2012}, with selective invertibility parameterised by
  a decidable predicate on triples. The Fano plane of $\PG(2,2)$ is
  the canonical instance (\S\ref{sec:fsi}).
...
% ================================================================
\section{Fano-Selective Invertibility}\label{sec:fsi}
% ================================================================

\begin{definition}[Fano-selective associator structure]\label{def:fsi}
Let $A$ be a $2^k$-dimensional algebra with basis $\{e_0, \ldots, e_{2^k-1}\}$,
binary product $e_i \cdot e_j = \sigma(i,j,k) \cdot e_{i \oplus j}$,
and let $B = \{1, \ldots, 2^k - 1\}$ index the imaginary basis elements.
A \emph{Fano-selective associator structure} on $A$ consists of:

\begin{enumerate}
\item A decidable predicate $\Fano: B^3 \to \{0,1\}$ on ordered basis
  triples.

\item For every triple $(i,j,l) \in B^3$, a right-reassociation
  rewrite
  \[
    \alpha^{R}_{i,j,l}: (e_i \cdot e_j) \cdot e_l \;\longmapsto\;
    e_i \cdot (e_j \cdot e_l)
  \]
  that is always valid (the rewrite preserves the result up to sign).

\item For triples with $\Fano(i,j,l) = 1$, an inverse rewrite
  $\alpha^{L}_{i,j,l}$ such that the two parenthesisations yield
  identical results: $(e_i \cdot e_j) \cdot e_l = e_i \cdot (e_j \cdot e_l)$.

\item For triples with $\Fano(i,j,l) = 0$, the two parenthesisations
  yield different results---the reassociation changes the sign by a
  factor of $-1$---so no valid inverse rewrite exists.
\end{enumerate}
...
\item The dagger negates the associator coefficient on non-associative
  triples: $\Fano(i,j,l) = 0 \implies \wave(l,j,i) = -\wave(i,j,l)$.
\end{itemize}
\end{definition}

\begin{remark}
This definition operates at the level of basis elements within a single
algebra. To establish the categorical lift, we must verify that the
reassociation factor $\varphi(a,b,c) := \alpha \cdot \beta \in \{+1,-1\}$
satisfies the pentagon coherence axiom of Lack--Street
\cite{lackstreet2012}. We do so in \S\ref{sec:coherence}.
\end{remark}
...
\begin{theorem}[Skew Monoidal Coherence]\label{thm:coherence}
The Cayley--Dickson octonion algebra ($k = 3$) with tensor $\oplus$
(XOR), unit $e_0$, and associator factor $\varphi$ satisfies all five
skew monoidal coherence axioms of Lack--Street \cite{lackstreet2012}.
\end{theorem}
...
\begin{remark}
The associator $\alpha_{a,b,c}$ multiplies the product by
$\varphi(a,b,c)$. Since $(-1)^{-1} = -1$, the associator is always
invertible as a map. However, for non-Fano triples ($\varphi = -1$),
the reassociation \emph{changes the sign of the computed value}. From
the compiler's perspective, a sign-changing rewrite is not
semantics-preserving: $(e_i \cdot e_j) \cdot e_k \neq e_i \cdot (e_j \cdot e_k)$
even though the two sides differ only by a sign. The \texttt{NonAssoc}
effect tracks this distinction: the compiler blocks reassociation
unless $\varphi = 1$, i.e., unless the triple is Fano.
\end{remark}
...
octonions. The Fano predicate provides finer-grained control for
expressions with three or more distinct variables.
...
formalism (TSVF) of Aharonov, Bergmann, and Lebowitz \cite{abl1964}.
