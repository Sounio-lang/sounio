...
- **Complete GRI-Mech 3.0 H/O sub-mechanism with native UQ** (`gri30_h2.sio`): all 28 H/O-only reactions / 10 species from grimech30.dat — modified Arrhenius, three-body collision efficiencies, Troe falloff (2 OH (+M) ⇌ H2O2 (+M)) — with reverse rates from NASA-7 thermochemistry via detailed balance (k_rev = k_fwd/Kc(T)), and first-order GUM propagation of per-reaction rate uncertainties (representative 1σ from Baulch 2005 / Konnov 2008 / Hong 2011) through the RK4 trajectory, returning a native 1σ band per species. Cross-checked against an independent Python replication (rates exact, thermo exact, pre-ignition-front trajectory <1%, uncertainty band <1%); the ignition front itself is exponentially phase-sensitive and documented as not parity-testable. Run-pass coverage: tests/stdlib/chemistry/test_gri30_h2.sio.
