...
```bash
SOUNIO_STDLIB_PATH=./stdlib ./scripts/ci/souc-native-wrapper.sh run tests/run-pass/viz_scene_package.sio
```
...
## Workbench Session Timeline Proof

Run:

```bash
SOUNIO_STDLIB_PATH=./stdlib ./scripts/ci/souc-native-wrapper.sh run tests/run-pass/viz_workbench_session_timeline.sio
```
...
The test records baseline and comparison Workbench runs, builds a notebook, scrubs both sessions at a shared early frame and at a late baseline/comparison frame pair, verifies timeline hashes, frame hashes, audit hashes, time deltas, hash distances, applies a `VizWorkbenchExperimentDiffOverlay` onto the comparison scene, proves the overlay changes the Canvas frame hash, packages the notebook/player/overlay state as `VizWorkbenchExperimentDiffArtifact` values, indexes those values in a `VizWorkbenchExperimentDiffLibrary`, applies a `VizWorkbenchExperimentDiffLibraryBrowser` back onto native Visual IR controls, drives a `VizWorkbenchExperimentCompareMode` with key and mouse events to change selected/baseline/compare artifacts, proves the browser/mode changes Canvas pixels, wraps notebook/library/browser/mode/frame/audit hashes in `VizWorkbenchExperimentPackage`, restores that package into a fresh Workbench scene with verified library/browser/mode/frame/audit hashes, saves initial and final packages into a fixed-slot `VizWorkbenchExperimentPackageStore`, clicks native store controls to save another package, cycle active slots, set baseline/compare, restore the active package, binds store/notebook/package/timeline state as `VizWorkbenchExperimentTimelineCockpit`, clicks native time controls through `VizWorkbenchExperimentCockpitPlayback`, wraps the package/store/restore/cockpit/playback/export hashes and Visual IR scene snapshot buffers as `VizWorkbenchExperimentExportPacket`, imports that packet into another fresh Workbench scene as `VizWorkbenchExperimentImportReplayReport` with zero frame/audit drift against the exported packet, and emits passive player, overlay, artifact, library, library-browser, compare-mode, package, package-restore, package-store, timeline-cockpit, cockpit-playback, export-packet, and import-replay metadata before static HTML/SVG export.
...
## Workbench Project Workspace Controls Proof

Run:

```bash
SOUNIO_STDLIB_PATH=./stdlib ./scripts/ci/souc-native-wrapper.sh run tests/run-pass/viz_workbench_project_workspace_controls.sio
```
