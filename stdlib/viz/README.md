...
- `VizWorkbenchProjectWorkspace` keeps two complete Visual Project Format slots for A/B scientific scene work. Native `saveA`, `saveB`, `loadA`, `loadB`, `diff`, and `ab` viewport controls are Visual IR nodes handled by `viz_workbench_workspace_app_handle_event`; save/load/compare update active slot, comparison slot, deterministic diff hash, time metadata, and visible audit metadata while the project envelopes remain Sounio data.
...
- `viz_workbench_replay_record_session`, `viz_workbench_replay_verify_session`, `viz_workbench_replay_dump_session`, and `viz_workbench_replay_emit_html_archive` lift Workbench replay into a Sounio-owned session tape with event steps, tick steps, per-frame Canvas hashes, audit hashes, integer scene time, a deterministic session signature, and a passive HTML/SVG archive comment. Rebuilt scenes can verify every frame against the recorded session without JavaScript or Python semantics.
