...
- `viz::workbench` composes charts, rendered molecule nodes, scalar/vector fields, trajectory, spectrum, lattice/phonon dynamics, particle-event traces, mesh, controls, inspector/edit/replay/export hooks into a reusable native visual workbench scene blueprint.
- Workbench time controls synchronize lattice/phonon and particle-event slots in-place, so interaction changes Sounio-owned physchem state rather than recreating a renderer-only scene.
