...
- `viz::workbench` composes charts, rendered molecule nodes, scalar/vector fields, trajectory, spectrum, lattice/phonon dynamics, particle-event traces, time-driven mesh, controls, inspector/edit/replay/export hooks into a reusable native visual workbench scene blueprint.
- Workbench time controls synchronize lattice/phonon, particle-event, and 3D mesh slots in-place, so interaction changes Sounio-owned physchem/geometry state rather than recreating a renderer-only scene.
