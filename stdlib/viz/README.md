...
- `viz_workbench_replay_record_session`, `viz_workbench_replay_verify_session`, and `viz_workbench_replay_dump_session` lift Workbench replay into a Sounio-owned session tape with event steps, tick steps, per-frame Canvas hashes, audit hashes, and integer scene time. Rebuilt scenes can verify every frame against the recorded session without JavaScript or Python semantics.
