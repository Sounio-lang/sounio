...
// One value of R, matching the Sounio modules and the Python replicas exactly.
// The bit-for-bit claim in the header depends on it: this file is only a valid
// cross-check of the replica while both compute from the same constants.
constexpr double R_SI = 8.31446261815324;   // J/(mol*K) = k_B * N_A, both exact by SI
// Cantera converts the cal/mol Ea of gri30.yaml with 4.184 J/cal exactly and its own
// R, so the mechanism it reads uses R/4.184.  The older 1.9872041 is 7.98e-08 away,
// inside exp(-Ea/(R*T)), and made this file compute DIFFERENT rate constants from
// the mechanism it claims to implement.
constexpr double R_CAL = 1.9872042586408316;
...
static Run band(const Mech& m, double T, double t_end, double dt, bool with_unc) {
    ...
    // mol/cm^3 at 1 atm is P0/(R*T)*1e-6.  The 1/(82.057*T) shorthand this carried
    // truncated the molar gas volume and ran 4.461e-06 high.
    const double mtot = P0 / (R_SI * T) * 1e-6;
