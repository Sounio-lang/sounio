...
| `formal/lean4/SounioSqrtField.lean` | **B7 — abstract real-field interface**: `SqrtField` (ordered field + √, ℝ-satisfiable); `nonneg_sqrt_unique`, `mul_sqrt` (`√a·√b=√(ab)`), radical map `r` | abstract `SqrtField` | the target a `QFTransfer` ℝ-instance plugs into; `GeneratorLawObligation` staged (no Mathlib, `lake build SounioSqrtField`) |
