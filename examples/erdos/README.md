...
| `formal/lean4/SounioMultiquadHom.lean` | **B8 — QF→SqrtField ring homomorphism**: Mathlib-free finite-sum library `fsum` (add/zero/congr/mul-distrib/`fsum_mul_fsum`/map/`fsum_perm`/`fsum_comm`/`fsum_xor`/`fsum_neg`); `evalNum l = Σ_{i<16} ofInt(lᵢ)·r i`; **`evalNum_qmul` PROVED**; **fraction map `φ(c,d)=(Σcᵢrᵢ)·inv(ofInt d)` with guarded ring-hom laws `phi_qmul`/`phi_qadd`/`phi_qsub` PROVED** (den≠0) | abstract `SqrtField` / 16 | promotes per-generator `generator_law` to the full bilinear convolution identity (`perm_range_xor` reindex) and completes it to a **den-aware field homomorphism** (frac-add identities `frac_add`/`frac_sub` + char-0 inverse toolkit); `phi_qadd`/`phi_qsub` `[propext, Quot.sound]`, `phi_qmul` inherits `perm_range_xor` certs (`lake build SounioMultiquadHom`) |
