...
| `formal/lean4/MadoreSpindleVitrine.lean` | Single-entry Madore/Q311 showcase: legacy `phi311`, parametric `evalS [3,11]`, normalized `standardRealPlaneUnit`, and Euclidean Nat-edge adapter | ℚ(√3,√11) | **χ(ℝ²) ≥ 4 base case closed** for the Mathlib-free standard real-plane unit relation; packaging only, no new theorem beyond imported proofs (`lake build MadoreSpindleVitrine`) |
| `formal/lean4/DeGreyChi5Vitrine.lean` | Single-entry G529 showcase: current `{3,5,11}` QF fragment, named `RootedField3511`, derived `phi3511` evaluator, scoped minimal-support boundary, and Q311 cross-vitrine boundary | ℚ(√3,√5,√11) current embedding | Re-exports the exact current support certificate and `current_embedding_3511_minimality_with_q311_real_boundary`; this is scoped to the checked embedding, not universal minimality over all G529 embeddings (`lake build DeGreyChi5Vitrine`) |
| `formal/lean4/ErdosVitrine.lean` + `test_madore_q311_g529_3511_unified_vitrine_gate.sh` | Unified Q311/G529 vitrine plus a separate chi>=6 interface-smoke boundary | Q311 + current G529 | Packaging gate, not a new theorem: Q311 is a proper-support boundary, not a G529 LRAT support; chi>=6 remains decoupled smoke-only with no Euclidean no-five lower-bound witness (`lake build ErdosVitrine`, then the shell gate) |
| `make_chi6_frontier_refute_success_sat_manifest.py` + `test_chi6_frontier_refute_success_sat_manifest.sh` | Frontier-success packager: validates a `REFUTE_SUCCESS_UNPROMOTABLE` ledger row and delegates to arbitrary cube-cover SAT/Lean packaging | finite `colourCNF` only | Emits a non-promotable `candidate.manifest` plus lineage JSON for the selected edge/cube batch; no geometry, Euclidean, or χ(ℝ²) claim (`promotable=0`) |
| `make_chi6_frontier_refute_success_promotion_preflight.py` + `test_chi6_frontier_refute_success_promotion_preflight.sh` | Source-bound frontier-success joiner: follows the successful ledger row back to `campaign_preflight_json`, verifies source/edge/cube hashes, then runs the integrated exact-geometry + arbitrary SAT preflight | exact rational source package + finite `colourCNF` | Emits promotion-preflight lineage only. `READY_FOR_CANDIDATE_PROMOTION_WIRING` is allowed only when source, geometry, and SAT all pass for the same graph identity; otherwise it stays incomplete and `promotable=0` |
| `cube_sieve_gpu_parity.py` + `test_cube_sieve_gpu_parity.sh` | Backend parity harness for the GPU-lane cube propagation producer | finite DIMACS cube propagation | Compares any backend with the deterministic CPU manifest producer and rejects canonical manifest drift; parity is search plumbing only (`promotable=0`, no proof artifact) |
| `make_chi6_rtx8000_gpu_job.py` + `test_chi6_rtx8000_gpu_job_manifest.sh` | Emits a Darwin RTX 8000 Kubernetes job manifest for the bounded chi>=6 frontier sweep | RTX 8000 search lane + OrangeFS scratch | Search-only launcher: pins the validated sm_75 GPU lane, requires an explicit Sounio cluster image, writes under `/scratch`, refuses node-local scratch/LLM-serving markers, and preserves `verified_claim=none` until DRAT/LRAT/Lean verification exists |
| **souc_sat cube support (owned for X>6)** | Native CDCL worker reads zero-based cube file as original unit clauses (before n_orig); emits CNF+units + deletion-free DRAT for the subproblem (CNF(edge,k) + cube). Enables cube-and-conquer for chi6+ frontier refutes / larger candidates toward χ(ℝ²)>6. Gate `test_souc_sat_cube_units.sh` (incl. k=6 smoke + dup/bad errors) green; used by `cube_sieve_refute_batch.py`. | — | Subproblem LRAT/DRAT for cover; no global chromatic claim until cube cover + geometry + Lean. |
...
## Fib / 1/137 rational directions (added for χ>6 hunt)

To bias the unit-step cloud toward 5-fold symmetry (pentagons, golden-ratio angles φ that appear in Moser spindle / de Grey / Golomb components for χ=5), the scout and campaign now support `--denom-mode fib` (Fib numbers as the c in a^2+b^2=c^2 triples; also (b,a) swaps + full sign/quadrant variants + gcd=1 primitives).

- `--denom-mode fib --max-den 34` (or 233 etc) produces different candidate graphs than uniform pythag.
- 1/137 is covered by `--max-den 137` (no special mode needed).
- Both preserve exact rational coords + all-pairs dist2==1 check (Lean-ready).
- Recorded in frontier-scout.json and campaign.json as "denom_mode".
- Fractal sweep surface accepts `--denom-mode fib` (CellParams + manifests); full guided propagation is incremental.
- Gate: test_chi6_rational_frontier_scout.sh exercises fib path (light smoke).
- Contract: still promotable=0 until full geometry+SAT+cover+Lean on a real hard cell. This is search bias only.

See `chi6_rational_frontier_scout.py:fib_rational_unit_directions`, campaign/fractal updates, and x6-fib-local-campaign-* snapshots.
