...
| `formal/lean4/SounioMultiquadHom.lean` | **B8 — QF→SqrtField numerator hom core**: Mathlib-free finite-sum library `fsum` (add/zero/congr/mul-distrib/`fsum_mul_fsum`/map/`fsum_perm`/`fsum_comm`/`fsum_xor`); `evalNum l = Σ_{i<16} ofInt(lᵢ)·r i`; **`evalNum_qmul` PROVED** (`evalNum(qmul x y) = mul(evalNum x)(evalNum y)`) | abstract `SqrtField` / 16 | promotes per-generator `generator_law` to the full bilinear convolution identity via the `perm_range_xor` XOR reindex — the **den-free multiplicative heart** of the fraction map φ:QF→F; new code axioms `[propext, Quot.sound]` (inherited `Classical.choice`+native certs from `perm_range_xor`; `lake build SounioMultiquadHom`) |
