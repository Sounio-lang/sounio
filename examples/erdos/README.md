...
| `formal/lean4/SounioSqrtField.lean` | **B7 — abstract real-field interface**: `SqrtField` (ordered field + √, ℝ-satisfiable); `nonneg_sqrt_unique`, `mul_sqrt` (`√a·√b=√(ab)`), radical map `r`; **`generator_law` PROVED** (`rᵢ·rⱼ = ofNatProd(i∧j)·r_{i⊕j}`, four-bit factorisation) | abstract `SqrtField` | the target a `QFTransfer` ℝ-instance plugs into; `GeneratorLawObligation` discharged — multiplicative core of `QF↪ℝ` (no Mathlib/Classical, `lake build SounioSqrtField`) |
