...
| `gen_lean_sat.sh` → `formal/lean4/SounioSatK76.lean` | **B1 SAT leg INTERNALISED (full chain)**: souc_sat's *own* CDCL LRAT cert, re-checked by Lean core's **verified LRAT checker** (`check_sound` + `native_decide`), then the bridge ⟹ chromatic bound — **no external checker, no Mathlib** | K₇ / 6 | **🏁 `k76_not_colourable : ¬ 6-colourable K₇` (χ≥7), axioms `[propext, Classical.choice, Quot.sound, native_decide.ax]`, no `sorry`** (`lake build SounioSatK76`, ~3 min). G₅₂₉ (98 k-line LRAT) needs file-loaded reflection — see `B1_SAT_LEG_IN_LEAN.md` |
