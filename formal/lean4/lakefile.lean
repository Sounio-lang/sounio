...
-- Non-default: exhaustively scans all 65536 4x4 binary matrix codes.
...
-- Concrete FORMAL_PARITY replay of the frozen 33-attempt mutation admission trace.
-- Non-default: canonicalizes 33 mutations across GL(4,F2) x swap x gauge.
lean_lib «SounioPireusOperatorOrbitAdmissionReconstruction» where
  moreLeanArgs := #["-j", "1"]

-- Abstract finite-action canonical-minimum iff orbit theorem for Pireus.
-- Non-default: the concrete V13 GL4 x swap x gauge instantiation remains open.
lean_lib «SounioPireusFiniteActionCanonicalization» where
  moreLeanArgs := #["-j", "1"]

-- Concrete 2048-word basis-fixed gauge coboundary action for Pireus V13.
-- Non-default: tree-section minimality and the GL4 action remain open.
...
-- Direct 11-bit gauge section as a unique orbit representative.
-- Non-default: closes the gauge quotient only, not the outer GL4 x swap fold.
lean_lib «SounioPireusGaugeSectionCanonicalization» where
  moreLeanArgs := #["-j", "1"]

-- XOR-linear pullback plus concrete input swap and unrestricted coboundary transport.
-- Non-default: the 11-bit rebase and concrete 20160-code GL4 bridge remain open.
lean_lib «SounioPireusLinearSwapGaugeDescent» where
  moreLeanArgs := #["-j", "1"]

-- Rebase XOR-linear gauge pullbacks into the frozen 11-bit basis-fixed section.
-- Non-default: concrete matrix-code witnesses and the 40320-view minimum remain open.
lean_lib «SounioPireusBasisFixedGaugeRebase» where
  moreLeanArgs := #["-j", "1"]

-- Structural bridge from each invertible V13 matrix code to an XOR-lane equivalence.
-- Non-default: the complete 20160 witness list and 40320-view minimum remain open.
...
-- Kernel-checked ordered-basis census: 15 * 14 * 12 * 8 = |GL(4,2)|.
-- Non-default: the bijection to the frozen matrix-code scan remains open.
