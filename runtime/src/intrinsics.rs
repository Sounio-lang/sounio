...
#[no_mangle]
pub extern "C" fn sounio_bootstrap_from_samples(
    estimate: f64,
    ...
    let samples = unsafe { std::slice::from_raw_parts(samples_ptr, n_samples) };
...
#[no_mangle]
pub extern "C" fn sounio_bootstrap_add(a: BootstrapKnowledge, b: BootstrapKnowledge) -> BootstrapKnowledge {
...
#[no_mangle]
pub extern "C" fn sounio_bootstrap_sub(a: BootstrapKnowledge, b: BootstrapKnowledge) -> BootstrapKnowledge {
    let estimate = a.estimate - b.estimate;
    let bootstrap_se = (a.bootstrap_se.powi(2) + b.bootstrap_se.powi(2)).sqrt();
    let a_half_width = (a.percentile_upper - a.percentile_lower) / 2.0;
    let b_half_width = (b.percentile_upper - b.percentile_lower) / 2.0;
...
#[no_mangle]
pub extern "C" fn sounio_bootstrap_mul(a: BootstrapKnowledge, b: BootstrapKnowledge) -> BootstrapKnowledge {
    let estimate = a.estimate * b.estimate;
    // Relative SE propagation
    let rel_a = if a.estimate != 0.0 { a.bootstrap_se / a.estimate.abs() } else { 0.0 };
    let rel_b = if b.estimate != 0.0 { b.bootstrap_se / b.estimate.abs() } else { 0.0 };
...
#[no_mangle]
pub extern "C" fn sounio_bootstrap_div(a: BootstrapKnowledge, b: BootstrapKnowledge) -> BootstrapKnowledge {
    if b.estimate == 0.0 {
        return BootstrapKnowledge {
            estimate: f64::NAN,
...
#[no_mangle]
pub extern "C" fn sounio_cv_from_folds(folds_ptr: *const f64, k: usize) -> CVKnowledge {
    ...
    let folds = unsafe { std::slice::from_raw_parts(folds_ptr, k) };
...
pub extern "C" fn sounio_cv_to_knowledge(cv: CVKnowledge) -> Knowledge {
    ...
    let confidence = if cv.k >= 10 { 0.95 } else if cv.k >= 5 { 0.90 } else { 0.80 };
...
pub extern "C" fn sounio_cv_ci_lower(cv: CVKnowledge) -> f64 {
    ...
    let t_value = if cv.k <= 2 { 12.71 } else if cv.k <= 5 { 2.78 } else if cv.k <= 10 { 2.26 } else { 1.96 };
...
mod tests {
    ...
    fn test_bootstrap_from_samples() {
        ...
        let boot = sounio_bootstrap_from_samples(10.0, samples.as_ptr(), samples.len(), 0.05);
    ...
    fn test_cv_from_folds() {
        ...
        let cv = sounio_cv_from_folds(folds.as_ptr(), folds.len());
