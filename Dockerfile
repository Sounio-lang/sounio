...
#
# Usage:
#   Build:   docker build -t sounio:0.99.0 .
#   Run:     docker run -v $(pwd):/workspace sounio:0.99.0 check /workspace/myfile.sio
#   Tests:   docker build --target test -t sounio:test .
#   Shell:   docker run -it -v $(pwd):/workspace --entrypoint /bin/bash sounio:0.99.0
#   REPL:    docker run -it sounio:0.99.0 repl
#
# For scientific reproducibility, use the specific version tag:
#   docker pull ghcr.io/sounio-lang/sounio:0.99.0
...
RUN cargo test --release --features "jit,pkg" -- --test-threads=1
