...
///
/// # Safety
/// `effect_ptr` and `op_ptr` must be valid null-terminated C strings.
/// `args_ptr` must be a valid pointer to an array of args_len f64 values if not null.
#[no_mangle]
pub unsafe extern "C" fn __sounio_dispatch_generic(
    effect_ptr: *const u8,
    op_ptr: *const u8,
    args_ptr: *const f64,
    args_len: usize,
    ...
    let effect = CStr::from_ptr(effect_ptr as *const std::ffi::c_char)
        .to_str()
        .unwrap_or("");

    let op = CStr::from_ptr(op_ptr as *const std::ffi::c_char)
        .to_str()
        .unwrap_or("");
