...
impl Arena {
    ...
    pub fn allocate(&mut self, size: usize, align: usize) -> *mut u8 {
        ...
        if aligned_offset + size <= self.chunk_size {
            let ptr = self.chunks[self.current]
                .as_mut_ptr()
                .wrapping_add(aligned_offset);
...
impl<T> RcBox<T> {
    ...
    ///
    /// # Safety
    /// `ptr` must be a valid pointer to a RefCounted<T>
    ...
    /// Decrement reference count, returns true if object should be deallocated
    ///
    /// # Safety
    /// `ptr` must be a valid pointer to a RefCounted<T>
    ...
    ///
    /// # Safety
    /// `ptr` must be a valid pointer to a RefCounted<T> and caller must ensure exclusive access
