...
#[unsafe(no_mangle)]
pub extern "C" fn __sounio_gpu_alloc(size: usize) -> u64 {
    if let Ok(mut bridge) = get_gpu_bridge().lock() {
        bridge.alloc(size).unwrap_or(0)
    } else {
        0
    }
}
...
/// Returns 1 on success, 0 on failure.
#[unsafe(no_mangle)]
pub extern "C" fn __sounio_gpu_copy_htod(buffer_id: u64, src: *const u8, size: usize) -> u64 {
    if src.is_null() {
        return 0;
    }
    ...
    if let Ok(mut bridge) = get_gpu_bridge().lock() {
        ...
        if bridge.copy_htod(buffer_id, slice).is_ok() {
            1
        } else {
            0
        }
...
/// Returns 1 on success, 0 on failure.
#[unsafe(no_mangle)]
pub extern "C" fn __sounio_gpu_copy_dtoh(buffer_id: u64, dst: *mut u8, size: usize) -> u64 {
    if dst.is_null() {
        return 0;
    }

    if let Ok(mut bridge) = get_gpu_bridge().lock() {
        let slice = unsafe { std::slice::from_raw_parts_mut(dst, size) };
        if bridge.copy_dtoh(buffer_id, slice).is_ok() {
            1
        } else {
            0
        }
...
/// Load PTX kernel
///
/// Returns kernel ID on success, 0 on failure.
#[unsafe(no_mangle)]
pub extern "C" fn __sounio_gpu_load_ptx(
    ptx: *const u8,
    ptx_len: usize,
    name: *const u8,
    name_len: usize,
) -> u64 {
    if ptx.is_null() || name.is_null() {
        return 0;
    }

    let ptx_str = unsafe {
        let slice = std::slice::from_raw_parts(ptx, ptx_len);
    ...
    if let Ok(mut bridge) = get_gpu_bridge().lock() {
        bridge.load_ptx(ptx_str, name_str).unwrap_or(0)
    } else {
        0
    }
}
...
/// Returns 1 on success, 0 on failure.
#[unsafe(no_mangle)]
pub extern "C" fn __sounio_gpu_launch(
    kernel_id: u64,
    grid_x: u32,
    grid_y: u32,
    grid_z: u32,
    ...
    if let Ok(mut bridge) = get_gpu_bridge().lock() {
        // Convert buffer IDs to kernel arguments
        let arg_ids = if arg_count > 0 {
            unsafe { std::slice::from_raw_parts(args, arg_count) }
        } else {
            &[]
        };

        let kernel_args: Vec<_> = arg_ids
            .iter()
            .filter_map(|id| bridge.buffer_as_arg(*id))
            .collect();

        if kernel_args.len() != arg_count {
            return 0; // Some buffer IDs were invalid
        }

        let result = bridge.launch_kernel(
            kernel_id,
...
mod tests {
    ...
    #[test]
    fn test_c_dispatch_alloc_free() {
        // Test the C-callable functions
        let buffer_id = __sounio_gpu_alloc(2048);
        // With simulated backend, allocation should succeed
        assert!(buffer_id > 0);

        let result = __sounio_gpu_free(buffer_id);
        assert_eq!(result, 1);
    ...
    #[test]
    fn test_c_dispatch_sync() {
        let result = __sounio_gpu_sync();
        assert_eq!(result, 1);
    }

    #[test]
    fn test_c_dispatch_invalid_args() {
        // Null pointer should fail
        let result = __sounio_gpu_copy_htod(1, std::ptr::null(), 100);
        assert_eq!(result, 0);

        let result = __sounio_gpu_copy_dtoh(1, std::ptr::null_mut(), 100);
        assert_eq!(result, 0);
    }
}
