    ...
    NETWORK_OPERATIONS.get_or_init(|| {
        vec![
            ...
            OperationSpec::new("delete", "Response")
                .with_params(vec!["String"])
                .with_confidence_factor(NETWORK_FETCH_CONFIDENCE_FACTOR),
            OperationSpec::new("send", "Unit").with_params(vec!["String", "String"]),
            OperationSpec::new("websocket_connect", "ConnId").with_params(vec!["String"]),
            OperationSpec::new("websocket_send", "Unit").with_params(vec!["Int", "String"]),
            OperationSpec::new("websocket_receive", "String")
                .with_params(vec!["Int"])
                .with_confidence_factor(NETWORK_FETCH_CONFIDENCE_FACTOR),
            OperationSpec::new("websocket_close", "Unit").with_params(vec!["Int"]),
            OperationSpec::new("resolve_dns", "String")
                .with_params(vec!["String"])
                .with_confidence_factor(DNS_CONFIDENCE_FACTOR),
            OperationSpec::new("is_connected", "Bool").with_params(vec![]),
            OperationSpec::new("set_timeout", "Unit").with_params(vec!["Int"]),
            OperationSpec::new("get_status_code", "Int").with_params(vec!["Response"]),
        ]
    })
}
...
impl NetworkHandler {
    ...
    fn get_ws_counter(state: &mut HandlerState) -> i64 {
        match state.named_state.get(WS_CONNECTION_COUNTER_KEY) {
            ...
            _ => {
                state
                    .named_state
                    .insert(WS_CONNECTION_COUNTER_KEY.to_string(), Value::Int(1));
                1
            }
        }
    }
    ...
    fn next_ws_connection_id(state: &mut HandlerState) -> i64 {
        ...
        state
            .named_state
            .insert(WS_CONNECTION_COUNTER_KEY.to_string(), Value::Int(id + 1));
        id
    }
    ...
    fn get_request_counter(state: &mut HandlerState) -> i64 {
        match state.named_state.get(REQUEST_COUNTER_KEY) {
            ...
            _ => {
                state
                    .named_state
                    .insert(REQUEST_COUNTER_KEY.to_string(), Value::Int(1));
                1
            }
        }
    }
    ...
    fn next_request_id(state: &mut HandlerState) -> i64 {
        ...
        state
            .named_state
            .insert(REQUEST_COUNTER_KEY.to_string(), Value::Int(id + 1));
        id
    }
    ...
    fn add_bytes_sent(state: &mut HandlerState, bytes: i64) {
        ...
        state
            .named_state
            .insert(BYTES_SENT_KEY.to_string(), Value::Int(current + bytes));
    }
    ...
    fn add_bytes_received(state: &mut HandlerState, bytes: i64) {
        ...
        state
            .named_state
            .insert(BYTES_RECEIVED_KEY.to_string(), Value::Int(current + bytes));
    }
    ...
    fn validate_url(url: &str, op: &str) -> Result<(), HandlerResult> {
            ...
            && !url.starts_with("wss://")
        {
            return Err(HandlerResult::Abort(HandlerError::new(
                ...
                op,
                format!(
                    "invalid URL scheme: {}. Expected http://, https://, ws://, or wss://",
                    url
                ),
            )));
        }
    ...
    fn handle_set_timeout(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        state
            .named_state
            .insert("__network_timeout".to_string(), Value::Int(timeout_ms));
    ...
    fn handle_get_status_code(&self, args: &[Value]) -> HandlerResult {
        ...
        match &args[0] {
            Value::Tuple(elems) if !elems.is_empty() => match &elems[0] {
    ...
    pub fn set_connected(state: &mut HandlerState, connected: bool) {
        state
            .named_state
            .insert(CONNECTIVITY_KEY.to_string(), Value::Bool(connected));
    }
}
...
impl HandlerCapability for NetworkHandler {
    ...
    fn epistemic_impact(&self, operation: &str) -> EpistemicImpact {
        match operation {
            ...
            "resolve_dns" => EpistemicImpact::with_confidence(DNS_CONFIDENCE_FACTOR),
    ...
    fn operation_linearity(&self, operation: &str) -> crate::effects::linearity::Linearity {
        ...
        // Network operations have external side effects - mostly one-shot
        match operation {
            "fetch" | "get" | "post" | "put" | "delete" | "send" | "websocket_connect"
            | "websocket_send" | "websocket_receive" | "websocket_close" | "resolve_dns"
            | "set_timeout" => Linearity::ExactlyOnce,
            // Status queries are safe for multi-shot
            "is_connected" | "get_status_code" => Linearity::MultiShot,
...
mod tests {
    ...
    fn test_get_with_headers() {
        ...
        let headers = Value::Array(Rc::new(RefCell::new(vec![Value::Tuple(vec![
    ...
    fn test_post_basic() {
        ...
        match result {
            HandlerResult::Resume(Value::Tuple(elems)) => match &elems[0] {
