...
fn get_div_operations() -> &'static [OperationSpec] {
    DIV_OPERATIONS.get_or_init(|| {
        vec![
            OperationSpec::new("div", "Float").with_params(vec!["Float", "Float"]),
            OperationSpec::new("checked_div", "Option").with_params(vec!["Float", "Float"]),
            OperationSpec::new("safe_div", "Float").with_params(vec!["Float", "Float", "Float"]),
            OperationSpec::new("int_div", "Int").with_params(vec!["Int", "Int"]),
            OperationSpec::new("rem", "Int").with_params(vec!["Int", "Int"]),
            OperationSpec::new("mod", "Float").with_params(vec!["Float", "Float"]),
        ]
    })
}
...
impl DivHandler {
    ...
    fn handle_div(&self, args: &[Value]) -> HandlerResult {
        ...
        if b == 0.0 {
            return HandlerResult::Abort(HandlerError::new("Div", "div", "division by zero"));
        }
    ...
    fn handle_mod(&self, args: &[Value]) -> HandlerResult {
        ...
        if b == 0.0 {
            return HandlerResult::Abort(HandlerError::new("Div", "mod", "modulo by zero"));
        }
...
impl HandlerCapability for DivHandler {
    ...
    fn operation_linearity(&self, operation: &str) -> Linearity {
        // Division is deterministic and idempotent - safe for multi-shot
        // All operations can be resumed multiple times without side effects
        match operation {
            "div" | "mod" | "div_checked" | "mod_checked" => Linearity::MultiShot,
...
mod tests {
    ...
    fn test_div_with_ints() {
        ...
        let result = handler.handle("div", &[Value::Int(10), Value::Int(4)], cont(), &mut state);
    ...
    fn test_rem_basic() {
        ...
        let result = handler.handle("rem", &[Value::Int(10), Value::Int(3)], cont(), &mut state);
    ...
    fn test_rem_by_zero() {
        ...
        let result = handler.handle("rem", &[Value::Int(10), Value::Int(0)], cont(), &mut state);
