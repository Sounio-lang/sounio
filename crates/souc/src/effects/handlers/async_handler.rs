...
impl AsyncHandler {
    ...
    fn handle_spawn(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        if !Self::is_function(&args[0]) {
            return HandlerResult::Abort(HandlerError::new(
                "Async",
                "spawn",
                format!("expected Function for task, got {:?}", args[0].type_name()),
            ));
        }
    ...
    fn handle_await(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        if matches!(
            state.named_state.get(&cancelled_key),
        ...
        if state.named_state.contains_key(&future_key) {
            ...
            state.named_state.insert(
                ...
                Value::Tuple(vec![Value::String("await".into()), Value::Int(future_id)]),
            );
    ...
    fn handle_join(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        let futures_rc = match &args[0] {
            ...
            _ => {
                return HandlerResult::Abort(HandlerError::new(
                    "Async",
                    "join",
                    format!("expected Array of FutureIds, got {:?}", args[0].type_name()),
                ));
            }
        };
        ...
        if pending.is_empty() {
            ...
            state.named_state.insert(
                Self::suspend_key(suspension_id.0),
                Value::Tuple(vec![Value::String("join".into()), args[0].clone()]),
            );
            HandlerResult::Suspend(suspension_id)
        }
    }
    ...
    fn handle_select(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        state.named_state.insert(
            Self::suspend_key(suspension_id.0),
            Value::Tuple(vec![Value::String("select".into()), args[0].clone()]),
        );
        HandlerResult::Suspend(suspension_id)
    }
    ...
    fn handle_cancel(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        let future_id = match &args[0] {
            ...
            _ => {
                return HandlerResult::Abort(HandlerError::new(
                    "Async",
                    "cancel",
                    format!("expected FutureId (I64), got {:?}", args[0].type_name()),
                ));
            }
        };
        ...
        {
            // Mark as cancelled
            state.named_state.insert(cancelled_key, Value::Bool(true));
...
impl HandlerCapability for AsyncHandler {
    ...
    fn operation_linearity(&self, operation: &str) -> crate::effects::linearity::Linearity {
        ...
        // Async operations create/modify async state - mostly one-shot
        match operation {
            "spawn" | "cancel" | "yield" | "sleep" => Linearity::ExactlyOnce,
            // await, join, select consume futures - one-shot
            "await" | "join" | "select" => Linearity::ExactlyOnce,
            // Status queries are safe for multi-shot
            "is_ready" => Linearity::MultiShot,
