...
impl ConfidenceModifier {
    ...
    pub fn to_transformation(&self, effect: &str, operation: &str) -> Transformation {
        ...
        Transformation::new(&name, TransformationKind::ExternalCall).with_confidence(self.factor)
    }
}
...
impl EpistemicImpactRegistry {
    ...
    pub fn new() -> Self {
        ...
        registry.register(
            "Epistemic",
            "revise",
            ConfidenceModifier::with_provenance("revised"),
        );
...
mod tests {
    ...
    fn test_modifier_with_bounds() {
        let m = ConfidenceModifier::with_factor(0.5)
            .with_min(0.3)
            .with_max(0.8);
