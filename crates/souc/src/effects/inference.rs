...
impl TypeInfo {
    ...
    pub fn type_name(ty: &Type) -> Option<String> {
        match ty {
            ...
            Type::Array { element, .. } => Self::type_name(element).map(|n| format!("[{}]", n)),
...
impl<'a> EffectChecker<'a> {
    ...
    fn is_refutable_pattern(&self, pattern: &Pattern) -> bool {
        match pattern {
            ...
            Pattern::Tuple(elements) => elements.iter().any(|p| self.is_refutable_pattern(p)),
            ...
            Pattern::Or(patterns) => patterns.iter().all(|p| self.is_refutable_pattern(p)),
        }
    }
                    ...
                    if let Expr::Closure { .. } = arg {
                        if !arg_effects.is_pure() {
                            ...
                            for effect_name in &arg_effects.effects {
                                if !self.effect_sources.contains_key(effect_name) {
                                    self.effect_sources
                                        .insert(effect_name.clone(), self.expr_span_from_id(*id));
                                }
                            }
                        }
                    }
    ...
    fn get_callee_name(&self, callee: &Expr) -> Option<String> {
        match callee {
            Expr::Path { path, .. } if path.is_simple() => Some(path.segments[0].clone()),
            _ => None,
        }
    }
                ...
                if let Some(type_info) = self.type_info {
                    ...
                    if let Some(receiver_type) = self.infer_receiver_type_name(inner_receiver) {
                        if let Some(return_ty) =
                            type_info.get_method_return_type(&receiver_type, method)
                        {
        ...
        match &self.kind {
            EffectErrorKind::UndeclaredEffect { effect, source } => {
                ...
                match source {
                    ...
                    EffectSource::HigherOrderFunction {
                        hof,
                        closure_effect,
                    } => {
                        write!(
                            f,
                            " (from {} with effectful closure having {})",
                            hof, closure_effect
                        )?;
                    }
            ...
            EffectErrorKind::UnhandledEffect { effect } => {
                ...
                write!(
                    f,
                    "\n  help: wrap in a handler or propagate with `with {}`",
                    effect
                )
            }
            ...
            EffectErrorKind::EffectfulClosureArg { effect, hof_name } => {
                write!(
                    f,
                    "closure passed to `{}` has effect `{}`",
                    hof_name, effect
                )?;
                write!(
                    f,
                    "\n  help: the enclosing function must declare `with {}`",
                    effect
                )
            }
