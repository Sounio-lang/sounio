...
impl Interval {
    ...
    pub fn mul(&self, other: &Self) -> Self {
        match (self, other) {
            ...
            (Interval::Range { lo: l1, hi: h1 }, Interval::Range { lo: l2, hi: h2 }) => {
                ...
                let lo = corners.iter().cloned().min().unwrap_or(Bound::NegInf);
                let hi = corners.iter().cloned().max().unwrap_or(Bound::PosInf);
                Interval::new(lo, hi)
            }
        }
    }
    ...
    pub fn lt(&self, other: &Self) -> BoolDomain {
        match (self, other) {
            ...
            (Interval::Range { hi: h1, .. }, Interval::Range { lo: l2, .. }) => {
                if *h1 < *l2 {
                    BoolDomain::True
    ...
    pub fn assume_lt(&self, bound: i64) -> Self {
        match self {
            Interval::Bottom => Interval::Bottom,
            Interval::Range { lo, .. } => Interval::new(*lo, Bound::Finite(bound - 1)).meet(self),
        }
    }
    ...
    pub fn assume_le(&self, bound: i64) -> Self {
        match self {
            Interval::Bottom => Interval::Bottom,
            Interval::Range { lo, .. } => Interval::new(*lo, Bound::Finite(bound)).meet(self),
        }
    }
    ...
    pub fn assume_gt(&self, bound: i64) -> Self {
        match self {
            Interval::Bottom => Interval::Bottom,
            Interval::Range { hi, .. } => Interval::new(Bound::Finite(bound + 1), *hi).meet(self),
        }
    }
    ...
    pub fn assume_ge(&self, bound: i64) -> Self {
        match self {
            Interval::Bottom => Interval::Bottom,
            Interval::Range { hi, .. } => Interval::new(Bound::Finite(bound), *hi).meet(self),
        }
    }
}
