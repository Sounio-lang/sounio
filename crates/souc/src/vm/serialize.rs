...
impl Opcode {
    fn from_u8(v: u8) -> Option<Self> {
        match v {
            ...
            0x16 => Some(Opcode::ToInt),
            0x17 => Some(Opcode::ToFloat),
            0x18 => Some(Opcode::ToBool),
            0x20 => Some(Opcode::Eq),
            0x21 => Some(Opcode::Ne),
            0x22 => Some(Opcode::Lt),
            0x23 => Some(Opcode::Le),
            ...
            0x36 => Some(Opcode::BitAnd),
            0x37 => Some(Opcode::BitOr),
...
impl<W: Write> BytecodeSerializer<W> {
    ...
    fn write_instruction(&mut self, instr: &Bytecode) -> Result<(), SerializeError> {
        match instr {
            ...
            Bytecode::ToInt => self.writer.write_all(&[Opcode::ToInt as u8])?,
            Bytecode::ToFloat => self.writer.write_all(&[Opcode::ToFloat as u8])?,
            Bytecode::ToBool => self.writer.write_all(&[Opcode::ToBool as u8])?,
            ...
            Bytecode::BitAnd => self.writer.write_all(&[Opcode::BitAnd as u8])?,
            Bytecode::BitOr => self.writer.write_all(&[Opcode::BitOr as u8])?,
            Bytecode::Jump(addr) => {
...
impl<R: Read> BytecodeDeserializer<R> {
    ...
    fn read_instruction(&mut self) -> Result<Bytecode, SerializeError> {
        ...
        match opcode {
            ...
            Opcode::ToInt => Ok(Bytecode::ToInt),
            Opcode::ToFloat => Ok(Bytecode::ToFloat),
            Opcode::ToBool => Ok(Bytecode::ToBool),
            ...
            Opcode::BitAnd => Ok(Bytecode::BitAnd),
            Opcode::BitOr => Ok(Bytecode::BitOr),
