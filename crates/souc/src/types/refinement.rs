...
impl RefinementChecker {
    ...
    pub fn check(&self, pred: &Predicate) -> RefinementResult {
        ...
        let to_check = SmtFormula::Implies(Box::new(context_formula), Box::new(formula));
        ...
        match pred {
            ...
            Predicate::Compare(op, left, right) => {
                ...
                match op {
                    ...
                    CompareOp::Ne => {
                        SmtFormula::Not(Box::new(SmtFormula::Eq(Box::new(l), Box::new(r))))
                    }
...
mod tests {
    ...
    fn test_smt_subtype_check() {
        ...
        assert!(
            !result.is_invalid(),
            "Positive should be subtype of non-negative"
        );
    }
