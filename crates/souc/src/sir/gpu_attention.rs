...
impl GpuAttentionScore {
    ...
    pub fn weighted_sum(&self, config: &GpuAttentionConfig) -> f64 {
        ...
        let cost =
            config.w_register * self.register_pressure + config.w_occupancy * self.occupancy_impact;
...
mod tests {
    ...
    fn test_occupancy_for_registers() {
        ...
        assert!(
            (occ_32 - 1.0).abs() < 0.01,
            "Expected ~100% occupancy at 32 regs"
        );
    ...
    fn test_optimal_register_target() {
        ...
        assert!(
            target_50 >= 32 && target_50 <= 64,
            "Expected ~64 regs for 50% occupancy"
        );
        ...
        assert!(
            target_25 > target_50,
            "Lower occupancy should allow more registers"
        );
    }
    ...
    fn test_allocation_metrics() {
        ...
        assert!(
            quality > 1.0,
            "Quality should be good with few high-confidence spills"
        );
