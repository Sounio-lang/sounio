...
pub use super::alloc_policy::{AllocationMetrics, AttentionConfig};
...
impl X86Reg {
    ...
    /// Does this register need any REX prefix to access its low byte?
    /// In x86-64, without REX: encodings 4-7 are AH,CH,DH,BH (high bytes)
    /// With any REX prefix: encodings 4-7 are SPL,BPL,SIL,DIL (low bytes)
    /// So RSP/RBP/RSI/RDI (4-7) need a REX prefix for byte operations
    pub fn needs_rex_for_byte_reg(&self) -> bool {
        let enc = *self as u8;
        // R8-R15 need REX.B, RSP/RBP/RSI/RDI need any REX for low-byte access
        (enc >= 4 && enc < 16) && !self.is_xmm()
    }
...
impl RegisterAllocator {
    ...
    fn infer_instruction_type(&self, inst: &SirInst, _func: &SirFunction) -> SirType {
        match inst {
            ...
            SirInst::Memory(MemoryOp::ExtractField { field_ty, .. }) => field_ty.clone(),
    ...
    /// If a value crosses a call, it MUST be spilled because there are no
    /// callee-saved XMM registers to use.
    fn allocate_xmm_reg(&mut self, crosses_call: bool) -> Option<X86Reg> {
        if crosses_call {
            // No callee-saved XMM registers exist in System V ABI.
            // Force spill so the value survives across function calls.
            return None;
        }
...
pub struct X86_64Emitter {
    ...
    spill_ops_by_position:
        std::collections::HashMap<usize, Vec<crate::backend::native::alloc::SpillReloadOp>>,
    ...
    /// Epistemic mode for Knowledge<T> operations
...
impl X86_64Emitter {
    pub fn new() -> Self {
        Self {
            ...
            current_block: None,
            epistemic_mode: EpistemicMode::Compact, // Default to compact for performance
        }
    }

    /// Set epistemic mode for Knowledge<T> operations
    pub fn set_epistemic_mode(&mut self, mode: EpistemicMode) {
        self.epistemic_mode = mode;
    }
    ...
    pub fn set_external_alloc_result(
        &mut self,
        result: &crate::backend::native::alloc::AllocResult,
    ) {
                ...
                if let Some(reg) = interval.assigned {
                    ...
                    if let Some(x86_reg) = self.virt_reg_to_x86_reg(reg) {
                        ...
                        self.register_allocator
                            .value_to_interval
                            .insert(value_id, interval_idx);

                        // Create the LiveInterval with the assigned register
                        let has_prov = !matches!(
                            interval.epistemic.provenance,
                            crate::backend::native::metrics::Provenance::Unknown
                        );
    ...
    fn virt_reg_to_x86_reg(
        &self,
        phys_reg: crate::backend::native::alloc::PhysReg,
    ) -> Option<X86Reg> {
    ...
    fn emit_movsd_mem_disp_r64(&mut self, base: X86Reg, disp: i32, src: X86Reg) {
        self.emit_byte(0xF2);
        // REX prefix: need REX.B for R8-R15 base, REX.R for XMM8-15 src
        let rex_b = base.needs_rex();
        let rex_r = src.needs_rex();
        if rex_b || rex_r {
            let rex = 0x40 | if rex_r { 0x04 } else { 0 } | if rex_b { 0x01 } else { 0 };
            self.emit_byte(rex);
        }
        self.emit_bytes(&[0x0F, 0x11]);

        // RSP and R12 require SIB byte (their encoding 0b100 means "SIB follows")
        let needs_sib = (base.encoding() & 0x7) == 0x4;
        // RBP/R13 with disp=0 needs mod=01 with disp8=0 (mod=00 rm=101 means RIP-relative)
        let rbp_base = (base.encoding() & 0x7) == 0x5;
        ...
        if disp == 0 && !rbp_base {
            ...
            if needs_sib {
                self.emit_byte(0x24); // SIB: scale=0, index=RSP(none), base=RSP
            }
        } else if (disp >= -128 && disp <= 127) || (disp == 0 && rbp_base) {
            self.emit_modrm(
                0b01,
                (src as u8) & 0x7,
                if needs_sib {
    ...
    fn emit_movsd_r64_mem_disp(&mut self, dst: X86Reg, base: X86Reg, disp: i32) {
        self.emit_byte(0xF2);
        // REX prefix: need REX.B for R8-R15 base, REX.R for XMM8-15 dst
        let rex_b = base.needs_rex();
        let rex_r = dst.needs_rex();
        if rex_b || rex_r {
            let rex = 0x40 | if rex_r { 0x04 } else { 0 } | if rex_b { 0x01 } else { 0 };
            self.emit_byte(rex);
        }
    ...
    fn emit_setcc(&mut self, cc: CondCode, dst: X86Reg) {
        // Must emit REX prefix for:
        // - R8-R15 (need REX.B bit for extended register)
        // - RSP/RBP/RSI/RDI (need any REX to access SPL/BPL/SIL/DIL instead of AH/CH/DH/BH)
        // Note: emit_rex() skips 0x40, but we need it for byte register access
        if dst.needs_rex_for_byte_reg() {
            let rex = 0x40 | if dst.needs_rex() { 0x01 } else { 0x00 };
            self.emit_byte(rex);
        }
    ...
    /// Reload a float value from spill slot into XMM register
    fn emit_reload_float(&mut self, xmm_reg: X86Reg, spill_slot: i32) {
        // Always use MOVSD for float reload
        self.emit_movsd_rbp_mem(xmm_reg, spill_slot);
    }
    ...
    /// Get a value into an XMM register (for float values)
    fn get_value_reg_float(&mut self, value: ValueId, scratch: X86Reg) -> X86Reg {
        if let Some(reg) = self.register_allocator.get_reg(value) {
            reg
        } else if let Some(slot) = self.register_allocator.get_spill_slot(value) {
            // Reload float from spill slot using movsd
            self.emit_reload_float(scratch, slot);
            scratch
        } else {
            // Value not found - return scratch register as fallback
            scratch
        }
    }
...
impl X86_64Emitter {
    ...
    pub fn emit_module_with_alloc_results(
        &mut self,
        module: &SirModule,
        alloc_results: Option<
            std::collections::HashMap<
                crate::sir::values::FuncId,
                crate::backend::native::alloc::AllocResult,
            >,
        >,
        ...
        for block in &func.blocks {
            ...
            for inst in &block.instructions {
                    ...
                    .get(&self.current_instruction_pos)
                {
                    ...
                    for op in ops {
                        match op {
                            crate::backend::native::alloc::SpillReloadOp::Spill {
                                reg,
                                slot,
                                ..
                            } => {
                            ...
                            crate::backend::native::alloc::SpillReloadOp::Reload {
                                reg,
                                slot,
                                ..
                            } => {
...
impl X86_64Emitter {
    ...
    fn emit_instruction(
        ...
        match &inst.inst {
            ...
            SirInst::Memory(mem_op) => {
                self.emit_memory_op(inst.result, mem_op, func)?;
            }
            SirInst::Select {
                cond,
                then_val,
    ...
    /// For structs with float fields: allocates stack space and uses movsd
    ...
    fn emit_unary_float(
        ...
        let allocated_dst = self.register_allocator.get_reg(result);

        // Float operations must use XMM registers for computation.
        // If the allocator assigned a GPR, we compute in XMM0 then move to GPR.
        let dst = match allocated_dst {
            Some(reg) if reg.is_xmm() => reg,
            _ => X86Reg::XMM0, // Compute in XMM0, then move if needed
        };

        match op {
            UnaryFloatOp::Neg => {
                // Negate by XORing with sign bit mask (0x8000000000000000)
                // Use a scratch register that doesn't conflict with dst or src.
                // First move src to dst, then XOR with mask in scratch.
                if dst != src {
                    self.emit_movsd_rr(dst, src);
                }
                // Choose scratch register that doesn't conflict with dst
                let scratch = if dst == X86Reg::XMM14 {
                    X86Reg::XMM13
                } else {
                    X86Reg::XMM14
                };
                self.emit_load_f64_const(scratch, 0x8000_0000_0000_0000);
                self.emit_xorpd(dst, scratch);
            }
            UnaryFloatOp::Abs => {
                // Absolute value by ANDing with mask that clears sign bit (0x7FFFFFFFFFFFFFFF)
                // First move src to dst, then AND with mask in scratch.
                if dst != src {
                    self.emit_movsd_rr(dst, src);
                }
                // Choose scratch register that doesn't conflict with dst
                ...
                self.emit_load_f64_const(scratch, 0x7FFF_FFFF_FFFF_FFFF);
                self.emit_andpd(dst, scratch);
            }
            UnaryFloatOp::Sqrt => {
        ...
        // If allocator assigned a GPR instead of XMM, move result from XMM to GPR
        if let Some(gpr) = allocated_dst {
            if !gpr.is_xmm() {
                // Move float bits from XMM0 to the GPR
                self.emit_movq_r64_xmm(gpr, dst);
            }
        }
    ...
    ///
    /// # Modes
    /// - **Full**: Calls runtime functions for GUM-compliant propagation with confidence intervals
    /// - **Compact**: Uses inline assembly with simplified propagation (faster)
    /// - **Erased**: Operations become no-ops (zero overhead)
    fn emit_epistemic_op(
        ...
        // Handle mode-based dispatch for runtime calls
        if self.epistemic_mode == EpistemicMode::Full {
            return self.emit_epistemic_op_runtime(result, op, func);
        }

        // Erased mode: epistemic operations become no-ops (just compute value)
        if self.epistemic_mode == EpistemicMode::Erased {
            return self.emit_epistemic_op_erased(result, op, func);
        }

        // Compact mode: inline assembly with simplified propagation
        match op {
            EpistemicOp::Create { value, confidence } => {
                ...
                if let SirType::Epistemic(inner_ty) = &result_ty {
                    ...
                    let is_float = matches!(
                        &**inner_ty,
                        SirType::Scalar(ScalarType::F32) | SirType::Scalar(ScalarType::F64)
                    );
                    ...
                    return Err(EmitError::UnsupportedInstruction(format!(
                        "Create epistemic value but result type is not epistemic: {:?}",
                        result_ty
                    )));
                }
            }
    ...
    /// Emit epistemic operations using runtime function calls (Full mode)
    ///
    /// Calls C-compatible runtime functions for GUM-compliant uncertainty propagation
    /// with full confidence intervals and provenance tracking.
    fn emit_epistemic_op_runtime(
        &mut self,
        result: ValueId,
        op: &super::ops::EpistemicOp,
        func: &SirFunction,
        ...
        match op {
            EpistemicOp::PropagateAdd { conf_a, conf_b } => {
                // Call: sounio_epistemic_add_full(a_ptr, b_ptr, result_ptr)
                // Setup arguments per System V ABI: RDI, RSI, RDX
                let a_reg = self.get_value_reg(*conf_a, X86Reg::RDI);
                let b_reg = self.get_value_reg(*conf_b, X86Reg::RSI);
                if a_reg != X86Reg::RDI {
                ...
                // Allocate space for result and pass pointer in RDX
                let result_reg = self
                    .register_allocator
                    .get_reg(result)
                    .unwrap_or(X86Reg::RAX);
                self.emit_sub_rsp_imm(64); // Full knowledge size
                self.emit_mov_rr(X86Reg::RDX, X86Reg::RSP);

                self.emit_call_extern("sounio_epistemic_add_full");

                // Result pointer is on stack
                self.emit_mov_rr(result_reg, X86Reg::RSP);
                self.store_value(result, result_reg);
            }

            EpistemicOp::PropagateSub { conf_a, conf_b } => {
                // Call: sounio_epistemic_sub_full(a_ptr, b_ptr, result_ptr)
                let a_reg = self.get_value_reg(*conf_a, X86Reg::RDI);
                let b_reg = self.get_value_reg(*conf_b, X86Reg::RSI);
                if a_reg != X86Reg::RDI {
                    self.emit_mov_rr(X86Reg::RDI, a_reg);
                ...
                let result_reg = self
                    ...
                    .get_reg(result)
                    .unwrap_or(X86Reg::RAX);
                self.emit_sub_rsp_imm(64);
                self.emit_mov_rr(X86Reg::RDX, X86Reg::RSP);

                self.emit_call_extern("sounio_epistemic_sub_full");
            ...
            EpistemicOp::PropagateMul {
                ...
                // Call: sounio_epistemic_mul_full(a_ptr, b_ptr, result_ptr)
                let a_reg = self.get_value_reg(*conf_a, X86Reg::RDI);
                let b_reg = self.get_value_reg(*conf_b, X86Reg::RSI);
                if a_reg != X86Reg::RDI {
                    self.emit_mov_rr(X86Reg::RDI, a_reg);
                ...
                self.emit_call_extern("sounio_epistemic_mul_full");
            ...
            EpistemicOp::PropagateDiv {
                ...
                // Call: sounio_epistemic_div_full(a_ptr, b_ptr, result_ptr)
                let a_reg = self.get_value_reg(*conf_a, X86Reg::RDI);
                let b_reg = self.get_value_reg(*conf_b, X86Reg::RSI);
                if a_reg != X86Reg::RDI {
                    self.emit_mov_rr(X86Reg::RDI, a_reg);
                ...
                self.emit_call_extern("sounio_epistemic_div_full");
            ...
            EpistemicOp::Meet { conf_a, conf_b } => {
                // Call: sounio_epistemic_meet(a_conf, b_conf) -> result_conf
                let a_reg = self.get_value_reg(*conf_a, X86Reg::XMM0);
                let b_reg = self.get_value_reg(*conf_b, X86Reg::XMM1);
                if a_reg != X86Reg::XMM0 {
                    self.emit_movsd_rr(X86Reg::XMM0, a_reg);
                }
                if b_reg != X86Reg::XMM1 {
                    self.emit_movsd_rr(X86Reg::XMM1, b_reg);
                }

                self.emit_call_extern("sounio_epistemic_meet");
            ...
            EpistemicOp::Join { conf_a, conf_b } => {
                // Call: sounio_epistemic_join(a_conf, b_conf) -> result_conf
                let a_reg = self.get_value_reg(*conf_a, X86Reg::XMM0);
                let b_reg = self.get_value_reg(*conf_b, X86Reg::XMM1);
                if a_reg != X86Reg::XMM0 {
                    self.emit_movsd_rr(X86Reg::XMM0, a_reg);
                ...
                self.emit_call_extern("sounio_epistemic_join");
            ...
            // For operations not yet implemented in runtime, fall back to compact mode
            _ => {
                // Temporarily use compact mode for remaining operations
                let save_mode = self.epistemic_mode;
                self.epistemic_mode = EpistemicMode::Compact;
                let inner_result = self.emit_epistemic_op(result, op, func);
                self.epistemic_mode = save_mode;
                return inner_result;
            }
        }
    ...
    /// Emit epistemic operations in erased mode (zero overhead)
    ///
    /// In erased mode, epistemic operations become no-ops - we just compute the value
    /// and ignore confidence tracking entirely.
    fn emit_epistemic_op_erased(
        &mut self,
        result: ValueId,
        op: &super::ops::EpistemicOp,
        func: &SirFunction,
        ...
        match op {
            EpistemicOp::Create {
                ...
                // In erased mode, just pass through the value
                let value_reg = self.get_value_reg(*value, X86Reg::RAX);
                let result_reg = self
                    .register_allocator
                    .get_reg(result)
                ...
                if value_reg != result_reg {
                    self.emit_mov_rr(result_reg, value_reg);
                }
                self.store_value(result, result_reg);
            }

            EpistemicOp::ExtractValue(epistemic_val) => {
                // In erased mode, the epistemic value IS the value
                let value_reg = self.get_value_reg(*epistemic_val, X86Reg::RAX);
                let result_reg = self
                    .register_allocator
                    .get_reg(result)
                    .unwrap_or(X86Reg::RAX);
            ...
            EpistemicOp::ExtractConfidence(_) => {
                // In erased mode, confidence is always 1.0
                let result_reg = self
                    .register_allocator
                    .get_reg(result)
                    .unwrap_or(X86Reg::XMM0);
                ...
                // Load constant 1.0
                self.emit_load_f64_constant(result_reg, 1.0);
                self.store_value(result, result_reg);
            }

            // Propagation operations become no-ops in erased mode
            EpistemicOp::PropagateAdd { .. }
            | EpistemicOp::PropagateSub { .. }
            | EpistemicOp::PropagateMul { .. }
            | EpistemicOp::PropagateDiv { .. }
            ...
            | EpistemicOp::Join { .. }
            | EpistemicOp::CondPropagate { .. } => {
                // In erased mode, confidence propagation always returns 1.0
                let result_reg = self
                    .register_allocator
                    .get_reg(result)
                    .unwrap_or(X86Reg::XMM0);
    ...
    /// Helper: Load f64 constant into XMM register
    fn emit_load_f64_constant(&mut self, dst: X86Reg, value: f64) {
        // Load constant using MOV to GPR then MOVQ to XMM
        let bits = value.to_bits();
        // mov rax, imm64 (10 bytes)
        self.emit_byte(0x48); // REX.W
        self.emit_byte(0xB8); // MOV RAX, imm64
        self.emit_bytes(&bits.to_le_bytes());

        // MOVQ xmm, rax: 66 48 0F 6E /r
        self.emit_bytes(&[0x66, 0x48, 0x0F, 0x6E]);
        self.emit_modrm(0b11, (dst as u8) & 0x7, 0); // RAX = 0
    }
    ...
    fn get_value_unit(
        ...
        value_id: ValueId,
        func: &SirFunction,
    ) -> Option<super::values::PhysicalUnit> {
        // Try to find value in function parameters
    ...
    fn units_compatible(
        &self,
        lhs: &super::values::PhysicalUnit,
        rhs: &super::values::PhysicalUnit,
    ) -> bool {
    ...
    fn infer_instruction_type(&self, inst: &super::ops::SirInst) -> SirType {
        match inst {
            ...
            super::ops::SirInst::Memory(super::ops::MemoryOp::ExtractField {
                field_ty, ..
            }) => field_ty.clone(),
    ...
    fn emit_scientific_op(
        ...
        match op {
                ...
                match method {
                    ...
                    OdeMethod::DoPri5 | OdeMethod::CashKarp | OdeMethod::BDF | OdeMethod::LSODA => {
                        ...
                        if state_dim <= 0 || state_dim > 1_000_000 {
                            return Err(EmitError::InvalidInput(format!(
                                "Invalid ODE state dimension: {}",
                                state_dim
                            )));
                        }
            ...
            _ => {
                return Err(EmitError::UnsupportedInstruction(format!(
                    "Scientific operation {:?} not yet implemented",
                    op
                )));
            }
        }
    ...
    fn emit_builtin_print(
        ...
        match arg_ty {
            ...
            SirType::Scalar(ScalarType::F64) => {
                // NOTE: XMM registers are caller-saved, so if there were any function calls
                // between the value definition and here, the register may be clobbered.
                // We must reload from the spill slot if available.
                eprintln!(
                    "[emit_builtin_print F64] arg={:?}, spill_slot={:?}, reg={:?}",
                    arg,
                    self.register_allocator.get_spill_slot(arg),
                    self.register_allocator.get_reg(arg)
                );
                if let Some(slot) = self.register_allocator.get_spill_slot(arg) {
                    // Value is spilled - reload to XMM0
                    eprintln!("[emit_builtin_print F64] RELOADING from slot {}", slot);
                    self.emit_reload_float(X86Reg::XMM0, slot);
                } else if let Some(reg) = self.register_allocator.get_reg(arg) {
                    // Value in register - need to check if it's XMM or GPR
                    eprintln!(
                        "[emit_builtin_print F64] USING reg {:?}, is_xmm={}",
                        reg,
                        reg.is_xmm()
                    );
                    if reg != X86Reg::XMM0 {
                        if reg.is_xmm() {
                            // XMM to XMM move
                            self.emit_movsd_rr(X86Reg::XMM0, reg);
                        } else {
                            // GPR to XMM move (float stored as bits in GPR)
                            self.emit_movq_xmm_r64(X86Reg::XMM0, reg);
                        }
                    }
                } else {
                    // No register or spill slot - fallback
                    eprintln!(
                        "[emit_builtin_print] WARNING: f64 value {:?} has no reg or spill slot!",
                        arg
                    );
                }
                self.emit_call_extern("_demetrios_print_f64");
    ...
    fn emit_memory_op(
        ...
        match op {
            ...
            MemoryOp::Store { ptr, val, .. } => {
                ...
                // Get the type of the value being stored to determine which instruction to use
                let val_ty = self.get_value_type(*val, func);

                if val_ty.is_float() {
                    // For float stores, use MOVSD with XMM register
                    let val_reg = self.get_value_reg_float(*val, X86Reg::XMM0);
                    // MOVSD [ptr], xmm
            ...
            MemoryOp::GetElementPtr { ptr, ty, indices } => {
                ...
                if let Some(result_id) = result {
                    ...
                    // Store the computed pointer so subsequent instructions can find it
            ...
            MemoryOp::ExtractField {
                ptr,
                field_offset,
                field_ty,
            } => {
                // Extract field from struct: compute ptr + field_offset and load
                if let Some(result_id) = result {
                    let base_reg = self.get_value_reg(*ptr, X86Reg::R10);
                    let dst_reg = self.register_allocator.get_reg(result_id).unwrap_or(
                        if field_ty.is_float() {
                            X86Reg::XMM0
                        } else {
                            X86Reg::RAX
...
mod tests {
    ...
    fn test_live_interval_overlap() {
        ...
        assert!(
            iv1.overlaps(&iv2),
            "Intervals [0,5) and [3,8) should overlap"
        );
        assert!(
            iv2.overlaps(&iv1),
            "Intervals [3,8) and [0,5) should overlap"
        );
        ...
        assert!(
            !iv3.overlaps(&iv4),
            "Intervals [0,3) and [5,8) should not overlap"
        );
        assert!(
            !iv4.overlaps(&iv3),
            "Intervals [5,8) and [0,3) should not overlap"
        );
    }
