    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Layout::Swizzled {
                ...
                write!(f, "Swizzled({}x{}, xor={:#x})", block_m, block_n, xor_mask)
            }
            Layout::Custom {
                stride_inner,
                stride_outer,
...
impl RngMode {
    ...
    pub fn is_deterministic(&self) -> bool {
        matches!(
            self,
            RngMode::CounterBased | RngMode::Deterministic | RngMode::Philox | RngMode::Threefry
        )
    }
...
impl CostEnvelope {
    ...
    pub fn requires_determinism(&self) -> bool {
        let reduction_deterministic = self
            ...
            .unwrap_or(false);
        let rng_deterministic = self.rng_mode.map(|m| m.is_deterministic()).unwrap_or(false);
...
mod tests {
    ...
    fn test_kernel_variant_set() {
        let set = KernelVariantSet::new("matmul")
            ...
            .add_named_variant("fp32", CostEnvelope::new().with_precision(Precision::Fp32));
