...
impl VariantSpace {
    ...
    pub fn active_dimensions(&self) -> usize {
        let mut count = 0;
        if self.tile_options.len() > 1 {
            count += 1;
        }
        if self.unroll_options.len() > 1 {
            count += 1;
        }
        if self.layout_options.len() > 1 {
            count += 1;
        }
        if self.vector_widths.len() > 1 {
            count += 1;
        }
        if self.shared_mem_options.len() > 1 {
            count += 1;
        }
        if self.precision_options.len() > 1 {
            count += 1;
        }
        ...
        if self.coarsening_factors.len() > 1 {
            count += 1;
        }
        if self.prefetch_distances.len() > 1 {
            count += 1;
        }
        count
    }
    ...
    pub fn for_stencil() -> Self {
        Self {
            tile_options: vec![(16, 16, 1), (32, 32, 1), (32, 16, 1), (16, 32, 1)],
            unroll_options: vec![1, 2, 4],
            layout_options: vec![
                Layout::RowMajor,
...
impl VariantGenerator {
    ...
    pub fn generate_all(&mut self, kernel_name: &str) -> Vec<KernelVariant> {
        ...
        let tiles = if self.space.tile_options.is_empty() {
            vec![(16, 16, 16)]
        } else {
            self.space.tile_options.clone()
        };
        let unrolls = if self.space.unroll_options.is_empty() {
            vec![1]
        } else {
            self.space.unroll_options.clone()
        };
        let layouts = if self.space.layout_options.is_empty() {
            vec![Layout::RowMajor]
        } else {
            self.space.layout_options.clone()
        };
        let vectors = if self.space.vector_widths.is_empty() {
            vec![1]
        } else {
            self.space.vector_widths.clone()
        };
        let shareds = if self.space.shared_mem_options.is_empty() {
            vec![0]
        } else {
            self.space.shared_mem_options.clone()
        };
        let precisions = if self.space.precision_options.is_empty() {
            vec![Precision::F32]
        } else {
            self.space.precision_options.clone()
        };
        let blocks = if self.space.block_sizes.is_empty() {
            vec![256]
        } else {
            self.space.block_sizes.clone()
        };
        let pipelines = if self.space.pipeline_stages.is_empty() {
            vec![1]
        } else {
            self.space.pipeline_stages.clone()
        };
        let coarsens = if self.space.coarsening_factors.is_empty() {
            vec![1]
        } else {
            self.space.coarsening_factors.clone()
        };
        let prefetches = if self.space.prefetch_distances.is_empty() {
            vec![0]
        } else {
            self.space.prefetch_distances.clone()
        };
                                    ...
                                    for &pipeline in &pipelines {
                                        for &coarsen in &coarsens {
                                            for &prefetch in &prefetches {
                                                ...
                                                let mut variant =
                                                    KernelVariant::new(id, kernel_name);
    ...
    pub fn generate_sobol(&mut self, kernel_name: &str, n: usize) -> Vec<KernelVariant> {
        ...
        for i in 0..n {
            ...
            if !self.space.shared_mem_options.is_empty() {
                let idx =
                    self.sobol_index(gray, dim_offset, dims, self.space.shared_mem_options.len());
                variant.shared_mem_bytes = self.space.shared_mem_options[idx];
                dim_offset += 1;
            }
            ...
            if !self.space.precision_options.is_empty() {
                let idx =
                    self.sobol_index(gray, dim_offset, dims, self.space.precision_options.len());
                variant.precision = self.space.precision_options[idx];
                dim_offset += 1;
            }
            ...
            if !self.space.pipeline_stages.is_empty() {
                let idx =
                    self.sobol_index(gray, dim_offset, dims, self.space.pipeline_stages.len());
                variant.pipeline_stages = self.space.pipeline_stages[idx];
                dim_offset += 1;
            }
            ...
            if !self.space.coarsening_factors.is_empty() {
                let idx =
                    self.sobol_index(gray, dim_offset, dims, self.space.coarsening_factors.len());
                variant.coarsening_factor = self.space.coarsening_factors[idx];
                dim_offset += 1;
            }
            ...
            if !self.space.prefetch_distances.is_empty() {
                let idx =
                    self.sobol_index(gray, dim_offset, dims, self.space.prefetch_distances.len());
                variant.prefetch_distance = self.space.prefetch_distances[idx];
            }
...
impl VariantRegistry {
    ...
    pub fn get_measured_variants(
        &self,
        kernel_name: &str,
    ) -> Vec<(&KernelVariant, &PerformanceMeasurement)> {
        self.kernel_variants
            .get(kernel_name)
            .map(|ids| {
                ids.iter()
    ...
    pub fn prune_unmeasured(&mut self) {
        ...
        let measured_set: std::collections::HashSet<VariantId> =
            measured_ids.iter().copied().collect();
...
impl<'a> VariantSelector<'a> {
    ...
    pub fn select_best_score(&self, kernel_name: &str) -> Option<VariantId> {
        self.registry
            ...
            .max_by(|(_, m1), (_, m2)| {
                m1.composite_score()
                    .partial_cmp(&m2.composite_score())
    ...
    pub fn select_most_stable(&self, kernel_name: &str) -> Option<VariantId> {
        self.registry
            ...
            .into_iter()
            .max_by(|(_, m1), (_, m2)| {
                m1.epistemic_quality
                    .partial_cmp(&m2.epistemic_quality)
                    .unwrap_or(std::cmp::Ordering::Equal)
            })
            .map(|(v, _)| v.id)
    }
    ...
    pub fn select_within_threshold(
        &self,
        kernel_name: &str,
...
mod tests {
    ...
    fn test_variant_generator_lhs() {
        let space =
            VariantSpace::new().with_tile_options(vec![(16, 16, 16), (32, 32, 16), (64, 64, 16)]);
    ...
    fn test_generate_sobol() {
        ...
        let unique_configs: std::collections::HashSet<_> =
            variants.iter().map(|v| v.config_hash()).collect();
        assert!(
            unique_configs.len() >= 5,
            "Sobol should produce at least 5 unique configurations from 20 variants, got {}",
            unique_configs.len()
        );
    }
}
