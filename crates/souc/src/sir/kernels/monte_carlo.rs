    ...
    pub fn sir_type() -> SirType {
        SirType::Struct(
            StructType::new(vec![
                (
                    Some("position".into()),
                    SirType::Array(ArrayType::new(SirType::f64(), 3)),
                ),
                (
                    Some("velocity".into()),
                    SirType::Array(ArrayType::new(SirType::f64(), 3)),
...
impl MonteCarloKernel {
    ...
    pub fn initialize_from_prior(&mut self, rng_seed: u64) {
        ...
        for i in 0..self.config.num_particles {
            let position = [
                self.config.prior.position_mean[0]
                    + self.config.prior.position_std[0] * next_normal(),
                self.config.prior.position_mean[1]
                    + self.config.prior.position_std[1] * next_normal(),
                self.config.prior.position_mean[2]
                    + self.config.prior.position_std[2] * next_normal(),
            ];

            let velocity = [
                self.config.prior.velocity_mean[0]
                    + self.config.prior.velocity_std[0] * next_normal(),
                self.config.prior.velocity_mean[1]
                    + self.config.prior.velocity_std[1] * next_normal(),
                self.config.prior.velocity_mean[2]
                    + self.config.prior.velocity_std[2] * next_normal(),
            ];
    ...
    fn partition_particles(&mut self) {
        let (high, low) =
            partition_indices_by_confidence(&self.particles, self.config.confidence_threshold);
    ...
    pub fn apply_weights(&mut self, observation: [f64; 3], observation_confidence: f64) {
        ...
        for p in &mut self.particles {
            ...
            p.confidence =
                (p.confidence * 0.9 + match_factor * observation_confidence * 0.1).clamp(0.0, 1.0);
    ...
    pub fn resample(&mut self, rng_seed: u64) {
        ...
        let mut rng_state = rng_seed
            .wrapping_add(self.current_step as u64)
            .wrapping_add(42);
        rng_state ^= rng_state << 13;
        rng_state ^= rng_state >> 7;
        rng_state ^= rng_state << 17;
...
pub fn emit_monte_carlo_sir(config: &MonteCarloConfig) -> SirModule {
    ...
    let reduction_type = SirType::Struct(
        StructType::new(vec![
            (
                Some("mean_position".into()),
                SirType::Array(ArrayType::new(SirType::f64(), 3)),
            ),
            (
                Some("mean_velocity".into()),
                SirType::Array(ArrayType::new(SirType::f64(), 3)),
            ),
            (
                Some("position_variance".into()),
                SirType::Array(ArrayType::new(SirType::f64(), 3)),
            ),
            (
                Some("velocity_variance".into()),
                SirType::Array(ArrayType::new(SirType::f64(), 3)),
            ),
...
fn emit_reduction_function(module: &mut SirModule, config: &MonteCarloConfig) {
    let particle_ptr_ty = SirType::ptr(ParticleState::sir_type());
    let reduction_ptr_ty = SirType::ptr(SirType::Struct(
        StructType::new(vec![
            (
                Some("mean_position".into()),
                SirType::Array(ArrayType::new(SirType::f64(), 3)),
...
mod tests {
    ...
    fn test_monte_carlo_full_run() {
        ...
        let observations = vec![([0.0, 0.0, 0.0], 0.9), ([0.0, 0.0, 0.0], 0.9)];
