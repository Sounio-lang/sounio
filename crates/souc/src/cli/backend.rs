...
pub fn extract_build_args(arg_matches: &clap::ArgMatches) -> Result<BuildArgs, String> {
    ...
    if let Some(backend_str) = arg_matches.get_one::<String>("backend") {
        build_args.backend = backend_str
            .parse()
    ...
    if let Some(features_str) = arg_matches.get_one::<String>("target_cpu") {
        build_args.native_opts.cpu_features = features_str
            .split(',')
...
pub fn compile(args: &BuildArgs) -> CompileOutput {
    ...
    for input in &args.input {
        if !input.exists() {
            output
                .errors
                .push(format!("Input file not found: {}", input.display()));
            return output;
        }
    }
...
fn compile_native(args: &BuildArgs) -> Result<(PathBuf, Option<NativeMetrics>), String> {
    ...
    if args.native_opts.enable_thermal && max_temp_k > 350.0 {
        ...
        if args.verbose {
            println!(
                "   Recommended: V={:.3}V, f={:.2}GHz",
                new_voltage, new_freq
            );
        }
    }
    ...
    let alloc_metrics = if args.native_opts.alloc_strategy == AllocStrategy::Epistemic {
        ...
        for func in &sir_module.functions {
            ...
            if !result.is_successful() && args.verbose {
                println!(
                    "⚠️  Warning: Allocation for function '{}' had {} critical spills",
    ...
    if args.verbose {
        ...
        if let Some(ref metrics) = alloc_metrics {
            println!(
                "  Allocated: {}, Spilled: {}, Avg confidence: {:.3}",
                metrics.intervals_allocated, metrics.intervals_spilled, metrics.avg_confidence
            );
    ...
    if args.native_opts.alloc_strategy == AllocStrategy::Epistemic && alloc_results.is_empty() {
        if args.verbose {
            println!(
                "⚠️  Warning: Epistemic allocation produced no results, falling back to internal allocator"
            );
        }
    }
    ...
    match format {
        OutputFormat::Object => {
            ...
            let elf_bytes = backend
                .compile_to_object(&sir_module)
        ...
        OutputFormat::Executable | OutputFormat::SharedLib => {
            ...
            use crate::backend::native::linker::{LinkMode, Linker, LinkerConfig};
            ...
            linker
                .link(&input_refs, &output_path, link_mode)
    ...
    let metrics = NativeMetrics {
        intervals_allocated: alloc_metrics
            .as_ref()
            .map(|m| m.intervals_allocated)
            .unwrap_or(0),
        intervals_spilled: alloc_metrics
            .as_ref()
            .map(|m| m.intervals_spilled)
            .unwrap_or(0),
        avg_confidence_allocated: alloc_metrics
            .as_ref()
...
fn convert_sir_block_to_metrics_block(
    block: &crate::sir::blocks::BasicBlock,
) -> crate::backend::native::metrics::SirBlock {
    use crate::backend::native::metrics::{OpClass, SirBlock, SirInstruction};
    ...
    for inst in &block.instructions {
        ...
        let op_class = match &inst.inst {
            crate::sir::ops::SirInst::BinOp { op, .. } => match op {
                crate::sir::ops::ArithOp::Add | crate::sir::ops::ArithOp::Sub => {
                    OpClass::IntArithmetic
                }
                crate::sir::ops::ArithOp::Mul => OpClass::IntMultiply,
                crate::sir::ops::ArithOp::SDiv | crate::sir::ops::ArithOp::UDiv => {
                    OpClass::IntDivide
                }
                crate::sir::ops::ArithOp::FAdd | crate::sir::ops::ArithOp::FSub => {
            ...
            crate::sir::ops::SirInst::UnaryFloat { op, .. } => match op {
                crate::sir::ops::UnaryFloatOp::Sqrt => OpClass::FloatSqrt,
                _ => OpClass::FloatArithmetic,
            },
            crate::sir::ops::SirInst::BinaryFloat { op, .. } => match op {
                crate::sir::ops::BinaryFloatOp::Pow => OpClass::FloatMultiply,
                _ => OpClass::FloatArithmetic,
            },
            crate::sir::ops::SirInst::Memory(op) => match op {
...
fn compile_cranelift(args: &BuildArgs) -> Result<(PathBuf, Option<NativeMetrics>), String> {
    #[cfg(feature = "jit")]
    {
        ...
        let obj_bytes = aot_compiler
            .compile_to_object(&hlir)
        ...
        let format =
            args.format
                .unwrap_or_else(|| match output_path.extension().and_then(|e| e.to_str()) {
                    Some("so") | Some("dylib") | Some("dll") => OutputFormat::SharedLib,
                    Some("o") => OutputFormat::Object,
        ...
        match format {
            ...
            OutputFormat::Executable | OutputFormat::SharedLib => {
                ...
                if format == OutputFormat::Executable {
                    let output_dir = output_path
                        ...
                        .unwrap_or_else(|| std::path::Path::new("."));
                    let runtime_obj =
                        crate::backend::native::runtime::build_runtime_object_without_start(
                            output_dir,
                        )
                        .map_err(|e| format!("Runtime build error: {}", e))?;
                    link_inputs.push(runtime_obj);
            ...
            _ => {
                return Err(format!(
                    "Cranelift: Unsupported output format: {:?}",
                    format
                ));
            }
        }
        ...
        if args.verbose {
            println!(
                "Cranelift: Total compilation time: {}ms",
...
mod tests {
    ...
    fn test_thermal_model_parsing() {
        assert_eq!(
            "7nm".parse::<ThermalModel>().unwrap(),
            ThermalModel::Arrhenius7nm
        );
        assert_eq!(
            "5nm".parse::<ThermalModel>().unwrap(),
            ThermalModel::Arrhenius5nm
        );
        assert_eq!("none".parse::<ThermalModel>().unwrap(), ThermalModel::None);
        assert_eq!(
            "conservative".parse::<ThermalModel>().unwrap(),
            ThermalModel::Conservative
        );
    }
