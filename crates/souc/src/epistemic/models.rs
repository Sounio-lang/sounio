...
pub enum UncertaintyModel {
    ...
    ///
    /// Spectral uncertainty propagation using orthogonal polynomial basis.
...
impl UncertaintyModel {
    ...
    pub fn supports_distributions(&self) -> bool {
        matches!(
            self,
            UncertaintyModel::Bayesian(_) | UncertaintyModel::Bootstrap(_)
        )
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            UncertainValue::CrossValidation {
                ...
                write!(f, "CV(score={:.4}±{:.4}, k={})", mean, std_dev, k)
            }
        }
    }
}
