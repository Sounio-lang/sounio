...
impl fmt::Debug for EffectHandler {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("EffectHandler")
            ...
            .field(
                "cases",
                &self.cases.iter().map(|c| &c.operation).collect::<Vec<_>>(),
            )
            .finish()
    }
}
...
impl CapabilityAdapter {
    ...
    pub fn to_effect_handler(&self) -> EffectHandler {
        let effect_kind =
            EffectKind::from_str(self.handler.effect_name()).unwrap_or(EffectKind::IO); // Default to IO if unknown
        ...
        for op_spec in self.handler.operations() {
            ...
            handler = handler.with_case(
                &op_name,
                move |args: &[Value], _state: &mut HandlerState| {
                    ...
                    let result =
                        capability_handler.handle(&op_name, args, continuation, &mut cap_state);
    ...
    pub fn dispatch_direct(
        ...
        let result = self
            .handler
            .handle(operation, args, continuation, &mut state);
...
pub struct EffectContext {
    ...
    continuation_context: crate::effects::ContinuationContext,
}
...
impl EffectContext {
    ...
    pub fn new() -> Self {
        let mut ctx = Self {
            ...
            continuation_context: crate::effects::ContinuationContext::new(),
        };
    ...
    pub fn install_from_registry(&mut self, registry: &HandlerRegistry) {
        for handler in registry.handlers() {
            let adapter =
                CapabilityAdapter::new(Box::new(CloneableHandlerWrapper(handler.clone())));
    ...
    pub fn dispatch(
        ...
        if let Some(idx) = handler_idx {
            ...
            // during the call
            let handler_fn_ptr: *const dyn Fn(
                &[Value],
                &mut HandlerState,
            ) -> Result<Value, EffectError> = {
    ...
    fn dispatch_via_registry_with_suspension(
        ...
        // Look up the handler in the registry and clone the Arc
        // This allows us to release the borrow on self.registry before capturing continuation
        let handler = {
            let registry = self
                .registry
                .as_ref()
        ...
        // Push the continuation onto the active stack so that the `resume` expression
        // in the handler can find it
        self.continuation_context.push(cont_id);

        // Create a continuation for the handler (currently just metadata)
        let continuation = CapabilityContinuation::new();
        ...
        match result {
            CapabilityResult::Return(v) | CapabilityResult::Resume(v) => {
                // Pop the continuation from the active stack since we're done with it
                self.continuation_context.pop();
                Ok(DispatchResult::Completed(v))
            }
            CapabilityResult::Abort(err) => {
                // Pop the continuation since we're aborting
                self.continuation_context.pop();
            ...
            CapabilityResult::Suspend(handler_suspension_id) => {
                // Don't pop the continuation - it remains active for later resumption
    ...
    ///
    /// This is used by the `resume` expression to find which continuation to invoke.
    /// Returns `None` if no continuation is currently active (i.e., not inside a handler).
    pub fn current_continuation(&self) -> Option<ContinuationId> {
        self.continuation_context.current()
    }

    /// Check if any continuation is currently active.
    ///
    /// Returns `true` if we're currently inside a handler with an active continuation.
    pub fn has_active(&self) -> bool {
        self.continuation_context.has_active()
    }

    /// Push a continuation onto the active stack.
    ///
    /// This should be called when entering a handler with a captured continuation.
    pub fn push_continuation(&mut self, id: ContinuationId) {
        self.continuation_context.push(id);
    }

    /// Pop the current continuation from the active stack.
    ///
    /// This should be called when exiting a handler.
    pub fn pop_continuation(&mut self) -> Option<ContinuationId> {
        self.continuation_context.pop()
    }
}
...
pub enum PerformResult {
    ...
    Suspended { continuation_id: ContinuationId },
}
...
fn interpreter_dist_to_prob(dist: &Distribution) -> prob::Distribution {
    match dist {
        ...
        Distribution::Uniform { a, b } => prob::Distribution::Uniform { low: *a, high: *b },
...
fn value_to_prob_distribution(
    ...
    match name {
        ...
        "Bernoulli" => Ok(prob::Distribution::Bernoulli { p: get_float("p")? }),
...
mod tests {
    ...
    fn test_causal_do_dispatch() {
        ...
        let result = ctx.do_intervention(Value::String("X".to_string()), Value::Float(1.0));
    ...
    fn test_handler_state_reset() {
        ...
        state
            .interventions
            .push(("X".to_string(), Value::Float(1.0)));
        state
            .observations
            .push(("Y".to_string(), Value::Float(2.0)));
    ...
    fn test_capability_adapter_direct_dispatch() {
        ...
        let result = adapter.dispatch_direct("add", &[Value::Int(2), Value::Int(3)], continuation);
    ...
    fn test_dispatch_via_registry_div() {
        ...
        let result =
            ctx.dispatch_by_name("Div", "div", vec![Value::Float(10.0), Value::Float(2.0)]);
        assert!(result.is_ok());
        if let Ok(Value::Float(v)) = result {
    ...
    fn test_dispatch_sensor_via_registry() {
        ...
        let result = ctx.dispatch_by_name(
            "Sensor",
            "is_available",
            vec![Value::String("temp_sensor".into())],
        );
        assert!(result.is_ok());
    }
    ...
    fn test_capture_interpreter_continuation_with_closure_state() {
        ...
        let id =
            ctx.capture_interpreter_continuation("Test", "add_captured", false, move |value| {
                match value {
    ...
    fn test_capture_interpreter_continuation_error() {
        ...
        let id = ctx.capture_interpreter_continuation("Test", "fail", false, |_value| {
    ...
    fn test_capture_interpreter_continuation_one_shot_enforcement() {
        ...
        let id = ctx.capture_interpreter_continuation("Test", "one_shot", false, |value| Ok(value));
    ...
    fn test_interpreter_continuation_label() {
        ...
        let id = ctx.capture_interpreter_continuation("MyEffect", "myOp", false, |value| Ok(value));
    ...
    fn test_interpreter_continuation_chaining() {
        ...
        let id1 =
            ctx.capture_interpreter_continuation("Math", "mul2", false, |value| match value {
                Value::Int(n) => Ok(Value::Int(n * 2)),
                _ => Ok(value),
            });
        ...
        let id2 =
            ctx.capture_interpreter_continuation("Math", "add10", false, |value| match value {
    ...
    fn test_multi_shot_interpreter_continuation_basic() {
        ...
        let id =
            ctx.capture_multi_shot_interpreter_continuation(
                "Test",
                "double",
                |value| match value {
                    Value::Int(n) => Ok(Value::Int(n * 2)),
    ...
    fn test_multi_shot_interpreter_continuation_multiple_resumes_same_value() {
        ...
        let id = ctx.capture_multi_shot_interpreter_continuation("Test", "counter", move |value| {
    ...
    fn test_multi_shot_interpreter_continuation_with_captured_state() {
        ...
        let id =
            ctx.capture_multi_shot_interpreter_continuation("Prob", "accumulate", move |value| {
                match value {
                    Value::Int(n) => {
    ...
    fn test_multi_shot_interpreter_continuation_label() {
        ...
        let id =
            ctx.capture_multi_shot_interpreter_continuation("Amb", "choose", |value| Ok(value));
    ...
    fn test_multi_shot_vs_one_shot_behavior() {
        ...
        let one_shot_id =
            ctx.capture_interpreter_continuation("Test", "one_shot", false, |value| Ok(value));

        // Multi-shot continuation
        let multi_shot_id =
            ctx.capture_multi_shot_interpreter_continuation("Test", "multi_shot", |value| {
                Ok(value)
            });
    ...
    fn test_multi_shot_interpreter_continuation_error_propagation() {
        ...
        let id =
            ctx.capture_multi_shot_interpreter_continuation("Test", "conditional_error", |value| {
                match value {
                    Value::Int(n) if n < 0 => Err(ContinuationError::NotImplemented(
    ...
    fn test_multi_shot_interpreter_continuation_for_probabilistic_sampling() {
        ...
        let id = ctx.capture_multi_shot_interpreter_continuation("Prob", "sample", |value| {
    ...
    fn test_multi_shot_interpreter_continuation_for_backtracking() {
        ...
        let id = ctx.capture_multi_shot_interpreter_continuation("Amb", "choose", |value| {
    ...
    fn test_multi_shot_interpreter_continuation_resume_point_properties() {
        ...
        let resume_point = ResumePoint::interpreter_multi_shot(|v| Ok(v), Some("test"));
    ...
    fn test_multi_shot_interpreter_continuation_debug_format() {
        ...
        let resume_point = ResumePoint::interpreter_multi_shot(|v| Ok(v), Some("debug test"));
    ...
    fn test_resume_suspension_with_interpreter_closure() {
        ...
        let continuation_id =
            ctx.capture_interpreter_continuation("Test", "double", false, |value| match value {
                Value::Int(n) => Ok(Value::Int(n * 2)),
                _ => Ok(value),
            });
    ...
    fn test_handler_stack_with_continuation_completed() {
        ...
        let custom_handler = EffectHandler::new(EffectKind::IO, "custom_io")
            .with_case("test_op", |_args, _state| {
                Ok(Value::String("custom".into()))
            });
        ctx.push_handler(custom_handler);

        // Dispatch should hit the stack handler and complete
        let result = ctx.dispatch_with_continuation(EffectKind::IO, "test_op", vec![]);
