...
impl Interpreter {
    ...
    fn unwrap_knowledge(val: Value) -> (Value, f64, bool, u32) {
        match val {
            Value::Knowledge {
                value,
                confidence,
                knightian,
                provenance_id,
            } => (*value, confidence, knightian, provenance_id),
            _ => (val, 1.0, false, 0), // Non-Knowledge values: full confidence, not Knightian
        }
    }
    ...
    /// Knightian flag propagates: if either input is ε=⊥, result is ε=⊥.
    fn wrap_knowledge(
        result: Value,
        conf1: f64,
        conf2: f64,
        ...
        // If either input was Knowledge (confidence < 1.0, provenance, or Knightian), wrap result.
        if conf1 < 1.0 || conf2 < 1.0 || prov1 != 0 || prov2 != 0 || k1 || k2 {
            Value::Knowledge {
    ...
    fn eval_binary(&self, op: HirBinaryOp, lhs: Value, rhs: Value) -> Result<Value, ControlFlow> {
        ...
        let (lhs, lhs_conf, lhs_knightian, lhs_prov) = Self::unwrap_knowledge(lhs);
        let (rhs, rhs_conf, rhs_knightian, rhs_prov) = Self::unwrap_knowledge(rhs);
