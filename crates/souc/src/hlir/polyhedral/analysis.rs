...
use crate::hlir::{
    BinaryOp, BlockId, HlirConstant, HlirFunction, HlirInstr, HlirTerminator, HlirType, Op, ValueId,
};
...
impl PolyhedralAnalysis {
            ...
            for (j, stmt2) in scop.statements.iter().enumerate() {
                ...
                for write in &stmt1.writes {
                    ...
                    for read in &stmt2.reads {
                        if let Some(dep) =
                            self.check_dependence(stmt1, stmt2, write, read, DependenceType::Flow)
                        {
    ...
    fn classify_loops(&mut self) {
        ...
        for &header1 in &headers {
            for &header2 in &headers {
                ...
                let loop1_blocks = self
                    .loops
                    .get(&header1)
                    .map(|l| l.blocks.clone())
                    .unwrap_or_default();
                let loop2_blocks = self
                    .loops
                    .get(&header2)
                    .map(|l| l.blocks.clone())
                    .unwrap_or_default();
            ...
            if let Op::Phi { incoming } = &instr.op {
                if let Some(result) = instr.result {
                    ...
                    if let (Some(init), Some(loop_val)) = (init_value, loop_value) {
                        ...
                        if let Some(iv) =
                            self.check_induction_pattern(func, loop_info, result, init, loop_val)
                        {
                            return Some(iv);
                        }
                    }
    ...
    fn get_loop_bound(
        ...
        (
            InductionBound::Value(ValueId::UNIT),
    ...
    fn check_dependence(
        ...
        for dim in 0..source_access
            .map
            .num_outputs
            .min(sink_access.map.num_outputs)
        {
