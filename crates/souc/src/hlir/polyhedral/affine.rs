...
impl AffineMap {
    ...
    pub fn identity(n: usize) -> Self {
        let outputs = (0..n).map(|i| AffineExpr::iter_var(n, i)).collect();
        Self {
            num_inputs: n,
            num_outputs: n,
            num_params: 0,
    ...
    pub fn project(n: usize, k: usize) -> Self {
        assert!(k <= n);
        let outputs = (0..(n - k)).map(|i| AffineExpr::iter_var(n, i)).collect();
        Self {
            num_inputs: n,
            num_outputs: n - k,
            num_params: 0,
...
impl AccessRelation {
    ...
    pub fn new(array_name: String, map: AffineMap, is_write: bool, statement_id: usize) -> Self {
        Self {
            array_name,
            map,
            is_write,
...
impl std::fmt::Display for AccessRelation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}[S{}] -> {}[{}]",
            access_type, self.statement_id, self.array_name, self.map
        )
    }
}
