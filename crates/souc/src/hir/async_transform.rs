...
impl AsyncTransformer {
    ...
    fn extract_awaits_recursive(
        &mut self,
        expr: &HirExpr,
        awaits: &mut Vec<AwaitExtraction>,
    ) -> HirExpr {
        match &expr.kind {
            ...
            HirExprKind::If {
                ...
                let new_else = else_branch
                    .as_ref()
                    .map(|e| Box::new(self.extract_awaits_recursive(e, awaits)));
                ...
                let new_arms = arms
                    ...
                    .map(|arm| HirMatchArm {
                        ...
                        guard: arm
                            .guard
                            .as_ref()
                            .map(|g| Box::new(self.extract_awaits_recursive(g, awaits))),
    ...
    fn build_await_sequence(
        ...
        if extracted.awaits.is_empty() {
            ...
            match binding {
                ...
                BindingTarget::None => {
                    self.current_stmts
                        .push(HirStmt::Expr(extracted.expr.clone()));
                }
            }
...
mod tests {
    ...
    fn test_extract_await_in_if_condition() {
        ...
        match &extracted.expr.kind {
            HirExprKind::If { condition, .. } => match &condition.kind {
                HirExprKind::Local(name) => {
                    assert!(name.starts_with("__await_tmp_"));
                }
    ...
    fn test_extract_await_in_while_condition() {
        ...
        match &extracted.expr.kind {
            HirExprKind::While { condition, .. } => match &condition.kind {
                HirExprKind::Local(name) => {
                    assert!(name.starts_with("__await_tmp_"));
                }
                _ => panic!("Expected local reference in condition"),
    ...
    fn test_extract_await_in_match_scrutinee() {
        ...
        match &extracted.expr.kind {
            HirExprKind::Match { scrutinee, .. } => match &scrutinee.kind {
                HirExprKind::Local(name) => {
                    assert!(name.starts_with("__await_tmp_"));
                }
