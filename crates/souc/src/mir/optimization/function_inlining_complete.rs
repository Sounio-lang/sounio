...
impl CompleteFunctionInlining {
    ...
    fn remap_instruction(&self, instr: &mut MirInstruction) -> Result<(), String> {
        match instr {
            ...
            | MirInstruction::ZExt { result, source, .. }
            | MirInstruction::SExt { result, source, .. }
            | MirInstruction::Trunc { result, source, .. }
            | MirInstruction::FPToSI { result, source, .. }
            | MirInstruction::SIToFP { result, source, .. }
            | MirInstruction::FPToUI { result, source, .. }
            | MirInstruction::UIToFP { result, source, .. }
                    ...
                    } = instr
                    {
                        ...
                        if let Some(callee) = module.functions.iter().find(|f| f.name == *func_name)
                        {
                            ...
                            if callee.name != func.name && should_inline_function(callee) {
                                ...
                                call_sites_to_inline.push((func_idx, call_site, callee.clone()));
                            }
                        }
                    }
                }
