            ...
            for succ in succs {
                ...
                if self.dominates(succ, block.id) {
                    ...
                    let loop_info = self
                        .loops
                        .entry(succ)
                        .or_insert_with(|| LoopInfo::new(succ));
...
impl MIRPass for LoopInvariantCodeMotion {
    ...
    fn run_on_function(&self, func: &mut MirFunction) -> Result<bool, String> {
        ...
        for header in loop_headers {
            ...
            let preheader_id =
                self.get_or_create_preheader(func, &loop_info, &loop_analysis, &mut state)?;
...
impl LoopInvariantCodeMotion {
    ...
    fn get_or_create_preheader(
        ...
        if outside_preds.len() == 1 {
            ...
            let candidate_succs = loop_analysis
                .successors
                .get(&candidate)
                .cloned()
                .unwrap_or_default();
...
fn has_side_effects(instr: &MirInstruction) -> bool {
    match instr {
        ...
        MirInstruction::Call { .. } | MirInstruction::CallIndirect { .. } => true,
...
mod tests {
    ...
    fn test_loop_detection_simple() {
        ...
        //                              -> exit
        let mut builder = FunctionBuilder::new(FuncId(0), "test_loop".to_string(), MirType::I32);
    ...
    fn test_invariant_detection() {
        ...
        let mut builder =
            FunctionBuilder::new(FuncId(0), "test_invariant".to_string(), MirType::I32);
    ...
    fn test_licm_pass_runs() {
        ...
        let mut func_builder = module_builder.create_function("simple".to_string(), MirType::I32);
    ...
    fn test_dominator_computation() {
        ...
        let mut builder = FunctionBuilder::new(FuncId(0), "test_dom".to_string(), MirType::Unit);
