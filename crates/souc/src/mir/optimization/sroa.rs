...
impl Sroa {
    ...
    fn is_aggregate_type(ty: &MirType) -> bool {
        matches!(
            ty,
            MirType::Struct { .. } | MirType::Array(_, _) | MirType::Tuple(_)
        )
    }
                    ...
                    match instr {
                            ...
                            if *base == current_value {
                                ...
                                for idx_value in indices {
                                    ...
                                    if let Some(const_val) =
                                        Self::get_constant_value(func, *idx_value)
                                    {
                                ...
                                if all_const {
                                    ...
                                    info.rejection_reason =
                                        Some("Non-constant GEP index".to_string());
                                    return info;
                                }
                            }
                        }
                        ...
                        MirInstruction::Store { address, value } => {
                            if *address == current_value {
                                ...
                                info.is_splittable = false;
                                info.rejection_reason =
                                    Some("Address of aggregate stored".to_string());
                                return info;
                            }
                        }
                        ...
                        | MirInstruction::CallIndirect { args, .. } => {
                            if args.contains(&current_value) {
                                info.is_splittable = false;
                                info.rejection_reason =
                                    Some("Pointer escapes to function call".to_string());
                                return info;
                            }
                        }
                        ...
                        MirInstruction::Phi { incoming, .. } => {
                            for (_, val) in incoming {
                                if *val == current_value {
                                    info.is_splittable = false;
                                    info.rejection_reason =
                                        Some("Pointer used in phi node".to_string());
                                    return info;
                                }
                            }
                        }
...
mod tests {
    ...
    fn test_is_aggregate_type() {
        ...
        assert!(!Sroa::is_aggregate_type(&MirType::Ptr(Box::new(
            MirType::I32
        ))));
    }
    ...
    fn test_get_field_type() {
        ...
        // Access start.x
        assert_eq!(
            Sroa::get_field_type(&nested_ty, &[0, 0]),
            Some(MirType::F64)
        );

        // Access end.y
        assert_eq!(
            Sroa::get_field_type(&nested_ty, &[1, 1]),
            Some(MirType::F64)
        );
    }
    ...
    fn test_sroa_simple_struct() {
        ...
        builder.build_gep(
            ...
            point,
            vec![idx_zero],
            MirType::Ptr(Box::new(MirType::F64)),
        );
        ...
        assert!(
            pass.stats.allocas_analyzed > 0,
            "Expected allocas to be analyzed"
        );
    }
    ...
    fn test_sroa_escaping_pointer() {
        ...
        builder.build_call(None, "external_func".to_string(), vec![data], MirType::Unit);
