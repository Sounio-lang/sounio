...
impl SCCPAnalysis {
    ...
    fn evaluate_select(
        &self,
        cond: &LatticeValue,
        true_val: &LatticeValue,
        false_val: &LatticeValue,
    ) -> LatticeValue {
        match cond {
    ...
    fn fold_unary(&self, op: MirUnaryOp, operand: &MirConstant, _ty: &MirType) -> LatticeValue {
        match op {
            MirUnaryOp::Neg => match operand {
                MirConstant::Int(v) => match v.checked_neg() {
            ...
            MirUnaryOp::Not => match operand {
                MirConstant::Bool(v) => LatticeValue::Constant(MirConstant::Bool(!v)),
                _ => LatticeValue::Bottom,
            },
            MirUnaryOp::BitNot => match operand {
                MirConstant::Int(v) => LatticeValue::Constant(MirConstant::Int(!v)),
                MirConstant::UInt(v) => LatticeValue::Constant(MirConstant::UInt(!v)),
                _ => LatticeValue::Bottom,
            },
        }
    }
    ...
    fn evaluate_compare(
        ...
        match (left, right) {
            (LatticeValue::Top, _) | (_, LatticeValue::Top) => LatticeValue::Top,
            (LatticeValue::Bottom, _) | (_, LatticeValue::Bottom) => LatticeValue::Bottom,
            (LatticeValue::Constant(l), LatticeValue::Constant(r)) => self.fold_compare(op, l, r),
        }
    }
...
mod tests {
    ...
    fn test_constant_folding_float() {
        ...
        assert_eq!(
            result,
            LatticeValue::Constant(MirConstant::Float("12".to_string()))
        );
    }
    ...
    fn test_constant_propagation_with_bottom() {
        ...
        assert!(
            !modified,
            "function should not be modified when operand is Bottom"
        );
    }
}
