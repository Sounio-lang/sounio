...
pub fn generate_double_buffered_f32_gemm_ptx(
    ...
    // emit cp.async for smem_a0: 4 elements per thread (ty-dim)
    // cp.async.ca.shared.global [dst], [src], 4  (.ca = cache-all, required for <16-byte copies)
...
mod tests {
    ...
    fn test_double_buffered_ptx_structure() {
        ...
        // cp.async machinery
        assert!(ptx.contains("cp.async.ca.shared.global"), "must use cp.async.ca for 4-byte copies");
