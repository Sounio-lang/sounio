    ...
    fn generate_op(&mut self, op: &GpuOp) -> Result<Word, SpirvError> {
        match op {
            ...
            GpuOp::AbsF32(val) => {
                ...
                wrap_err(
                    self.builder
                        .ext_inst(ty, None, ext, 4, vec![Operand::IdRef(v)]),
                )
            }
            ...
            GpuOp::AbsI32(val) => {
                ...
                wrap_err(
                    self.builder
                        .ext_inst(ty, None, ext, 5, vec![Operand::IdRef(v)]),
                )
            }
            ...
            GpuOp::MinF32(lhs, rhs) => {
                ...
                let ty = self.get_type("f32")?;
                let ext = self.get_or_import_glsl_ext();
                wrap_err(self.builder.ext_inst(
                    ty,
                    None,
                    ext,
                    37,
            ...
            GpuOp::FMulAdd(a, b, c) => {
                ...
                wrap_err(self.builder.ext_inst(
                    ...
                    ext,
                    50,
                    vec![Operand::IdRef(va), Operand::IdRef(vb), Operand::IdRef(vc)],
                ))
            }
