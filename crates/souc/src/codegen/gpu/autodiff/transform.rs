...
use super::super::ir::{
    BlockId, GpuBlock, GpuKernel, GpuModule, GpuOp, GpuParam, GpuTerminator, GpuType, MemorySpace,
    ValueId,
};
...
impl AutoDiffTransform {
    ...
    pub fn transform_module(&mut self, module: &GpuModule) -> Result<GpuModule, DiffError> {
        let mut new_module = GpuModule::new(format!("{}_autodiff", module.name), module.target);
    ...
    fn generate_tape_record(
        ...
        ops.push((
            tape_ptr_id,
            GpuOp::Param(self.ctx.inputs.len() as u32 + self.ctx.outputs.len() as u32),
        ));
        ...
        ops.push((
            entry_val_id,
            GpuOp::ConstInt(packed_entry as i64, GpuType::U64),
        ));
    ...
    fn generate_backward_blocks(&mut self, kernel: &GpuKernel) -> Result<Vec<GpuBlock>, DiffError> {
        ...
        for (i, output) in outputs.iter().enumerate() {
            ...
            entry_block.add_instruction(grad_ptr, GpuOp::Param((num_original_params + i) as u32));
            ...
            // Load gradient value
            entry_block.add_instruction(grad_val, GpuOp::Load(grad_elem_ptr, MemorySpace::Global));
...
mod tests {
    ...
    fn test_diff_attribute_parsing() {
        ...
        assert_eq!(
            DiffAttribute::from_str("diff"),
            Some(DiffAttribute::Differentiable)
        );
        assert_eq!(
            DiffAttribute::from_str("no_grad"),
            Some(DiffAttribute::NoGrad)
        );
