...
impl AutoDiff {
    ...
    pub fn differentiate_module(&mut self, module: &GpuModule) -> Result<GpuModule, DiffError> {
        let mut diff_module = GpuModule::new(format!("{}_autodiff", module.name), module.target);
    ...
    fn is_differentiable(&self, kernel: &GpuKernel) -> bool {
        ...
        let has_float_output = kernel
            .params
            .iter()
            .any(|p| is_float_type(&p.ty) && matches!(p.space, MemorySpace::Global));
...
impl std::fmt::Display for DiffError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DiffError::NonDifferentiable(op) => {
                write!(f, "Operation '{}' is not differentiable", op)
            }
            DiffError::NoDifferentiableParams => {
                write!(f, "Kernel has no differentiable parameters")
            }
...
mod tests {
    ...
    fn test_is_float_type() {
        ...
        assert!(is_float_type(&GpuType::Ptr(
            Box::new(GpuType::F32),
            MemorySpace::Global
        )));
