...
impl HlirToGpuLowering {
    ...
    fn lower_type(&self, ty: &HlirType) -> GpuType {
        match ty {
            ...
            HlirType::QuatLinear => {
                GpuType::Array(Box::new(GpuType::Vec4(Box::new(GpuType::F32))), 0)
            }
            HlirType::QuatConv2d => {
                GpuType::Array(Box::new(GpuType::Vec4(Box::new(GpuType::F32))), 0)
            }
            ...
            // Sedenion: 16 x f32 (16D hypercomplex)
            HlirType::Sedenion => GpuType::Array(Box::new(GpuType::F32), 16),
        }
    }
    ...
    fn emit_epistemic_binary(
        ...
        if let (Some(l_valid), Some(r_valid)) = (left_valid, right_valid) {
            ...
            block.add_instruction(result_validity, GpuOp::And(l_valid, l_valid));
        // Copy
