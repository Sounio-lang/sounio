...
impl QatQuantState {
    ...
    pub fn observe(&mut self, values: &[f32]) {
        ...
        if !self.initialized {
            ...
            self.running_min = self.momentum * batch_min + (1.0 - self.momentum) * self.running_min;
            self.running_max = self.momentum * batch_max + (1.0 - self.momentum) * self.running_max;
        }
        ...
        match self.scheme {
            ...
            QuantScheme::Asymmetric => {
                ...
                if range > 0.0 {
                    ...
                    self.zero_point = (qmin as f32 - self.running_min / self.scale).round() as i32;
