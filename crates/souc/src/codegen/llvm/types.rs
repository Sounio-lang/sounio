...
impl<'ctx> TypeConverter<'ctx> {
    ...
    pub fn convert(&mut self, ty: &HlirType) -> BasicTypeEnum<'ctx> {
        match ty {
            ...
            // Double precision SIMD vector types
            HlirType::Vec2d => {
                // 2x f64 vector
                self.context.f64_type().vec_type(2).into()
            }
            HlirType::Vec3d => {
                // 3x f64 padded to 4 for SIMD alignment
            ...
            | HlirType::QuatGate => self.context.ptr_type(AddressSpace::default()).into(),
            ...
            HlirType::Knowledge { inner, mode, .. } => {
                ...
                match mode {
                    ...
                    EpistemicMode::Compact => {
                        ...
                        self.context
                            .struct_type(&[inner_ty, u16_ty.into(), padding_ty.into()], false)
                            .into()
                    }
    ...
    pub fn size_bits(&self, ty: &HlirType) -> u64 {
        match ty {
            ...
            HlirType::Vec2d => 128,    // 2x f64 = 128 bits
