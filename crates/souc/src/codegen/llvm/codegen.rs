...
impl<'ctx> LLVMCodegen<'ctx> {
    ...
    fn compile_effect_dispatch(
        ...
        for arg_id in args {
            ...
            let arg_ty = self.value_types.get(arg_id).cloned();
            let converted = self.value_to_f64(val, arg_ty.as_ref())?;
    ...
    fn convert_f64_to_type(
        ...
        match ty {
            ...
            HlirType::U8 | HlirType::U16 | HlirType::U32 | HlirType::U64 | HlirType::U128 => {
                let bits = self.types.int_bit_width(ty).unwrap_or(64);
                let int_ty = self.types.int_type_for_bits(bits);
                self.builder
    ...
    fn compile_constant(&mut self, constant: &HlirConstant) -> Option<BasicValueEnum<'ctx>> {
        match constant {
            ...
            HlirConstant::CString(s) => {
                ...
                let global = self.builder.build_global_string_ptr(s, "cstr").ok()?;
                let ptr = global.as_pointer_value();
                self.strings.insert(s.clone(), ptr);
                Some(ptr.into())
            }
