...
impl<'ctx> DebugBuilder<'ctx> {
    ...
    pub fn create_type_info(&self, ty: &HlirType) -> DIType<'ctx> {
        match ty {
            ...
            HlirType::Vec2d => self.create_basic_type("vec2d", 128, DW_ATE_FLOAT),
            ...
            HlirType::QuatGate => self.create_basic_type("quat_gate", 64, DW_ATE_ADDRESS),
            HlirType::Knowledge { inner, .. } => {
                // Knowledge types are represented as pointer to the inner type
                let inner_ty = self.create_type_info(inner);
                self.create_pointer_type(Some(inner_ty), "knowledge")
            }
        }
    }
...
fn type_size_bits(ty: &HlirType) -> u64 {
    match ty {
        ...
        | HlirType::QuatGate => 64,
        HlirType::Knowledge { inner, mode, .. } => {
            let inner_bits = type_size_bits(inner);
            match mode {
                crate::runtime::EpistemicMode::Full => inner_bits + 512,
                crate::runtime::EpistemicMode::Compact => inner_bits + 128,
                crate::runtime::EpistemicMode::Erased => inner_bits,
            }
        }
    }
}
