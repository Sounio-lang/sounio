...
impl JitEffectState {
    ...
    fn dispatch_predefined_handler(
        &self,
        handler_id: u32,
        _effect: &str,
        op: &str,
...
fn dispatch_causal_effect(handler_id: u32, op: &str, args: &[f64]) -> Option<f64> {
    ...
    let result = match op {
        ...
        _ if op.starts_with("do_") => Some(args.first().copied().unwrap_or(0.0)),
    ...
    match handler_id {
        ...
        HANDLER_EPISTEMIC_SIMPLE => {
            match op {
                ...
                "get_confidence" => Some(args.first().copied().unwrap_or(1.0)),
                "clamp" => {
                    let min = args.first().copied().unwrap_or(0.0);
                    let max = args.get(1).copied().unwrap_or(1.0);
                    let conf = args.get(2).copied().unwrap_or(1.0);
        ...
        HANDLER_EPISTEMIC_AUDIT => match op {
            "degrade" => {
                let factor = args.first().copied().unwrap_or(1.0);
...
extern "C" fn runtime_jacobian(
    ...
    let result_ptr = unsafe {
        std::alloc::alloc(
            std::alloc::Layout::from_size_align(result_size, 8).expect("valid layout"),
        )
    } as *mut f64;
...
extern "C" fn runtime_mut_exists(name_ptr: *const u8) -> i64 {
    ...
    if let Ok(state) = get_jit_effect_state().lock() {
        if state.mutable_state.contains_key(name) {
            1
        } else {
            0
        }
...
#[cfg(feature = "jit")]
extern "C" fn runtime_handler_push(
    effect_ptr: *const u8,
    name_ptr: *const u8,
    handler_id: u32,
) -> i64 {
...
extern "C" fn runtime_handler_has(effect_ptr: *const u8) -> i64 {
    ...
    if let Ok(state) = get_jit_effect_state().lock() {
        ...
        if state.find_handler(effect).is_some() {
            1
        } else {
            0
        }
...
impl CraneliftAot {
    ...
    pub fn compile_to_object(&self, module: &HlirModule) -> Result<Vec<u8>, String> {
        ...
        flag_builder
            .set("is_pic", "true")
            .map_err(|e| e.to_string())?; // PIC for shared libs/executables
...
impl JitCompiler {
    fn new(optimize: bool) -> Result<Self, String> {
        ...
        jit_builder.symbol(
            "runtime_prob_sample_uniform",
            runtime_prob_sample_uniform as *const u8,
        );
        jit_builder.symbol(
            "runtime_prob_sample_bernoulli",
            runtime_prob_sample_bernoulli as *const u8,
        );
        ...
        jit_builder.symbol(
            "runtime_effect_set_seed",
            runtime_effect_set_seed as *const u8,
        );

        // Register generic effect dispatch functions (for registry-based handlers)
        jit_builder.symbol(
            "runtime_effect_dispatch",
            runtime_effect_dispatch as *const u8,
        );
        jit_builder.symbol(
            "runtime_dispatch_effect_f64",
            runtime_dispatch_effect_f64 as *const u8,
        );
        ...
        jit_builder.symbol(
            "runtime_io_append_file",
            runtime_io_append_file as *const u8,
        );
        jit_builder.symbol(
            "runtime_io_file_exists",
            runtime_io_file_exists as *const u8,
        );
        ...
        jit_builder.symbol(
            ...
            runtime_handler_dispatch as *const u8,
        );
        jit_builder.symbol(
            "runtime_continuation_create",
            runtime_continuation_create as *const u8,
        );
        jit_builder.symbol(
            "runtime_continuation_resume",
            runtime_continuation_resume as *const u8,
        );
        jit_builder.symbol(
            "runtime_continuation_get_value",
            runtime_continuation_get_value as *const u8,
        );
        jit_builder.symbol(
            "runtime_continuation_clear",
            runtime_continuation_clear as *const u8,
        );
    ...
    fn declare_runtime_functions(&mut self) -> Result<(), String> {
        ...
        self.func_sigs.insert(
            "runtime_prob_sample_bernoulli".to_string(),
            sig_prob_bernoulli,
        );
        ...
        self.func_ids.insert("runtime_prob_observe".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_effect_reset".to_string(), id);
        self.func_sigs
        ...
        self.func_sigs
            .insert("runtime_math_sin".to_string(), sig_math_sin);
        ...
        self.func_sigs
            .insert("runtime_math_cos".to_string(), sig_math_cos);
        ...
        self.func_sigs
            .insert("runtime_math_exp".to_string(), sig_math_exp);
        ...
        self.func_sigs
            .insert("runtime_math_log".to_string(), sig_math_log);
        ...
        self.func_sigs
            .insert("runtime_math_sqrt".to_string(), sig_math_sqrt);
        ...
        self.func_sigs
            .insert("runtime_math_tan".to_string(), sig_math_tan);
        ...
        self.func_sigs
            .insert("runtime_math_atan".to_string(), sig_math_atan);
        ...
        self.func_sigs
            .insert("runtime_math_fabs".to_string(), sig_math_fabs);
        ...
        self.func_sigs
            .insert("runtime_math_asin".to_string(), sig_math_asin);
        ...
        self.func_sigs
            .insert("runtime_math_acos".to_string(), sig_math_acos);
        ...
        self.func_sigs
            .insert("runtime_math_sinh".to_string(), sig_math_sinh);
        ...
        self.func_sigs
            .insert("runtime_math_cosh".to_string(), sig_math_cosh);
        ...
        self.func_sigs
            .insert("runtime_math_tanh".to_string(), sig_math_tanh);
        ...
        self.func_sigs
            .insert("runtime_math_asinh".to_string(), sig_math_asinh);
        ...
        self.func_sigs
            .insert("runtime_math_acosh".to_string(), sig_math_acosh);
        ...
        self.func_sigs
            .insert("runtime_math_atanh".to_string(), sig_math_atanh);
        ...
        self.func_sigs
            .insert("runtime_math_log2".to_string(), sig_math_log2);
        ...
        self.func_sigs
            .insert("runtime_math_log10".to_string(), sig_math_log10);
        ...
        self.func_sigs
            .insert("runtime_math_atan2".to_string(), sig_math_atan2);
        ...
        self.func_sigs
            .insert("runtime_jacobian".to_string(), sig_jacobian);
        ...
        self.func_sigs
            .insert("runtime_hessian".to_string(), sig_hessian);
        ...
        self.func_ids.insert("runtime_io_read_line".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_io_read_file".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_mut_modify".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_mut_exists".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_mut_delete".to_string(), id);
        self.func_sigs
        ...
        self.func_sigs
            .insert("runtime_alloc".to_string(), sig_alloc);
        ...
        self.func_ids.insert("runtime_alloc_array".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_alloc_clear".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_alloc_total".to_string(), id);
        self.func_sigs
        ...
        self.func_ids.insert("runtime_alloc_count".to_string(), id);
        self.func_sigs
        ...
        self.func_sigs
            .insert("runtime_handler_push".to_string(), sig_handler_push);
        ...
        self.func_sigs
            .insert("runtime_handler_pop".to_string(), sig_handler_pop);
        ...
        self.func_ids
            .insert("runtime_handler_depth".to_string(), id);
        self.func_sigs
            .insert("runtime_handler_depth".to_string(), sig_handler_depth);
        ...
        self.func_sigs
            .insert("runtime_handler_has".to_string(), sig_handler_has);
        ...
        let id = self
            ...
            .declare_function(
                "runtime_handler_dispatch",
                Linkage::Import,
                &sig_handler_dispatch,
            )
            .map_err(|e| format!("Failed to declare runtime_handler_dispatch: {}", e))?;
        self.func_ids
            .insert("runtime_handler_dispatch".to_string(), id);
        self.func_sigs
            .insert("runtime_handler_dispatch".to_string(), sig_handler_dispatch);
        ...
        self.func_ids
            .insert("runtime_continuation_create".to_string(), id);
        self.func_sigs
            .insert("runtime_continuation_create".to_string(), sig_cont_create);
        ...
        self.func_ids
            .insert("runtime_continuation_resume".to_string(), id);
        self.func_sigs
            .insert("runtime_continuation_resume".to_string(), sig_cont_resume);
        ...
        self.func_ids
            .insert("runtime_continuation_get_value".to_string(), id);
        self.func_sigs
            .insert("runtime_continuation_get_value".to_string(), sig_cont_get);
        ...
        self.func_ids
            .insert("runtime_continuation_clear".to_string(), id);
        self.func_sigs
            .insert("runtime_continuation_clear".to_string(), sig_cont_clear);
...
fn translate_block(
    ...
    for instr in &block.instructions {
        let result = translate_instruction(
            builder,
            instr,
            values,
            string_values,
    ...
    match &instr.op {
        ...
        Op::CallDirect { name, args } => {
            ...
            if name == "dual_add" {
                if arg_vals.len() >= 2 {
                    ...
                    let a_val = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        a_ptr,
                        0,
                    );
                    let a_der = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        a_ptr,
                        8,
                    );
                    let b_val = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        b_ptr,
                        0,
                    );
                    let b_der = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        b_ptr,
                        8,
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
                }
                return Ok(None);
            }
            ...
            if name == "dual_sin" {
                if !arg_vals.is_empty() {
                    ...
                    let sin_func = func_refs
                        .get("runtime_math_sin")
                        .ok_or("runtime_math_sin not found")?;
                    let cos_func = func_refs
                        .get("runtime_math_cos")
                        .ok_or("runtime_math_cos not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), sin_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_cos" {
                if !arg_vals.is_empty() {
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), cos_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_exp" {
                if !arg_vals.is_empty() {
                    ...
                    let exp_func = func_refs
                        .get("runtime_math_exp")
                        .ok_or("runtime_math_exp not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), exp_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_log" {
                if !arg_vals.is_empty() {
                    ...
                    let log_func = func_refs
                        .get("runtime_math_log")
                        .ok_or("runtime_math_log not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), log_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_sqrt" {
                if !arg_vals.is_empty() {
                    ...
                    let sqrt_func = func_refs
                        .get("runtime_math_sqrt")
                        .ok_or("runtime_math_sqrt not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), sqrt_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_pow" {
                if arg_vals.len() >= 2 {
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), pow_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_tan" {
                if !arg_vals.is_empty() {
                    ...
                    let tan_func = func_refs
                        .get("runtime_math_tan")
                        .ok_or("runtime_math_tan not found")?;
                    let cos_func = func_refs
                        .get("runtime_math_cos")
                        .ok_or("runtime_math_cos not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), tan_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_atan" {
                if !arg_vals.is_empty() {
                    ...
                    let atan_func = func_refs
                        .get("runtime_math_atan")
                        .ok_or("runtime_math_atan not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), atan_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_abs" {
                if !arg_vals.is_empty() {
                    ...
                    let fabs_func = func_refs
                        .get("runtime_math_fabs")
                        .ok_or("runtime_math_fabs not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), abs_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_asin" {
                if !arg_vals.is_empty() {
                    ...
                    let asin_func = func_refs
                        .get("runtime_math_asin")
                        .ok_or("runtime_math_asin not found")?;
                    let sqrt_func = func_refs
                        .get("runtime_math_sqrt")
                        .ok_or("runtime_math_sqrt not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), asin_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_acos" {
                if !arg_vals.is_empty() {
                    ...
                    let acos_func = func_refs
                        .get("runtime_math_acos")
                        .ok_or("runtime_math_acos not found")?;
                    let sqrt_func = func_refs
                        .get("runtime_math_sqrt")
                        .ok_or("runtime_math_sqrt not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), acos_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_sinh" {
                if !arg_vals.is_empty() {
                    ...
                    let sinh_func = func_refs
                        .get("runtime_math_sinh")
                        .ok_or("runtime_math_sinh not found")?;
                    let cosh_func = func_refs
                        .get("runtime_math_cosh")
                        .ok_or("runtime_math_cosh not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), sinh_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_cosh" {
                if !arg_vals.is_empty() {
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), cosh_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_tanh" {
                if !arg_vals.is_empty() {
                    ...
                    let tanh_func = func_refs
                        .get("runtime_math_tanh")
                        .ok_or("runtime_math_tanh not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), tanh_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_asinh" {
                if !arg_vals.is_empty() {
                    ...
                    let asinh_func = func_refs
                        .get("runtime_math_asinh")
                        .ok_or("runtime_math_asinh not found")?;
                    let sqrt_func = func_refs
                        .get("runtime_math_sqrt")
                        .ok_or("runtime_math_sqrt not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), asinh_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_acosh" {
                if !arg_vals.is_empty() {
                    ...
                    let acosh_func = func_refs
                        .get("runtime_math_acosh")
                        .ok_or("runtime_math_acosh not found")?;
                    let sqrt_func = func_refs
                        .get("runtime_math_sqrt")
                        .ok_or("runtime_math_sqrt not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), acosh_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_atanh" {
                if !arg_vals.is_empty() {
                    ...
                    let atanh_func = func_refs
                        .get("runtime_math_atanh")
                        .ok_or("runtime_math_atanh not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), atanh_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_log2" {
                if !arg_vals.is_empty() {
                    ...
                    let log2_func = func_refs
                        .get("runtime_math_log2")
                        .ok_or("runtime_math_log2 not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), log2_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_log10" {
                if !arg_vals.is_empty() {
                    ...
                    let log10_func = func_refs
                        .get("runtime_math_log10")
                        .ok_or("runtime_math_log10 not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), log10_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "dual_atan2" {
                if arg_vals.len() >= 2 {
                    ...
                    let y_val = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        y_ptr,
                        0,
                    );
                    let y_der = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        y_ptr,
                        8,
                    );
                    let x_val = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        x_ptr,
                        0,
                    );
                    let x_der = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        x_ptr,
                        8,
                    ...
                    let atan2_func = func_refs
                        .get("runtime_math_atan2")
                        .ok_or("runtime_math_atan2 not found")?;
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), atan2_val, ptr, 0);
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), result_der, ptr, 8);
                    return Ok(Some(ptr));
            ...
            if name == "grad" {
                if arg_vals.len() >= 2 {
                    ...
                    builder
                        .ins()
                        .store(cranelift_codegen::ir::MemFlags::new(), one, input_ptr, 8);
                    ...
                    let derivative = builder.ins().load(
                        types::F64,
                        cranelift_codegen::ir::MemFlags::new(),
                        result_ptr,
                        8,
            ...
            if name == "jacobian" {
                if arg_vals.len() >= 3 {
                    ...
                    let jacobian_func = func_refs
                        .get("runtime_jacobian")
                        .ok_or("runtime_jacobian not found")?;
            ...
            if name == "hessian" {
                if arg_vals.len() >= 2 {
                    ...
                    let hessian_func = func_refs
                        .get("runtime_hessian")
                        .ok_or("runtime_hessian not found")?;
                        ...
                        if let Some(base_sig) = func_sigs.get(name) {
                            ...
                            for i in fixed_param_count..promoted_args.len() {
                                ...
                                if arg_type == types::F32 {
                                    promoted_args[i] =
                                        builder.ins().fpromote(types::F64, promoted_args[i]);
                                } else if arg_type == types::I8 || arg_type == types::I16 {
                                    promoted_args[i] =
                                        builder.ins().sextend(types::I32, promoted_args[i]);
                                }
                            }
                            ...
                            let call =
                                builder
                                    .ins()
                                    .call_indirect(sig_ref, func_addr, &promoted_args);
        ...
        Op::PushHandler {
            ...
            if let Some(&func_ref) = func_refs.get("runtime_handler_push") {
                ...
                builder
                    .ins()
                    .call(func_ref, &[effect_val, name_val, handler_id_val]);
            }
            Ok(Some(builder.ins().iconst(types::I64, 0)))
        }
            ...
            if let Some(&dispatch_ref) = func_refs.get("runtime_handler_dispatch") {
                ...
                if !arg_vals.is_empty() {
                    ...
                    for (i, &val) in arg_vals.iter().enumerate() {
                        ...
                        builder
                            .ins()
                            .store(MemFlags::new(), f64_val, args_ptr, (i * 8) as i32);
                    }

                    let args_len = builder.ins().iconst(types::I64, arg_vals.len() as i64);
                    let dispatch_call = builder
                        .ins()
                        .call(dispatch_ref, &[effect_val, op_val, args_ptr, args_len]);
                    ...
                    let dispatch_call = builder
                        .ins()
                        .call(dispatch_ref, &[effect_val, op_val, null_ptr, zero_len]);
                    let results = builder.inst_results(dispatch_call);
                    if !results.is_empty() {
                        return Ok(Some(results[0]));
                    }
