...
impl NativeBackend {
    ...
    pub fn compile(
        &mut self,
        module: &crate::sir::module::SirModule,
    ...
    pub fn compile_to_object(
        ...
        for sym in &code_segment.symbols {
            let size =
                elf::calculate_symbol_size(sym, &code_segment.symbols, code_segment.code.len());
        ...
        // Add relocations
        for reloc in &code_segment.relocations {
            let sym_idx = *symbol_indices.get(&reloc.symbol).ok_or_else(|| {
                CompileError::EmissionFailed(format!("Symbol not found: {}", reloc.symbol))
            })?;
...
mod tests {
    use super::*;
    use metrics::{OpClass, Operand, SirBlock, SirInstruction};
