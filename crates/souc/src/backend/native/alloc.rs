...
impl fmt::Display for AllocStatistics {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            ...
            self.avg_allocated_confidence
        )?;
        writeln!(
            f,
            "  Avg confidence (spilled): {:.3}",
            self.avg_spilled_confidence
        )?;
        writeln!(
            f,
            "  Confidence preserved: {:.1}%",
            self.confidence_preserved * 100.0
        )?;
...
impl EpistemicAllocator {
                ...
                match spill_result {
                    ...
                    SpillDecision::SpillOther {
                        ...
                        if let Some(pos) = self.active.iter().position(|i| i.vreg == victim_vreg) {
                            ...
                            victim
                                .epistemic
                                .apply_spill_degradation(self.config.spill_confidence_factor);
    ...
    fn spill_at_interval(
            ...
            || (victim_score < self.config.min_confidence_threshold && !victim.epistemic.critical)
        {
            let reg = victim
                .assigned
                .expect("Active interval should have register");
            let slot = self.spill_manager.allocate(victim.vreg);

            // Generate spill code
            let spill_code = vec![SpillReloadOp::Spill {
                vreg: victim.vreg,
                reg,
                slot,
                pos: current.start,
...
mod tests {
    ...
    fn make_interval(vreg: u32, start: usize, end: usize, confidence: f64) -> LiveInterval {
        let mut interval = LiveInterval::new(VirtReg(vreg), RegClass::GeneralPurpose, start);
        interval.end = end;
    ...
    fn test_spill_low_confidence() {
        ...
        for interval in &result.spilled {
            ...
            println!(
                "Spilled v{} with confidence {}",
                interval.vreg.0, interval.epistemic.confidence
            );
        }
    ...
    fn test_critical_preservation() {
        ...
        let critical_spilled = result.spilled.iter().any(|i| i.vreg == VirtReg(0));
    ...
    for (block_idx, block) in func.blocks.iter().enumerate() {
        for inst in &block.instructions {
            ...
            if let Some(result_val) = inst.result {
                ...
                if matches!(
                    inst.inst,
                    SirInst::Memory(crate::sir::ops::MemoryOp::Alloca { .. })
                ) {
...
fn determine_register_class(inst: &crate::sir::ops::SirInst, default: &RegClass) -> RegClass {
    use crate::sir::ops::SirInst;
    ...
    match inst {
        ...
        SirInst::Memory(mem_op) => {
            use crate::sir::ops::MemoryOp;
            use crate::sir::types::{ScalarType, SirType};
            match mem_op {
                MemoryOp::ExtractField { field_ty, .. } => match field_ty {
                    SirType::Scalar(ScalarType::F32 | ScalarType::F64) => RegClass::FloatingPoint,
                    _ => RegClass::GeneralPurpose,
                },
                MemoryOp::Load { ty, .. } => match ty {
                    SirType::Scalar(ScalarType::F32 | ScalarType::F64) => RegClass::FloatingPoint,
                    _ => RegClass::GeneralPurpose,
                },
                MemoryOp::Alloca { ty, .. } => {
                    // Alloca returns a pointer, which is GeneralPurpose
...
fn infer_result_type(inst: &crate::sir::ops::SirInst) -> Option<crate::sir::types::SirType> {
    ...
    match inst {
        SirInst::BinOp { op, .. } => match op {
            ...
            | crate::sir::ops::ArithOp::FMul
            | crate::sir::ops::ArithOp::FDiv
            | crate::sir::ops::ArithOp::FRem => Some(SirType::Scalar(ScalarType::F64)),
            _ => Some(SirType::Scalar(ScalarType::I64)),
        },
        SirInst::BuildAggregate { ty, .. } => Some(ty.clone()),
        SirInst::Const(c) => match c {
            crate::sir::values::Constant::F64(_) => Some(SirType::Scalar(ScalarType::F64)),
            crate::sir::values::Constant::F32(_) => Some(SirType::Scalar(ScalarType::F32)),
            crate::sir::values::Constant::I64(_)
            | crate::sir::values::Constant::I32(_)
            | crate::sir::values::Constant::I16(_)
            | crate::sir::values::Constant::I8(_) => Some(SirType::Scalar(ScalarType::I64)),
            crate::sir::values::Constant::Bool(_) => Some(SirType::Scalar(ScalarType::Bool)),
            _ => None,
        },
        SirInst::Memory(mem_op) => {
            use crate::sir::ops::MemoryOp;
            match mem_op {
                MemoryOp::ExtractField { field_ty, .. } => Some(field_ty.clone()),
                MemoryOp::Load { ty, .. } => Some(ty.clone()),
                MemoryOp::Alloca { ty, .. } => {
                    // Alloca returns a pointer to the type
                    Some(SirType::ptr(ty.clone()))
                }
                _ => None,
            }
        }
