// Integration test: self-hosted import resolution parsing
//
// Verifies that the self-hosted suite loads and parses correctly with
// the new selective import syntax (use path::{A, B, C}). Execution
// correctness is verified by Gate 2.

use sounio::module_loader;
use std::path::Path;

#[test]
fn imports_selfhost_parses() {
    ...
    let ast = match module_loader::load_program_ast(&suite_path) {
        Ok(ast) => ast,
        Err(e) => {
            panic!("Failed to load self-hosted suite with import extensions: {:?}", e);
        }
    };
