...
use sounio::codegen::gpu::autodiff::{
    AutoDiff, AutoDiffTransform, BackwardCodegen, DiffConfig, DiffContext, DiffPrimitive, GpuTape,
    GpuTapeConfig, KernelAnalysis, ParamInfo, PrimitiveRegistry, TapeEntry, TapeOp,
};
...
fn create_chain_rule_kernel() -> GpuKernel {
    ...
    block.add_instruction(
        ValueId(10),
        GpuOp::GetElementPtr(ValueId(9), vec![ValueId(0)]),
    );
    block.add_instruction(
        ValueId(11),
...
fn test_differentiate_mul_kernel() {
    ...
    assert!(
        backward.gradient_params.contains_key("x")
            || backward.requires_grad.contains(&"x".to_string())
    );
    assert!(
        backward.gradient_params.contains_key("w")
            || backward.requires_grad.contains(&"w".to_string())
    );
}
...
fn test_fma_differentiation() {
    ...
    block.add_instruction(
        ValueId(8),
        GpuOp::GetElementPtr(ValueId(7), vec![ValueId(0)]),
    );
    ...
    block.add_instruction(
        ValueId(10),
        GpuOp::FMulAdd(ValueId(3), ValueId(6), ValueId(9)),
    );
    ...
    block.add_instruction(
        ValueId(12),
        GpuOp::GetElementPtr(ValueId(11), vec![ValueId(0)]),
    );
    block.add_instruction(
        ValueId(13),
