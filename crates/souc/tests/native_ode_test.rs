...
use sounio::backend::native::ode_runtime::{sounio_ode_cashkarp_step, sounio_ode_dopri5_step};
...
unsafe extern "C" fn exp_decay_derivatives(_state: *mut f64, _t: f64, dydt: *mut f64) {
...
unsafe extern "C" fn lotka_volterra_derivatives(state: *mut f64, _t: f64, dydt: *mut f64) {
...
fn test_error_estimation() {
    ...
    assert!(
        err < 1.0,
        "Error estimate should be less than 1.0 for accepted step"
    );
}
...
fn test_step_size_adaptation() {
    ...
    if !err.is_infinite() {
        ...
        assert!(
            dt < initial_dt,
            "Step size should be reduced after rejection"
        );
    }
}
