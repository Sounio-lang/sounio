...
fn test_minimal_object_file() {
    ...
    compile_with_native_backend(&input, &output, "o").expect("Failed to compile minimal test");
...
fn test_complex_object_file() {
    ...
    compile_with_native_backend(&input, &output, "o").expect("Failed to compile complex test");
...
fn test_complex_has_functions() {
    ...
    compile_with_native_backend(&input, &output, "o").expect("Failed to compile");

    // Use readelf to check for function symbols
    let readelf_output = Command::new("readelf").arg("-s").arg(&output).output();
...
fn test_complex_has_loops() {
    ...
    compile_with_native_backend(&input, &output, "o").expect("Failed to compile code with loops");
...
fn test_elf_has_text_section() {
    ...
    if let Ok(output) = readelf_output {
        ...
        assert!(stdout.contains(".text"), "ELF should have .text section");
    } else {
