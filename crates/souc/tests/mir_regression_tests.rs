...
use sounio::mir::instructions::{MirBinaryOp, MirInstruction, MirTerminator};
use sounio::mir::optimization::{
    ConstantPropagation, FunctionInlining, MIRPass, StrengthReduction,
};
use sounio::mir::types::{MirConstant, MirType};
...
fn test_function_inlining_semantics() {
    ...
    // Note: Inlining behavior depends on implementation
    ...
    // The exact number of operations depends on whether inlining occurred
...
fn test_loop_analysis_control_flow() {
    ...
    // Entry block
    let entry_block = func_builder.create_block("entry");
    let loop_header = func_builder.create_block("loop_header");
    let loop_body = func_builder.create_block("loop_body");
    let exit = func_builder.create_block("exit");
    ...
    func_builder.build_compare(
        cmp,
        sounio::mir::instructions::MirCompareOp::Lt,
        i_var,
        const_10,
        MirType::I64,
    );
    ...
    // Loop detection depends on the analysis implementation
    ...
    assert_eq!(func.blocks.len(), 5); // auto-entry, entry, header, body, exit
}
...
fn test_ssa_properties_after_optimization() {
    ...
    let strength_red = StrengthReduction::new();
...
fn test_optimization_edge_cases_empty_function() {
    ...
    let constant_prop = ConstantPropagation;
    let strength_red = StrengthReduction::new();
    let func_inlining = FunctionInlining::new();
...
fn test_optimization_recursive_functions() {
    ...
    factorial.build_compare(
        cmp_result,
        sounio::mir::instructions::MirCompareOp::Le,
        param,
        const_1,
        MirType::I64,
    );
...
fn test_floating_point_optimizations() {
    ...
    // Verify the function structure is preserved
    let entry = &func.blocks[0];
    // Note: Constant propagation may fold the addition into a constant
    // The important thing is that the optimization doesn't crash or produce invalid IR
    assert!(
        !entry.instructions.is_empty(),
...
fn test_function_signature_preservation() {
    ...
    assert_eq!(func.return_type, MirType::F64);
}
