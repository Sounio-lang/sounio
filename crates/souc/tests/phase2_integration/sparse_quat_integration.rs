...
mod nxm_sparsity_tests {
    ...
    fn test_nxm_flexibility() {
        let patterns = vec![(1, 2), (1, 4), (1, 8), (2, 4), (2, 8), (4, 8)];
...
mod csr_format_tests {
    ...
    fn test_csr_format_conversion_dense_to_sparse() {
        let dense = vec![1.0, 0.0, 2.0, 0.0, 0.0, 3.0, 0.0, 4.0, 5.0, 0.0, 0.0, 6.0];
    ...
    fn test_csr_row_wise_pattern() {
        let _matrix = vec![1.0, 2.0, 0.0, 0.0, 0.0, 3.0, 4.0, 0.0, 5.0, 0.0, 6.0, 7.0];

        // Row 0: 2 non-zeros
        // Row 1: 2 non-zeros
        // Row 2: 3 non-zeros
        let row_ptrs = vec![0, 2, 4, 7];
        let row_nnz: Vec<usize> = row_ptrs.windows(2).map(|w| w[1] - w[0]).collect();
...
mod end_to_end_sparse_tests {
    ...
    fn test_sparse_quat_accuracy_loss() {
        ...
        let dense_values: Vec<f32> = quats
            .iter()
            .flat_map(|q| vec![q.w, q.x, q.y, q.z])
            .collect();
