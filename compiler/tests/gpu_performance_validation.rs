...
use sounio::codegen::gpu::{
    ArchPeakPerf, BlockId, CudaArch, GpuBlock, GpuKernel, KernelProfiler,
};
    ...
    pub fn measure_dispatch_overhead(&self) -> DispatchOverheadAnalysis {
        ...
        for (name, kernel) in kernels {
            ...
            let dispatch_pct = if profile.cost_estimate.total_cycles > 0 {
                (estimated_dispatch_cycles as f64 / profile.cost_estimate.total_cycles as f64) * 100.0
            } else {
                0.0
            };
...
impl WmmaUtilizationValidator {
    ...
    fn generate_wmma_recommendations(
        ...
        if utilization < 50.0 {
            ...
            recommendations.push(
                "Increase tensor core operations relative to other compute".to_string(),
            );
        }

        // Check for memory bandwidth bottlenecks
        if profile.score.memory_efficiency < 0.5 {
            recommendations.push("Memory bandwidth is limiting - improve data locality".to_string());
        }

        // Check for occupancy issues
        if profile.score.occupancy_score < 0.6 {
            recommendations.push("Low occupancy limiting performance - reduce registers/shared memory"
                .to_string());
        }
...
impl AccuracyComparator {
    ...
    pub fn compare_precision(&self) -> PrecisionComparison {
        ...
        let test_cases = vec![
            ("matrix_multiply_512x512", self.generate_matmul_data(512, 512, 512)),
...
fn test_profiler_tools_detection() {
    ...
    for gpu in &tools.gpu_devices {
        println!(
            ...
            gpu.index, gpu.name, gpu.compute_capability.0, gpu.compute_capability.1, gpu.has_tensor_cores
        );
    }
}
...
fn test_dispatch_overhead_measurement() {
    ...
    for kernel in &analysis.overhead_by_kernel {
        println!("  {}: {} cycles ({:.2}%)", kernel.name, kernel.dispatch_cycles, kernel.dispatch_percentage);
    }
...
fn test_int8_vs_fp32_accuracy() {
    ...
    for result in &comparison.test_results {
        println!(
            ...
            if result.passes_threshold { "PASS" } else { "FAIL" }
        );
    }
...
pub fn generate_performance_report(
    ...
    for result in wmma_results {
        report.push_str(&format!(
            ...
            if result.achieves_target { "✓ PASS" } else { "✗ FAIL" }
        ));
    }
    report.push_str("\n");
    ...
    for result in &precision_comparison.test_results {
        report.push_str(&format!(
            "- **{}**: L2={:.2e}, L∞={:.2e}, Relative={:.4}%\n",
            result.test_name, result.l2_error, result.linf_error, result.relative_error * 100.0
        ));
    }
    report.push_str(&format!(
