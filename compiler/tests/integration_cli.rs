...
"#,
    ...
    if !output.status.success() {
        assert!(
            stderr.contains("error") || stderr.contains("Error") || stderr.contains("not implemented"),
...
fn test_cli_help() {
    ...
    assert!(stdout.contains("build"), "help should mention 'build' command");
}
...
fn test_cli_check_multiple_files_fail() {
    let temp_dir = TempDir::new().unwrap();
    let test_file1 = create_test_file(
        &temp_dir,
        "test1.sio",
        "fn main() -> i64 { 1 }",
    );
