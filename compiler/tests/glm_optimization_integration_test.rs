...
mod glm_optimization_integration {
    use sounio::mir::optimization::glm_integration::{
        GLMManager, GLMConfig, OptimizationSuggestion, OptimizationType
    };
    use sounio::mir::{MirModule, MirFunction, MirBlock, MirInstruction};
    ...
    fn test_optimization_suggestion_creation() {
        ...
        assert_eq!(suggestion.optimization_type, OptimizationType::ConstantPropagation);
        assert_eq!(suggestion.confidence, 0.95);
        assert_eq!(suggestion.target, "test_function");
        assert_eq!(suggestion.parameters.get("threshold"), Some(&"0.8".to_string()));
    }
    ...
    fn test_optimization_suggestion_serialization() {
        ...
        assert_eq!(parsed.parameters.get("unroll_factor"), Some(&"4".to_string()));
    }
    ...
    fn test_epistemic_optimization_type() {
        ...
        assert_eq!(suggestion.optimization_type, OptimizationType::UncertaintyAwareOptimization);
        assert!(suggestion.confidence >= 0.9);
        assert_eq!(suggestion.parameters.get("optimize_knowledge_ops"), Some(&"true".to_string()));
    }
    ...
    async fn test_glm_api_integration() {
        ...
        let mut test_function = MirFunction::new(
        ...
        test_block.add_instruction(MirInstruction::Return(
            MirOperand::Constant(MirConstant::I32(42))
        ));
    ...
    fn test_optimization_level_integration() {
        ...
        assert!(!OptimizationLevel::O0.should_enable_pass(
            &std::marker::PhantomData::<sounio::mir::optimization::ml_guided_optimization::MLGuidedOptimization>
        ));
...
mod glm_optimization_integration {
    ...
    fn test_glm_feature_disabled() {
        ...
        assert!(true, "GLM feature tests are disabled when feature is not enabled");
    }
}