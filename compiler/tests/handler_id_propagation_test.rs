...
use sounio::sir::module::{get_handler_id, HandlerMetadata, SirModule, EFFECT_HANDLER_MAP};
...
fn test_handler_id_mapping() {
    ...
    for (effect_name, expected_id) in expected_effects {
        ...
        assert!(handler_id.is_some(), "No handler ID for effect: {}", effect_name);
        
        let id = handler_id.unwrap();
        assert_eq!(id, expected_id, "Wrong handler ID for {}: expected {}, got {}", 
...
fn test_effect_handler_map_constant() {
    ...
    assert_eq!(EFFECT_HANDLER_MAP.len(), 13, "Should have 13 standard effects");
    
    // Verify no duplicate IDs
    let mut ids = std::collections::HashSet::new();
    for (effect, id) in EFFECT_HANDLER_MAP {
        assert!(!ids.contains(id), "Duplicate handler ID: {} used by {}", id, effect);
        ids.insert(*id);
    }
    
    println!("Handler ID map has {} entries with no duplicates", EFFECT_HANDLER_MAP.len());
}
...
fn test_handler_id_uniqueness() {
    ...
    for (effect, id) in EFFECT_HANDLER_MAP {
        assert!(*id > 0 && *id <= 13, "Handler ID {} for {} is out of range", id, effect);
