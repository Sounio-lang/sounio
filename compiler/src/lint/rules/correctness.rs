...
fn is_zero_literal(expr: &Expr) -> bool {
    match expr {
        Expr::Literal { value: Literal::Int(0), .. } => true,
        Expr::Literal { value: Literal::Float(f), .. } => *f == 0.0,
        _ => false,
    }
}
...
fn is_literal_true(expr: &Expr) -> bool {
    matches!(expr, Expr::Literal { value: Literal::Bool(true), .. })
}
    ...
    match expr {
        ...
        Expr::If { then_branch, else_branch, .. } => {
            has_break_or_return(then_branch)
                || else_branch.as_ref().map_or(false, |e| has_break_or_return_expr(e))
        }
