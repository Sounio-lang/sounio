...
fn is_small_type_expr(expr: &Expr) -> bool {
    match expr {
        ...
        Expr::Tuple { elements, .. } => elements.len() <= 2 && elements.iter().all(is_small_type_expr),
        _ => false,
    }
}
...
fn is_large_type(ty: &TypeExpr) -> bool {
    match ty {
        TypeExpr::Array { size: Some(n), .. } => {
            ...
            if let Expr::Literal { value: crate::ast::Literal::Int(size), .. } = n.as_ref() {
                *size > 16
            } else {
                false
            }
        ...
        TypeExpr::Named { path, .. } => {
            ...
            path.segments.last().map_or(false, |name| large_types.contains(&name.as_str()))
        }
...
fn is_very_large_type(ty: &TypeExpr) -> bool {
    match ty {
        ...
        TypeExpr::Named { path, .. } => {
            ...
            path.segments.last().map_or(false, |name| very_large.contains(&name.as_str()))
        }
...
mod tests {
    ...
    fn test_is_large_type() {
        ...
        assert!(is_large_type(&TypeExpr::Tuple(vec![
            TypeExpr::Named { path: Path::simple("i32"), args: vec![], unit: None },
            TypeExpr::Named { path: Path::simple("i32"), args: vec![], unit: None },
            TypeExpr::Named { path: Path::simple("i32"), args: vec![], unit: None },
            TypeExpr::Named { path: Path::simple("i32"), args: vec![], unit: None },
            TypeExpr::Named { path: Path::simple("i32"), args: vec![], unit: None },
