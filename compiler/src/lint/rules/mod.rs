...
mod tests {
    ...
    #[test]
    fn test_lint_groups() {
        assert!(LintGroup::Linear.lint_ids().contains(&"unused_linear_value"));
        assert!(LintGroup::Epistemic.lint_ids().contains(&"low_confidence_propagation"));
    }

    #[test]
    fn test_group_from_name() {
        assert_eq!(LintGroup::from_name("linear"), Some(LintGroup::Linear));
        assert_eq!(LintGroup::from_name("EPISTEMIC"), Some(LintGroup::Epistemic));
