...
use crate::lint::{
    Applicability, Edit, Lint, LintCategory, LintContext, LintLevel, Suggestion,
};
            ...
            for (name, span) in declared {
                if !used.contains(&name) && !name.starts_with('_') {
                    ctx.report_with_suggestion(
                        ...
                        span,
                        Suggestion {
                            message: format!("if this is intentional, prefix with underscore: `_{}`", name),
                            edits: vec![Edit {
...
fn is_snake_case(name: &str) -> bool {
    ...
    if !name.chars().next().map_or(false, |c| c.is_lowercase() || c.is_numeric()) {
        return false;
    }
