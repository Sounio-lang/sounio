...
pub enum DimensionError {
    ...
    MatmulInnerMismatch { left_cols: DimSize, right_rows: DimSize },
    /// Cannot broadcast shapes
    BroadcastIncompatible { left: Vec<DimSize>, right: Vec<DimSize> },
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::RankMismatch { expected, found } => {
                write!(f, "rank mismatch: expected {} dimensions, found {}", expected, found)
            }
            Self::MatmulInnerMismatch { left_cols, right_rows } => {
                write!(
                    f,
            ...
            Self::InfiniteDimension(var) => {
                write!(f, "infinite dimension: ?D{} occurs in its own definition", var.0)
            }
...
impl DimensionSolver {
    ...
    pub fn matmul_shape(
        ...
        self.unify(k1.clone(), k2.clone()).map_err(|_| {
            DimensionError::MatmulInnerMismatch {
                left_cols: k1.clone(),
                right_rows: k2.clone(),
            }
        })?;
...
mod tests {
    ...
    fn test_concat_shape() {
        ...
        let shapes = vec![TensorShape::Static(vec![3, 4]), TensorShape::Static(vec![5, 4])];
