...
impl PortUsage {
    pub fn alu() -> Self {
        Self { p0: true, p1: true, p5: true, ..Default::default() }
    }
    pub fn fma() -> Self {
        Self { p0: true, p1: true, ..Default::default() }
    }
    pub fn load() -> Self {
        Self { p23: true, ..Default::default() }
    }
    pub fn store() -> Self {
        Self { p4: true, p7: true, ..Default::default() }
    }
    pub fn div() -> Self {
        Self { p0: true, ..Default::default() }
    }
}
...
impl CycleModel {
    ...
    pub fn skylake_like() -> Self {
        ...
        latencies.insert(OpClass::IntArithmetic, CycleSpec {
            latency: 1,
            throughput_recip: 0.25,  // 4 per cycle
            confidence: 0.98,
            ports: PortUsage::alu(),
        });
        latencies.insert(OpClass::IntMultiply, CycleSpec {
            latency: 3,
            throughput_recip: 1.0,
            confidence: 0.95,
            ports: PortUsage { p1: true, ..Default::default() },
        });
        latencies.insert(OpClass::IntDivide, CycleSpec {
            latency: 26,  // 64-bit: 26-90 cycles, highly variable
            throughput_recip: 6.0,
            confidence: 0.60,  // High variance
            ports: PortUsage::div(),
        });
        latencies.insert(OpClass::IntCompare, CycleSpec {
            latency: 1,
        ...
        // Floating-point operations
        latencies.insert(OpClass::FloatArithmetic, CycleSpec {
            latency: 4,
            throughput_recip: 0.5,
            confidence: 0.95,
            ports: PortUsage::fma(),
        });
        latencies.insert(OpClass::FloatMultiply, CycleSpec {
            latency: 4,
            throughput_recip: 0.5,
            confidence: 0.95,
            ports: PortUsage::fma(),
        });
        latencies.insert(OpClass::FloatDivide, CycleSpec {
            latency: 14,  // divsd: 13-14 cycles
            throughput_recip: 4.0,
            confidence: 0.85,
            ports: PortUsage::div(),
        });
        latencies.insert(OpClass::FloatSqrt, CycleSpec {
            latency: 18,  // sqrtsd: 15-18 cycles
            throughput_recip: 4.5,
            confidence: 0.85,
            ports: PortUsage::div(),
        });
        latencies.insert(OpClass::FloatFma, CycleSpec {
            latency: 4,
            throughput_recip: 0.5,
            confidence: 0.95,
            ports: PortUsage::fma(),
        });
        latencies.insert(OpClass::FloatConvert, CycleSpec {
            latency: 5,
            throughput_recip: 1.0,
            confidence: 0.90,
            ports: PortUsage { p1: true, ..Default::default() },
        ...
        latencies.insert(OpClass::MemoryLoad, CycleSpec {
            latency: 4,  // L1 hit
            throughput_recip: 0.5,  // 2 loads per cycle
            confidence: 0.70,  // Cache-dependent variance
            ports: PortUsage::load(),
        });
        latencies.insert(OpClass::MemoryStore, CycleSpec {
            latency: 1,  // Store doesn't block
            throughput_recip: 1.0,
            confidence: 0.80,
            ports: PortUsage::store(),
        });
        latencies.insert(OpClass::MemoryLoadUnaligned, CycleSpec {
            latency: 5,  // +1 cycle penalty
            throughput_recip: 1.0,
            confidence: 0.65,
            ports: PortUsage::load(),
        });
        latencies.insert(OpClass::MemoryPrefetch, CycleSpec {
        ...
        // Control flow
        latencies.insert(OpClass::Branch, CycleSpec {
            latency: 1,  // Predicted correctly
            throughput_recip: 0.5,
            confidence: 0.60,  // Branch prediction variance
            ports: PortUsage { p0: true, ..Default::default() },
        });
        latencies.insert(OpClass::BranchIndirect, CycleSpec {
            latency: 2,
            throughput_recip: 1.0,
            confidence: 0.50,  // Harder to predict
            ports: PortUsage { p0: true, ..Default::default() },
        });
        latencies.insert(OpClass::Call, CycleSpec {
            latency: 2,
            throughput_recip: 1.0,
            confidence: 0.85,
            ports: PortUsage { p0: true, ..Default::default() },
        });
        latencies.insert(OpClass::Return, CycleSpec {
            latency: 1,
            throughput_recip: 1.0,
            confidence: 0.85,
            ports: PortUsage { p0: true, ..Default::default() },
        ...
        // SIMD operations
        latencies.insert(OpClass::Simd128, CycleSpec {
            latency: 4,
            throughput_recip: 0.5,
            confidence: 0.92,
            ports: PortUsage::fma(),
        });
        latencies.insert(OpClass::Simd256, CycleSpec {
            latency: 4,
            throughput_recip: 0.5,  // AVX2 full width
            confidence: 0.90,
            ports: PortUsage::fma(),
        });
        latencies.insert(OpClass::Simd512, CycleSpec {
            latency: 6,
            throughput_recip: 1.0,  // AVX-512 often downclocks
            confidence: 0.75,  // Frequency throttling variance
            ports: PortUsage::fma(),
        });
        latencies.insert(OpClass::SimdShuffle, CycleSpec {
            latency: 1,
            throughput_recip: 1.0,
            confidence: 0.90,
            ports: PortUsage { p5: true, ..Default::default() },
        });
        latencies.insert(OpClass::SimdHorizontal, CycleSpec {
            latency: 6,  // Horizontal ops are slower
            throughput_recip: 2.0,
            confidence: 0.88,
            ports: PortUsage { p0: true, p5: true, ..Default::default() },
        });

        // Special operations
        latencies.insert(OpClass::Crypto, CycleSpec {
            latency: 4,  // AES-NI single round
            throughput_recip: 1.0,
            confidence: 0.95,
            ports: PortUsage { p0: true, ..Default::default() },
        });
        latencies.insert(OpClass::Atomic, CycleSpec {
            latency: 20,  // lock prefix: ~20 cycles uncontended
            throughput_recip: 20.0,
            confidence: 0.50,  // Highly contention-dependent
            ports: PortUsage::load(),
        });
        latencies.insert(OpClass::Syscall, CycleSpec {
            latency: 150,  // System call overhead
            throughput_recip: 150.0,
            confidence: 0.30,  // Extreme variance
            ports: PortUsage::default(),
        });
        latencies.insert(OpClass::Nop, CycleSpec {
            latency: 0,
            throughput_recip: 0.25,
            confidence: 1.0,
            ports: PortUsage::default(),
        ...
        latencies.insert(OpClass::EpistemicPropagate, CycleSpec {
            latency: 8,   // sqrt + multiply for δ propagation
            throughput_recip: 2.0,
            confidence: 0.85,
            ports: PortUsage::fma(),
        });
        latencies.insert(OpClass::EpistemicMerge, CycleSpec {
            latency: 4,   // min/max + some bookkeeping
            throughput_recip: 1.0,
            confidence: 0.90,
            ports: PortUsage::alu(),
        });
        latencies.insert(OpClass::EpistemicCheck, CycleSpec {
...
impl PowerModel {
    ...
    pub fn finfet_7nm() -> Self {
        ...
        // Integer operations
        energy.insert(OpClass::IntArithmetic, EnergySpec {
            dynamic_pj: 0.03,
            leakage_uw: 0.05,
            confidence: 0.90,
        });
        energy.insert(OpClass::IntMultiply, EnergySpec {
            dynamic_pj: 0.35,
            leakage_uw: 0.15,
            confidence: 0.88,
        });
        energy.insert(OpClass::IntDivide, EnergySpec {
            dynamic_pj: 2.5,
            leakage_uw: 0.8,
            confidence: 0.75,
        });
        energy.insert(OpClass::IntCompare, EnergySpec {
            dynamic_pj: 0.02,
            leakage_uw: 0.03,
            confidence: 0.92,
        });
        ...
        energy.insert(OpClass::FloatArithmetic, EnergySpec {
            dynamic_pj: 0.9,
            leakage_uw: 0.3,
            confidence: 0.88,
        });
        energy.insert(OpClass::FloatMultiply, EnergySpec {
            dynamic_pj: 1.5,
            leakage_uw: 0.5,
            confidence: 0.85,
        });
        energy.insert(OpClass::FloatDivide, EnergySpec {
            dynamic_pj: 8.0,
            leakage_uw: 2.0,
            confidence: 0.75,
        });
        energy.insert(OpClass::FloatSqrt, EnergySpec {
            dynamic_pj: 10.0,
            leakage_uw: 2.5,
            confidence: 0.75,
        });
        energy.insert(OpClass::FloatFma, EnergySpec {
            dynamic_pj: 2.0,
            leakage_uw: 0.6,
            confidence: 0.85,
        });
        energy.insert(OpClass::FloatConvert, EnergySpec {
            dynamic_pj: 0.5,
            leakage_uw: 0.2,
            confidence: 0.88,
        });
        ...
        energy.insert(OpClass::MemoryLoad, EnergySpec {
            dynamic_pj: 5.0,   // L1D read
            leakage_uw: 2.0,
            confidence: 0.70,  // Cache-dependent
        });
        energy.insert(OpClass::MemoryStore, EnergySpec {
            dynamic_pj: 3.5,   // L1D write
            leakage_uw: 1.5,
            confidence: 0.72,
        });
        energy.insert(OpClass::MemoryLoadUnaligned, EnergySpec {
            dynamic_pj: 7.0,
            leakage_uw: 2.5,
            confidence: 0.65,
        });
        energy.insert(OpClass::MemoryPrefetch, EnergySpec {
            dynamic_pj: 1.0,
            leakage_uw: 0.5,
            confidence: 0.80,
        });
        ...
        // Control flow
        energy.insert(OpClass::Branch, EnergySpec {
            dynamic_pj: 0.1,
            leakage_uw: 0.05,
            confidence: 0.85,
        });
        energy.insert(OpClass::BranchIndirect, EnergySpec {
            dynamic_pj: 0.15,
            leakage_uw: 0.08,
            confidence: 0.80,
        });
        energy.insert(OpClass::Call, EnergySpec {
            dynamic_pj: 0.2,
            leakage_uw: 0.1,
            confidence: 0.82,
        });
        energy.insert(OpClass::Return, EnergySpec {
            dynamic_pj: 0.15,
            leakage_uw: 0.08,
            confidence: 0.82,
        });
        ...
        // SIMD operations
        energy.insert(OpClass::Simd128, EnergySpec {
            dynamic_pj: 3.0,
            leakage_uw: 1.0,
            confidence: 0.85,
        });
        energy.insert(OpClass::Simd256, EnergySpec {
            dynamic_pj: 5.5,
            leakage_uw: 1.8,
            confidence: 0.82,
        });
        energy.insert(OpClass::Simd512, EnergySpec {
            dynamic_pj: 12.0,
            leakage_uw: 4.0,
            confidence: 0.70,  // Frequency throttling effects
        });
        energy.insert(OpClass::SimdShuffle, EnergySpec {
            dynamic_pj: 1.5,
            leakage_uw: 0.5,
            confidence: 0.85,
        });
        energy.insert(OpClass::SimdHorizontal, EnergySpec {
            dynamic_pj: 4.0,
            leakage_uw: 1.2,
            confidence: 0.80,
        });
        ...
        // Special operations
        energy.insert(OpClass::Crypto, EnergySpec {
            dynamic_pj: 4.0,
            leakage_uw: 1.5,
            confidence: 0.88,
        });
        energy.insert(OpClass::Atomic, EnergySpec {
            dynamic_pj: 15.0,
            leakage_uw: 5.0,
            confidence: 0.50,  // Highly variable
        });
        energy.insert(OpClass::Syscall, EnergySpec {
            dynamic_pj: 500.0,  // Context switch overhead
            leakage_uw: 50.0,
            confidence: 0.30,
        });
        energy.insert(OpClass::Nop, EnergySpec {
            dynamic_pj: 0.01,
            leakage_uw: 0.01,
            confidence: 0.99,
        });
        ...
        energy.insert(OpClass::EpistemicPropagate, EnergySpec {
            dynamic_pj: 15.0,  // sqrt + muls + adds
            leakage_uw: 3.0,
            confidence: 0.80,
        });
        energy.insert(OpClass::EpistemicMerge, EnergySpec {
            dynamic_pj: 5.0,
            leakage_uw: 1.0,
            confidence: 0.85,
        });
        energy.insert(OpClass::EpistemicCheck, EnergySpec {
            dynamic_pj: 2.0,
            leakage_uw: 0.5,
            confidence: 0.88,
        });
...
impl CycleEstimate {
    ...
    pub fn effective_cycles(&self) -> u64 {
        let throughput_limited: f64 = self.breakdown.values()
...
impl fmt::Display for PowerEstimate {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            self.average_power_uw, self.peak_power_uw, 
            self.dynamic_energy_pj, self.confidence, self.temperature_rise_k
        )
    }
}
...
impl SirMetricsEstimator {
    ...
    pub fn estimate_cycles(&self, block: &SirBlock) -> CycleEstimate {
        let mut estimate = CycleEstimate::new(
            Provenance::sir_backend(&self.cycle_model.arch_name)
        );
                ...
                for src in &inst.sources {
                    if matches!(src, Operand::Memory { .. }) {
                        ...
                        let miss_penalty = (self.cycle_model.l2_hit_cycles - 
                                           self.cycle_model.l1_hit_cycles) as f64 * 0.1;
    ...
    pub fn estimate_power(&self, block: &SirBlock, cycles: &CycleEstimate) -> PowerEstimate {
        let mut estimate = PowerEstimate::new(
            Provenance::sir_backend(&self.power_model.tech_node)
        );
            ...
            if let Some(ref ep) = inst.epistemic {
                if ep.propagates_uncertainty {
                    if let Some(prop_spec) = self.power_model.get_spec(OpClass::EpistemicPropagate) {
                        estimate.add_instruction(OpClass::EpistemicPropagate, prop_spec, voltage_scale);
                    }
                }
            }
        }
        ...
        let execution_time_ns = cycles.effective_cycles() as f64 / 
                                self.power_model.current_frequency;  // GHz -> ns
...
mod tests {
    ...
    fn sample_block() -> SirBlock {
        ...
        block.push(SirInstruction {
            op_class: OpClass::MemoryLoad,
            sources: vec![Operand::Memory { base: 0, offset: 8, scale: 1 }],
        ...
        block.push(SirInstruction {
            ...
            destination: Some(Operand::Memory { base: 0, offset: 16, scale: 1 }),
            epistemic: None,
        });
