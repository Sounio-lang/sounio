...
fn bdf_coefficients(order: usize) -> (Vec<f64>, f64, f64) {
    match order {
        ...
        3 => {
            ...
            (vec![11.0 / 6.0, -3.0, 3.0 / 2.0, -1.0 / 3.0], 1.0, 3.0 / 22.0)
        }
        4 => {
            // BDF4
            (vec![25.0 / 12.0, -4.0, 3.0, -4.0 / 3.0, 1.0 / 4.0], 1.0, 12.0 / 125.0)
        }
        5 => {
            // BDF5
            (vec![137.0 / 60.0, -5.0, 5.0, -10.0 / 3.0, 5.0 / 4.0, -1.0 / 5.0], 1.0, 10.0 / 137.0)
        }
...
pub unsafe extern "C" fn sounio_ode_bdf_step(
    ...
    if need_new_jacobian {
        ...
        for i in 0..n {
            for j in 0..n {
                let idx = i * n + j;
                bdf_state.lu_matrix[idx] = if i == j { 1.0 } else { 0.0 }
                    - gamma * bdf_state.jacobian_cache[idx];
            }
        }

        // LU decomposition
        bdf_state.lu_valid = lu_decompose(
            &mut bdf_state.lu_matrix,
            &mut bdf_state.lu_pivot,
            n,
        );
...
fn adams_moulton_coefficients(order: usize) -> Vec<f64> {
    match order {
        ...
        4 => vec![251.0 / 720.0, 646.0 / 720.0, -264.0 / 720.0, 106.0 / 720.0, -19.0 / 720.0],
...
pub unsafe extern "C" fn sounio_ode_lsoda_step(
    ...
    if step_accepted {
        ...
        *t = t_new;
        ...
        let max_history = if lsoda_state.method == LSODAMethod::Adams { 13 } else { 6 };
        ...
        if lsoda_state.steps_since_switch >= 10 {
            let is_stiff = detect_stiffness_lsoda(
                state,
                current_t,
                h,
                derivatives,
        ...
        // Order control
        let max_order = if lsoda_state.method == LSODAMethod::Adams { 4 } else { 5 };
        if error_norm < 0.1
...
unsafe fn lsoda_bdf_step(
    ...
    if !state.lu_valid || jacobian_age > h * 10.0 {
        ...
        for i in 0..n {
            for j in 0..n {
                let idx = i * n + j;
                state.lu_matrix[idx] = if i == j { 1.0 } else { 0.0 }
                    - gamma * state.jacobian_cache[idx];
            }
        }
