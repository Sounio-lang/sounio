...
impl EffectOp {
    ...
    pub fn effect_name(&self) -> &str {
        match self {
            EffectOp::IoRead | EffectOp::IoPrint | EffectOp::IoPrintln
            | EffectOp::IoWrite | EffectOp::IoFlush => "IO",
            ...
            EffectOp::ProbSample | EffectOp::ProbObserve | EffectOp::ProbFactor
            | EffectOp::ProbScore => "Prob",

            EffectOp::GradForward | EffectOp::GradReverse | EffectOp::GradJvp
            | EffectOp::GradVjp => "Grad",

            EffectOp::CausalDo | EffectOp::CausalObserve | EffectOp::CausalCounterfactual
            | EffectOp::CausalQuery | EffectOp::CausalAte | EffectOp::CausalCate => "Causal",
...
impl EffectDispatch for UnifiedEffectRuntime {
    fn dispatch_effect(&mut self, effect: &str, op: &str, args: &[f64]) -> Result<f64, EffectError> {
        ...
        let result = match effect_op {
            ...
            EffectOp::IoRead => {
                crate::runtime::handler_stack::__sounio_dispatch_io_read()
            }
            ...
            EffectOp::MutGet => {
                if !args.is_empty() {
                    ...
                    return Err(EffectError::InvalidArgs("MutGet requires key argument".to_string()));
                }
            }
            EffectOp::MutSet => {
                if args.len() >= 2 {
                    ...
                    return Err(EffectError::InvalidArgs("MutSet requires key and value arguments".to_string()));
                }
            }
            EffectOp::MutModify => {
                if args.len() >= 2 {
                    ...
                    return Err(EffectError::InvalidArgs("MutModify requires key and delta arguments".to_string()));
                }
            }
            ...
            EffectOp::DivDiv => {
                if args.len() >= 2 {
                    ...
                    return Err(EffectError::InvalidArgs("DivDiv requires two arguments".to_string()));
                }
            }
            ...
            EffectOp::ProbSample => {
                if args.len() >= 2 {
                    ...
                    return Err(EffectError::InvalidArgs("ProbSample requires low and high arguments".to_string()));
                }
            }
            EffectOp::ProbObserve => {
                if args.len() >= 3 {
                    crate::runtime::handler_stack::__sounio_dispatch_prob_observe(args[0], args[1], args[2])
                } else {
                    return Err(EffectError::InvalidArgs("ProbObserve requires mean, std, and value arguments".to_string()));
                }
            }
            ...
            EffectOp::AllocAlloc => {
                if !args.is_empty() {
                    ...
                    return Err(EffectError::InvalidArgs("AllocAlloc requires size argument".to_string()));
                }
            }
            EffectOp::AllocDealloc => {
                if !args.is_empty() {
                    ...
                    return Err(EffectError::InvalidArgs("AllocDealloc requires pointer argument".to_string()));
                }
            }

            // Panic operations
            EffectOp::PanicPanic => {
                crate::runtime::handler_stack::__sounio_dispatch_panic_panic()
            }

            // GPU operations
            EffectOp::GpuSync => {
                crate::runtime::handler_stack::__sounio_dispatch_gpu_sync()
            }
            EffectOp::GpuLaunch => {
                if args.len() >= 4 {
                    ...
                    return Err(EffectError::InvalidArgs("GpuLaunch requires kernel_id, grid_x, grid_y, grid_z arguments".to_string()));
                }
            }
            ...
            EffectOp::AsyncSpawn => {
                if !args.is_empty() {
                    ...
                    return Err(EffectError::InvalidArgs("AsyncSpawn requires task_id argument".to_string()));
                }
            }
            EffectOp::AsyncYield => {
                crate::runtime::handler_stack::__sounio_dispatch_async_yield()
            }
            EffectOp::AsyncAwait => {
                if !args.is_empty() {
                    crate::runtime::handler_stack::__sounio_dispatch_async_await(args[0])
                } else {
                    return Err(EffectError::InvalidArgs("AsyncAwait requires task_id argument".to_string()));
                }
            }
            ...
            EffectOp::GradForward => {
                if args.len() >= 2 {
                    ...
                    return Err(EffectError::InvalidArgs("GradForward requires value and derivative arguments".to_string()));
                }
            }
            EffectOp::GradReverse => {
                if args.len() >= 2 {
                    ...
                    return Err(EffectError::InvalidArgs("GradReverse requires value and adjoint arguments".to_string()));
                }
            }
            ...
            EffectOp::CausalDo => {
                if !args.is_empty() {
                    ...
                    return Err(EffectError::InvalidArgs("CausalDo requires value argument".to_string()));
                }
            }
            EffectOp::CausalObserve => {
                if !args.is_empty() {
                    ...
                    return Err(EffectError::InvalidArgs("CausalObserve requires value argument".to_string()));
                }
            }
