...
impl MixedPrecisionTransform {
    ...
    fn classify_op(&self, op: &GpuOp) -> OpCategory {
        match op {
            ...
            GpuOp::FastExp(_) | GpuOp::FastLog(_) | GpuOp::FastSqrt(_) |
            GpuOp::FastSin(_) | GpuOp::FastCos(_) | GpuOp::FastRsqrt(_) => {
                OpCategory::MustBeFloat32
            }

            // Quantization and special functions that need precision
            GpuOp::QuantizeF32ToInt8 { .. } | GpuOp::DequantizeInt8ToF32 { .. } |
            GpuOp::QuantizeF32ToUint8 { .. } | GpuOp::DequantizeUint8ToF32 { .. } => {
                OpCategory::MustBeFloat32
            }
            ...
            GpuOp::Load { .. } | GpuOp::Store { .. } | GpuOp::Phi(_) => {
    ...
    pub fn classify_operations(&mut self, kernel: &GpuKernel) {
        ...
        for block in &kernel.blocks {
            for (value_id, op) in &block.instructions {
                ...
                self.precision_map.insert(*value_id, should_be_low_precision);
            }
        }
    }
