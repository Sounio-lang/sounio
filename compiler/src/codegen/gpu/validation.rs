...
impl NumericalCorrectnessValidator {
    pub fn validate(
        ...
        if rms_error > 0.005 * output_fp32.iter().map(|x| x.abs()).sum::<f32>() / output_fp32.len() as f32 {
            result.passed = false;
            result.add_diagnostic(&format!(
                "RMS error {:.6} exceeds threshold",
                rms_error
            ));
        }
...
impl FeatureIsolationValidator {
    pub fn validate(
        baseline_loss: f32,
        individual_losses: &[(String, f32)],
    ) -> ValidationResult {
        let mut result = ValidationResult::new("feature_isolation", true);

        for (feature_name, feature_loss) in individual_losses {
            let rel_impact = ((feature_loss - baseline_loss).abs() / baseline_loss).abs().max(0.0);
            result.add_metric(
                &format!("impact_{}", feature_name),
                rel_impact,
            );
...
impl IntegrationValidator {
    pub fn validate(
        ...
        for (features, loss) in combined_losses {
            let rel_diff = ((loss - baseline_loss).abs() / baseline_loss).abs().max(0.0);
            result.add_metric(
                &format!("combined_{}", features.join("+")),
                rel_diff,
            );
...
impl AccuracyPreservationValidator {
    pub fn validate(
        ...
        for (feature_name, accuracy) in feature_accuracies {
            let acc_loss = ((baseline_accuracy - accuracy) / baseline_accuracy).abs().max(0.0);
            result.add_metric(
                &format!("acc_loss_{}", feature_name),
                acc_loss,
            );
    ...
    pub fn validate(weights: &[f32], pattern: &str) -> ValidationResult {
        ...
        match pattern {
            ...
            "csr" => {
                ...
                result.add_metric("nnz", weights.iter().filter(|&&w| w.abs() > 1e-6).count() as f32);
            }
            _ => {
