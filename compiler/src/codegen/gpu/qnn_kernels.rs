...
impl QuatLinearBNReLUConfig {
    ...
    pub fn validate(&self, in_features: usize, out_features: usize, batch_size: usize) -> Result<(), KernelError> {
...
impl QuatLinearBNReLUParams {
    ...
    pub fn validate(&self) -> Result<(), KernelError> {
        ...
        if self.input.is_null() || self.weights.is_null() || self.bias.is_null() ||
           self.output.is_null() || self.bn_mean.is_null() || self.bn_var.is_null() {
            return Err(KernelError::InvalidConfig("Null device pointers".to_string()));
        }
...
pub fn compute_launch_config(
    ...
    if grid_x > (1 << 20) || grid_y > (1 << 20) {
        return Err(KernelError::InvalidConfig("Grid dimensions too large".to_string()));
    }
    ...
    for b in 0..batch_size {
        for o in 0..out_features {
            ...
            for i in 0..in_features {
                ...
                let qw = in_q[0] * w_q[0] - in_q[1] * w_q[1]
                    - in_q[2] * w_q[2]
                    - in_q[3] * w_q[3];
                let qx = in_q[0] * w_q[1] + in_q[1] * w_q[0]
                    + in_q[2] * w_q[3]
                    - in_q[3] * w_q[2];
                let qy = in_q[0] * w_q[2] - in_q[1] * w_q[3]
                    + in_q[2] * w_q[0]
                    + in_q[3] * w_q[1];
                let qz = in_q[0] * w_q[3] + in_q[1] * w_q[2]
                    - in_q[2] * w_q[1]
                    + in_q[3] * w_q[0];
