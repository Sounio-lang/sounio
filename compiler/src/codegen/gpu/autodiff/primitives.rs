...
pub enum BackwardOp {
    ...
    MulByInput { input_idx: usize, other_input_idx: usize },

    /// dx = dz * f(inputs) where f is a custom expression
    Custom { input_idx: usize, expr: BackwardExpr },

    /// dx = dz / input (for division numerator)
    DivByInput { input_idx: usize, divisor_idx: usize },

    /// dx = -dz * numerator / (input^2) (for division denominator)
    DivDenomGrad { input_idx: usize, numerator_idx: usize },
}
...
pub enum BackwardExpr {
    ...
    Indicator { input_idx: usize, reduction_result_idx: usize },
