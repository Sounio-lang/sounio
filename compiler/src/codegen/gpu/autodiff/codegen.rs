...
impl<'a> BackwardCodegen<'a> {
    ...
    fn generate_backward_blocks(
        ...
        let backward_ops = self.generate_backward_operations(forward, analysis, &output_gradients)?;
    ...
    fn apply_backward_op(
        ...
        match backward_op {
            BackwardOp::PassThrough { input_idx } => {
                // dx = dz
                let input = operands.get(*input_idx).copied().ok_or_else(|| {
                    DiffError::Internal("Invalid operand index".to_string())
                })?;
            ...
            BackwardOp::MulByInput { input_idx, other_input_idx } => {
                // dx = dz * other
                let input = operands.get(*input_idx).copied().ok_or_else(|| {
                    DiffError::Internal("Invalid operand index".to_string())
                })?;
                let other = operands.get(*other_input_idx).copied().ok_or_else(|| {
                    DiffError::Internal("Invalid operand index".to_string())
                })?;
            ...
            BackwardOp::DivByInput { input_idx, divisor_idx } => {
                // dx = dz / divisor
                let input = operands.get(*input_idx).copied().ok_or_else(|| {
                    DiffError::Internal("Invalid operand index".to_string())
                })?;
                let divisor = operands.get(*divisor_idx).copied().ok_or_else(|| {
                    DiffError::Internal("Invalid operand index".to_string())
                })?;
            ...
            BackwardOp::DivDenomGrad { input_idx, numerator_idx } => {
                // dx = -dz * numerator / x^2
                let input = operands.get(*input_idx).copied().ok_or_else(|| {
                    DiffError::Internal("Invalid operand index".to_string())
                })?;
                let numerator = operands.get(*numerator_idx).copied().ok_or_else(|| {
                    DiffError::Internal("Invalid operand index".to_string())
                })?;
