...
impl NvidiaNCUProfiler {
    ...
    pub fn profile_cuda_kernel(&self, kernel_path: &Path) -> ProfilerResult<KernelMetrics> {
        ...
        cmd.arg("--set").arg("full")
        // Collect all metrics sets
            .arg("--output-format").arg("csv")
            // CSV output for easy parsing
            .arg("--app-managed-memory").arg("off")
            // Let ncu manage memory
            .arg("--target-processes").arg("pid")
        ...
        for line in output.lines() {
            ...
            match metric_name {
                ...
                "dram__sectors_read.sum" => {
                    ...
                    metrics.memory_reads_bytes =
                        metric_value.parse::<u64>().unwrap_or(0) * 32;
                }
                "dram__sectors_write.sum" => {
                    metrics.memory_writes_bytes =
                        metric_value.parse::<u64>().unwrap_or(0) * 32;
                }
                ...
                "sm__warps_active.avg.pct_of_peak_sustained_active" => {
                    metrics.warp_efficiency = metric_value
                        .parse::<f64>()
                        .unwrap_or(0.0) / 100.0;
                }
...
impl ProfilerManager {
    ...
    pub fn list_available_profilers() -> Vec<String> {
        ...
        if NvidiaNCUProfiler::new().map(|p| p.is_available()).unwrap_or(false) {
            available.push("NVIDIA ncu".to_string());
        }

        if LinuxPerfProfiler::new().map(|p| p.is_available()).unwrap_or(false) {
