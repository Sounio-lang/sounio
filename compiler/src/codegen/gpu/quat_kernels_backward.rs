...
impl QuatLinearBackwardConfig {
    ...
    pub fn validate(&self, in_features: usize, out_features: usize) -> Result<(), BackwardError> {
        if self.tile_out == 0 || self.tile_in == 0 {
            return Err(BackwardError::InvalidConfig("Tile sizes must be > 0".to_string()));
        }
        ...
        if self.bn_epsilon <= 0.0 {
            return Err(BackwardError::InvalidConfig("BN epsilon must be > 0".to_string()));
        }
...
impl QuatBackwardParams {
    ...
    pub fn validate(&self) -> Result<(), BackwardError> {
        ...
        {
            return Err(BackwardError::MemoryError("Null pointer in parameters".to_string()));
        }

        // Dimension validation
        if self.batch_size == 0 || self.in_features == 0 || self.out_features == 0 {
            return Err(BackwardError::InvalidConfig("Invalid dimensions".to_string()));
        }
            ...
            for o in 0..params.out_features {
                ...
                for c in 0..4 {
                    ...
                    if !params.grad_bn_gamma.is_null() {
                        *params.grad_bn_gamma.add(bn_idx + c) +=
                            grad_bn[c] * (linear_out[c] - bn_mean[c])
