...
use super::ir::Doc;
use crate::ast::{
    Ast, AssignOp, BinaryOp, Block, CausalModelDef, EffectDef, EnumDef, Expr, ExternBlock,
    ExternItem, FnDef, GenericParam, GlobalDef, HandlerDef, ImplDef, ImplItem,
    ImportDef, Item, Literal, MatchArm, ModuleDef, OdeDef, Param, Pattern, PdeDef, SelectArm,
    Stmt, StructDef, TraitDef, TraitItem, TypeAliasDef, TypeExpr, UnaryOp, VariantData,
    VariantDef, Visibility, WherePredicate,
};
...
impl<'a> FormatVisitor<'a> {
    ...
    fn visit_ontology_import(&self, o: &crate::ast::OntologyImportDef) -> Doc {
        let mut parts = Vec::new();
        parts.push(Doc::text(format!("ontology {} from \"{}\"", o.prefix, o.source)));
    ...
    pub fn visit_type(&self, ty: &TypeExpr) -> Doc {
        match ty {
            ...
            TypeExpr::Tile { element_type, tile_m, tile_n, layout } => {
                let mut parts = Vec::new();
            ...
            TypeExpr::Refinement { var, base_type, predicate } => {
                Doc::concat(vec![
                    Doc::text(format!("{{ {}: ", var)),
    ...
    pub fn visit_expr(&self, expr: &Expr) -> Doc {
        match expr {
            ...
            Expr::Counterfactual { factual, intervention, outcome, .. } => {
                Doc::concat(vec![
                    Doc::text("counterfactual { ... }"),
                ])
            }
