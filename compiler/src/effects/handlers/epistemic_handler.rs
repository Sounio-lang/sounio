...
fn get_epistemic_operations() -> &'static [OperationSpec] {
    EPISTEMIC_OPERATIONS.get_or_init(|| {
        vec![
            OperationSpec::new("degrade", "Unit")
                .with_params(vec!["Float"]),
            OperationSpec::new("assert_confidence", "Unit")
                .with_params(vec!["Float"]),
            OperationSpec::new("firewall", "Unit")
                .with_params(vec!["String", "Float"]),
            OperationSpec::new("record_provenance", "Unit")
                .with_params(vec!["String"]),
            OperationSpec::new("switch_model", "Unit")
                .with_params(vec!["String"]),
            OperationSpec::new("merge", "Value")
                .with_params(vec!["String", "Array"]),
            OperationSpec::new("get_confidence", "Float")
                .with_params(vec![]),
            OperationSpec::new("boost", "Unit")
                .with_params(vec!["Float"]),
            OperationSpec::new("clamp", "Unit")
                .with_params(vec!["Float", "Float"]),
            OperationSpec::new("isolate", "Unit")
                .with_params(vec![]),
            OperationSpec::new("reset_confidence", "Unit")
                .with_params(vec!["Float"]),
            OperationSpec::new("get_provenance", "Array")
                .with_params(vec![]),
            OperationSpec::new("get_model", "String")
                .with_params(vec![]),
        ]
    })
}
...
impl EpistemicHandler {
    ...
    fn set_confidence(state: &mut HandlerState, confidence: f64) {
        ...
        state.named_state.insert(CONFIDENCE_KEY.to_string(), Value::Float(clamped));
    }
    ...
    fn set_model(state: &mut HandlerState, model: String) {
        state.named_state.insert(MODEL_KEY.to_string(), Value::String(model));
    }
    ...
    fn handle_assert_confidence(&self, args: &[Value], state: &HandlerState) -> HandlerResult {
        ...
        if min_confidence < 0.0 || min_confidence > 1.0 {
            return HandlerResult::Abort(HandlerError::new(
                "Epistemic",
                "assert_confidence",
                format!("minimum confidence must be in [0, 1], got {}", min_confidence),
            ));
        }
    ...
    fn handle_firewall(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        match mode.as_str() {
            ...
            "clamp" => {
                ...
                state.named_state.insert(CONFIDENCE_FLOOR_KEY.to_string(), Value::Float(min));
                state.named_state.insert(CONFIDENCE_CEILING_KEY.to_string(), Value::Float(max));
            ...
            "isolate" => {
                ...
                state.named_state.insert(
                    PROVENANCE_KEY.to_string(),
                    Value::Array(Rc::new(RefCell::new(vec![Value::String("isolated".to_string())]))),
                );
            }
            _ => {
                return HandlerResult::Abort(HandlerError::new(
...
mod tests {
    ...
    fn test_assert_confidence_pass() {
        ...
        let result = handler.handle("assert_confidence", &[Value::Float(0.8)], cont(), &mut state);
        match result {
            HandlerResult::Resume(Value::Unit) => {}
    ...
    fn test_record_provenance() {
        ...
        handler.handle("record_provenance", &[Value::String("validated".to_string())], cont(), &mut state);
        handler.handle("record_provenance", &[Value::String("transformed".to_string())], cont(), &mut state);
    ...
    fn test_switch_model() {
        ...
        handler.handle("switch_model", &[Value::String("bayesian".to_string())], cont(), &mut state);
    ...
    fn test_switch_model_invalid() {
        ...
        let result = handler.handle("switch_model", &[Value::String("invalid".to_string())], cont(), &mut state);
        match result {
            HandlerResult::Abort(e) => {
    ...
    fn test_firewall_isolate() {
        ...
        handler.handle("record_provenance", &[Value::String("before".to_string())], cont(), &mut state);
        handler.handle("firewall", &[Value::String("isolate".to_string())], cont(), &mut state);
    ...
    fn test_clamp() {
        ...
        handler.handle("clamp", &[Value::Float(0.3), Value::Float(0.8)], cont(), &mut state);
    ...
    fn test_reset_confidence() {
        ...
        handler.handle("degrade", &[Value::Float(0.3)], cont(), &mut state);
        handler.handle("reset_confidence", &[Value::Float(0.95)], cont(), &mut state);
    ...
    fn test_get_provenance() {
        ...
        handler.handle("record_provenance", &[Value::String("step1".to_string())], cont(), &mut state);
        handler.handle("record_provenance", &[Value::String("step2".to_string())], cont(), &mut state);
