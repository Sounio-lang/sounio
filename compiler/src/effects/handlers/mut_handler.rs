...
impl MutHandler {
    ...
    pub fn new() -> Self {
        let operations = vec![
            ...
            OperationSpec::new("enter_scope", "Value")
    ...
    fn handle_set(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        if args.len() < 2 {
            return HandlerResult::Abort(HandlerError::new(
                ...
                "set",
                format!(
                    "set requires 2 arguments (name, value), got {}",
                    args.len()
                ),
            ));
        }
...
mod tests {
    ...
    fn test_modify_with_direct_value() {
        ...
        assert!(matches!(
            get_result,
            HandlerResult::Resume(Value::Int(20))
        ));
    }
    ...
    fn test_with_state_returns_tuple() {
        ...
        // Set an initial value
        state.named_state.insert("counter".to_string(), Value::Int(0));
