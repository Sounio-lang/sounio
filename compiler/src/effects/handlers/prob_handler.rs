...
fn get_prob_operations() -> &'static [OperationSpec] {
    PROB_OPERATIONS.get_or_init(|| {
        vec![
            ...
            OperationSpec::new("observe", "Unit")
                .with_params(vec!["Distribution", "Float"]),
            OperationSpec::new("condition", "Unit")
                .with_params(vec!["Bool"]),
            ...
            OperationSpec::new("exponential", "Float")
                .with_params(vec!["Float"])
                .with_confidence_factor(SAMPLE_CONFIDENCE_FACTOR),
            OperationSpec::new("log_prob", "Float")
                .with_params(vec!["Distribution", "Float"]),
            OperationSpec::new("score", "Unit")
                .with_params(vec!["Float"]),
            OperationSpec::new("get_log_prob", "Float")
                .with_params(vec![]),
        ]
    })
}
...
impl ProbHandler {
    ...
    fn get_seed(state: &mut HandlerState) -> u64 {
        match state.named_state.get(RNG_SEED_KEY) {
            ...
            _ => {
                ...
                let seed = 12345u64;
                state.named_state.insert(RNG_SEED_KEY.to_string(), Value::Int(seed as i64));
                seed
            }
        }
    }
    ...
    fn next_seed(state: &mut HandlerState) -> u64 {
        ...
        state.named_state.insert(RNG_SEED_KEY.to_string(), Value::Int(new_seed as i64));
        new_seed
    }
    ...
    fn add_log_prob(state: &mut HandlerState, lp: f64) {
        ...
        state.named_state.insert(LOG_PROB_KEY.to_string(), Value::Float(current + lp));
    }
    ...
    fn increment_observe_count(state: &mut HandlerState) {
        ...
        state.named_state.insert(OBSERVE_COUNT_KEY.to_string(), Value::Int(current + 1));
    }
    ...
    fn sample_distribution(dist: &Distribution, state: &mut HandlerState) -> f64 {
        match dist {
            Distribution::Normal { mean, std } => {
                mean + std * Self::random_normal(state)
            }
            Distribution::Uniform { a, b } => {
                a + (b - a) * Self::random_uniform(state)
            }
            Distribution::Beta { alpha, beta } => {
        ...
        let p = match &args[0] {
            ...
            _ => {
                return HandlerResult::Abort(HandlerError::new(
                    "Prob",
                    "bernoulli",
                    format!("expected Float for probability, got {:?}", args[0].type_name()),
                ));
            }
        };
        ...
        let probs_rc = match &args[0] {
            ...
            _ => {
                return HandlerResult::Abort(HandlerError::new(
                    "Prob",
                    "categorical",
                    format!("expected Array of probabilities, got {:?}", args[0].type_name()),
                ));
            }
        };
    ...
    fn handle_beta(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        if alpha <= 0.0 || beta_param <= 0.0 {
            return HandlerResult::Abort(HandlerError::new(
                "Prob",
                "beta",
                format!("alpha and beta must be positive, got alpha={}, beta={}", alpha, beta_param),
            ));
        }

        let dist = Distribution::Beta { alpha, beta: beta_param };
        let value = Self::sample_distribution(&dist, state);
        HandlerResult::Resume(Value::Float(value))
    }
    ...
    fn extract_float(value: &Value, effect: &str, op: &str, param: &str) -> Result<f64, HandlerResult> {
        match value {
            Value::Float(f) => Ok(*f),
    ...
    pub fn reset_log_prob(state: &mut HandlerState) {
        state.named_state.insert(LOG_PROB_KEY.to_string(), Value::Float(0.0));
    }
...
impl HandlerCapability for ProbHandler {
    ...
    fn epistemic_impact(&self, operation: &str) -> EpistemicImpact {
        match operation {
            ...
            "sample" | "bernoulli" | "uniform" | "normal" | "categorical" | "beta" | "exponential" => {
                EpistemicImpact::with_confidence(SAMPLE_CONFIDENCE_FACTOR)
            }
...
mod tests {
    ...
    fn test_uniform_range() {
        ...
        for _ in 0..20 {
            ...
            match result {
                HandlerResult::Resume(Value::Float(v)) => {
                    assert!(v >= 10.0 && v < 20.0, "uniform should be in [10, 20): {}", v);
                }
        ...
        for _ in 0..10 {
            ...
            match result {
                HandlerResult::Resume(Value::Int(i)) => {
                    assert!(i >= 0 && i < 3, "categorical should return 0, 1, or 2: {}", i);
                }
    ...
    fn test_sample_with_distribution() {
        ...
        let dist = Value::Distribution(Distribution::Normal { mean: 0.0, std: 1.0 });
    ...
    fn test_condition_true() {
        ...
        let lp = ProbHandler::get_log_prob_value(&state);
        assert!((lp - 0.0).abs() < 0.001, "condition(true) should not change log prob");
    }
    ...
    fn test_condition_false() {
        ...
        let lp = ProbHandler::get_log_prob_value(&state);
        assert!(lp == f64::NEG_INFINITY, "condition(false) should set log prob to -inf");
    }
    ...
    fn test_log_prob_operation() {
        ...
        match result {
            HandlerResult::Resume(Value::Float(lp)) => {
                ...
                assert!(lp < 0.0 && lp > -2.0, "log prob should be around -0.919: {}", lp);
            }
            other => panic!("expected Resume(Float), got {:?}", other),
        }
    }
    ...
    fn test_observe_count() {
        ...
        let dist = Value::Distribution(Distribution::Normal { mean: 0.0, std: 1.0 });
        handler.handle("observe", &[dist.clone(), Value::Float(0.0)], cont(), &mut state);
        assert_eq!(ProbHandler::get_observe_count(&state), 1);

        handler.handle("observe", &[dist.clone(), Value::Float(1.0)], cont(), &mut state);
