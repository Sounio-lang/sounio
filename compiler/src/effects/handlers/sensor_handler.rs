    ...
    SENSOR_OPERATIONS.get_or_init(|| {
        vec![
            ...
            OperationSpec::new("read_with_unit", "Measurement")
                .with_params(vec!["String"])
                .with_confidence_factor(SENSOR_READ_CONFIDENCE_FACTOR),
            OperationSpec::new("calibrate", "Unit")
                .with_params(vec!["String", "Float"]),
            OperationSpec::new("get_accuracy", "Float")
                .with_params(vec!["String"]),
            OperationSpec::new("get_precision", "Float")
                .with_params(vec!["String"]),
            OperationSpec::new("get_range", "Tuple")
                .with_params(vec!["String"]),
            OperationSpec::new("is_available", "Bool")
                .with_params(vec!["String"]),
            ...
            OperationSpec::new("set_simulated_value", "Unit")
                .with_params(vec!["String", "Float"]),
            OperationSpec::new("get_reading_count", "Int")
                .with_params(vec![]),
        ]
    })
}
...
impl SensorHandler {
    ...
    fn get_rng_seed(state: &mut HandlerState) -> u64 {
        match state.named_state.get(SENSOR_RNG_KEY) {
            ...
            _ => {
                let seed = 54321u64;
                state.named_state.insert(SENSOR_RNG_KEY.to_string(), Value::Int(seed as i64));
                seed
            }
        }
    }
    ...
    fn next_rng(state: &mut HandlerState) -> u64 {
        ...
        state.named_state.insert(SENSOR_RNG_KEY.to_string(), Value::Int(new_seed as i64));
        new_seed
    }
    ...
    fn increment_reading_count(state: &mut HandlerState) {
        ...
        state.named_state.insert(SENSOR_READINGS_COUNT_KEY.to_string(), Value::Int(count + 1));
    }
        ...
        let sensor_ids_rc = match &args[0] {
            ...
            _ => {
                return HandlerResult::Abort(HandlerError::new(
                    "Sensor",
                    "batch_read",
                    format!("expected Array for sensor_ids, got {:?}", args[0].type_name()),
                ));
            }
        };
            ...
            let sensor_id = match id_value {
                ...
                _ => {
                    return HandlerResult::Abort(HandlerError::new(
                        "Sensor",
                        "batch_read",
                        format!("expected String at index {}, got {:?}", i, id_value.type_name()),
                    ));
                }
            };
    ...
    fn handle_set_simulated_value(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        if args.len() < 2 {
            return HandlerResult::Abort(HandlerError::new(
                "Sensor",
                "set_simulated_value",
...
impl HandlerCapability for SensorHandler {
    ...
    fn epistemic_impact(&self, operation: &str) -> EpistemicImpact {
        match operation {
            ...
            "read_averaged" => {
                EpistemicImpact::with_confidence(SENSOR_AVERAGED_CONFIDENCE_FACTOR)
            }
