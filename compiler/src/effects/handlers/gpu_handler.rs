...
fn get_gpu_operations() -> &'static [OperationSpec] {
    GPU_OPERATIONS.get_or_init(|| {
        vec![
            ...
            OperationSpec::new("sync", "Unit")
                .with_params(vec![]),
            OperationSpec::new("alloc_device", "DevicePtr")
                .with_params(vec!["Int"]),
            OperationSpec::new("free_device", "Unit")
                .with_params(vec!["DevicePtr"]),
            OperationSpec::new("copy_to_device", "Unit")
                .with_params(vec!["Array", "DevicePtr", "Int"]),
            OperationSpec::new("copy_from_device", "Array")
                .with_params(vec!["DevicePtr", "Int"])
                .with_confidence_factor(GPU_COMPUTE_CONFIDENCE_FACTOR),
            OperationSpec::new("get_device_count", "Int")
                .with_params(vec![]),
            OperationSpec::new("set_device", "Unit")
                .with_params(vec!["Int"]),
            OperationSpec::new("stream_create", "StreamId")
                .with_params(vec![]),
            OperationSpec::new("stream_sync", "Unit")
                .with_params(vec!["StreamId"]),
            OperationSpec::new("stream_destroy", "Unit")
                .with_params(vec!["StreamId"]),
            OperationSpec::new("get_device_properties", "DeviceProperties")
                .with_params(vec!["Int"]),
            OperationSpec::new("memset_device", "Unit")
                .with_params(vec!["DevicePtr", "Int", "Int"]),
        ]
    })
}
...
impl GpuHandler {
    ...
    fn get_alloc_counter(state: &mut HandlerState) -> u64 {
        match state.named_state.get(GPU_ALLOCATIONS_KEY) {
            ...
            _ => {
                state.named_state.insert(GPU_ALLOCATIONS_KEY.to_string(), Value::Int(1000));
                1000
            }
        }
    }
    ...
    fn next_device_ptr(state: &mut HandlerState) -> u64 {
        ...
        state.named_state.insert(GPU_ALLOCATIONS_KEY.to_string(), Value::Int((ptr + 1) as i64));
        ptr
    }
    ...
    fn get_stream_counter(state: &mut HandlerState) -> u64 {
        match state.named_state.get(GPU_STREAM_COUNTER_KEY) {
            Some(Value::Int(n)) => *n as u64,
            _ => {
                state.named_state.insert(GPU_STREAM_COUNTER_KEY.to_string(), Value::Int(1));
                1
            }
        }
    }
    ...
    fn next_stream_id(state: &mut HandlerState) -> u64 {
        ...
        state.named_state.insert(GPU_STREAM_COUNTER_KEY.to_string(), Value::Int((id + 1) as i64));
        id
    }
    ...
    fn add_pending_kernel(state: &mut HandlerState) {
        ...
        state.named_state.insert(GPU_PENDING_KERNELS_KEY.to_string(), Value::Int(current + 1));
    }

    /// Clear pending kernels
    fn clear_pending_kernels(state: &mut HandlerState) {
        state.named_state.insert(GPU_PENDING_KERNELS_KEY.to_string(), Value::Int(0));
    }
    ...
    fn update_total_allocated(state: &mut HandlerState, delta: i64) {
        ...
        state.named_state.insert(GPU_TOTAL_ALLOCATED_KEY.to_string(), Value::Int(current + delta));
    }
    ...
    fn extract_dims(value: &Value, name: &str) -> Result<(u32, u32, u32), HandlerResult> {
        match value {
            ...
            _ => Err(HandlerResult::Abort(HandlerError::new(
                "GPU",
                "launch",
                format!("expected tuple of 3 u32 for {}, got {:?}", name, value.type_name()),
            ))),
        }
    }
    ...
    fn handle_launch(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        if threads_per_block > 1024 {
            return HandlerResult::Abort(HandlerError::new(
                "GPU",
                "launch",
                format!("threads per block ({}) exceeds maximum (1024)", threads_per_block),
            ));
        }
        ...
        match &args[0] {
            ...
            _ => {
                return HandlerResult::Abort(HandlerError::new(
                    "GPU",
                    "copy_to_device",
                    format!("expected Array for host_data, got {:?}", args[0].type_name()),
                ));
            }
        }
    ...
    fn handle_set_device(&self, args: &[Value], state: &mut HandlerState) -> HandlerResult {
        ...
        if device_id < 0 || device_id >= self.device_count {
            return HandlerResult::Abort(HandlerError::new(
                "GPU",
                "set_device",
                format!("device_id {} out of range [0, {})", device_id, self.device_count),
            ));
        }

        state.named_state.insert(GPU_CURRENT_DEVICE_KEY.to_string(), Value::Int(device_id));
        HandlerResult::Resume(Value::Unit)
    }
    ...
    fn handle_get_device_properties(&self, args: &[Value]) -> HandlerResult {
        ...
        let properties = vec![
            ("name".to_string(), Value::String("Simulated GPU".to_string())),
            ("total_memory".to_string(), Value::Int(8 * 1024 * 1024 * 1024)), // 8GB
...
mod tests {
    ...
    fn test_sync_clears_pending() {
        ...
        handler.handle("launch", &[kernel.clone(), grid.clone(), block.clone()], cont(), &mut state);
        handler.handle("launch", &[kernel.clone(), grid.clone(), block.clone()], cont(), &mut state);
    ...
    fn test_get_device_properties() {
        ...
        let result = handler.handle("get_device_properties", &[Value::Int(0)], cont(), &mut state);
    ...
    fn test_multiple_allocations() {
        ...
        assert_eq!(GpuHandler::total_allocated_memory(&state), 1024 + 2048 + 512);
    }
}
