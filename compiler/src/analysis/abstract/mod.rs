...
pub use fixpoint::{FixpointConfig, FixpointEngine, FixpointResult, SccDecomposition, simple_fixpoint};
...
impl AbstractInterpreter {
    ...
    pub fn analyze_function_numeric(
        &self,
        func: &HlirFunction,
    ) -> FixpointResult<NumericDomain> {
...
impl LoopInvariant {
    ...
    pub fn to_predicate(&self, counter_name: &str, bound_name: &str) -> Predicate {
        ...
        if self.counter_less_than_bound {
            preds.push(Predicate::lt(Term::var(counter_name), Term::var(bound_name)));
        }
