...
impl<D1: AbstractDomain, D2: AbstractDomain> AbstractDomain for Product<D1, D2> {
    ...
    fn widen(&self, other: &Self) -> Self {
        Self::new(self.first.widen(&other.first), self.second.widen(&other.second))
    }

    fn narrow(&self, other: &Self) -> Self {
        Self::new(self.first.narrow(&other.first), self.second.narrow(&other.second))
    }
...
impl AbstractDomain for IntervalSign {
    ...
    fn join(&self, other: &Self) -> Self {
        Self::new(self.interval.join(&other.interval), self.sign.join(&other.sign))
    }

    fn meet(&self, other: &Self) -> Self {
        Self::new(self.interval.meet(&other.interval), self.sign.meet(&other.sign))
    }

    fn widen(&self, other: &Self) -> Self {
        Self::new(self.interval.widen(&other.interval), self.sign.join(&other.sign))
    }

    fn narrow(&self, other: &Self) -> Self {
        Self::new(self.interval.narrow(&other.interval), self.sign.meet(&other.sign))
    }
...
impl IntervalCongruence {
    ...
    pub fn new(interval: Interval, congruence: Congruence) -> Self {
        let mut result = Self { interval, congruence };
        result.reduce();
        result
    }
    ...
    pub fn is_even(&self) -> bool {
        self.congruence.is_even() || self.interval.is_constant().map(|c| c % 2 == 0).unwrap_or(false)
    }

    /// Check if value is odd
    pub fn is_odd(&self) -> bool {
        self.congruence.is_odd() || self.interval.is_constant().map(|c| c % 2 == 1).unwrap_or(false)
    }
}
...
impl NumericDomain {
    ...
    pub fn new(interval: Interval, sign: Sign, congruence: Congruence) -> Self {
        let mut result = Self { interval, sign, congruence };
        result.reduce();
        result
    }
...
mod tests {
    ...
    fn test_product_basic() {
        let prod: Product<Interval, Sign> = Product::new(
    ...
    fn test_interval_congruence_alignment() {
        let ic = IntervalCongruence::new(
            Interval::from_range(0, 100),
            Congruence::divisible_by(8),
        );
    ...
    fn test_product_meet() {
        let a = IntervalSign::new(
            Interval::from_range(0, 10),
            Sign::top(),
        );
        let b = IntervalSign::new(
            Interval::from_range(5, 15),
            Sign::positive(),
        );
