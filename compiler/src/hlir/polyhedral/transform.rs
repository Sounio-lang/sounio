...
impl ScheduleNode {
    ...
    pub fn total_dims(&self) -> usize {
        match self {
            ScheduleNode::Band { num_dims, child, .. } => num_dims + child.total_dims(),
...
impl<'a> PolyhedralTransform<'a> {
    ...
    fn tile_schedule(&self, schedule: &AffineMap, tile_sizes: &[usize]) -> PolyResult<AffineMap> {
        ...
        Ok(AffineMap::new(schedule.num_inputs, schedule.num_params, outputs))
    }
}
...
mod tests {
    ...
    fn test_schedule_node_tiling() {
        ...
        match tiled {
            ScheduleNode::Band { tile_sizes, is_tilable, .. } => {
                assert!(is_tilable);
