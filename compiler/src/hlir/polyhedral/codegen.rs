...
use crate::hlir::{
    BlockId, BinaryOp, HlirBlock, HlirConstant, HlirFunction, HlirInstr, HlirTerminator,
    HlirType, Op, ValueId,
};
...
impl PolyhedralCodegen {
    ...
    fn generate_loop_band(
        ...
        // Store loop index
        self.index_values.insert(format!("i{}", num_dims - 1), loop_idx);
    ...
    fn generate_affine_expr(&mut self, expr: &AffineExpr) -> PolyResult<ValueId> {
        ...
        for (i, coeff) in expr.iter_coeffs.iter().enumerate() {
            ...
            let idx_val = self.index_values.get(&idx_name).copied().unwrap_or_else(|| {
        ...
        for (i, coeff) in expr.param_coeffs.iter().enumerate() {
            ...
            let param_val = self.param_values.get(&param_name).copied().unwrap_or_else(|| {
                let v = self.fresh_value_id();
                self.emit(HlirInstr {
                    result: Some(v),
    ...
    fn generate_from_schedules(
        ...
        for nest in &scop.loop_nests {
            ...
            let (entry, exit) = self.generate_loop_band(
                depth,
                &ScheduleNode::Leaf,
                scop,
                schedule,
