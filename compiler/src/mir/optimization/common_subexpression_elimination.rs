...
use crate::mir::{
    ...
    ValueId, BlockId, MirType, MirBinaryOp, MirUnaryOp, MirCompareOp,
    MirTerminator,
};
...
impl Expression {
    ...
    pub fn from_instruction(instr: &MirInstruction) -> Option<Self> {
        match instr {
            MirInstruction::Binary { op, left, right, ty, .. } => {
            ...
            MirInstruction::Unary { op, operand, ty, .. } => {
            ...
            MirInstruction::Compare { op, left, right, ty, .. } => {
            ...
            MirInstruction::GetElementPtr { base, indices, ty, .. } => {
            ...
            MirInstruction::Cast { source, source_ty, target_ty, .. } => {
            ...
            MirInstruction::ZExt { source, source_ty, target_ty, .. } => {
            ...
            MirInstruction::SExt { source, source_ty, target_ty, .. } => {
            ...
            MirInstruction::Trunc { source, source_ty, target_ty, .. } => {
            ...
            MirInstruction::FPToSI { source, target_ty, .. } => {
                Some(Expression::FPToSI {
                    source: *source,
                    target_ty: target_ty.clone(),
                })
            }
            MirInstruction::SIToFP { source, target_ty, .. } => {
                Some(Expression::SIToFP {
                    source: *source,
                    target_ty: target_ty.clone(),
                })
            }
            MirInstruction::FPToUI { source, target_ty, .. } => {
                Some(Expression::FPToUI {
                    source: *source,
                    target_ty: target_ty.clone(),
                })
            }
            MirInstruction::UIToFP { source, target_ty, .. } => {
                Some(Expression::UIToFP {
                    source: *source,
                    target_ty: target_ty.clone(),
                })
            }
            MirInstruction::PtrCast { source, source_ty, target_ty, .. } => {
            ...
            MirInstruction::Select { condition, true_value, false_value, ty, .. } => {
    ...
    pub fn used_values(&self) -> Vec<ValueId> {
        match self {
            ...
            | Expression::Cast { source: operand, .. }
            | Expression::ZExt { source: operand, .. }
            | Expression::SExt { source: operand, .. }
            | Expression::Trunc { source: operand, .. }
            | Expression::FPToSI { source: operand, .. }
            | Expression::SIToFP { source: operand, .. }
            | Expression::FPToUI { source: operand, .. }
            | Expression::UIToFP { source: operand, .. }
            | Expression::PtrCast { source: operand, .. } => {
                vec![*operand]
            }
            ...
            Expression::Select { condition, true_value, false_value, .. } => {
...
impl AvailableExpressions {
    ...
    pub fn analyze(func: &MirFunction) -> Self {
        ...
        let all_expressions: HashSet<Expression> = analysis.generated.values()
        ...
        for block in &func.blocks {
            if block.id == func.entry_block {
                ...
                analysis.available_in.insert(block.id, all_expressions.clone());
            }
        ...
        while changed && iterations < MAX_ITERATIONS {
            ...
            for block in &func.blocks {
                ...
                let preds = analysis.predecessors.get(&block.id).cloned().unwrap_or_default();
                ...
                let gen_set = analysis.generated.get(&block.id).cloned().unwrap_or_default();
...
mod tests {
    ...
    fn test_expression_from_binary() {
        ...
        if let Some(Expression::Binary { op, left, right, ty }) = expr {
    ...
    fn test_expression_from_call_returns_none() {
        ...
        let expr = Expression::from_instruction(&instr);
        assert!(expr.is_none(), "Call should not produce an expression (side effects)");
    }
    ...
    fn test_cse_simple() {
        ...
        let add_count = entry.instructions.iter().filter(|i| {
            matches!(i, MirInstruction::Binary { op: MirBinaryOp::Add, .. })
        }).count();
