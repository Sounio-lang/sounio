...
impl RankNChecker {
    ...
    pub fn generalize(&mut self, env_vars: &HashSet<TypeVar>, ty: &Type) -> Type {
        ...
        let free_not_in_env: Vec<TypeVar> = free_in_type
            .difference(env_vars)
            .copied()
            .collect();
    ...
    fn can_unify(&self, ty1: &Type, ty2: &Type) -> bool {
        ...
        match (&ty1, &ty2) {
            ...
            (Type::Ref { inner: i1, mutable: m1, .. }, Type::Ref { inner: i2, mutable: m2, .. }) => {
            ...
            (Type::Named { name: n1, args: a1 }, Type::Named { name: n2, args: a2 }) => {
                n1 == n2 && a1.len() == a2.len() && a1.iter().zip(a2.iter()).all(|(a, b)| self.can_unify(a, b))
            }
            _ => false,
        }
    }
    ...
    fn unify(&mut self, ty1: &Type, ty2: &Type) -> bool {
        ...
        match (&ty1, &ty2) {
            ...
            // Reference types
            (
                Type::Ref { inner: i1, mutable: m1, lifetime: l1 },
                Type::Ref { inner: i2, mutable: m2, lifetime: l2 },
            ) => {
            ...
            (
                ...
                p1.len() == p2.len() && {
                    let param_pairs: Vec<_> = p1.iter().cloned().zip(p2.iter().cloned()).collect();
            ...
            (
                Type::Forall { vars: v1, inner: i1 },
                Type::Forall { vars: v2, inner: i2 },
            ) => {
...
mod tests {
    ...
    fn test_subsumption_concrete_not_to_forall() {
        ...
        assert!(result.is_fail(), "i32 -> i32 should not subsume forall T. T -> T");
    }
