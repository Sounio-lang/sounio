...
impl std::str::FromStr for AllocPolicy {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(format!("Unknown policy: '{}'. Use: classic, attention, calibrated", s)),
        }
    }
}
...
pub fn compute_attention_score(
    ...
    let epistemic_score = config.w_confidence * tau
...
impl MetricsCollector {
    ...
    pub fn finalize(&mut self) {
        ...
        if !self.score_samples.is_empty() {
            self.metrics.min_attention_score = self.score_samples
                .iter().cloned().fold(f64::INFINITY, f64::min);
            self.metrics.max_attention_score = self.score_samples
                .iter().cloned().fold(f64::NEG_INFINITY, f64::max);
            self.metrics.avg_attention_score = self.score_samples.iter().sum::<f64>()
                / self.score_samples.len() as f64;
        }

        if !self.spilled_scores.is_empty() {
            self.metrics.spilled_avg_score = self.spilled_scores.iter().sum::<f64>()
                / self.spilled_scores.len() as f64;
        }

        if !self.kept_scores.is_empty() {
            self.metrics.kept_avg_score = self.kept_scores.iter().sum::<f64>()
                / self.kept_scores.len() as f64;
        }
...
mod tests {
    ...
    fn test_attention_score_ordering() {
        ...
        assert!(score_critical > score_dispensable,
    ...
    fn test_confidence_classification() {
        ...
        let high = EpistemicMetadata { confidence: Some(0.85), ..Default::default() };
        let medium = EpistemicMetadata { confidence: Some(0.5), ..Default::default() };
        let low = EpistemicMetadata { confidence: Some(0.2), ..Default::default() };
    ...
    fn test_policy_parsing() {
        assert_eq!("classic".parse::<AllocPolicy>().unwrap(), AllocPolicy::Classic);
