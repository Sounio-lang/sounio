...
impl SpirVModuleBuilder {
    ...
    pub fn add_decoration(&mut self, target: ResultId, decoration: Decoration) {
        self.decorations.push(SpirVInst::Decorate {
            target,
            decoration,
        });
    }
...
impl SpirVBackend {
    ...
    fn lower_sir_function(&mut self, func: &SirFunction) -> Result<(), SpirVError> {
        ...
        let param_types: Vec<SpirVType> = func.params.iter().map(|(_, ty)| self.lower_type(ty)).collect();
    ...
    fn lower_instruction(
        ...
        match inst {
            ...
            SirInst::Const(constant) => {
                ...
                self.value_map
                    .insert(ValueId(result_id.0), const_id);
            }
