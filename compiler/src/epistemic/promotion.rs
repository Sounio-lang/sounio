...
impl UncertaintyLevel {
    ...
    pub fn can_promote_to(&self, target: Self) -> bool {
        ...
        match (*self, target) {
            ...
            (Self::Interval, Self::Bootstrap | Self::Affine | Self::Distribution | Self::Particles) => true,
...
impl PromotedValue {
    ...
    pub fn bounds(&self) -> (f64, f64) {
        match self {
            ...
            Self::CrossValidation { mean, std_error, k, .. } => {
                // Use t-distribution approximation for CI
                let t_value = if *k <= 2 { 12.71 } else if *k <= 5 { 2.78 } else if *k <= 10 { 2.26 } else { 1.96 };
...
impl Promoter {
    ...
    pub fn promote_bootstrap(
        ...
        match target {
            ...
            UncertaintyLevel::Particles => {
                let particles = self.generate_normal_samples_n(estimate + bias, bootstrap_se, self.default_particles);
                let weights = vec![1.0 / self.default_particles as f64; self.default_particles];
                Ok(PromotedValue::Particles {
                    particles,
                    weights,
    ...
    pub fn promote_cv(
        ...
        match target {
            ...
            UncertaintyLevel::Particles => {
                let particles = self.generate_normal_samples_n(mean, std_dev, self.default_particles);
                let weights = vec![1.0 / self.default_particles as f64; self.default_particles];
                Ok(PromotedValue::Particles {
                    particles,
                    weights,
...
mod tests {
    ...
    fn test_bootstrap_parse() {
        assert_eq!(UncertaintyLevel::parse("bootstrap"), Some(UncertaintyLevel::Bootstrap));
        assert_eq!(UncertaintyLevel::parse("boot"), Some(UncertaintyLevel::Bootstrap));
        assert_eq!(UncertaintyLevel::parse("resampling"), Some(UncertaintyLevel::Bootstrap));
    }

    #[test]
    fn test_promote_bootstrap_to_distribution() {
        let p = Promoter::new().with_samples(100);
        let r = p.promote_bootstrap(
            10.0,  // estimate
    ...
    #[test]
    fn test_cv_parse() {
        assert_eq!(UncertaintyLevel::parse("crossvalidation"), Some(UncertaintyLevel::CrossValidation));
        assert_eq!(UncertaintyLevel::parse("cv"), Some(UncertaintyLevel::CrossValidation));
        assert_eq!(UncertaintyLevel::parse("kfold"), Some(UncertaintyLevel::CrossValidation));
    }
    ...
    fn test_bootstrap_cv_incomparable() {
        ...
        assert_eq!(
            l.meet(UncertaintyLevel::Bootstrap, UncertaintyLevel::CrossValidation),
            UncertaintyLevel::Point
        );
        assert_eq!(
            l.join(UncertaintyLevel::Bootstrap, UncertaintyLevel::CrossValidation),
            UncertaintyLevel::Distribution
        );
    }
}
