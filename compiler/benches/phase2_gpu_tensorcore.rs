...
fn fused_linear_bn_relu(input: &[f32], weights: &[f32], bias: &[f32],
                        bn_scale: &[f32], output: &mut [f32], n: usize) {
    for i in 0..n {
        let mut sum = 0.0;
        for j in 0..n {
...
fn bench_batched_matmul(c: &mut Criterion) {
    ...
    group.bench_function("batch_4x256x256", |b| {
        let matrices: Vec<Vec<f32>> = (0..4)
            .map(|_| black_box(generate_matrix(256)))
            .collect();
