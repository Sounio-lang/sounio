...
impl Quaternion {
    ...
    fn normalize(self) -> Quaternion {
        ...
        if n > 1e-10 {
            let inv_n = 1.0 / n;
            Quaternion::new(self.w * inv_n, self.x * inv_n, self.y * inv_n, self.z * inv_n)
        } else {
            Quaternion::identity()
        }
    }
...
fn benchmark_quaternion_multiplication(c: &mut Criterion) {
    ...
    c.bench_function("quat_mul_single", |b| {
        b.iter(|| q1.mul(q2))
    });
...
fn benchmark_quaternion_norm(c: &mut Criterion) {
    ...
    c.bench_function("quat_norm_sq", |b| {
        b.iter(|| q.norm_sq())
    });

    c.bench_function("quat_norm", |b| {
        b.iter(|| q.norm())
    });
...
fn benchmark_quaternion_batch(c: &mut Criterion) {
    ...
    for batch_size in [64, 256, 1024, 4096].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("mul", batch_size), batch_size, |b, &size| {
            ...
            b.iter(|| {
                quats.iter().map(|q| q.mul(q_rot)).collect::<Vec<_>>()
            })
        });

        group.bench_with_input(BenchmarkId::new("rotate", batch_size), batch_size, |b, &size| {
            ...
            b.iter(|| {
                vectors.iter().map(|v| q.rotate_vector(*v)).collect::<Vec<_>>()
            })
        });
    }
    group.finish();
