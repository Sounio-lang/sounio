...
WORKER_CHECK_TRACE="${WORKER_CHECK_TRACE:-0}"
WORKER_CHECK_STMT_TRACE_FN="${WORKER_CHECK_STMT_TRACE_FN:-}"
...
Environment:
  ...
  WORKER_LAYOUT_TRACE,
  WORKER_NV2_IR_TRACE, WORKER_CHECK_TRACE, WORKER_CHECK_STMT_TRACE_FN,
  WORKER_PRESERVE_DIAGNOSTICS,
  WORKER_INTO_ACC_NO_RESET,
...
Set WORKER_CHECK_TRACE=1 only for checker crash localization. It prints the
module, function, and phase reached by the worker's raw ELF.

Set WORKER_CHECK_STMT_TRACE_FN to a function name only for checker crash
localization. It prints statement, expression, and call phases for that function.
...
[[ "$WORKER_CHECK_TRACE" == '0' || "$WORKER_CHECK_TRACE" == '1' ]] || fail "invalid WORKER_CHECK_TRACE: $WORKER_CHECK_TRACE"
[[ -z "$WORKER_CHECK_STMT_TRACE_FN" || "$WORKER_CHECK_STMT_TRACE_FN" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || fail "invalid WORKER_CHECK_STMT_TRACE_FN: $WORKER_CHECK_STMT_TRACE_FN"
...
WORKER_CHECK_TRACE="$WORKER_CHECK_TRACE"
WORKER_CHECK_STMT_TRACE_FN="$WORKER_CHECK_STMT_TRACE_FN"
...
echo "worker_check_trace=\$WORKER_CHECK_TRACE"
echo "worker_check_stmt_trace_fn=\$WORKER_CHECK_STMT_TRACE_FN"
...
if [[ "\$WORKER_CHECK_TRACE" == '1' ]]; then
  export SOUNIO_CHECK_TRACE=1
fi

if [[ -n "\$WORKER_CHECK_STMT_TRACE_FN" ]]; then
  export SOUNIO_CHECK_STMT_TRACE_FN="\$WORKER_CHECK_STMT_TRACE_FN"
fi
