#!/usr/bin/env bash
# slurm-jobs/erdos90/submit_subset_array.sh — sbatch array for ℤ² subset n=100 (N=5)
#
# Usage:
#   bash slurm-jobs/erdos90/submit_subset_array.sh
#   ARRAY_MAX=8 bash slurm-jobs/erdos90/submit_subset_array.sh
#
# Output:
#   /orangefs/training/sounio/erdos90-subset-runs/<RUN_ID>/results/<seed>.log
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SOUC="${SOUC:-$ROOT/bin/souc}"
SRC="${SRC:-$ROOT/stdlib/research/erdos90_subset_cluster.sio}"
RUN_ID="${RUN_ID:-erdos90-sub-$(date -u +%Y%m%dT%H%M%S)-${BASHPID}}"
STAGE_ROOT="${STAGE_ROOT:-/orangefs/training/sounio/erdos90-subset-runs/${RUN_ID}}"
...
echo "target:      n=100 N=5 subset (beat grid10x10=288)"

echo
echo "[1/4] compile subset cluster ELFs ..."
for i in $(seq 0 "$ARRAY_MAX"); do
    ...
    chmod +x "$elf"
    echo "  compiled seed=$seed"
done

echo
echo "[2/4] stage ELFs to ${STAGE_ROOT}/bin ..."
...
for i in $(seq 0 "$ARRAY_MAX"); do
    ...
    base64 -w0 "$elf" | srun --partition="$PARTITION" --time=00:10:00 --chdir=/orangefs/training bash -c "
      base64 -d > '${STAGE_ROOT}/bin/e90_sub_${seed}'
      chmod +x '${STAGE_ROOT}/bin/e90_sub_${seed}'
    "
done
...
BIN="${STAGE_ROOT}/bin/e90_sub_\${SEED}"
OUT="${STAGE_ROOT}/results/\${SEED}.log"

echo "=== Erdos90 subset n=100 seed=\${SEED} on \$(hostname) ==="
echo "start=\$(date -Is)"
t0=\$(date +%s)
"\$BIN" | tee "\$OUT"
echo "wall=\$(( \$(date +%s) - t0 ))s end=\$(date -Is)"
grep -E 'BEST_N100|BEAT-GRID288|CLUSTER_DONE' "\$OUT" || true
EOF
...
cat <<SUM
  ...
  Leaderboard: grep -h BEST_N100 ${STAGE_ROOT}/results/*.log | sort -t= -k3 -nr | head
SUM