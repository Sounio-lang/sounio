#!/usr/bin/env bash
# slurm-jobs/erdos90/submit_subset1521_array.sh — n=1521 subset array (N=25)
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SOUC="${SOUC:-$ROOT/bin/souc}"
SRC="${SRC:-$ROOT/stdlib/research/erdos90_subset1521_cluster.sio}"
RUN_ID="${RUN_ID:-erdos90-1521-$(date -u +%Y%m%dT%H%M%S)-${BASHPID}}"
STAGE_ROOT="${STAGE_ROOT:-/orangefs/training/sounio/erdos90-1521-runs/${RUN_ID}}"
...
echo "target:      n=1521 N=25 subset (beat grid39x39=7692)"
...
for i in $(seq 0 "$ARRAY_MAX"); do
    ...
    base64 -w0 "$elf" | srun --partition="$PARTITION" --time=00:10:00 --chdir=/orangefs/training bash -c "
      base64 -d > '${STAGE_ROOT}/bin/e90_1521_${seed}'
      chmod +x '${STAGE_ROOT}/bin/e90_1521_${seed}'
    "
done
...
BIN="${STAGE_ROOT}/bin/e90_1521_\${SEED}"
OUT="${STAGE_ROOT}/results/\${SEED}.log"
"\$BIN" | tee "\$OUT"
grep -E 'BEST_N1521|BEAT-GRID7692|CLUSTER1521_DONE' "\$OUT" || true
EOF

JOB_ID="$(sbatch --parsable "$SBATCH_FILE")"
echo "submitted: job ${JOB_ID}"
echo "monitor:   squeue -j ${JOB_ID}"
echo "leader:    grep -h BEST_N1521 ${STAGE_ROOT}/results/*.log | sort -t= -k3 -nr | head"