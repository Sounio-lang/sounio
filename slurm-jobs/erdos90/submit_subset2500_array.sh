#!/usr/bin/env bash
# slurm-jobs/erdos90/submit_subset2500_array.sh — n=2500 subset array (N=25)
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SOUC="${SOUC:-$ROOT/bin/souc}"
SRC="${SRC:-$ROOT/stdlib/research/erdos90_subset2500_cluster.sio}"
RUN_ID="${RUN_ID:-erdos90-2500-$(date -u +%Y%m%dT%H%M%S)-${BASHPID}}"
STAGE_ROOT="${STAGE_ROOT:-/orangefs/training/sounio/erdos90-2500-runs/${RUN_ID}}"
...
echo "target:      n=2500 N=25 subset (beat grid50x50=13148)"
...
for i in $(seq 0 "$ARRAY_MAX"); do
    ...
    base64 -w0 "$elf" | srun --partition="$PARTITION" --time=00:10:00 --chdir=/orangefs/training bash -c "
      base64 -d > '${STAGE_ROOT}/bin/e90_2500_${seed}'
      chmod +x '${STAGE_ROOT}/bin/e90_2500_${seed}'
    "
done
...
BIN="${STAGE_ROOT}/bin/e90_2500_\${SEED}"
OUT="${STAGE_ROOT}/results/\${SEED}.log"
"\$BIN" | tee "\$OUT"
grep -E 'BEST_N2500|BEAT-GRID13148|CLUSTER2500_DONE' "\$OUT" || true
EOF

JOB_ID="$(sbatch --parsable "$SBATCH_FILE")"
echo "submitted: job ${JOB_ID}"
echo "monitor:   squeue -j ${JOB_ID}"
echo "leader:    grep -h BEST_N2500 ${STAGE_ROOT}/results/*.log | sort -t= -k3 -nr | head"