#!/usr/bin/env bash
# Gate for stdlib data::dataframe_colops under default Madaros.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if ! grep -qF 'Madaros v' <<<"$engine_info"; then
  echo "FAIL: dataframe-colops gate requires default Madaros" >&2
  printf '%s\n' "$engine_info" >&2
  exit 1
fi
...
if $SOUC compile tests/stdlib/data/test_dataframe_colops_stdlib.sio -o "$OUT/x.elf" >/dev/null 2>&1; then
