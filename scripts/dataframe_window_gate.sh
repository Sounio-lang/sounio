...
if ! grep -qF 'Madaros v' <<<"$engine_info"; then
  echo "FAIL: dataframe-window gate requires default Madaros" >&2
  printf '%s\n' "$engine_info" >&2
  exit 1
fi
...
if $SOUC compile tests/stdlib/data/test_dataframe_window_stdlib.sio -o "$OUT/x.elf" >/dev/null 2>&1; then
