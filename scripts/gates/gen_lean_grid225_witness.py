#!/usr/bin/env python3
"""Generate Lean witness from erdos90_grid225_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/grid225_witness.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Grid225Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² grid witness at n=225 (machine-checked)

Generated from `stdlib/research/erdos90_grid225_export.sio`. Full 15×15 grid; unit²={unit_n}.
-/

namespace Sounio.Erdos90Grid225
...
theorem grid_n225_count : countGridUnit25 witness225 = {edges} := by
  native_decide

theorem grid_n225_beats_harb : harb 225 < countGridUnit25 witness225 := by
  native_decide

theorem grid_n225_meets_harb : harb 225 ≤ countGridUnit25 witness225 := by
  native_decide

theorem u225_grid_lower_bound : {edges} ≤ countGridUnit25 witness225 := by
  native_decide

end Sounio.Erdos90Grid225
"""
