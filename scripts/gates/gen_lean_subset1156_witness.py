#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1156_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1156.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1156Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1156, u(1156) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1156_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 2543200

namespace Sounio.Erdos90Subset1156
...
theorem subset_n1156_count : countGridUnit25 witness1156 = {edges} := by
  native_decide

theorem subset_n1156_beats_grid34x34 : 5692 < countGridUnit25 witness1156 := by
  native_decide

theorem subset_n1156_meets_harb : harb 1156 ≤ countGridUnit25 witness1156 := by
  native_decide

theorem u1156_subset_lower_bound : {edges} ≤ countGridUnit25 witness1156 := by
  native_decide

end Sounio.Erdos90Subset1156
"""
