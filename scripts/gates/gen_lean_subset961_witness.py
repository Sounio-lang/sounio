#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset961_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset961.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset961Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=961, u(961) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset961_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 2114200

namespace Sounio.Erdos90Subset961
...
theorem subset_n961_count : countGridUnit25 witness961 = {edges} := by
  native_decide

theorem subset_n961_beats_grid31x31 : 4636 < countGridUnit25 witness961 := by
  native_decide

theorem subset_n961_meets_harb : harb 961 ≤ countGridUnit25 witness961 := by
  native_decide

theorem u961_subset_lower_bound : {edges} ≤ countGridUnit25 witness961 := by
  native_decide

end Sounio.Erdos90Subset961
"""
