#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1089_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1089.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1089Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1089, u(1089) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1089_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 2395800

namespace Sounio.Erdos90Subset1089
...
theorem subset_n1089_count : countGridUnit25 witness1089 = {edges} := by
  native_decide

theorem subset_n1089_beats_grid33x33 : 5328 < countGridUnit25 witness1089 := by
  native_decide

theorem subset_n1089_meets_harb : harb 1089 ≤ countGridUnit25 witness1089 := by
  native_decide

theorem u1089_subset_lower_bound : {edges} ≤ countGridUnit25 witness1089 := by
  native_decide

end Sounio.Erdos90Subset1089
"""
