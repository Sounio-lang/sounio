#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1296_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1296.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1296Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1296, u(1296) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1296_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 2851200

namespace Sounio.Erdos90Subset1296
...
theorem subset_n1296_count : countGridUnit25 witness1296 = {edges} := by
  native_decide

theorem subset_n1296_beats_grid36x36 : 6456 < countGridUnit25 witness1296 := by
  native_decide

theorem subset_n1296_meets_harb : harb 1296 ≤ countGridUnit25 witness1296 := by
  native_decide

theorem u1296_subset_lower_bound : {edges} ≤ countGridUnit25 witness1296 := by
  native_decide

end Sounio.Erdos90Subset1296
"""
