#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset900_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset900.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset900Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=900, u(900) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset900_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset900
...
theorem subset_n900_count : countGridUnit25 witness900 = {edges} := by
  native_decide

theorem subset_n900_beats_grid30x30 : 4308 < countGridUnit25 witness900 := by
  native_decide

theorem subset_n900_meets_harb : harb 900 ≤ countGridUnit25 witness900 := by
  native_decide

theorem u900_subset_lower_bound : {edges} ≤ countGridUnit25 witness900 := by
  native_decide

end Sounio.Erdos90Subset900
"""
