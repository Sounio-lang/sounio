#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset484_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset484.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset484Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=484, u(484) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset484_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset484
...
theorem subset_n484_count : countGridUnit25 witness484 = {edges} := by
  native_decide

theorem subset_n484_beats_grid22x22 : 2116 < countGridUnit25 witness484 := by
  native_decide

theorem subset_n484_meets_harb : harb 484 ≤ countGridUnit25 witness484 := by
  native_decide

theorem u484_subset_lower_bound : {edges} ≤ countGridUnit25 witness484 := by
  native_decide

end Sounio.Erdos90Subset484
"""
