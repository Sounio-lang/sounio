#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset2025_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset2025.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset2025Witness.lean"
    )
    ...
    if len(pts) != 2025:
        print(f"expected 2025 points, got {len(pts)}", file=sys.stderr)
        return 1
    if len(set(pts)) != 2025:
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=2025, u(2025) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset2025_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 10125000

namespace Sounio.Erdos90Subset2025
...
def witness2025 : List (Int × Int) := [
  {entries}
]

theorem subset_n2025_count : countGridUnit25 witness2025 = {edges} := by
  native_decide

theorem subset_n2025_beats_grid45x45 : 10488 < countGridUnit25 witness2025 := by
  native_decide

theorem subset_n2025_meets_harb : harb 2025 ≤ countGridUnit25 witness2025 := by
  native_decide

theorem u2025_subset_lower_bound : {edges} ≤ countGridUnit25 witness2025 := by
  native_decide

end Sounio.Erdos90Subset2025
"""
