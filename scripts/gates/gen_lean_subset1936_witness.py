#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1936_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1936.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1936Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1936, u(1936) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1936_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 7744000

namespace Sounio.Erdos90Subset1936
...
theorem subset_n1936_count : countGridUnit25 witness1936 = {edges} := by
  native_decide

theorem subset_n1936_beats_grid44x44 : 9992 < countGridUnit25 witness1936 := by
  native_decide

theorem subset_n1936_meets_harb : harb 1936 ≤ countGridUnit25 witness1936 := by
  native_decide

theorem u1936_subset_lower_bound : {edges} ≤ countGridUnit25 witness1936 := by
  native_decide

end Sounio.Erdos90Subset1936
"""
