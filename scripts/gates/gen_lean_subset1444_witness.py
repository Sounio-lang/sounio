#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1444_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1444.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1444Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1444, u(1444) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1444_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 3176800

namespace Sounio.Erdos90Subset1444
...
theorem subset_n1444_count : countGridUnit25 witness1444 = {edges} := by
  native_decide

theorem subset_n1444_beats_grid38x38 : 7268 < countGridUnit25 witness1444 := by
  native_decide

theorem subset_n1444_meets_harb : harb 1444 ≤ countGridUnit25 witness1444 := by
  native_decide

theorem u1444_subset_lower_bound : {edges} ≤ countGridUnit25 witness1444 := by
  native_decide

end Sounio.Erdos90Subset1444
"""
