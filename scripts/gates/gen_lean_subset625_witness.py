#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset625_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset625.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset625Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=625, u(625) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset625_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset625
...
theorem subset_n625_count : countGridUnit25 witness625 = {edges} := by
  native_decide

theorem subset_n625_beats_grid25x25 : 2848 < countGridUnit25 witness625 := by
  native_decide

theorem subset_n625_meets_harb : harb 625 ≤ countGridUnit25 witness625 := by
  native_decide

theorem u625_subset_lower_bound : {edges} ≤ countGridUnit25 witness625 := by
  native_decide

end Sounio.Erdos90Subset625
"""
