#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1521_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1521.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1521Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1521, u(1521) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1521_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 3346200

namespace Sounio.Erdos90Subset1521
...
theorem subset_n1521_count : countGridUnit25 witness1521 = {edges} := by
  native_decide

theorem subset_n1521_beats_grid39x39 : 7692 < countGridUnit25 witness1521 := by
  native_decide

theorem subset_n1521_meets_harb : harb 1521 ≤ countGridUnit25 witness1521 := by
  native_decide

theorem u1521_subset_lower_bound : {edges} ≤ countGridUnit25 witness1521 := by
  native_decide

end Sounio.Erdos90Subset1521
"""
