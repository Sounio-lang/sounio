#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1369_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1369.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1369Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1369, u(1369) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1369_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 3011800

namespace Sounio.Erdos90Subset1369
...
theorem subset_n1369_count : countGridUnit25 witness1369 = {edges} := by
  native_decide

theorem subset_n1369_beats_grid37x37 : 6856 < countGridUnit25 witness1369 := by
  native_decide

theorem subset_n1369_meets_harb : harb 1369 ≤ countGridUnit25 witness1369 := by
  native_decide

theorem u1369_subset_lower_bound : {edges} ≤ countGridUnit25 witness1369 := by
  native_decide

end Sounio.Erdos90Subset1369
"""
