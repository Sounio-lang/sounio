#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1849_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1849.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1849Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1849, u(1849) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1849_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 4067800

namespace Sounio.Erdos90Subset1849
...
theorem subset_n1849_count : countGridUnit25 witness1849 = {edges} := by
  native_decide

theorem subset_n1849_beats_grid43x43 : 9508 < countGridUnit25 witness1849 := by
  native_decide

theorem subset_n1849_meets_harb : harb 1849 ≤ countGridUnit25 witness1849 := by
  native_decide

theorem u1849_subset_lower_bound : {edges} ≤ countGridUnit25 witness1849 := by
  native_decide

end Sounio.Erdos90Subset1849
"""
