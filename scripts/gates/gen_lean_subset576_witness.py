#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset576_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset576.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset576Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=576, u(576) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset576_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset576
...
theorem subset_n576_count : countGridUnit25 witness576 = {edges} := by
  native_decide

theorem subset_n576_beats_grid24x24 : 2592 < countGridUnit25 witness576 := by
  native_decide

theorem subset_n576_meets_harb : harb 576 ≤ countGridUnit25 witness576 := by
  native_decide

theorem u576_subset_lower_bound : {edges} ≤ countGridUnit25 witness576 := by
  native_decide

end Sounio.Erdos90Subset576
"""
