#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset784_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset784.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset784Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=784, u(784) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset784_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset784
...
theorem subset_n784_count : countGridUnit25 witness784 = {edges} := by
  native_decide

theorem subset_n784_beats_grid28x28 : 3688 < countGridUnit25 witness784 := by
  native_decide

theorem subset_n784_meets_harb : harb 784 ≤ countGridUnit25 witness784 := by
  native_decide

theorem u784_subset_lower_bound : {edges} ≤ countGridUnit25 witness784 := by
  native_decide

end Sounio.Erdos90Subset784
"""
