#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset289_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset289.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset289Witness.lean"
    )
    ...
    if len(pts) != 289:
        print(f"expected 289 points, got {len(pts)}", file=sys.stderr)
        return 1
    if len(set(pts)) != 289:
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=289, u(289) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset289_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset289
...
def witness289 : List (Int × Int) := [
  {entries}
]

theorem subset_n289_count : countGridUnit25 witness289 = {edges} := by
  native_decide

theorem subset_n289_beats_grid17x17 : 1136 < countGridUnit25 witness289 := by
  native_decide

theorem subset_n289_meets_harb : harb 289 ≤ countGridUnit25 witness289 := by
  native_decide

theorem u289_subset_lower_bound : {edges} ≤ countGridUnit25 witness289 := by
  native_decide

end Sounio.Erdos90Subset289
"""
