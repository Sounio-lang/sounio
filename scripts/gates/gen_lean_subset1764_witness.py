#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1764_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1764.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1764Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1764, u(1764) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1764_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 3880800

namespace Sounio.Erdos90Subset1764
...
theorem subset_n1764_count : countGridUnit25 witness1764 = {edges} := by
  native_decide

theorem subset_n1764_beats_grid42x42 : 9036 < countGridUnit25 witness1764 := by
  native_decide

theorem subset_n1764_meets_harb : harb 1764 ≤ countGridUnit25 witness1764 := by
  native_decide

theorem u1764_subset_lower_bound : {edges} ≤ countGridUnit25 witness1764 := by
  native_decide

end Sounio.Erdos90Subset1764
"""
