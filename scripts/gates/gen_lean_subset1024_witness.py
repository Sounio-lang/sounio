#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1024_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1024.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1024Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1024, u(1024) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1024_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 2252800

namespace Sounio.Erdos90Subset1024
...
theorem subset_n1024_count : countGridUnit25 witness1024 = {edges} := by
  native_decide

theorem subset_n1024_beats_grid32x32 : 4976 < countGridUnit25 witness1024 := by
  native_decide

theorem subset_n1024_meets_harb : harb 1024 ≤ countGridUnit25 witness1024 := by
  native_decide

theorem u1024_subset_lower_bound : {edges} ≤ countGridUnit25 witness1024 := by
  native_decide

end Sounio.Erdos90Subset1024
"""
