#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset196_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset196.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset196Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=196, u(196) ≥ {edges} (machine-checked)
...
theorem subset_n196_count : countGridUnit25 witness196 = {edges} := by
  native_decide
...
theorem u196_subset_lower_bound : {edges} ≤ countGridUnit25 witness196 := by
  native_decide
