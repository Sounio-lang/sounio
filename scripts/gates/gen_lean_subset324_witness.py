#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset324_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset324.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset324Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=324, u(324) ≥ {edges} (machine-checked)
...
theorem subset_n324_count : countGridUnit25 witness324 = {edges} := by
  native_decide
...
theorem u324_subset_lower_bound : {edges} ≤ countGridUnit25 witness324 := by
  native_decide
