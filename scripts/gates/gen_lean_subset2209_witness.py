#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset2209_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset2209.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset2209Witness.lean"
    )
    ...
    if len(pts) != 2209:
        print(f"expected 2209 points, got {len(pts)}", file=sys.stderr)
        return 1
    if len(set(pts)) != 2209:
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=2209, u(2209) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset2209_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 11045000

namespace Sounio.Erdos90Subset2209
...
def witness2209 : List (Int × Int) := [
  {entries}
]

theorem subset_n2209_count : countGridUnit25 witness2209 = {edges} := by
  native_decide

theorem subset_n2209_beats_grid47x47 : 11516 < countGridUnit25 witness2209 := by
  native_decide

theorem subset_n2209_meets_harb : harb 2209 ≤ countGridUnit25 witness2209 := by
  native_decide

theorem u2209_subset_lower_bound : {edges} ≤ countGridUnit25 witness2209 := by
  native_decide

end Sounio.Erdos90Subset2209
"""
