#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset2116_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset2116.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset2116Witness.lean"
    )
    ...
    if len(pts) != 2116:
        print(f"expected 2116 points, got {len(pts)}", file=sys.stderr)
        return 1
    if len(set(pts)) != 2116:
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=2116, u(2116) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset2116_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 10580000

namespace Sounio.Erdos90Subset2116
...
def witness2116 : List (Int × Int) := [
  {entries}
]

theorem subset_n2116_count : countGridUnit25 witness2116 = {edges} := by
  native_decide

theorem subset_n2116_beats_grid46x46 : 10996 < countGridUnit25 witness2116 := by
  native_decide

theorem subset_n2116_meets_harb : harb 2116 ≤ countGridUnit25 witness2116 := by
  native_decide

theorem u2116_subset_lower_bound : {edges} ≤ countGridUnit25 witness2116 := by
  native_decide

end Sounio.Erdos90Subset2116
"""
