#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset729_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset729.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset729Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=729, u(729) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset729_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset729
...
theorem subset_n729_count : countGridUnit25 witness729 = {edges} := by
  native_decide

theorem subset_n729_beats_grid27x27 : 3396 < countGridUnit25 witness729 := by
  native_decide

theorem subset_n729_meets_harb : harb 729 ≤ countGridUnit25 witness729 := by
  native_decide

theorem u729_subset_lower_bound : {edges} ≤ countGridUnit25 witness729 := by
  native_decide

end Sounio.Erdos90Subset729
"""
