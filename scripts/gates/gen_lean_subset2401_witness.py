#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset2401_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset2401.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset2401Witness.lean"
    )
    ...
    if len(pts) != 2401:
        print(f"expected 2401 points, got {len(pts)}", file=sys.stderr)
        return 1
    if len(set(pts)) != 2401:
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=2401, u(2401) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset2401_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 12005000

namespace Sounio.Erdos90Subset2401
...
def witness2401 : List (Int × Int) := [
  {entries}
]

theorem subset_n2401_count : countGridUnit25 witness2401 = {edges} := by
  native_decide

theorem subset_n2401_beats_grid49x49 : 12592 < countGridUnit25 witness2401 := by
  native_decide

theorem subset_n2401_meets_harb : harb 2401 ≤ countGridUnit25 witness2401 := by
  native_decide

theorem u2401_subset_lower_bound : {edges} ≤ countGridUnit25 witness2401 := by
  native_decide

end Sounio.Erdos90Subset2401
"""
