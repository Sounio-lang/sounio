#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset529_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset529.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset529Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=529, u(529) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset529_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset529
...
theorem subset_n529_count : countGridUnit25 witness529 = {edges} := by
  native_decide

theorem subset_n529_beats_grid23x23 : 2348 < countGridUnit25 witness529 := by
  native_decide

theorem subset_n529_meets_harb : harb 529 ≤ countGridUnit25 witness529 := by
  native_decide

theorem u529_subset_lower_bound : {edges} ≤ countGridUnit25 witness529 := by
  native_decide

end Sounio.Erdos90Subset529
"""
