#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1681_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1681.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1681Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1681, u(1681) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1681_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 3698200

namespace Sounio.Erdos90Subset1681
...
theorem subset_n1681_count : countGridUnit25 witness1681 = {edges} := by
  native_decide

theorem subset_n1681_beats_grid41x41 : 8576 < countGridUnit25 witness1681 := by
  native_decide

theorem subset_n1681_meets_harb : harb 1681 ≤ countGridUnit25 witness1681 := by
  native_decide

theorem u1681_subset_lower_bound : {edges} ≤ countGridUnit25 witness1681 := by
  native_decide

end Sounio.Erdos90Subset1681
"""
