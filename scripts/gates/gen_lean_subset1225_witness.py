#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1225_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1225.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1225Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1225, u(1225) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1225_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 2695000

namespace Sounio.Erdos90Subset1225
...
theorem subset_n1225_count : countGridUnit25 witness1225 = {edges} := by
  native_decide

theorem subset_n1225_beats_grid35x35 : 6068 < countGridUnit25 witness1225 := by
  native_decide

theorem subset_n1225_meets_harb : harb 1225 ≤ countGridUnit25 witness1225 := by
  native_decide

theorem u1225_subset_lower_bound : {edges} ≤ countGridUnit25 witness1225 := by
  native_decide

end Sounio.Erdos90Subset1225
"""
