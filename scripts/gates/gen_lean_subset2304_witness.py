#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset2304_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset2304.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset2304Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=2304, u(2304) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset2304_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 11520000

namespace Sounio.Erdos90Subset2304
...
theorem subset_n2304_count : countGridUnit25 witness2304 = {edges} := by
  native_decide

theorem subset_n2304_beats_grid48x48 : 12048 < countGridUnit25 witness2304 := by
  native_decide

theorem subset_n2304_meets_harb : harb 2304 ≤ countGridUnit25 witness2304 := by
  native_decide

theorem u2304_subset_lower_bound : {edges} ≤ countGridUnit25 witness2304 := by
  native_decide

end Sounio.Erdos90Subset2304
"""
