#!/usr/bin/env python3
"""Generate Lean witness from erdos90_grid289_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/grid289_witness.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Grid289Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² grid witness at n=289 (machine-checked)
...
theorem grid_n289_count : countGridUnit25 witness289 = {edges} := by
  native_decide

theorem grid_n289_beats_harb : harb 289 < countGridUnit25 witness289 := by
  native_decide

theorem grid_n289_meets_harb : harb 289 ≤ countGridUnit25 witness289 := by
  native_decide

theorem u289_grid_lower_bound : {edges} ≤ countGridUnit25 witness289 := by
  native_decide

end Sounio.Erdos90Grid289
"""
