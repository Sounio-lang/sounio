#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset841_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset841.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset841Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=841, u(841) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset841_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 1000000

namespace Sounio.Erdos90Subset841
...
theorem subset_n841_count : countGridUnit25 witness841 = {edges} := by
  native_decide

theorem subset_n841_beats_grid29x29 : 3992 < countGridUnit25 witness841 := by
  native_decide

theorem subset_n841_meets_harb : harb 841 ≤ countGridUnit25 witness841 := by
  native_decide

theorem u841_subset_lower_bound : {edges} ≤ countGridUnit25 witness841 := by
  native_decide

end Sounio.Erdos90Subset841
"""
