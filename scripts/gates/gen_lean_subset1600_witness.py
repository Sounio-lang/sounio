#!/usr/bin/env python3
"""Generate Lean witness from erdos90_subset1600_export.sio stdout."""
import re
import sys
from pathlib import Path

def main() -> int:
    log_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/subset1600.log")
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else Path(
        "formal/lean4/SounioErdos90Subset1600Witness.lean"
    )
    ...
    lean = f"""/-!
# Erdős [90] — ℤ² subset witness at n=1600, u(1600) ≥ {edges} (machine-checked)

Generated from `stdlib/research/erdos90_subset1600_export.sio`.
Seed={seed}, winR={win_r}, unit²={unit_n}. Not a claim of global optimality.
-/

set_option maxHeartbeats 3520000

namespace Sounio.Erdos90Subset1600
...
theorem subset_n1600_count : countGridUnit25 witness1600 = {edges} := by
  native_decide

theorem subset_n1600_beats_grid40x40 : 8128 < countGridUnit25 witness1600 := by
  native_decide

theorem subset_n1600_meets_harb : harb 1600 ≤ countGridUnit25 witness1600 := by
  native_decide

theorem u1600_subset_lower_bound : {edges} ≤ countGridUnit25 witness1600 := by
  native_decide

end Sounio.Erdos90Subset1600
"""
