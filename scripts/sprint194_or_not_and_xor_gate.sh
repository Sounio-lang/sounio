...
echo ""
echo "--- Self-tests: T830-T835 ---"
L="$(mktemp)"
if [ ! -x "$SOUC" ]; then
    for i in $(seq 830 835); do TOTAL=$((TOTAL+1)); echo "NOT_RUN  selftest:T$i"; NOT_RUN=$((NOT_RUN+1)); done
else
    timeout 180 "$SOUC" run self-hosted/compiler/main.sio -- --self-test > "$L" 2>&1 || _ec=$?
    for i in $(seq 830 835); do cl "selftest:T$i" "T$i OK" "$L"; done
fi
rm -f "$L"
