#!/usr/bin/env bash
# sprint212_xor_and_or_simp_gate.sh — Sprint 212: Block DV XOR-AND-OR simplify gate
set -eo pipefail
...
cg "src:dv_xor_rr"    "as_xor_rr_valid\[dv_s1 as usize\]" "$O"
cg "src:dv_or_rr"     "ao_or_rr_valid\[dv_s2 as usize\]" "$O"
cg "src:dv_copy"      "ir_copy.*dv_instr\.dst.*dv_s1" "$O"
echo ""; echo "--- Tests ---"
