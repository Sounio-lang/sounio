#!/usr/bin/env bash
# sprint168_zero_minus_gate.sh — Sprint 168: Block CD zero-minus normalization
#
# Block CD: 0 - x → neg(x)  (IrUnaryOp::OpNeg)
#   Zero new arrays: uses is_const/const_val + def_at.
#   SOTA: LLVM InstCombineAddSub sub(0,X) → neg X; additive inverse normalization.
set -eo pipefail
...
check_log_line() {
    ...
    elif ! grep -qF "T674" "$log_file" 2>/dev/null; then echo "NOT_RUN  $name (OOM before T674)"; NOT_RUN=$((NOT_RUN+1))
...
check_grep "src:cd_zero_check"     "const_val\[cd_s1 as usize\] == 0" "$OPT_FILE"
check_grep "src:cd_unary_neg"      "IrUnaryOp.*OpNeg|ir_unaryop.*OpNeg" "$OPT_FILE"
