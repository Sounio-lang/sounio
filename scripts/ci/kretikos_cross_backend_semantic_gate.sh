...
if [[ "$vec_ptx_add_marker" != true ||
      ...
      "$epi_spirv_valid_and_marker" != true ]]; then
  status="fail"
else
  status="pass"
fi
...
artifacts_json="{\"epistemic_dual_output_f32\":{\"metal\":{\"bytes\":$(size_file "$EPI_METAL_OUT"),\"path\":\"${EPI_METAL_OUT#$ROOT_DIR/}\",\"sha256\":\"$(sha_file "$EPI_METAL_OUT")\"},\"ptx\":{\"bytes\":$(size_file "$EPI_PTX_OUT"),\"path\":\"${EPI_PTX_OUT#$ROOT_DIR/}\",\"sha256\":\"$(sha_file "$EPI_PTX_OUT")\"},\"spirv\":{\"bytes\":$(size_file "$EPI_SPIRV_OUT"),\"path\":\"${EPI_SPIRV_OUT#$ROOT_DIR/}\",\"sha256\":\"$(sha_file "$EPI_SPIRV_OUT")\"}},\"vec_add_f32\":{\"metal\":{\"bytes\":$(size_file "$VEC_METAL_OUT"),\"path\":\"${VEC_METAL_OUT#$ROOT_DIR/}\",\"sha256\":\"$(sha_file "$VEC_METAL_OUT")\"},\"ptx\":{\"bytes\":$(size_file "$VEC_PTX_OUT"),\"path\":\"${VEC_PTX_OUT#$ROOT_DIR/}\",\"sha256\":\"$(sha_file "$VEC_PTX_OUT")\"},\"spirv\":{\"bytes\":$(size_file "$VEC_SPIRV_OUT"),\"path\":\"${VEC_SPIRV_OUT#$ROOT_DIR/}\",\"sha256\":\"$(sha_file "$VEC_SPIRV_OUT")\"}}}"

./bin/kretikos json-emit \
  --raw-json "artifacts=$artifacts_json" \
  --array-strings "boundaries=structural_cross_backend_gate_only|does_not_claim_runtime_execution_for_metal_or_spirv|does_not_claim_general_backend_equivalence|does_not_claim_full_user_kernel_lowering|vec_add_oracle_is_scalar_add_1_plus_2_equals_3|epistemic_dual_output_oracle_is_value_add_eps_square_sum_valid_and" \
...
./bin/kretikos kaxi-validate-evidence "$REPORT_JSON" \
  ...
  --expect "checks.epistemic_dual_output_f32_spirv_valid_and_marker=true" >/dev/null
