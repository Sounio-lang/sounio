...
assert_contains "$VERSION_OUT" "souc " "souc --version"
assert_contains "$VERSION_OUT" "stage0:" "souc --version"

echo "[real-runner] 2/7 souc info"
INFO_OUT="$(./bin/souc info 2>&1)"
assert_contains "$INFO_OUT" "stage0:" "souc info"
assert_contains "$INFO_OUT" "target:" "souc info"
...
trap 'rm -f "$TMP_ELF" "$TMP_RAW" "$TMP_MADAROS_SRC" "$TMP_MADAROS_ELF"' EXIT
...
REPL_OUT="$(printf '%s\n' \
  ...
  'dbl(21)' \
  ':quit' \
  | timeout 120 ./bin/souc repl --no-restore 2>&1)"
assert_contains "$REPL_OUT" "42" "souc repl"

if [[ "${SOUNIO_REAL_RUNNER_SKIP_MADAROS:-0}" != "1" ]]; then
