...
echo "[project-spine] 1/15 public runner baseline"
bash scripts/ci/real_language_runner_gate.sh

echo "[project-spine] 2/15 checked example project"
...
echo "[project-spine] 3/15 souc init project loop"
...
echo "[project-spine] 4/15 missing import negative"
...
echo "[project-spine] 5/15 package import manifest gate"
bash scripts/ci/package_import_science_gate.sh

echo "[project-spine] 6/15 thin multimodule runtime"
...
echo "[project-spine] 7/15 Madaros identity and single-file native-v2"
...
echo "[project-spine] 8/15 Madaros source body semantics"
bash scripts/ci/madaros_source_semantics_gate.sh

echo "[project-spine] 9/15 native-v2 global address + pointer aggregate"
bash scripts/ci/native_v2_global_addr_ptr_gate.sh

echo "[project-spine] 10/15 Madaros project import runtime"
...
echo "[project-spine] 11/15 Madaros imported argument-call runtime"
set +e
...
set +e
./bin/madaros run tests/multimodule/thin_six_literal_args_main.sio
MADAROS_THIN_SIX_ARGS_RC=$?
set -e
[[ "$MADAROS_THIN_SIX_ARGS_RC" -eq 63 ]] || fail "Madaros thin imported six-arg call expected exit 63, got $MADAROS_THIN_SIX_ARGS_RC"
set +e
./bin/madaros run tests/multimodule/thin_seven_literal_args_main.sio
...
set +e
./bin/madaros run tests/multimodule/thin_runtime_mixed_seven_main.sio
MADAROS_THIN_RUNTIME_MIXED_SEVEN_ARGS_RC=$?
set -e
[[ "$MADAROS_THIN_RUNTIME_MIXED_SEVEN_ARGS_RC" -eq 31 ]] || fail "Madaros mixed runtime/literal imported seven-arg call expected exit 31, got $MADAROS_THIN_RUNTIME_MIXED_SEVEN_ARGS_RC"
set +e
./bin/madaros run tests/multimodule/thin_runtime_local_mixed_seven_main.sio
MADAROS_THIN_RUNTIME_LOCAL_MIXED_SEVEN_ARGS_RC=$?
set -e
[[ "$MADAROS_THIN_RUNTIME_LOCAL_MIXED_SEVEN_ARGS_RC" -eq 50 ]] || fail "Madaros local/runtime/literal imported seven-arg call expected exit 50, got $MADAROS_THIN_RUNTIME_LOCAL_MIXED_SEVEN_ARGS_RC"
set +e
./bin/madaros run tests/multimodule/thin_runtime_two_local_seven_main.sio
MADAROS_THIN_RUNTIME_TWO_LOCAL_SEVEN_ARGS_RC=$?
set -e
[[ "$MADAROS_THIN_RUNTIME_TWO_LOCAL_SEVEN_ARGS_RC" -eq 69 ]] || fail "Madaros two-local imported seven-arg call expected exit 69, got $MADAROS_THIN_RUNTIME_TWO_LOCAL_SEVEN_ARGS_RC"
for witness in \
...
echo "[project-spine] 12/15 native aggregate ABI"
bash scripts/ci/native_aggregate_abi_gate.sh

echo "[project-spine] 13/15 imported source native-v2 compile"
