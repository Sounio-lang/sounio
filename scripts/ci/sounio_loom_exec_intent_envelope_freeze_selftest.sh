...
fail() {
  printf 'sounio-loom-exec-intent-envelope-freeze-selftest: FAIL: %s\n' "$*" >&2
  exit 1
}
...
expect_value schema loom-exec-intent-envelope-freeze-v1
expect_value stage SEMANTICS_FROZEN
expect_value producing_language Sounio
expect_value language_role SEMANTIC_AUTHORITY
expect_value action 9034
expect_value concept_id SOUNIO-LOOM-EXEC-INTENT-ENVELOPE
expect_value case_count 12
expect_value causal_sabotage PASS
expect_value load_bearing_rule command_sha256_equal
expect_value raw_event_is_semantic_identity false
expect_value raw_event_separate true
expect_value descriptor_transport unix-stream-inherited
expect_value descriptor_is_bearer false
expect_value pathname_is_authority false
expect_value expected_results_encoded_in_material_layer false
expect_value ocaml_projection_attached false
expect_value provider_lifecycle_attached false
expect_value arbitrary_command_projection false
...
for key in garden contract source entrypoint build_script selftest evidence \
  parent_9031_manifest parent_9033_manifest toolchain_wrapper toolchain_compiler; do
...
work="$(mktemp -d "${TMPDIR:-/tmp}/sounio-loom-exec-intent-freeze.XXXXXX")"
trap 'rm -rf "$work"' EXIT
for ordinal in one two; do
  SOUNIO_LOOM_EXEC_INTENT_ENVELOPE_OUTPUT="$work/runtime-$ordinal" \
...
fields="$(manifest_value fields_schema)
$(manifest_value envelope_schema)
$(manifest_value semantic_event_sha256)
$(manifest_value command_sha256)"
project_frame="$(manifest_value wire_schema) $(manifest_value project_word0) $(manifest_value common_word1)"
mismatch_frame="$(manifest_value wire_schema) $(manifest_value command_mismatch_word0) $(manifest_value common_word1)"
[[ "$(printf '%s\n' "$project_frame" | "$work/runtime-one")" == \
   "$(manifest_value project_decision)
$fields" ]] || fail 'project decision or intent fields drifted'
set +e
...
result="$(bash "$ROOT_DIR/scripts/ci/sounio_loom_exec_intent_envelope_selftest.sh")"
[[ "$result" == "$(manifest_value result)" ]] ||
  fail 'Sounio fixture result drifted'
[[ "$(printf '%s' "$result" | sha256sum | cut -d ' ' -f 1)" == \
   "$(manifest_value result_sha256)" ]] ||
  fail 'Sounio fixture result hash drifted'
printf '%s\n' "$result" |
  cmp - "$ROOT_DIR/$(manifest_value evidence_path)" ||
  fail 'evidence is not the exact executable result'

printf 'sounio-loom-exec-intent-envelope-freeze-selftest: PASS semantic_authority=Sounio action=9034 stage=SEMANTICS_FROZEN executable_sha256=%s treatment=PROJECT command_mismatch=DENY555 causal_sabotage=PASS schema=%s semantic_event_sha256=%s raw_event_separate=true python_executed=false rust_executed=false ocaml_projection_attached=false provider_lifecycle_attached=false arbitrary_command_projection=false exec_attached=false production_activation=false parity_open=false claim_ready=false\n' \
  "$(manifest_value executable_sha256)" \
  "$(manifest_value envelope_schema)" \
  "$(manifest_value semantic_event_sha256)"
