...
stack_kb="${SOUNIO_MADAROS_BOX_DEREF_STACK_KB:-524288}"
[[ "$stack_kb" =~ ^[1-9][0-9]*$ && ${#stack_kb} -le 9 ]] || { echo "[madaros-box-deref] FAIL: invalid stack size: $stack_kb" >&2; exit 1; }
stack_before="$(ulimit -S -s 2>/dev/null)" || { echo "[madaros-box-deref] FAIL: soft stack limit unavailable" >&2; exit 1; }
if [[ "$stack_before" != "unlimited" ]] && ((stack_before < stack_kb)); then
  ulimit -S -s "$stack_kb" 2>/dev/null || { echo "[madaros-box-deref] FAIL: could not raise soft stack limit to ${stack_kb} KiB" >&2; exit 1; }
fi
stack_after="$(ulimit -S -s 2>/dev/null)"
if [[ "$stack_after" != "unlimited" ]] && ((stack_after < stack_kb)); then
  echo "[madaros-box-deref] FAIL: soft stack limit remained below ${stack_kb} KiB: $stack_after" >&2
  exit 1
fi
printf '[madaros-box-deref] stack_kb before=%s after=%s requested=%s\n' "$stack_before" "$stack_after" "$stack_kb"
