...
SOURCE="$ROOT_DIR/tests/known_failures/global_f64_scratch_add.sio"
...
if [[ "$run_rc" != "0" ]]; then
  cat "$WORK/run.log" >&2
  fail "global f64 scratch witness exited rc=$run_rc"
fi
grep -Fxq 'PASS global_f64_scratch_add' "$WORK/run.log" || {
  cat "$WORK/run.log" >&2
  fail "exact PASS marker missing"
}

echo "[madaros-global-f64] PASS: rebuilt current-source Madaros preserves global f64-array element typing"
