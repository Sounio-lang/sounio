...
fail() {
  printf 'sounio-loom-process-witness-effect-root-v9-host-gate: FAIL reason=%s root_treatment=false bootstrap_sabotage=false material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false\n' "$*" >&2
  exit 1
}

unavailable() {
  printf 'sounio-loom-process-witness-effect-root-v9-host-gate: HOST_GATE_UNAVAILABLE reason=%s root_treatment=false bootstrap_sabotage=false material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false\n' "$*" >&2
  exit 77
}
...
$ROOT/loom/effect-policy-v9.freeze.v1
$ROOT/loom/payload
...
for manifest in "$ROOT/loom/payload.freeze.v1" \
                "$ROOT/loom/effect-policy-v9.freeze.v1"; do
...
[[ "$(sha256sum "$ROOT/loom/effect-policy-v9.freeze.v1" | cut -d ' ' -f 1)" == \
  9d747d937a6a2316dd8894b37e243180031b8518f2696b9200ee7d1f1d81868c ]] ||
  fail 'host root policy manifest hash drifted'

nonce="$$-$(date +%s%N)"
UNIT="sounio-loom-effect-root-v9-$nonce.service"
SABOTAGE_UNIT="sounio-loom-effect-root-v9-missing-incoming-$nonce.service"
SYS_SABOTAGE_UNIT="sounio-loom-effect-root-v9-missing-sys-$nonce.service"
VAR_TMP_SABOTAGE_UNIT="sounio-loom-effect-root-v9-missing-var-tmp-$nonce.service"
CONTROL="/run/sounio-loom-effect-root-v9-$nonce.control"
OUTPUT="/run/sounio-loom-effect-root-v9-$nonce.output"
ERROR="/run/sounio-loom-effect-root-v9-$nonce.error"
SABOTAGE_OUTPUT="/run/sounio-loom-effect-root-v9-missing-incoming-$nonce.output"
SABOTAGE_ERROR="/run/sounio-loom-effect-root-v9-missing-incoming-$nonce.error"
SYS_SABOTAGE_OUTPUT="/run/sounio-loom-effect-root-v9-missing-sys-$nonce.output"
SYS_SABOTAGE_ERROR="/run/sounio-loom-effect-root-v9-missing-sys-$nonce.error"
VAR_TMP_SABOTAGE_OUTPUT="/run/sounio-loom-effect-root-v9-missing-var-tmp-$nonce.output"
VAR_TMP_SABOTAGE_ERROR="/run/sounio-loom-effect-root-v9-missing-var-tmp-$nonce.error"
CLIENT_PID=''
CELL_PID=''
ROOT_MOUNTED=false
...
systemd-run --quiet --unit="$UNIT" --service-type=exec --pipe --wait \
  ...
  -- /loom/effect-cell --root-hold \
     --policy-manifest /loom/effect-policy-v9.freeze.v1 \
  < "$CONTROL" > "$OUTPUT" 2> "$ERROR" &
CLIENT_PID=$!

for attempt in $(seq 1 200); do
  if grep -q '^LOOM_PROCESS_WITNESS_EFFECT_POLICY_V9_ROOT_READY PASS ' "$OUTPUT" 2>/dev/null; then
    break
  fi
...
[[ "$ready" == LOOM_PROCESS_WITNESS_EFFECT_POLICY_V9_ROOT_READY\ PASS* ]] ||
...
for expectation in \
  ...
  'policy_manifest_sha256=9d747d937a6a2316dd8894b37e243180031b8518f2696b9200ee7d1f1d81868c' \
