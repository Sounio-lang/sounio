...
if bin/souc run "$PROBE" -- "$PART_SRC" "$PART_CACHE" "$PART_CACHE_SRC" >"$PART_LOG" 2>&1 &&
   ...
   grep -q 'payload_term_count=20' "$PART_LOG" &&
   ...
   grep -q 'mini_table_class_count=20' "$PART_LOG" &&
   ...
   grep -q 'mini_table_parent_edge_count=19' "$PART_LOG" &&
...
if grep -q 'ontology_bundle_directive_found=1' "$LOINC_LOG" &&
   ...
   grep -q 'payload_term_count=20' "$LOINC_LOG" &&
   ...
   grep -q 'mini_table_class_count=20' "$LOINC_LOG" &&
   ...
   grep -q 'mini_table_parent_edge_count=3' "$LOINC_LOG" &&
