...
OUT_DIR="${SOUNIO_SEMANTIC_ORC_KAXI_PACK_DIR:-$(mktemp -d /tmp/sounio-semantic-orc-kaxi-pack.XXXXXX)}"
...
RUN_OUT="$OUT_DIR/swow16_fixture_gate.out"

mkdir -p "$OUT_DIR"

echo "[semantic-orc-kaxi-pack] input=$INPUT_DIR"
echo "[semantic-orc-kaxi-pack] regime=$REGIME"
echo "[semantic-orc-kaxi-pack] support_index=$SUPPORT_INDEX"
echo "[semantic-orc-kaxi-pack] out=$OUT_DIR"
...
for token in "${required_tokens[@]}"; do
  if ! grep -Fqx "$token" "$RUN_OUT"; then
    echo "[semantic-orc-kaxi-pack] FAIL: missing token: $token" >&2
    cat "$RUN_OUT" >&2
    exit 1
  fi
done
...
if pack.get("schema") != "sounio.semantic_orc.swow16_kaxi_sinkhorn16_pack.v1":
    raise SystemExit("unexpected kaxi pack schema")
if len(manifest.get("selected_node_ids", [])) != 16:
    raise SystemExit("manifest must select 16 nodes")
if int(manifest.get("support_index", -1)) != int(pack.get("support_index", -2)):
    raise SystemExit("manifest/pack support_index mismatch")
if int(manifest.get("support_stride", -1)) != int(pack.get("support_stride", -2)):
    raise SystemExit("manifest/pack support_stride mismatch")
...
for forbidden in ("ptx_source", "cuda_kernel_params", "grid_dim", "block_dim", "runtime"):
    if forbidden in pack:
        raise SystemExit(f"K-AXI pack must not contain runtime launch field: {forbidden}")
for key in ("generator_sha256", "feature_file_sha256", "trajectory_file_sha256"):
...
path_keys = {
    ...
    "trajectory_file": "trajectory_file_sha256",
...
echo "[semantic-orc-kaxi-pack] PASS"
