#!/usr/bin/env bash
# CI gate for the level-8 (256-dim) ZD census fast exact contract.
#
# Compiles scripts/research/l8_zd_census_fast.c into a scratch dir, runs it,
...
if [[ ! -f "${SRC}" ]]; then
    echo "L8_ZD_CENSUS_GATE_FAIL: missing ${SRC}"
    exit 1
fi

CC_BIN="${CC:-cc}"
if ! command -v "${CC_BIN}" >/dev/null 2>&1; then
    echo "L8_ZD_CENSUS_GATE_FAIL: no C compiler (${CC_BIN})"
    exit 1
fi
...
echo "Running level-8 ZD census fast contract (dim 256)..."
OUT="$("${BIN}")" || {
    echo "${OUT}"
    echo "L8_ZD_CENSUS_GATE_FAIL: contract exited non-zero"
    exit 1
}
echo "${OUT}"

if ! grep -q '^L8_ZD_FAST_VERDICT PASS$' <<<"${OUT}"; then
    echo "L8_ZD_CENSUS_GATE_FAIL: verdict not PASS"
    exit 1
fi
...
if [[ -z "${C_HASH}" || "${C_HASH}" != "${PY_HASH}" ]]; then
    echo "L8_ZD_CENSUS_GATE_FAIL: sign-table hash mismatch"
    exit 1
fi
