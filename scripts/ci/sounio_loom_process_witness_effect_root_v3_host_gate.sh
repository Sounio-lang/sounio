...
fail() {
  printf 'sounio-loom-process-witness-effect-root-v3-host-gate: FAIL reason=%s root_treatment=false material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false\n' "$*" >&2
  exit 1
}

unavailable() {
  printf 'sounio-loom-process-witness-effect-root-v3-host-gate: HOST_GATE_UNAVAILABLE reason=%s root_treatment=false material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false\n' "$*" >&2
  exit 77
}
...
$ROOT/loom/effect-policy-v3.freeze.v1
$ROOT/loom/payload
...
for directory in "$ROOT" "$ROOT/loom" "$ROOT/dev" "$ROOT/proc" "$ROOT/tmp"; do
...
for manifest in "$ROOT/loom/payload.freeze.v1" \
                "$ROOT/loom/effect-policy-v3.freeze.v1"; do
...
[[ "$(sha256sum "$ROOT/loom/effect-policy-v3.freeze.v1" | cut -d ' ' -f 1)" == \
  40407323594e37d44b9002d1cdd390677416048221ace446693919f8415ca480 ]] ||
  fail 'host root policy manifest hash drifted'

nonce="$$-$(date +%s%N)"
UNIT="sounio-loom-effect-root-v3-$nonce.service"
CONTROL="/run/sounio-loom-effect-root-v3-$nonce.control"
OUTPUT="/run/sounio-loom-effect-root-v3-$nonce.output"
ERROR="/run/sounio-loom-effect-root-v3-$nonce.error"
CLIENT_PID=''
CELL_PID=''
ROOT_MOUNTED=false
...
for attempt in $(seq 1 200); do
  if grep -q '^LOOM_PROCESS_WITNESS_EFFECT_POLICY_V3_ROOT_READY PASS ' "$OUTPUT" 2>/dev/null; then
    break
  fi
...
[[ "$ready" == LOOM_PROCESS_WITNESS_EFFECT_POLICY_V3_ROOT_READY\ PASS* ]] ||
...
for expectation in \
  ...
  'policy_manifest_sha256=40407323594e37d44b9002d1cdd390677416048221ace446693919f8415ca480' \
...
printf 'sounio-loom-process-witness-effect-root-v3-host-gate: HOST_MEASUREMENT_PASS semantic_authority=Sounio producer=C++20+Sounio role=MATERIAL_PARITY action=9025 host=%s kernel=%s architecture=%s systemd_version=%s object_boundary=IMMUTABLE_ROOT_MOUNT_NAMESPACE tree_sha256=%s cell_sha256=%s payload_sha256=7249748c322ede756c779904cb2d87f561ba2e17d0691314a09feaf16ca2ed4d policy_manifest_sha256=40407323594e37d44b9002d1cdd390677416048221ace446693919f8415ca480 root_owned=true root_read_only=true root_exact=true dynamic_user=true uid=%s gid=%s mount_namespace=private network_namespace=private private_tmp=false private_devices=false proc_treatment=absent tmp_read_only=true fd_inventory=0+1+2 capabilities=zero no_new_privileges=true seccomp=true process_extinction=observed ready_sha256=%s root_treatment=true material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false recycle_open=false exec_attached=false commit_attached=false ci_attached=false parity_open=false claim_ready=false\n' \
