...
#   E251  Knowledge<Hyper<Sedenion>> * Knowledge<Hyper<Sedenion>>   refused
#         Knowledge<Hyper<Octonion>> * ...  and sedenion `+`         accepted  (Hurwitz boundary)
...
refuses E251 "sedenion Knowledge product (third axis)" tests/compile-fail/e251_knowledge_sedenion_product_shortcut.sio
accepts       "octonion Knowledge product + sedenion sum (Hurwitz control)" tests/run-pass/e251_control_knowledge_octonion_product_check.sio
...
# The pre-existing octonion algebra fixtures must keep checking (fano_selective on Octonion).
accepts       "algebra_decl_basic (fano_selective on Octonion)" tests/run-pass/algebra_decl_basic.sio
accepts       "algebra_commutative_default (no mul line)"       tests/run-pass/algebra_commutative_default.sio
