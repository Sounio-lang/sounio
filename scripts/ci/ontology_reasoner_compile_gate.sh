...
echo "[ontology-reasoner] SKIP: standalone native compiler still rejects nested Seq<struct> field receivers in reasoner.sio"
exit 0
...
if ! "$SOUC_BIN" "$DRV" "$TMP/reasoner_drv" >"$TMP/build.log" 2>&1; then
