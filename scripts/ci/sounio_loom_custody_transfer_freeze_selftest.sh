...
RUNTIME_B="$TEST_ROOT/sounio-custody-transfer-b"
...
fail() {
  printf 'sounio-loom-custody-transfer-freeze-selftest: FAIL: %s\n' "$*" >&2
  exit 1
}
...
[[ "$entrypoint_path" == tools/loom/custody_transfer_main.sio ]] ||
  fail 'unexpected entrypoint path'
[[ "$build_script_path" == scripts/dev/build_sounio_loom_custody_transfer.sh ]] ||
  fail 'unexpected build script'
[[ "$(file_hash "$ROOT_DIR/$source_path")" == "$(field source_sha256)" ]] ||
  fail 'working source drifted'
[[ "$(file_hash "$ROOT_DIR/$entrypoint_path")" == "$(field entrypoint_sha256)" ]] ||
  fail 'working entrypoint drifted'
[[ "$(git -C "$ROOT_DIR" show "$executable_commit:$source_path" | stream_hash)" == "$(field source_sha256)" ]] ||
  fail 'frozen source hash differs'
[[ "$(git -C "$ROOT_DIR" show "$executable_commit:$entrypoint_path" | stream_hash)" == "$(field entrypoint_sha256)" ]] ||
  fail 'frozen entrypoint hash differs'
...
git -C "$ROOT_DIR" archive "$executable_commit" \
  "$source_path" "$entrypoint_path" "$build_script_path" "$wrapper_path" "$compiler_path" |
  tar -x -C "$FROZEN_ROOT"
[[ "$(file_hash "$FROZEN_ROOT/$build_script_path")" == "$(field build_script_sha256)" ]] ||
  fail 'frozen build script drifted'
[[ "$(file_hash "$FROZEN_ROOT/$wrapper_path")" == "$(field toolchain_wrapper_sha256)" ]] ||
  fail 'frozen compiler wrapper drifted'
[[ "$(file_hash "$FROZEN_ROOT/$compiler_path")" == "$(field toolchain_compiler_sha256)" ]] ||
...
[[ "$toolchain_hash" == "$(field toolchain_record_sha256)" ]] ||
  fail 'toolchain record hash differs'
...
[[ "$hardware_hash" == "$(field hardware_record_sha256)" ]] ||
  fail 'hardware record hash differs'

SOUNIO_SOURCE_ROOT="$FROZEN_ROOT" \
  SOUNIO_LOOM_CUSTODY_TRANSFER_SOUC="$FROZEN_ROOT/$wrapper_path" \
  SOUNIO_LOOM_CUSTODY_TRANSFER_OUTPUT="$RUNTIME_A" \
  bash "$FROZEN_ROOT/$build_script_path" >/dev/null
SOUNIO_SOURCE_ROOT="$FROZEN_ROOT" \
  SOUNIO_LOOM_CUSTODY_TRANSFER_SOUC="$FROZEN_ROOT/$wrapper_path" \
  SOUNIO_LOOM_CUSTODY_TRANSFER_OUTPUT="$RUNTIME_B" \
  bash "$FROZEN_ROOT/$build_script_path" >/dev/null
[[ "$(file_hash "$RUNTIME_A")" == "$(field executable_sha256)" ]] ||
  fail 'first executable hash differs'
[[ "$(file_hash "$RUNTIME_B")" == "$(field executable_sha256)" ]] ||
  fail 'second executable hash differs'

command="$(field command)"
[[ "$command" == 'bash scripts/ci/sounio_loom_custody_transfer_selftest.sh' ]] ||
  fail 'unexpected gate command'
[[ "$(printf '%s\n' "$command" | stream_hash)" == "$(field command_sha256)" ]] ||
  fail 'command hash differs'
[[ "$(file_hash "$ROOT_DIR/scripts/ci/sounio_loom_custody_transfer_selftest.sh")" == "$(field command_script_sha256)" ]] ||
  fail 'gate command implementation drifted'
gate_result="$(bash "$ROOT_DIR/scripts/ci/sounio_loom_custody_transfer_selftest.sh")"
[[ "$gate_result" == "$(field result)" ]] || fail 'Sounio gate result differs'
[[ "$(printf '%s\n' "$gate_result" | stream_hash)" == "$(field result_sha256)" ]] ||
...
source_u32="${source_u32:-$(field source_sha256_u32)}"
semantics_u32="${semantics_u32:-$(field semantics_sha256_u32)}"
toolchain_u32="${toolchain_u32:-$(field toolchain_record_sha256_u32)}"
hardware_u32="${hardware_u32:-$(field hardware_record_sha256_u32)}"
command_u32="${command_u32:-$(field command_sha256_u32)}"
result_u32="${result_u32:-$(field result_sha256_u32)}"
...
[[ "$freeze_decision" == "$(field freeze_decision)" ]] ||
  fail 'Sounio freeze decision differs'
...
grep -Fq "manifest_sha256=$manifest_hash" "$EVIDENCE" ||
  fail 'evidence does not bind the freeze manifest'

printf '%s\n' \
  "sounio-loom-custody-transfer-freeze-selftest: PASS producer=Sounio role=SEMANTIC_AUTHORITY stage=SEMANTICS_FROZEN cases=$(field expected_result_cases) semantics_sha256=$(field semantics_sha256) manifest_sha256=$manifest_hash parity_open=false claim_ready=false"
