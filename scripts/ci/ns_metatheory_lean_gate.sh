...
[[ -n "${AXLINES}" ]] || fail "no '#print axioms' output captured (expected ≥ 16 lines)"
NAX="$(printf '%s\n' "${AXLINES}" | wc -l | tr -d ' ')"
[[ "${NAX}" -ge 16 ]] || fail "expected ≥ 16 axiom reports, got ${NAX}"
...
for thm in trueVar_append trueVar_mul naive_add_understates_iff naive_add_conservative_of_cov_nonpos naive_add_exact_iff partition_iff inner_nonpos_of_partition naive_add_conservative_of_partition inner_eq_neg_var_of_full_partition inner_disjoint covers_union covers_scale \
