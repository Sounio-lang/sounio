...
#   - f256, params/returns ABI, print_f128 builtin, GUM
#
set -euo pipefail
...
if grep -Fq 'fn lower_f128_binary_target' self-hosted/ir/lower.sio \
  ...
  && grep -Fq 'f128_bits_soft_mul' self-hosted/check/specializer.sio; then
  note_pass "lower_f128_mul_neg_cmp_markers"
else
  note_fail "lower_f128_mul_neg_cmp_markers_missing"
fi
...
echo "NOTE v0e52_deferred div=see_v0e53 f256=pending params_returns_abi=see_v0e54 print_builtin=pending gum=pending"
