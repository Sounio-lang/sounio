...
if grep -q 'falling back to full IR path' "$LOG_DIR/native_compile.log"; then
  echo "[native-v2-imported-captured-closure-boundary] FAIL: imported native-v2 path reported fallback" >&2
  cat "$LOG_DIR/native_compile.log" >&2 || true
  exit 1
fi
...
if ! grep -q 'module_native_driver: imported source uses modular IR path' "$LOG_DIR/native_compile.log" ||
  ...
  echo "[native-v2-imported-captured-closure-boundary] FAIL: native compile did not use an imported modular native path" >&2
  cat "$LOG_DIR/native_compile.log" >&2 || true
  exit 1
fi
