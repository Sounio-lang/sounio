...
MANIFEST="$ROOT_DIR/tools/loom/causal_workflow_run_grant_fixture.freeze.v1"

fail() {
  printf 'sounio-loom-causal-workflow-run-grant-fixture-freeze-selftest: FAIL: %s\n' "$*" >&2
  exit 1
}
...
expect() {
  [[ "$(value "$1")" == "$2" ]] || fail "manifest field $1 drifted"
}
...
expect_hash() {
  ...
  [[ -f "$ROOT_DIR/$path" && ! -L "$ROOT_DIR/$path" ]] ||
    fail "$key path is absent or linked"
  [[ "$(sha256sum "$ROOT_DIR/$path" | cut -d ' ' -f 1)" == \
     "$(value "${key}_sha256")" ]] || fail "$key hash drifted"
}

[[ -f "$MANIFEST" && ! -L "$MANIFEST" ]] || fail 'freeze manifest absent'
expect schema loom-causal-workflow-run-grant-fixture-freeze-v1
...
for key in source build_script selftest freeze_selftest first_manifest evidence \
  parent_9030_manifest parent_9037_manifest toolchain_wrapper \
...
work="$(mktemp -d "${TMPDIR:-/tmp}/sounio-loom-causal-run-grant-freeze.XXXXXX")"
trap 'rm -rf "$work"' EXIT
for ordinal in one two; do
  SOUNIO_LOOM_CAUSAL_RUN_GRANT_OUTPUT="$work/fixture-$ordinal" \
...
[[ "$(sha256sum "$work/fixture-one" | cut -d ' ' -f 1)" == \
   "$(value executable_sha256)" ]] || fail 'executable hash drifted'
[[ "$(sha256sum "$work/bundle-one" | cut -d ' ' -f 1)" == \
   "$(value bundle_sha256)" ]] || fail 'bundle hash drifted'
[[ "$(wc -l < "$work/bundle-one")" == "$(value bundle_lines)" ]] ||
  fail 'bundle line count drifted'

result="$(bash "$ROOT_DIR/scripts/ci/sounio_loom_causal_workflow_run_grant_fixture_selftest.sh")"
[[ "$result" == "$(value result)" ]] || fail 'selftest result drifted'
[[ "$(printf '%s' "$result" | sha256sum | cut -d ' ' -f 1)" == \
   "$(value result_sha256)" ]] || fail 'selftest result hash drifted'

printf 'sounio-loom-causal-workflow-run-grant-fixture-freeze-selftest: PASS semantic_authority=Sounio launch_action=9030 workflow_action=9037 stage=SEMANTICS_FROZEN source_sha256=%s executable_sha256=%s bundle_sha256=%s command_sha256=%s intent_sha256=%s event_sha256=%s artifact_sha256=%s command_mismatch=DENY492 causal_sabotage=PASS arbitrary_shell=false python_executed=false rust_executed=false material_grant=false material_execution=false host_launch_attached=false production_activation=false parity_open=false claim_ready=false\n' \
  "$(value source_sha256)" "$(value executable_sha256)" \
  "$(value bundle_sha256)" "$(value command_sha256)" \
  "$(value intent_sha256)" "$(value event_sha256)" \
