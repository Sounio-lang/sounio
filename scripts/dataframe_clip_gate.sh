#!/usr/bin/env bash
# Gate for stdlib data::dataframe_clip (bound values to a range) under default Madaros.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if ! grep -qF 'Madaros v' <<<"$engine_info"; then
  echo "FAIL: dataframe-clip gate requires default Madaros" >&2
  printf '%s\n' "$engine_info" >&2
  exit 1
fi
...
echo "== run-proof: clip =="
if $SOUC compile tests/stdlib/data/test_dataframe_clip_stdlib.sio -o "$OUT/x.elf" >/dev/null 2>&1; then
