...
ROOT_BUILDER="$ROOT_DIR/scripts/dev/build_loom_process_witness_effect_root_v3.sh"
HOST_GATE="$ROOT_DIR/scripts/ci/sounio_loom_process_witness_effect_root_v3_host_gate.sh"
NAMESPACE=beagle
NODE=t560-proxmox
SELECTOR='app.kubernetes.io/name=node-ephemeral-governance'

fail() {
  printf 'run-loom-process-witness-effect-root-v3-host-probe: REFUSE reason=%s root_treatment=false material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false\n' "$*" >&2
  exit 70
}
...
[[ -x "$ROOT_BUILDER" && -f "$ROOT_BUILDER" && ! -L "$ROOT_BUILDER" ]] ||
  fail 'V3 root builder is unavailable'
[[ -x "$HOST_GATE" && -f "$HOST_GATE" && ! -L "$HOST_GATE" ]] ||
  fail 'V3 host root gate is unavailable'
for tool in kubectl sha256sum mktemp timeout; do
  command -v "$tool" >/dev/null 2>&1 || fail "required transport tool is absent: $tool"
done

WORK="$(mktemp -d "${TMPDIR:-/tmp}/loom-effect-root-v3-host.XXXXXX")"
CAPSULE="$WORK/capsule"
build_receipt="$(SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V3_OUTPUT="$CAPSULE" \
  "$ROOT_BUILDER")"
[[ "$build_receipt" == BUILT_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V3\ * ]] ||
  fail 'V3 root builder receipt diverged'
...
[[ "$CELL_SHA256" =~ ^[0-9a-f]{64}$ && "$TREE_SHA256" =~ ^[0-9a-f]{64}$ ]] ||
  fail 'V3 root builder hashes are malformed'
...
nonce="$$-$(date +%s%N)"
REMOTE_ROOT="/var/tmp/loom-effect-root-v3-${TREE_SHA256:0:16}-$nonce"
REMOTE_GATE="/var/tmp/loom-effect-root-v3-gate-${CELL_SHA256:0:16}-$nonce"
cleanup() {
  if [[ -n "${POD:-}" ]]; then
...
kubectl -n "$NAMESPACE" exec "$POD" -- sh -c \
  "umask 077; install -d -m 0755 '/proc/1/root$REMOTE_ROOT' '/proc/1/root$REMOTE_ROOT/loom' '/proc/1/root$REMOTE_ROOT/dev' '/proc/1/root$REMOTE_ROOT/proc' '/proc/1/root$REMOTE_ROOT/tmp'"
transfer_file() {
...
transfer_file "$CAPSULE/loom/effect-policy-v3.freeze.v1" \
  "$REMOTE_ROOT/loom/effect-policy-v3.freeze.v1" 0444
transfer_file "$HOST_GATE" "$REMOTE_GATE" 0500
kubectl -n "$NAMESPACE" exec "$POD" -- sh -c \
  "mknod '/proc/1/root$REMOTE_ROOT/dev/null' c 1 3; chown 0:0 '/proc/1/root$REMOTE_ROOT/dev/null'; chmod 0666 '/proc/1/root$REMOTE_ROOT/dev/null'; chown -R 0:0 '/proc/1/root$REMOTE_ROOT'; chmod 0555 '/proc/1/root$REMOTE_ROOT' '/proc/1/root$REMOTE_ROOT/loom' '/proc/1/root$REMOTE_ROOT/dev' '/proc/1/root$REMOTE_ROOT/proc' '/proc/1/root$REMOTE_ROOT/tmp'"

remote_hashes="$(kubectl -n "$NAMESPACE" exec "$POD" -- sh -c \
  "sha256sum '/proc/1/root$REMOTE_ROOT/loom/effect-cell' '/proc/1/root$REMOTE_ROOT/loom/payload' '/proc/1/root$REMOTE_ROOT/loom/payload.freeze.v1' '/proc/1/root$REMOTE_ROOT/loom/effect-policy-v3.freeze.v1' | cut -d ' ' -f 1")"
...
[[ "${remote_hash_lines[0]:-}" == "$CELL_SHA256" &&
   ...
   "${remote_hash_lines[3]:-}" == 40407323594e37d44b9002d1cdd390677416048221ace446693919f8415ca480 ]] ||
...
[[ "$host_output" == sounio-loom-process-witness-effect-root-v3-host-gate:\ HOST_MEASUREMENT_PASS* ]] ||
...
[[ "$host_output" == *'root_treatment=true material_sabotages=0 material_coverage=false complete_effects=false material_execution=false'* ]] ||
  fail 'host immutable-root probe promoted beyond evidence'

printf '%s\n' "$host_output"
printf 'LOOM_PROCESS_WITNESS_EFFECT_ROOT_V3_HOST_TRANSPORT PASS namespace=%s node=%s pod=%s transport=kubectl+hostPID+nsenter tree_sha256=%s cell_sha256=%s host_output_sha256=%s root_treatment=true material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false parity_open=false claim_ready=false\n' \
