#!/usr/bin/env bash
# scripts/dev/madaros_wave12_tip_green_gate.sh
#
# Wave12 tip-green regression lock for origin/main Madaros science/compiler.
# Supersedes Wave11 tip-green by keeping the eight Wave11 locks and promoting
# the Wave11 multi-mod imported-f64 BSS closeouts (#1380 Defect A + #1382
# Defect A′) that were source-fixed but not yet tip-green-locked:
#
#   Wave11 retained:
...
#                          (Defect A GLOBAL_VAR_INIT suppress + Defect A′ multi-mod
#                           BSS offset remap; leaf+pad, lognormal science, A≠B bits)
#
# Exit 0 only when every sub-gate exits 0 and emits its expected sentinel.
# Writes a machine-readable tip receipt:
#   artifacts/compiler/madaros_wave12_tip_green_receipt.v1.json
#
...
#   SOUC=./bin/souc bash scripts/dev/madaros_wave12_tip_green_gate.sh
set -euo pipefail
...
RECEIPT="$RECEIPT_DIR/madaros_wave12_tip_green_receipt.v1.json"
LOG_DIR="$(mktemp -d /tmp/madaros_wave12_tip_green.XXXXXX)"
...
echo "== madaros_wave12_tip_green_gate =="
...
run_one() {
  ...
  # Extra multi-mod BSS invariants: refuse last-init-wins collision (both A and B
  # reading 2.5 bits) even if the outer sentinel were ever weakened.
...
cat >"$RECEIPT" <<EOF
{
  "schema": "madaros_wave12_tip_green_receipt.v1",
  "status": "$OVERALL",
  "wave": "wave12",
  ...
  "measurement": {
    "wave11_tip_green": "pass_or_see_gates",
    "imported_f64_bss": "pass_or_see_gates",
...
if [[ $fail -eq 0 ]]; then
  echo "MADAROS_WAVE12_TIP_GREEN_GATE_OK"
  exit 0
fi
echo "MADAROS_WAVE12_TIP_GREEN_GATE_FAIL" >&2
exit 1
