...
OUTPUT_ROOT="${SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V8_OUTPUT:-$ROOT_DIR/tools/loom/_build/effect-root-v8}"
CELL_BUILDER="$ROOT_DIR/scripts/dev/build_loom_process_witness_effect_policy_v8.sh"
...
fail() {
  printf 'build-loom-process-witness-effect-root-v8: FAIL reason=%s\n' "$*" >&2
  exit 1
}
...
[[ "$(sha256sum "$POLICY_MANIFEST" | cut -d ' ' -f 1)" == \
  ...
  fail 'frozen V8 policy manifest drifted'
...
SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_POLICY_V8_OUTPUT="$OUTPUT_ROOT/loom/effect-cell" \
...
install -m 0444 "$POLICY_MANIFEST" "$OUTPUT_ROOT/loom/effect-policy-v8.freeze.v1"
...
loom/effect-policy-v8.freeze.v1:0444:f97bd4c3c8cd93978da27b361bc7fec3d8316775fb58a9a4bf94ddf53513293a
