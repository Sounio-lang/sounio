...
BUILDER="$ROOT_DIR/scripts/dev/build_loom_kernel_peer_matrix_v12.sh"
SELFTEST="$ROOT_DIR/scripts/ci/sounio_loom_kernel_peer_matrix_v12_selftest.sh"
HOST_GATE="$ROOT_DIR/scripts/ci/sounio_loom_kernel_peer_matrix_v12_host_gate.sh"
...
fail() {
  printf 'run-loom-kernel-peer-matrix-v12-host-probe: REFUSE reason=%s decisive_pairs=0 controls_executed=false material_peer_matrix=false same_uid_peer_isolation=false action_9025=DENY451 claim_ready=false\n' "$*" >&2
  exit 70
}

for path in "$BUILDER" "$SELFTEST" "$HOST_GATE" "$LOADER_SOURCE"; do
  [[ -f "$path" && ! -L "$path" ]] || fail "required matrix component is absent or linked: $path"
done
...
SOUNIO_LOOM_KERNEL_PEER_MATRIX_V12_OUTPUT="$BASE_INITRAMFS" \
...
[[ ${#candidate_pods[@]} -eq 1 ]] || fail "expected one host transport pod; found ${#candidate_pods[@]}"
...
nonce="$$-$(date +%s%N)"
REMOTE_BASE="/var/tmp/loom-kernel-peer-matrix-v12-base-$nonce.cpio.gz"
REMOTE_PACKER="/var/tmp/loom-kernel-peer-matrix-v12-packer-$nonce"
REMOTE_LOADER_SOURCE="/var/tmp/loom-kernel-peer-matrix-v12-loader-$nonce.cpp"
REMOTE_GATE="/var/tmp/loom-kernel-peer-matrix-v12-gate-$nonce"
cleanup_remote() {
...
host_status=$?
set -e
[[ $host_status -eq 0 ]] || fail "host matrix gate failed status=$host_status output=$host_output"
summary="$(printf '%s\n' "$host_output" | grep '^sounio-loom-kernel-peer-matrix-v12-host-gate: HOST_MEASUREMENT_PASS ' || true)"
[[ -n "$summary" && "$(printf '%s\n' "$summary" | wc -l)" == 1 ]] || fail 'host summary absent or duplicated'

printf '%s\n' "$host_output"
printf 'LOOM_KERNEL_PEER_MATRIX_V12_HOST_TRANSPORT PASS namespace=%s node=%s pod=%s transport=kubectl+hostPID+nsenter base_initramfs_sha256=%s packer_sha256=%s bpf_object_sha256=%s loader_source_sha256=%s host_gate_sha256=%s kernel_sha256=%s host_output_sha256=%s semantic_authority=Sounio action=9025 operations=10 decisive_pairs=10 treatment_refused=10 mediator_removed_completed=10 same_kuid_pair_observed=true all_four_kernel_uid_slots_equal=true attacker_syscalls_open=true receiver_mediator_active=true only_delta_mediator=true competing_ptrace_lsms=absent guest_root_traversable=true principal_capability=CAP_SYS_NICE_ONLY all_epoch_objects_extinct=true controls_executed=false material_peer_matrix=false same_uid_peer_isolation=false action_9025_decision=DENY451 claim_ready=false\n' \
