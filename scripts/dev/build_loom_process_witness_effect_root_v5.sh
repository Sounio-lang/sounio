...
OUTPUT_ROOT="${SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V5_OUTPUT:-$ROOT_DIR/tools/loom/_build/effect-root-v5}"
CELL_BUILDER="$ROOT_DIR/scripts/dev/build_loom_process_witness_effect_policy_v5.sh"
...
fail() {
  printf 'build-loom-process-witness-effect-root-v5: FAIL reason=%s\n' "$*" >&2
  exit 1
}
...
[[ "$(sha256sum "$POLICY_MANIFEST" | cut -d ' ' -f 1)" == \
  ...
  fail 'frozen V5 policy manifest drifted'
...
install -d -m 0755 "$OUTPUT_ROOT" "$OUTPUT_ROOT/loom" "$OUTPUT_ROOT/dev" \
  ...
  "$OUTPUT_ROOT/sys"
SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_POLICY_V5_OUTPUT="$OUTPUT_ROOT/loom/effect-cell" \
...
install -m 0444 "$POLICY_MANIFEST" "$OUTPUT_ROOT/loom/effect-policy-v5.freeze.v1"
...
loom/effect-policy-v5.freeze.v1:0444:f17fc7d776db557d2655e00036f4014b4a7a38d8ed16e74786471415c49908f7
...
printf 'BUILT_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V5 path=%s producer=C++20+Sounio role=MATERIAL_PARITY semantic_authority=Sounio action=9025 capsule=true cell_sha256=%s payload_sha256=%s policy_manifest_sha256=f17fc7d776db557d2655e00036f4014b4a7a38d8ed16e74786471415c49908f7 payload_manifest_sha256=624ccd7297778803eff8d9972a33d5e55fb022f9e7e37f444f0aee13c22fb4da tree_sha256=%s static_cell=true static_payload=true systemd_mount=/run/systemd/incoming systemd_sys_mount=/sys dev_null=host_materialization_required host_root_ownership=false root_read_only=false material_coverage=false complete_effects=false material_execution=false launch_open=false\n' \
