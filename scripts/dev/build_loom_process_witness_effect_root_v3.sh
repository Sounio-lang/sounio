...
OUTPUT_ROOT="${SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V3_OUTPUT:-$ROOT_DIR/tools/loom/_build/effect-root-v3}"
CELL_BUILDER="$ROOT_DIR/scripts/dev/build_loom_process_witness_effect_policy_v3.sh"
...
fail() {
  printf 'build-loom-process-witness-effect-root-v3: FAIL reason=%s\n' "$*" >&2
  exit 1
}
...
[[ "$(sha256sum "$POLICY_MANIFEST" | cut -d ' ' -f 1)" == \
  ...
  fail 'frozen V3 policy manifest drifted'
...
install -d -m 0755 "$OUTPUT_ROOT" "$OUTPUT_ROOT/loom" "$OUTPUT_ROOT/dev" \
  "$OUTPUT_ROOT/proc" "$OUTPUT_ROOT/tmp"
SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_POLICY_V3_OUTPUT="$OUTPUT_ROOT/loom/effect-cell" \
...
install -m 0444 "$POLICY_MANIFEST" "$OUTPUT_ROOT/loom/effect-policy-v3.freeze.v1"
...
loom/effect-policy-v3.freeze.v1:0444:40407323594e37d44b9002d1cdd390677416048221ace446693919f8415ca480
dev/null:host-character-device:1:3
proc:empty:0555
tmp:empty-read-only:0555"
TREE_SHA256="$(printf '%s\n' "$TREE_RECORD" | sha256sum | cut -d ' ' -f 1)"

printf 'BUILT_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V3 path=%s producer=C++20+Sounio role=MATERIAL_PARITY semantic_authority=Sounio action=9025 capsule=true cell_sha256=%s payload_sha256=%s policy_manifest_sha256=40407323594e37d44b9002d1cdd390677416048221ace446693919f8415ca480 payload_manifest_sha256=624ccd7297778803eff8d9972a33d5e55fb022f9e7e37f444f0aee13c22fb4da tree_sha256=%s static_cell=true static_payload=true dev_null=host_materialization_required host_root_ownership=false root_read_only=false material_coverage=false complete_effects=false material_execution=false launch_open=false\n' \
