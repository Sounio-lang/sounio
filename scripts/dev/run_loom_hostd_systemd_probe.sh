...
[[ "$stage_output" == *'exec_cell_bundle_present=true '* &&
   ...
   "$stage_output" == *'exec_intent_projection_configured=true '* &&
...
[[ $host_status -eq 0 && "$host_output" == \
  'sounio-loom-hostd-systemd-host-selftest: HOST_MEASUREMENT_PASS '* &&
   ...
   "$host_output" == *' event_projection=Sounio-9034 event_override=false intent_command_mismatch=DENY555 '* &&
