...
BASE_REF="${E3A_BASE_REF:-origin/canon/madaros-v2-sota}"
...
E3A_PROTECTED=(self-hosted/compiler/main.sio self-hosted/ir self-hosted/native self-hosted/wasm self-hosted/gpu stdlib/runtime stdlib/eisa stdlib/math/dd64.sio stdlib/math/qd128.sio self-hosted/enir/ir.sio self-hosted/enir/interpreter.sio self-hosted/enir/qd.sio tools/eisa/eisa_evm_run.sio)
if [[ "${E3A_ALLOW_DOWNSTREAM_ENIR_EXTENSION:-0}" != "1" ]]; then E3A_PROTECTED+=(self-hosted/enir/source_lower.sio); fi
git diff --quiet "$BASE_REF" -- "${E3A_PROTECTED[@]}" || fail "E3A changed protected production, ENIR, qd, or oracle surfaces"
...
E2H_BASE_REF="$BASE_REF" bash scripts/dev/madaros_v2_e2h_enir_memory_move_poison_gate.sh >"$TMP_DIR/e2h-regression.log"
