...
python3 "$EXTERN_DECL_FRONT_HALF_RECEIPT_TOOL" emit \
  --compiler "$COMPILER" \
  --root "$ROOT_DIR" \
  --out-dir "$EXTERN_DECL_FRONT_HALF_RECEIPT_DIR"

if [[ ! -f "$EXTERN_DECL_FRONT_HALF_RECEIPT" ]]; then
  echo "[madaros-v2-s5-program-mir-abi] FAIL: missing extern declaration front-half receipt: $EXTERN_DECL_FRONT_HALF_RECEIPT" >&2
  exit 1
fi
...
for field in [
    ...
    "f128_native_bounded_truncated_two_limb_decimal_binary128_materialization_promoted",
...
required_value_native_cases = {
    ...
    "overflow_to_positive_infinity_plus_one": {"literal": "1e5001", "hex": "7fff0000000000000000000000000000", "metadata": [1, 0, 1, 1, -5001, 0]},
    "overflow_to_negative_infinity_plus_one": {"literal": "-1e5001", "hex": "ffff0000000000000000000000000000", "metadata": [-1, 0, 1, 1, -5001, 0]},
    "two_limb_overflow_to_positive_infinity": {"literal": "123456789012345678901234567890123456e5001", "hex": "7fff0000000000000000000000000000", "metadata": [1, 901234567890123456, 123456789012345678, 36, -5001, 0]},
    ...
    "negative_truncated_pi_tail20_algorithmic": {"literal": "-3.14159265358979323846264338327950288420", "hex": "c000921fb54442d18469898cc51701b8", "metadata": [-1, 846264338327950288, 314159265358979323, 39, 38, 3]},
    "truncated_euler_39_digits_algorithmic": {"literal": "2.71828182845904523536028747135266249775", "hex": "40005bf0a8b1457695355fb8ac404e7a", "metadata": [1, 536028747135266249, 271828182845904523, 39, 38, 3]},
    ...
    "truncated_one_seventh_39_repeating_below_one_algorithmic": {"literal": "0.142857142857142857142857142857142857142", "hex": "3ffc2492492492492492492492492492", "metadata": [1, 714285714285714285, 14285714285714285, 40, 39, 4]},
}
...
if extern_decl_front_half_receipt.get("schema") != "madaros.v2.s5.extern_declaration_front_half_receipt/0.1":
    raise SystemExit("bad S5 extern declaration front-half receipt schema")
if extern_decl_front_half_receipt.get("status") != "pass":
    raise SystemExit("program MIR/ABI gate requires passing extern declaration front-half receipt")
if extern_decl_front_half_receipt.get("stage_contract_level") != "S5_22_EXTERN_DECLARATION_IS_NOT_KERNEL_FRONT_HALF":
    raise SystemExit("extern declaration receipt must declare S5.22 front-half stage contract")
if extern_decl_front_half_receipt.get("case_count") != 4:
    raise SystemExit("extern declaration receipt must contain exact four cases")
if extern_decl_front_half_receipt.get("positive_case_count") != 3:
    raise SystemExit("extern declaration receipt must contain exact three positive extern cases")
if extern_decl_front_half_receipt.get("negative_case_count") != 1:
    raise SystemExit("extern declaration receipt must contain one negative kernel case")
for field in [
    "extern_decl_is_kernel_split_promoted",
    "extern_decl_no_kernel_e072_for_non_unit_returns",
    "extern_decl_no_empty_body_e008_for_non_unit_returns",
    "extern_decl_i64_typecheck_promoted",
    "extern_decl_f64_typecheck_promoted",
    "extern_decl_f128_typecheck_promoted",
    "real_kernel_e072_preserved",
    "ir_extern_strategy_no_longer_depends_on_is_kernel",
    "parser_has_explicit_is_extern_bit",
]:
    if extern_decl_front_half_receipt.get(field) is not True:
        raise SystemExit(f"extern declaration front-half receipt missing required true flag: {field}")
for field in [
    "f128_external_sysv_abi_promoted",
    ...
    if extern_decl_front_half_receipt.get(field) is not False:
        raise SystemExit(f"extern declaration front-half receipt must not overclaim {field}")
extern_cases = {row.get("case_id"): row for row in extern_decl_front_half_receipt.get("cases", [])}
required_extern_cases = {
    "extern_c_i64_return_decl_check",
    "extern_c_f64_arg_return_decl_check",
    "extern_c_f128_arg_return_decl_check",
    "kernel_nonunit_return_still_rejected",
}
if set(extern_cases) != required_extern_cases:
    raise SystemExit(f"extern declaration receipt cases mismatch: {sorted(extern_cases)}")
for case_id in [
    "extern_c_i64_return_decl_check",
    "extern_c_f64_arg_return_decl_check",
    "extern_c_f128_arg_return_decl_check",
]:
    row = extern_cases[case_id]
    if row.get("check_rc") != 0:
        raise SystemExit(f"{case_id} must check successfully")
    if row.get("kernel_e072_absent") is not True:
...
for field in [
    ...
    "native_v2_external_link_launcher_promoted",
    "f128_external_sysv_runtime_promoted",
    "f128_external_sysv_scalar_passthru_oracle_promoted",
    "f128_external_sysv_scalar_passthru_wrapper_link_promoted",
    "f128_external_sysv_argument_oracle_promoted",
    "f128_external_sysv_return_oracle_promoted",
    "f128_external_sysv_argument_wrapper_link_promoted",
    "f128_external_sysv_return_wrapper_link_promoted",
...
passthru_wrapper_link = external_sysv_cases["extern_c_passthru_f128_native_v2_wrapper_link_oracle"]
if passthru_wrapper_link.get("wrapper_mode") != "native-v2-link":
    raise SystemExit("external SysV f128 wrapper-link oracle must use madaros native-v2-link")
if passthru_wrapper_link.get("native_v2_link_rc") != 0:
    raise SystemExit("external SysV f128 wrapper-link oracle must link successfully")
if passthru_wrapper_link.get("linked_executable_exit_code") != 0:
    raise SystemExit("external SysV f128 wrapper-link oracle linked executable must exit zero")
for field in [
    "f128_external_sysv_scalar_passthru_wrapper_link_promoted",
    "f128_external_sysv_argument_wrapper_link_promoted",
    "f128_external_sysv_return_wrapper_link_promoted",
...
required_differential_flags = [
    ...
    "f128_binary128_native_materialization_differential_complete",
    "f128_ieee_class_helper_differential_recorded",
    "f128_ieee_predicate_helper_differential_recorded",
]
...
for category, expected_counts in required_differential_category_counts.items():
    rows = [row for row in differential_cases.values() if row.get("category") == category]
    actual_counts = {
        "total": len(rows),
        "matched": sum(1 for row in rows if row.get("status") == "matched"),
        "reference_unavailable": sum(1 for row in rows if row.get("status") == "reference_unavailable"),
    }
    if actual_counts != expected_counts:
        raise SystemExit(f"differential receipt {category} counts mismatch: {actual_counts} != {expected_counts}")
required_unavailable = {
    ...
    "f128_class_code_normal_one",
    "f128_class_code_normal_one_tenth",
    "f128_class_code_normal_negative_one_tenth",
    "f128_class_code_normal_smallest_binary128",
    "f128_class_code_subnormal_min_positive",
    "f128_class_code_subnormal_min_negative",
    "f128_class_code_infinity_positive_overflow",
    "f128_class_code_infinity_negative_overflow",
    "f128_class_code_nan_canonical_quiet_builtin",
    "f128_predicate_zero_positive_f128_is_zero",
    "f128_predicate_zero_positive_f128_is_finite",
    "f128_predicate_zero_negative_f128_is_zero",
    "f128_predicate_zero_negative_f128_is_finite",
    "f128_predicate_normal_one_f128_is_normal",
    "f128_predicate_normal_one_f128_is_finite",
    "f128_predicate_normal_one_tenth_f128_is_normal",
    "f128_predicate_normal_one_tenth_f128_is_finite",
    "f128_predicate_normal_negative_one_tenth_f128_is_normal",
    "f128_predicate_normal_negative_one_tenth_f128_is_finite",
    "f128_predicate_normal_smallest_binary128_f128_is_normal",
    "f128_predicate_normal_smallest_binary128_f128_is_finite",
    "f128_predicate_subnormal_min_positive_f128_is_subnormal",
    "f128_predicate_subnormal_min_positive_f128_is_finite",
    "f128_predicate_subnormal_min_negative_f128_is_subnormal",
    "f128_predicate_subnormal_min_negative_f128_is_finite",
    "f128_predicate_underflow_positive_zero_f128_is_zero",
    "f128_predicate_underflow_positive_zero_f128_is_finite",
    "f128_predicate_infinity_positive_overflow_f128_is_infinite",
    "f128_predicate_infinity_negative_overflow_f128_is_infinite",
    "f128_predicate_nan_canonical_quiet_builtin_f128_is_nan",
}
...
required_new_f128_differential_matched_cases = {
    case_id
...
not_promoted = [
    {
        ...
        "status": "not_promoted_beyond_bounded_siglo_scale18_two_limb_scale0_to_18_bounded_truncated_retained_scale35_range_1_over_8_to_10_and_extreme_saturation_cases",
        "reason": "native-v2 now emits exact dyadic, bounded sig_hi=0/no-truncation/scale10<=18 rounded decimals, algorithmic two-limb no-truncation scale0..18 decimals, bounded truncated two-limb retained-scale-35 decimals in [1/8, 10) using parser tail-info as rounding sticky, explicit subnormal anchors, and extreme decimal overflow-to-infinity/power10-underflow-to-zero saturation cases; arbitrary rounded decimal-to-binary128 materialization remains fail-closed",
    },
    {
...
module = {
    ...
    "extern_declaration_front_half_receipt": {
        "schema": extern_decl_front_half_receipt["schema"],
        "path": f"{extern_decl_front_half_receipt_path.parent.name}/{extern_decl_front_half_receipt_path.name}",
        "receipt_sha256": extern_decl_front_half_receipt["receipt_sha256"],
        "stage_contract_level": extern_decl_front_half_receipt["stage_contract_level"],
        "case_id": extern_decl_front_half_receipt["case_id"],
        "case_count": extern_decl_front_half_receipt["case_count"],
        "positive_case_count": extern_decl_front_half_receipt["positive_case_count"],
        "negative_case_count": extern_decl_front_half_receipt["negative_case_count"],
        "extern_decl_is_kernel_split_promoted": extern_decl_front_half_receipt["extern_decl_is_kernel_split_promoted"],
        "extern_decl_f128_typecheck_promoted": extern_decl_front_half_receipt["extern_decl_f128_typecheck_promoted"],
        "real_kernel_e072_preserved": extern_decl_front_half_receipt["real_kernel_e072_preserved"],
    ...
    "external_sysv_f128_blocker_receipt": {
        ...
        "native_v2_external_link_launcher_promoted": external_sysv_f128_blocker_receipt["native_v2_external_link_launcher_promoted"],
        ...
        "f128_external_sysv_scalar_passthru_wrapper_link_promoted": external_sysv_f128_blocker_receipt["f128_external_sysv_scalar_passthru_wrapper_link_promoted"],
    ...
    "roundtrip_contract": [
        ...
        "f128_binary128_value_contract_native_materialization_promoted_for_current_case_set_including_truncated_high_precision_decimals_and_extreme_decimal_saturation",
...
receipt = {
    ...
    "s5_extern_declaration_front_half_complete": True,
    ...
    "extern_declaration_front_half_receipt_sha256": extern_decl_front_half_receipt["receipt_sha256"],
    ...
    "f128_native_extreme_decimal_saturation_promoted": module["scalar_abi_receipts"]["f128_native_extreme_decimal_saturation_promoted"],
