...
CXX="${SOUNIO_LOOM_KERNEL_PEER_BPF_LOAD_V12_CXX:-c++}"
CLANG="${SOUNIO_LOOM_KERNEL_PEER_BPF_LOAD_V12_CLANG:-clang}"
...
MICROHOST_MANIFEST="$ROOT_DIR/tools/loom/kernel_peer_microhost_v12.freeze.v1"
OUTPUT="${SOUNIO_LOOM_KERNEL_PEER_BPF_LOAD_V12_OUTPUT:-$ROOT_DIR/tools/loom/_build/default/src/loom-kernel-peer-bpf-load-v12.base.cpio.gz}"
PACKER_OUTPUT="${SOUNIO_LOOM_KERNEL_PEER_BPF_LOAD_V12_PACKER_OUTPUT:-}"
BPF_OUTPUT="${SOUNIO_LOOM_KERNEL_PEER_BPF_LOAD_V12_BPF_OUTPUT:-}"

fail() {
  printf 'build-loom-kernel-peer-bpf-load-v12: FAIL: %s\n' "$*" >&2
  exit 1
}
...
[[ "$(sha256sum "$MICROHOST_MANIFEST" | cut -d ' ' -f 1)" == 0c0f49287a838b9fd1836d4771036ca4ba5bba62a69bd9993c5056ef65186328 ]] ||
  fail 'V12 BPF LSM microhost freeze drifted'
grep -Fxq 'next_stage=BPF_LSM_PROGRAM_LOAD' "$MICROHOST_MANIFEST" ||
  fail 'V12 microhost freeze does not authorize BPF program load'
...
mkdir -p "$TREE/dev" "$TREE/proc" "$TREE/sys/kernel/security" "$TREE/sys/fs/bpf" \
  "$TREE/tmp" "$TREE/loom" "$TREE/lib/x86_64-linux-gnu" "$TREE/lib64"
SOURCE_DATE_EPOCH=0 "$CXX" -std=c++20 -O2 -Wall -Wextra -Werror -pedantic -static \
  -fno-record-gcc-switches -frandom-seed=loom-kernel-peer-bpf-load-init-v12 \
  -ffile-prefix-map="$ROOT_DIR"=. -Wl,--build-id=none "$INIT_SOURCE" -o "$INIT"
...
printf 'BUILT_LOOM_KERNEL_PEER_BPF_LOAD_V12 base_initramfs=%s language=C+BPF+C++20 role=MATERIAL_BOOTSTRAP transitory=true semantic_authority=Sounio action=9025 init_sha256=%s loader_object_sha256=%s bpf_object_sha256=%s packer_sha256=%s base_initramfs_sha256=%s programs=3 guest_disk=none guest_network=none material_peer_matrix=false same_uid_peer_isolation=false action_9025_decision=DENY451 claim_ready=false\n' \
