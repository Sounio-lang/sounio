...
#   file  rc  first_error_code  error_count  parse_errors  first_error_line
#
...
sweep_one() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$f" "$rc" "${code:-none}" "${count:-0}" "${parse:-0}" "${line:-}"
}
export -f sweep_one
...

{
  printf '# engine\t%s\n' "$ENGINE_ID"
  printf '# trees\t%s\n' "${TREES[*]}"
  printf 'file\trc\tfirst_error_code\terror_count\tparse_errors\tfirst_error_line\n'
  sort "$BODY"
} > "$OUT"
rm -f "$BODY"
...
awk -F'\t' '/^#/ {next} NR>1 {n[$2]++} END {for (k in n) printf "  rc=%s\t%d\n", k, n[k]}' "$OUT" | sort

echo
echo "[sweep] rejection classes (first error per file):"
awk -F'\t' '/^#/ {next} NR>1 && $2!=0 {n[$3]++} END {for (k in n) printf "  %s\t%d\n", k, n[k]}' "$OUT" | sort -k2 -rn
