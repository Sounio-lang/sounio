...
BASE_REF="${E2F_BASE_REF:-origin/canon/madaros-v2-sota}"
...
git diff --quiet "$BASE_REF" -- \
  ...
  || fail "E2F changed compiler codegen/ABI/runtime, pinned qd semantics, or frozen METRON oracle"
...
E2E_BASE_REF="$BASE_REF" bash scripts/dev/madaros_v2_e2e_enir_qd128_gate.sh >"$TMP_DIR/e2e-regression.log"
