...
SOUC="${SOUNIO_LOOM_RESIDENT_MEMBRANE_V4_SOUC:-$ROOT_DIR/bin/souc}"
ENGINE="${SOUNIO_LOOM_RESIDENT_MEMBRANE_V4_ENGINE:-lean_single}"
...
[[ "$(manifest_value "$RESIDENT_V3_MANIFEST" parent_9029_sha256)" == "$(file_hash "$INVOCATION_MANIFEST")" ]] ||
  fail 'resident v3 does not bind the current action 9029 manifest'

work="$(mktemp -d "${TMPDIR:-/tmp}/sounio-loom-resident-membrane-v4-build.XXXXXX")"
trap 'rm -rf "$work"' EXIT
membrane_adapter="$work/subprocess_membrane_resident_v4_adapter.sio"
resident_adapter="$work/resident_authority_resident_v4_adapter.sio"
closure_adapter="$work/effect_closure_resident_v4_adapter.sio"
...
adapt_entrypoint "$CLOSURE_ENTRYPOINT" effect_closure_v4_decide_one "$closure_adapter"
adapt_entrypoint "$INVOCATION_ENTRYPOINT" kernel_invocation_cell_v4_decide_one "$invocation_adapter"
adapt_entrypoint "$GRANT_ENTRYPOINT" kernel_exec_grant_cell_v4_decide_one "$grant_adapter"

# Source assembly and adapter derivation are mechanical. Expected decisions
# remain in the five frozen Sounio functions and are checked byte-for-byte.
sed -n '1,$p' "$MEMBRANE_MODULE" "$membrane_adapter" "$RESIDENT_MODULE" \
  ...
  "$grant_adapter" "$DISPATCH_MAIN" > "$combined"
SOUNIO_SOUC_ENGINE="$ENGINE" "$SOUC" compile "$combined" -o "$compiled"
[[ -f "$compiled" ]] || fail 'compiler omitted the resident Sounio v4 executable'
...
evidence='1 1 11 11'
probe_frame="9030 3 1 $transition $parents $identity $peer $shape $consumption $revocation $extinction $outcome $authority $evidence $bindings"
probe="$(printf '%s\n' '5' "$probe_frame" '0' | "$OUTPUT")"
[[ "$probe" == 'SOUNIO_KERNEL_EXEC_GRANT_CELL_ALLOW code=0 reason=allow stage=SEMANTICS_FROZEN' ]] ||
  fail "resident Sounio v4 probe failed: $probe"

printf 'BUILT_RESIDENT_MEMBRANE_V4 path=%s language=Sounio engine=%s actions=9023,9024,9025,9029,9030\n' \
  "$OUTPUT" "$ENGINE"
