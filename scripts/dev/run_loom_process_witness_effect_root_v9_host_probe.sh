...
ROOT_BUILDER="$ROOT_DIR/scripts/dev/build_loom_process_witness_effect_root_v9.sh"
HOST_GATE="$ROOT_DIR/scripts/ci/sounio_loom_process_witness_effect_root_v9_host_gate.sh"
NAMESPACE=beagle
NODE=t560-proxmox
SELECTOR='app.kubernetes.io/name=node-ephemeral-governance'

fail() {
  printf 'run-loom-process-witness-effect-root-v9-host-probe: REFUSE reason=%s root_treatment=false bootstrap_sabotage=false material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false\n' "$*" >&2
  exit 70
}
...
[[ -x "$ROOT_BUILDER" && -f "$ROOT_BUILDER" && ! -L "$ROOT_BUILDER" ]] ||
  fail 'V9 root builder is unavailable'
[[ -x "$HOST_GATE" && -f "$HOST_GATE" && ! -L "$HOST_GATE" ]] ||
  fail 'V9 host root gate is unavailable'
for tool in kubectl sha256sum mktemp timeout; do
  command -v "$tool" >/dev/null 2>&1 || fail "required transport tool is absent: $tool"
done

WORK="$(mktemp -d "${TMPDIR:-/tmp}/loom-effect-root-v9-host.XXXXXX")"
CAPSULE="$WORK/capsule"
build_receipt="$(SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V9_OUTPUT="$CAPSULE" \
  "$ROOT_BUILDER")"
[[ "$build_receipt" == BUILT_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V9\ * ]] ||
  fail 'V9 root builder receipt diverged'
...
[[ "$CELL_SHA256" =~ ^[0-9a-f]{64}$ && "$TREE_SHA256" =~ ^[0-9a-f]{64}$ ]] ||
  fail 'V9 root builder hashes are malformed'
...
nonce="$$-$(date +%s%N)"
REMOTE_ROOT="/var/tmp/loom-effect-root-v9-${TREE_SHA256:0:16}-$nonce"
REMOTE_GATE="/var/tmp/loom-effect-root-v9-gate-${CELL_SHA256:0:16}-$nonce"
cleanup() {
  if [[ -n "${POD:-}" ]]; then
...
transfer_file "$CAPSULE/loom/effect-policy-v9.freeze.v1" \
  "$REMOTE_ROOT/loom/effect-policy-v9.freeze.v1" 0444
...
remote_hashes="$(kubectl -n "$NAMESPACE" exec "$POD" -- sh -c \
  "sha256sum '/proc/1/root$REMOTE_ROOT/loom/effect-cell' '/proc/1/root$REMOTE_ROOT/loom/payload' '/proc/1/root$REMOTE_ROOT/loom/payload.freeze.v1' '/proc/1/root$REMOTE_ROOT/loom/effect-policy-v9.freeze.v1' | cut -d ' ' -f 1")"
...
[[ "${remote_hash_lines[0]:-}" == "$CELL_SHA256" &&
   ...
   "${remote_hash_lines[3]:-}" == 9d747d937a6a2316dd8894b37e243180031b8518f2696b9200ee7d1f1d81868c ]] ||
...
printf '%s\n' "$host_output"
printf 'LOOM_PROCESS_WITNESS_EFFECT_ROOT_V9_HOST_TRANSPORT PASS namespace=%s node=%s pod=%s transport=kubectl+hostPID+nsenter tree_sha256=%s cell_sha256=%s host_output_sha256=%s typed_file_bounds=effect-cell-16MiB+payload-1MiB+manifests-64KiB effective_mount_truth=DynamicUser+disconnected+strict+read-only property_authority=CONFIGURATION_ONLY filesystem_authority=ROOT_HOST_MOUNTINFO temporary_sources=SAME_IMMUTABLE_ROOT_TMP temporary_read_only=true temporary_empty=true forbidden_mounts=/proc+/home+/root+/run+/var+/etc principal_readable=false principal_enumeration=forbidden root_observed_empty=true observer_split=principal+ROOT_HOST incoming_mount_extinction=observed root_treatment=true bootstrap_sabotage=true bootstrap_missing_incoming_status=226/NAMESPACE bootstrap_missing_sys_status=226/NAMESPACE bootstrap_missing_var_tmp_status=226/NAMESPACE material_sabotages=0 material_coverage=false complete_effects=false material_execution=false launch_open=false parity_open=false claim_ready=false\n' \
