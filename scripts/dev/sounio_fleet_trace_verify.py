    ...
    *,
    ...
    for row in rows:
        ...
        if event_type == "OBSERVATION":
            ...
            require(action in {"start", "accept-handoff"}, f"capability {capability_id} action is invalid")
            capabilities[capability_id] = {"issued": (row, payload), "published": None, "terminal": None}
            label = "IssueStartCapability" if action == "start" else "PrepareHandoff"
            ...
            if event_type == "CAPABILITY_CONSUMED":
                ...
                for key in ("slot", "desired_hash", "observation_fingerprint") if issued.get("action") == "start" else ("slot", "handoff_id", "evidence_set_digest", "to_agent", "to_lane"):
            ...
            require(issued.get("action") == "start", f"action uses non-start capability: {action_id}")
            ...
            actions[action_id] = {"requested": (row, payload), "terminal": None}
            label = "Stutter"
            ...
            require(payload.get("action") == "start", f"action result kind mismatch: {action_id}")
