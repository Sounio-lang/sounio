...
# The self-hosted frontend carries large fixed-capacity arenas on its stack.
# Keep the gate's execution contract aligned with the canonical Madaros gates
# so a host's small default stack cannot masquerade as a compiler regression.
ulimit -s unlimited
