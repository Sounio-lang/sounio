...
OUTPUT_ROOT="${SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V9_OUTPUT:-$ROOT_DIR/tools/loom/_build/effect-root-v9}"
CELL_BUILDER="$ROOT_DIR/scripts/dev/build_loom_process_witness_effect_policy_v9.sh"
...
fail() {
  printf 'build-loom-process-witness-effect-root-v9: FAIL reason=%s\n' "$*" >&2
  exit 1
}
...
[[ "$(sha256sum "$POLICY_MANIFEST" | cut -d ' ' -f 1)" == \
  ...
  fail 'frozen V9 policy manifest drifted'
...
SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_POLICY_V9_OUTPUT="$OUTPUT_ROOT/loom/effect-cell" \
...
install -m 0444 "$POLICY_MANIFEST" "$OUTPUT_ROOT/loom/effect-policy-v9.freeze.v1"
...
loom/effect-policy-v9.freeze.v1:0444:9d747d937a6a2316dd8894b37e243180031b8518f2696b9200ee7d1f1d81868c
