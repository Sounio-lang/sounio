...
OUTPUT_ROOT="${SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_ROOT_V10_OUTPUT:-$ROOT_DIR/tools/loom/_build/effect-root-v10}"
CELL_BUILDER="$ROOT_DIR/scripts/dev/build_loom_process_witness_effect_policy_v10.sh"
...
fail() {
  printf 'build-loom-process-witness-effect-root-v10: FAIL reason=%s\n' "$*" >&2
  exit 1
}
...
[[ "$(sha256sum "$POLICY_MANIFEST" | cut -d ' ' -f 1)" == \
  ...
  fail 'frozen V10 policy manifest drifted'
...
SOUNIO_LOOM_PROCESS_WITNESS_EFFECT_POLICY_V10_OUTPUT="$OUTPUT_ROOT/loom/effect-cell" \
...
install -m 0444 "$POLICY_MANIFEST" "$OUTPUT_ROOT/loom/effect-policy-v10.freeze.v1"
...
loom/effect-policy-v10.freeze.v1:0444:9e7f42fd4bd18fd2b5f996b279a67f46a50546a20ef6949e4dc069c16b3d0dda
