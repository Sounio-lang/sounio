#!/usr/bin/env bash
# sprint20_resolution_manifest_gate.sh — acquisition/recourse manifest schema gate
set -euo pipefail
...
TIMEOUT_SECS="${SOUNIO_SPRINT20_TIMEOUT_SECS:-360}"

mkdir -p "$(dirname "$OUT_JSON")"

CASES_TSV="/tmp/sprint20_cases_$$.tsv"
: > "$CASES_TSV"
...
assert isinstance(ep.get("acquisition_policies", []), list)
assert isinstance(ep.get("acquisition_plans", []), list)
assert isinstance(ep.get("recourse_policies", []), list)
assert isinstance(ep.get("recourse_plans", []), list)

if expected == "acquisition":
    plans = ep.get("acquisition_plans", [])
    assert plans
    plan = plans[0]
    assert plan.get("policy_name")
    assert plan.get("objective_name")
    assert plan.get("route_name")
    assert isinstance(plan.get("metric_snapshot"), dict)
else:
    plans = ep.get("recourse_plans", [])
    assert plans
    plan = plans[0]
    assert plan.get("policy_name")
    assert plan.get("strategy_name")
    assert plan.get("route_name")
    assert isinstance(plan.get("metric_snapshot"), dict)
PY
  then
    record "$case_name" "pass" "manifest_contains_resolution_metadata"
  else
    record "$case_name" "fail" "manifest_missing_resolution_metadata"
  fi
}
...
if grep -q '\\"acquisition_policies\\"' self-hosted/compiler/main.sio 2>/dev/null \
   && grep -q '\\"acquisition_plans\\"' self-hosted/compiler/main.sio 2>/dev/null \
   && grep -q '\\"recourse_policies\\"' self-hosted/compiler/main.sio 2>/dev/null \
   && grep -q '\\"recourse_plans\\"' self-hosted/compiler/main.sio 2>/dev/null; then
  record "manifest_printer_resolution_fields_present" "pass" "found"
else
  record "manifest_printer_resolution_fields_present" "fail" "missing_manifest_field"
fi

run_manifest_case "manifest_plan_acquisition_basic" "tests/frontend/plan_acquisition_basic.sio" "acquisition"
run_manifest_case "manifest_plan_recourse_basic" "tests/frontend/plan_recourse_basic.sio" "recourse"
...
payload = {
    "schema": "sounio.sprint20.resolution_manifest_gate.v1",
...
echo "sprint20_resolution_manifest_gate: out_json=$(to_rel "$OUT_JSON") status=$status"
[ "$status" = "pass" ] && exit 0
