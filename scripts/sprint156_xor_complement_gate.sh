#!/usr/bin/env bash
# Sprint 156: Block BR — XOR-complement x ^ ~x → -1
set -eo pipefail
...
check_log_line() {
    ...
    elif ! grep -qF "T602" "$log" 2>/dev/null; then echo "NOT_RUN  $name (OOM before T602)"; NOT_RUN=$((NOT_RUN+1))
...
check_grep "src:block_br_comment" "Block BR.*XOR.complement|Block BR.*x.*~x.*-1" "$OPT"
check_grep "src:br_bnot_check"    "is_bnot\[br_s2 as usize\]|is_bnot\[.*br_s" "$OPT"
check_grep "src:br_base_eq"       "bnot_src\[br_s2 as usize\] == br_s1|bnot_src\[.*br_s" "$OPT"
check_grep "src:br_load_neg1"     "ir_load_imm.*br_instr\.dst.*-1|ir_load_imm.*br.*-1" "$OPT"
echo "--- Tests ---"
MAIN="self-hosted/compiler/main.sio"
check_grep "main:T602_fn"  "fn compiler_main_test_br_xor_complement_basic" "$MAIN"
check_grep "main:T603_fn"  "fn compiler_main_test_br_xor_complement_comm" "$MAIN"
check_grep "main:total"    "let total: i64 = [0-9]+" "$MAIN"
echo "--- Self-tests ---"
LOG="$(mktemp)"
timeout 180 "$SOUC" run self-hosted/compiler/main.sio -- --self-test > "$LOG" 2>&1 || _ec=$?
for t in T602 T603 T604 T605 T606 T607; do check_log_line "selftest:$t" "$t OK" "$LOG"; done
rm -f "$LOG"
echo "--- Type-check ---"
TOTAL=$((TOTAL+1))
