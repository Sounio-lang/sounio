...
WORKER="$ROOT/scripts/research/cs6_v7b_target23_arb_tm2r_composability_carrier_worker.py"
...
set -e
if [[ $rc -ne 0 ]]; then
  mv "$tmp_stderr" "$OUT_DIR/support_${TILE}.incomplete.stderr.txt"
  echo "composability carrier worker failed for $TILE with rc=$rc" >&2
  exit "$rc"
fi
...
if payload.get("selected_source_chain_certificate") is not True:
    raise SystemExit("worker did not certify its complete source tile")
...
if not payload.get("carriers"):
    raise SystemExit("worker emitted no final carriers")
PY

mv "$tmp_result" "$result"
mv "$tmp_stderr" "$stderr"
trap - EXIT
echo "CS6_COMPOSABILITY_TILE_COMPLETE=$TILE"
