...
def pilot_env() -> dict[str, str]:
    env = dict(os.environ)
    root = tool_root()
    env["PATH"] = f"{root / 'bin'}{os.pathsep}{env.get('PATH', '')}"
    env["PYTHONPATH"] = f"{root / 'python'}{os.pathsep}{env.get('PYTHONPATH', '')}"
    env["SOUNIO_SOLVER_TOOL_ROOT"] = str(root)
    return env


def solver_availability(out_dir: Path, prepare_missing: bool) -> Path:
    cmd = ["python3", "scripts/research/bootstrap_solver_external_tools.py", "--out-dir", str(out_dir), "--tool-root", str(tool_root())]
    if prepare_missing:
        cmd.append("--prepare-missing")
    completed = run(cmd, env=pilot_env())
...
def fetch_corpus(args: argparse.Namespace, out_dir: Path) -> Path:
    ...
    if args.smt_manifest:
        cmd.extend(["--smt-manifest", str(args.smt_manifest)])
    completed = run(cmd, timeout=args.fetch_timeout_sec, env=pilot_env())
...
def run_external(corpus_rows: list[dict[str, str]], availability_rows: list[dict[str, str]], out_dir: Path, timeout_sec: int) -> Path:
    ...
    for inst in corpus_rows:
        ...
        for solver in solvers_by_domain.get(domain, []):
            ...
            try:
                completed = run(solver_command(solver["solver"], solver["path"], Path(inst["local_path"])), timeout=timeout_sec, env=pilot_env())
...
def compile_sounio(sio: Path, binary: Path, timeout_sec: int) -> tuple[subprocess.CompletedProcess[str], str, str]:
    default = run(["./bin/souc", "compile", str(sio), "-o", str(binary)], timeout=timeout_sec)
    if default.returncode == 0:
        return default, "madaros", "0"
    fallback_env = pilot_env()
    fallback_env["SOUNIO_SOUC_ENGINE"] = "lean_single"
    fallback = run(["./bin/souc", "compile", str(sio), "-o", str(binary)], timeout=timeout_sec, env=fallback_env)
    if fallback.returncode == 0:
        combined = subprocess.CompletedProcess(
            args=fallback.args,
            returncode=0,
            stdout=default.stdout + default.stderr + "\n--- lean_single fallback ---\n" + fallback.stdout,
            stderr=fallback.stderr,
        )
        return combined, "lean_single", "1"
    combined_fail = subprocess.CompletedProcess(
        args=fallback.args,
        returncode=fallback.returncode,
        stdout=default.stdout + default.stderr + "\n--- lean_single fallback ---\n" + fallback.stdout,
        stderr=fallback.stderr,
    )
    return combined_fail, "lean_single", "1"
...
def run_sounio(corpus_manifest: Path, out_dir: Path, timeout_sec: int) -> Path:
    ...
    if not manifest.exists():
        ...
        write_csv(path, rows, ["instance_id", "expected", "compile_exit", "run_exit", "result", "elapsed_ms", "compile_engine", "fallback_used", "sio_path", "binary_path"])
        return path
    for row in read_csv(manifest):
        ...
        start = time.monotonic_ns()
        compile_run, compile_engine, fallback_used = compile_sounio(sio, binary, timeout_sec)
...
def summarize(corpus: list[dict[str, str]], availability: list[dict[str, str]], external: list[dict[str, str]], sounio: list[dict[str, str]], cross: list[dict[str, str]], out_dir: Path) -> tuple[Path, Counter[str]]:
    ...
    opb_real = any(row["domain"] == "opb" and row["status"] == "OK" for row in corpus) and any(row["domain"] == "opb" for row in external)
    smt_real = any(row["domain"] == "smt" and row["status"] == "OK" for row in corpus) and any(row["domain"] == "smt" for row in external)
    ...
    summary["pilot_smt_real_evidence"] = 1 if smt_real else 0
    summary["pilot_opb_real_evidence"] = 1 if opb_real else 0
...
def write_blockers(corpus: list[dict[str, str]], external: list[dict[str, str]], sounio: list[dict[str, str]], summary: Counter[str], out_dir: Path) -> Path:
    rows: list[dict[str, str]] = []
    has_smt = any(row["domain"] == "smt" and row["status"] == "OK" for row in corpus)
    has_opb = any(row["domain"] == "opb" and row["status"] == "OK" for row in corpus)
    sat_definitive = any(row["domain"] == "sat" and row["result"] in {"SAT", "UNSAT"} for row in external)
    sounio_definitive = any(row["result"] in {"SAT", "UNSAT"} for row in sounio)

    def add(blocker_id: str, severity: str, klass: str, evidence: str, gate: str, next_action: str) -> None:
        rows.append(
            {
    ...
    for row in corpus:
        if row["status"] != "OK":
            add(
                f"SOLVER-PILOT-CORPUS-{row['domain'].upper()}-{row['instance_id']}",
                "S2",
                "corpus-routing",
                row["status"],
                f"corpus_manifest.csv has OK rows for {row['domain']}",
                "Pin/download a real small source or lower the operational filter with evidence.",
            )
    if not has_smt:
        add(
            "SOLVER-PILOT-SMT-CORPUS-2026-06-26",
            "S2",
            "corpus-routing",
            "no SMT QF_LIA OK rows in corpus_manifest.csv",
            "Pinned, hashed QF_LIA SMT-LIB rows run through an external SMT solver.",
            "Populate benchmarks/solver/qflia_pinned_manifest.csv with small real QF_LIA URLs and hashes.",
        )
    if not has_opb:
        add(
            "SOLVER-PILOT-OPB-SOLVER-2026-06-26",
            "S2",
            "tooling-or-corpus",
            "no OPB OK rows with solver output",
            "roundingsat or equivalent produces rows in external_results.csv for real OPB instances.",
            "Keep RoundingSat available and ensure SampleByIntSize members extract under the size filter.",
        )
    if not sat_definitive:
        add(
            "SOLVER-PILOT-SAT-MULTISOLVER-2026-06-26",
            "S2",
            "solver-timeout",
            "no definitive SAT external result under timeout",
            "z3 plus cadical or kissat produce SAT/UNSAT rows for real SATComp/GBD CNFs.",
            "Run the SLURM gate with 30s timeout or tune corpus selection away from timeout-only families.",
        )
    if not sounio_definitive:
        add(
            "SOLVER-PILOT-SOUNIO-HARNESS-COMPILE-2026-06-26",
            "S2",
            "harness-runtime",
            "no definitive Sounio SAT result under timeout",
            "Generated theorem::smt harnesses compile and execute without COMPILE_FAILED and return SAT/UNSAT.",
            "Use the SLURM gate and keep the lean_single fallback explicit until Madaros imported/native lowering is repaired.",
        )
    if summary["crosscheck_match_0"]:
        add(
            "SOLVER-PILOT-CROSSCHECK-MISMATCH-2026-06-26",
            "S1",
            "semantic-mismatch",
            "definitive crosscheck mismatch",
            "crosscheck_results.csv has zero definitive mismatches.",
            "Inspect mismatching harness and external solver logs before broadening corpus.",
        )
    path = out_dir / "blockers.csv"
    write_csv(path, rows, ["blocker_id", "severity", "class", "evidence_level", "owner", "worktree", "branch", "acceptance_gate", "next_action", "status"])
    return path


def current_branch() -> str:
    try:
        completed = run(["git", "branch", "--show-current"], timeout=10)
    except OSError:
        return ""
...
def main() -> int:
    ...
    parser.add_argument("--prepare-missing-tools", action="store_true", help="Build missing user-space solver tools before the pilot")
    ...
    try:
        availability_path = solver_availability(out_dir, args.prepare_missing_tools)
        ...
        sounio_rows = read_csv(sounio_path)
        summary_path, summary = summarize(corpus_rows, availability_rows, external_rows, sounio_rows, read_csv(cross_path), out_dir)
        blockers_path = write_blockers(corpus_rows, external_rows, sounio_rows, summary, out_dir)
    ...
    manifest.write_text(
        "\n".join(
            [
                ...
                f"prepare_missing_tools={1 if args.prepare_missing_tools else 0}",
                ...
                f"blockers={blockers_path}",
                f"pilot_sat_acceptance_candidate={summary['pilot_sat_acceptance_candidate']}",
                f"pilot_smt_real_evidence={summary['pilot_smt_real_evidence']}",
                f"pilot_opb_real_evidence={summary['pilot_opb_real_evidence']}",
                f"pilot_evidence_incomplete={summary['pilot_evidence_incomplete']}",
                f"slurm_job_id={os.environ.get('SLURM_JOB_ID', '')}",
                f"slurm_job_name={os.environ.get('SLURM_JOB_NAME', '')}",
                f"slurm_node_list={os.environ.get('SLURM_NODELIST', '')}",
