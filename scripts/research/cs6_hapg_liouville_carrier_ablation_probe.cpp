...
struct LiouvilleData {
  interval time;
  IVector initial_hull = IVector(4);
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (!lowercase_sha256(input_sha256) || !lowercase_sha256(run_challenge) ||
        !lowercase_sha256(frozen_contract_sha256) ||
        !lowercase_sha256(coordinate_manifest_sha256) ||
        !lowercase_sha256(run_contract_sha256) ||
        !lowercase_sha256(manifest_row_sha256) ||
        !lowercase_sha256(attempt_binding)) {
      throw std::runtime_error(
    ...
    write_vector(std::cout, "INITIAL", liouville.initial_hull);
    std::cout << '\n';
    ...
    if (argc == 13) {
      std::cerr << "V7_FAILURE_BINDING"
                << " LIOUVILLE_CARRIER=" << argv[7]
                << " ATTEMPT_BINDING=" << argv[12] << '\n';
    }
