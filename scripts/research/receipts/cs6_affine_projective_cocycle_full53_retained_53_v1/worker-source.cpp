...
RayChart parse_ray_chart(const char* token) {
  ...
  if (value == "PLUS") {
    return RayChart::kPlus;
  }
  if (value == "MINUS") {
    return RayChart::kMinus;
  }
  if (value == "NONE") {
    return RayChart::kNone;
  }
  throw std::runtime_error("unsupported frozen ray chart");
}

int parse_pivot_sign(const char* token) {
  const std::string value(token);
  if (value == "-1") {
    return -1;
  }
  ...
  throw std::runtime_error("unsupported frozen pivot sign");
}

FrozenRayContract parse_ray_contract(const char* chart_token,
                                     const char* sign_token) {
  const FrozenRayContract contract{parse_ray_chart(chart_token),
                                   parse_pivot_sign(sign_token)};
  const bool unavailable = contract.chart == RayChart::kNone;
  if (unavailable != (contract.expected_sign == 0)) {
    throw std::runtime_error("frozen chart and pivot sign contract mismatch");
  }
  return contract;
}
...
IMatrix section_tangent_block(const IMatrix& value) {
  IMatrix projected = value;
  for (int index = 0; index < kDimension; ++index) {
    if (!projected[kSectionCoordinate][index].contains(0.0)) {
      throw std::runtime_error("Poincare image was not tangent to section");
    }
    projected[kSectionCoordinate][index] = interval(0.0);
  }
  return projected;
...
struct FrozenInput {
  ...
  int s_depth;
  int s_index;
  std::array<FrozenRayContract, 4> apg_ray_contracts;
  ...
  FrozenInput(int u_depth_value, int u_index_value, int s_depth_value,
              int s_index_value,
              const std::array<FrozenRayContract, 4>& contracts)
      : u_depth(u_depth_value),
        u_index(u_index_value),
        s_depth(s_depth_value),
        s_index(s_index_value),
        apg_ray_contracts(contracts) {
    const bool exact_root = u_depth == 0 && u_index == 0 && s_depth == 0 &&
                            s_index == 0;
    const bool all_unavailable = std::all_of(
        apg_ray_contracts.begin(), apg_ray_contracts.end(),
        [](const FrozenRayContract& contract) {
          return contract.chart == RayChart::kNone &&
                 contract.expected_sign == 0;
        });
    const bool all_active = std::all_of(
        apg_ray_contracts.begin(), apg_ray_contracts.end(),
        [](const FrozenRayContract& contract) {
          return contract.chart != RayChart::kNone &&
                 (contract.expected_sign == -1 || contract.expected_sign == 1);
        });
    if (exact_root && !all_unavailable) {
      throw std::runtime_error(
          "the exact root requires the all-NONE/0 chart sentinel");
    }
    if (!exact_root && !all_active) {
      throw std::runtime_error(
          "frozen chart sentinel is permitted only for the exact root");
    }
  }

  const FrozenRayContract& apg_ray_contract(int event, int column) const {
    if ((event != 1 && event != 2) || column < 0 || column > 1) {
      throw std::runtime_error("invalid frozen chart request");
    }
    const FrozenRayContract& contract =
        apg_ray_contracts[static_cast<std::size_t>((event - 1) * 2 + column)];
    if (contract.chart == RayChart::kNone || contract.expected_sign == 0) {
      throw std::runtime_error("frozen APG chart contract is unavailable");
    }
    return contract;
  }
...
struct ResidentC2Data : C2Data {
  IVector postsection = IVector(kDimension);
  interval postsection_time;
};

class SectionResidentC2Map : public IC2PoincareMap {
 public:
  using IC2PoincareMap::IC2PoincareMap;

  ResidentC2Data one_return(C2Rect2Set source) {
    ResidentC2Data result;
    C2Rect2Set before = source;
    C2Rect2Set after = source;
    this->sectionDerivativesEnclosure.init(
        &result.time, &result.flow, &result.flow_hessian, nullptr);
    ...
    if (!this->crossSectionInOneStep(before, after, local_time,
                                     result.image)) {
      throw std::runtime_error(
          "one-step C2 Newton crossing was not available");
    }
    this->sectionDerivativesEnclosure.computeOneStepSectionEnclosure(
        before, this->m_solver, result.image, local_time);
    this->computeDP(result.image, result.flow, result.flow_hessian, result.dp,
                    result.d2p, result.dtime, result.d2time, result.time);
    this->m_solver.computeCoefficientsAtCenter(result.time, result.image, 2);
    for (int row = 0; row < kDimension; ++row) {
      result.second_time_coefficient[row] =
          this->m_solver.centerCoefficient(row, 2);
    }
    result.dp = section_tangent_block(result.dp);
...
ResidentC2Data resident_c2_return(const FrozenInput& input,
                                  C2Rect2Set source) {
  IMap field(kVectorField);
  field.setParameter("zs", input.zs);
  IC2OdeSolver solver(field, kOrder);
  ICoordinateSection section(kDimension, kSectionCoordinate);
  SectionResidentC2Map map(solver, section, capd::poincare::MinusPlus);
  ResidentC2Data result = map.one_return(source);
  result.normal_velocity =
      result.image[0] * result.image[1] - result.image[2] - input.zs;
  reconstruct_impact_time_derivatives(input, result);
  return result;
...
constexpr std::array<std::array<int, 2>, 6> kTM2Exponents = {{
    {{0, 0}}, {{1, 0}}, {{0, 1}}, {{2, 0}}, {{1, 1}}, {{0, 2}},
}};

struct TM2 {
  std::array<interval, 6> coefficient;
  interval remainder;

  TM2() : remainder(0.0) {
    coefficient.fill(interval(0.0));
  }
};

TM2 tm_constant(const interval& value) {
  TM2 result;
  result.coefficient[0] = value;
  return result;
}

TM2 tm_affine(const interval& center, const interval& coefficient0,
              const interval& coefficient1, const interval& remainder) {
  TM2 result;
  result.coefficient[0] = center;
  result.coefficient[1] = coefficient0;
  result.coefficient[2] = coefficient1;
  result.remainder = remainder;
  return result;
}

interval interval_power(const interval& value, int exponent) {
  switch (exponent) {
    case 0:
      return interval(1.0);
    case 1:
    ...
    default:
      throw std::runtime_error("TM2 exponent exceeds multiplication bound");
  }
}

interval tm_monomial_range(const IVector& delta, int exponent0,
                           int exponent1) {
  return interval_power(delta[0], exponent0) *
         interval_power(delta[1], exponent1);
}

interval tm_polynomial_range(const TM2& value, const IVector& delta) {
  interval result(0.0);
  for (std::size_t monomial = 0; monomial < value.coefficient.size();
       ++monomial) {
    result += value.coefficient[monomial] *
              tm_monomial_range(delta, kTM2Exponents[monomial][0],
                                kTM2Exponents[monomial][1]);
  }
  return result;
}

interval tm_range(const TM2& value, const IVector& delta) {
  return tm_polynomial_range(value, delta) + value.remainder;
}

TM2 tm_add(const TM2& left, const TM2& right) {
  TM2 result;
  for (std::size_t monomial = 0; monomial < result.coefficient.size();
       ++monomial) {
    result.coefficient[monomial] =
        left.coefficient[monomial] + right.coefficient[monomial];
  }
  result.remainder = left.remainder + right.remainder;
  return result;
}

TM2 tm_negate(const TM2& value) {
  TM2 result;
  for (std::size_t monomial = 0; monomial < result.coefficient.size();
       ++monomial) {
    result.coefficient[monomial] = -value.coefficient[monomial];
  }
  result.remainder = -value.remainder;
  return result;
}

TM2 tm_subtract(const TM2& left, const TM2& right) {
  return tm_add(left, tm_negate(right));
}

TM2 tm_scale(const TM2& value, const interval& scale) {
  TM2 result;
  for (std::size_t monomial = 0; monomial < result.coefficient.size();
       ++monomial) {
    result.coefficient[monomial] = value.coefficient[monomial] * scale;
  }
  result.remainder = value.remainder * scale;
  return result;
}

int tm_monomial_index(int exponent0, int exponent1) {
  for (std::size_t monomial = 0; monomial < kTM2Exponents.size();
       ++monomial) {
    if (kTM2Exponents[monomial][0] == exponent0 &&
        kTM2Exponents[monomial][1] == exponent1) {
      return static_cast<int>(monomial);
    }
  }
  return -1;
}

TM2 tm_multiply(const TM2& left, const TM2& right,
                const IVector& delta) {
  TM2 result;
  interval discarded(0.0);
  for (std::size_t left_index = 0;
       left_index < left.coefficient.size(); ++left_index) {
    for (std::size_t right_index = 0;
         right_index < right.coefficient.size(); ++right_index) {
      const int exponent0 = kTM2Exponents[left_index][0] +
                            kTM2Exponents[right_index][0];
      const int exponent1 = kTM2Exponents[left_index][1] +
                            kTM2Exponents[right_index][1];
      const interval product =
          left.coefficient[left_index] * right.coefficient[right_index];
      const int target = tm_monomial_index(exponent0, exponent1);
      if (target >= 0) {
        result.coefficient[static_cast<std::size_t>(target)] += product;
      } else {
        discarded += product *
                     tm_monomial_range(delta, exponent0, exponent1);
      }
    }
  }
  result.remainder =
      discarded + tm_polynomial_range(left, delta) * right.remainder +
      left.remainder * tm_polynomial_range(right, delta) +
      left.remainder * right.remainder;
  return result;
}

TM2 tm_reciprocal(const TM2& denominator, const IVector& delta) {
  const interval denominator_range = tm_range(denominator, delta);
  if (denominator_range.contains(0.0)) {
    throw std::runtime_error("TM2 reciprocal pivot contains zero");
  }
  const interval center = denominator.coefficient[0];
  if (center.contains(0.0)) {
    throw std::runtime_error("TM2 reciprocal center contains zero");
  }
  const TM2 z = tm_scale(
      tm_subtract(denominator, tm_constant(center)), interval(1.0) / center);
  const interval z_range = tm_range(z, delta);
  const double contraction =
      std::max(std::abs(z_range.leftBound()),
               std::abs(z_range.rightBound()));
  if (!(contraction < 1.0)) {
    throw std::runtime_error("TM2 reciprocal is noncontractive");
  }
  const TM2 z_squared = tm_multiply(z, z, delta);
  TM2 result = tm_add(tm_subtract(tm_constant(interval(1.0)), z),
                      z_squared);
  const interval exact_tail =
      -(z_range * z_range * z_range) / (interval(1.0) + z_range);
  result.remainder += exact_tail;
  return tm_scale(result, interval(1.0) / center);
}

TM2 tm_divide(const TM2& numerator, const TM2& denominator,
              const IVector& delta) {
  return tm_multiply(numerator, tm_reciprocal(denominator, delta), delta);
}

bool finite(const TM2& value) {
  if (!finite(value.remainder)) {
    return false;
  }
  return std::all_of(value.coefficient.begin(), value.coefficient.end(),
                     [](const interval& coefficient) {
                       return finite(coefficient);
                     });
}

void write_tm2(std::ostream& output, const std::string& prefix,
               const TM2& value, const IVector& delta) {
  constexpr std::array<const char*, 6> names = {
      "C00", "C10", "C01", "C20", "C11", "C02"};
  for (std::size_t monomial = 0; monomial < value.coefficient.size();
       ++monomial) {
    write_interval(output, prefix + names[monomial],
                   value.coefficient[monomial]);
  }
  write_interval(output, prefix + "R", value.remainder);
  write_interval(output, prefix + "POLY", tm_polynomial_range(value, delta));
  write_interval(output, prefix + "HULL", tm_range(value, delta));
}

using TM2Vector = std::array<TM2, kDimension>;
using TM2Matrix = std::array<std::array<TM2, kDimension>, kDimension>;

TM2 tm_linear_combination(const TM2& x, const TM2& y,
                          const interval& x_scale,
                          const interval& y_scale) {
  return tm_add(tm_scale(x, x_scale), tm_scale(y, y_scale));
}
...
RayChart frozen_chart(const FrozenInput& input, int event, int column) {
  return input.apg_ray_contract(event, column).chart;
}

int expected_pivot_sign(const FrozenInput& input, int event, int column) {
  return input.apg_ray_contract(event, column).expected_sign;
}

struct TMNormalizedRay {
  RayChart chart = RayChart::kNone;
  TM2 pivot;
  TM2 numerator;
  TM2 slope;
  TM2Vector normalized;
  interval pivot_hull;
  interval slope_hull;
  int expected_sign = 0;
  bool pivot_sign_certified = false;
};

TMNormalizedRay tm_normalize_ray(const TM2Vector& ray, RayChart chart,
                                 int expected_sign,
                                 const IVector& delta) {
  TMNormalizedRay result;
  result.chart = chart;
  result.expected_sign = expected_sign;
  switch (chart) {
    case RayChart::kX:
      result.pivot = ray[0];
      result.numerator = ray[1];
      break;
    case RayChart::kY:
      result.pivot = ray[1];
      result.numerator = ray[0];
      break;
    case RayChart::kPlus:
      result.pivot = tm_add(ray[0], ray[1]);
      result.numerator = tm_subtract(ray[1], ray[0]);
      break;
    case RayChart::kMinus:
      result.pivot = tm_subtract(ray[0], ray[1]);
      result.numerator = tm_add(ray[0], ray[1]);
      break;
    case RayChart::kNone:
      throw std::runtime_error("cannot normalize in an empty chart");
  }
  result.pivot_hull = tm_range(result.pivot, delta);
  result.pivot_sign_certified =
      expected_sign < 0 ? result.pivot_hull.rightBound() < 0.0
                        : result.pivot_hull.leftBound() > 0.0;
  if (!result.pivot_sign_certified) {
    throw std::runtime_error("frozen TM2 pivot sign was not certified");
  }
  result.slope = tm_divide(result.numerator, result.pivot, delta);
  result.slope_hull = tm_range(result.slope, delta);
  switch (chart) {
    case RayChart::kX:
      result.normalized[0] = tm_constant(interval(1.0));
      result.normalized[1] = result.slope;
      break;
    case RayChart::kY:
      result.normalized[0] = result.slope;
      result.normalized[1] = tm_constant(interval(1.0));
      break;
    case RayChart::kPlus:
      result.normalized[0] = tm_scale(
          tm_subtract(tm_constant(interval(1.0)), result.slope),
          interval(0.5));
      result.normalized[1] = tm_scale(
          tm_add(tm_constant(interval(1.0)), result.slope), interval(0.5));
      break;
    case RayChart::kMinus:
      result.normalized[0] = tm_scale(
          tm_add(tm_constant(interval(1.0)), result.slope), interval(0.5));
      result.normalized[1] = tm_scale(
          tm_subtract(result.slope, tm_constant(interval(1.0))),
          interval(0.5));
      break;
    case RayChart::kNone:
      throw std::runtime_error("cannot reconstruct an empty chart");
  }
  result.normalized[kSectionCoordinate] = tm_constant(interval(0.0));
  return result;
}

bool finite(const TMNormalizedRay& ray) {
  return finite(ray.pivot) && finite(ray.numerator) && finite(ray.slope) &&
         std::all_of(ray.normalized.begin(), ray.normalized.end(),
                     [](const TM2& value) { return finite(value); }) &&
         finite(ray.pivot_hull) && finite(ray.slope_hull);
}

void emit_tm_ray(const char* marker, const TMNormalizedRay& ray,
                 const IVector& delta) {
  std::cout << marker << " CHART=" << chart_name(ray.chart)
            << " EXPECTED_SIGN=" << ray.expected_sign
            << " PIVOT_SIGN_CERTIFIED=" << ray.pivot_sign_certified;
  write_tm2(std::cout, "P_", ray.pivot, delta);
  write_tm2(std::cout, "Q_", ray.numerator, delta);
  write_tm2(std::cout, "S_", ray.slope, delta);
  for (int row = 0; row < kDimension; ++row) {
    write_tm2(std::cout, "N" + std::to_string(row) + "_",
              ray.normalized[row], delta);
  }
  std::cout << '\n';
}
...
int main(int argc, char** argv) {
  ...
  try {
    if (argc != 17) {
      throw std::runtime_error(
          "usage: U_DEPTH U_INDEX S_DEPTH S_INDEX PARENT_INPUT_SHA256 "
          "PARENT_RECEIPT_SHA256 E1_R0_CHART E1_R0_SIGN E1_R1_CHART "
          "E1_R1_SIGN E2_R0_CHART E2_R0_SIGN E2_R1_CHART E2_R1_SIGN "
          "COORDINATE_MANIFEST_SHA256 RUN_CHALLENGE");
    }
    const int u_depth =
    ...
    const std::string parent_receipt_sha256 = argv[6];
    const std::array<FrozenRayContract, 4> contracts = {
        parse_ray_contract(argv[7], argv[8]),
        parse_ray_contract(argv[9], argv[10]),
        parse_ray_contract(argv[11], argv[12]),
        parse_ray_contract(argv[13], argv[14])};
    const std::string coordinate_manifest_sha256 = argv[15];
    const std::string run_challenge = argv[16];
    if (!lowercase_sha256(input_sha256) ||
        !lowercase_sha256(parent_receipt_sha256) ||
        !lowercase_sha256(coordinate_manifest_sha256) ||
        !lowercase_sha256(run_challenge)) {
      throw std::runtime_error(
          "input, receipt, manifest, and challenge must be lowercase SHA-256");
    }
    const FrozenInput input(u_depth, u_index, s_depth, s_index, contracts);
    ...
    const AffineCarrier affine_p1 =
        build_affine_carrier(c2_full_p1, c2_center_p1, delta);
    ...
    TM2Matrix event1_jacobian;
    for (int row = 0; row < kDimension; ++row) {
      for (int column = 0; column < kDimension; ++column) {
        event1_jacobian[row][column] = tm_affine(
            affine_p1.center[row][column],
            affine_p1.coefficient[0][row][column],
            affine_p1.coefficient[1][row][column],
            affine_p1.residual[row][column]);
      }
    }
    TM2Vector event1_state;
    TM2Vector event1_displacement;
    for (int row = 0; row < kDimension; ++row) {
      event1_state[row] =
          tm_affine(event1_center[row], event1_basis[row][0],
                    event1_basis[row][1], event1_residual[row]);
      event1_displacement[row] =
          tm_subtract(event1_state[row], tm_constant(event1_center[row]));
    }

    IMatrix local_identity(kDimension, kDimension);
    local_identity.setToIdentity();
    IVector apg_input_padding(kDimension);
    IVector apg_input_residual = event1_residual;
    for (int row = 0; row < kDimension; ++row) {
      if (row == kSectionCoordinate) {
        apg_input_padding[row] = interval(0.0);
        continue;
      }
      const double magnitude =
          std::max({1.0, std::abs(event1_center[row].leftBound()),
                    std::abs(event1_center[row].rightBound())});
      const double radius =
          64.0 * std::numeric_limits<double>::epsilon() * magnitude;
      apg_input_padding[row] = interval(-radius, radius);
      apg_input_residual[row] += apg_input_padding[row];
    }
    C2Rect2Set::C0BaseSet apg_full_c0(
        event1_center, event1_basis, delta, residual_basis,
        apg_input_residual);
    C2Rect2Set::C1BaseSet apg_full_c1(local_identity);
    C2Rect2Set apg_full_set(apg_full_c0, apg_full_c1, c2_full_p1.time);
    apg_full_set.setC0Factor(std::numeric_limits<double>::infinity());
    const IVector apg_local_input_hull = static_cast<IVector>(apg_full_set);
    C2Rect2Set::C0BaseSet apg_center_c0(event1_center);
    C2Rect2Set::C1BaseSet apg_center_c1(local_identity);
    C2Rect2Set apg_center_set(apg_center_c0, apg_center_c1,
                             c2_center_p1.time);
    const ResidentC2Data apg_local_full =
        resident_c2_return(input, apg_full_set);
    const ResidentC2Data apg_local_center =
        resident_c2_return(input, apg_center_set);

    TM2Matrix local_jacobian;
    for (int row = 0; row < kDimension; ++row) {
      for (int column = 0; column < kDimension; ++column) {
        const interval center = midpoint(apg_local_center.dp[row][column]);
        local_jacobian[row][column] = tm_constant(center);
        local_jacobian[row][column].remainder +=
            apg_local_center.dp[row][column] - center;
        for (int variable = 0; variable < 2; ++variable) {
          const interval derivative = actual_second_derivative(
              apg_local_full.d2p, row, column, variable);
          local_jacobian[row][column] = tm_add(
              local_jacobian[row][column],
              tm_scale(event1_displacement[variable], derivative));
        }
      }
    }

    std::array<TMNormalizedRay, 2> apg_event1_rays;
    std::array<TM2Vector, 2> apg_event2_raw;
    std::array<TMNormalizedRay, 2> apg_event2_rays;
    for (int column = 0; column < 2; ++column) {
      TM2Vector event1_ray;
      for (int row = 0; row < kDimension; ++row) {
        event1_ray[row] = event1_jacobian[row][column];
      }
      apg_event1_rays[column] = tm_normalize_ray(
          event1_ray, frozen_chart(input, 1, column),
          expected_pivot_sign(input, 1, column), delta);
      for (int row = 0; row < kDimension; ++row) {
        apg_event2_raw[column][row] = tm_constant(interval(0.0));
        for (int inner = 0; inner < 2; ++inner) {
          apg_event2_raw[column][row] = tm_add(
              apg_event2_raw[column][row],
              tm_multiply(local_jacobian[row][inner],
                          apg_event1_rays[column].normalized[inner], delta));
        }
      }
      apg_event2_rays[column] = tm_normalize_ray(
          apg_event2_raw[column], frozen_chart(input, 2, column),
          expected_pivot_sign(input, 2, column), delta);
    }

    std::array<TM2, 2> apg_total_scale;
    std::array<TM2Vector, 2> apg_reconstructed_columns;
    std::array<TM2Vector, 2> shared_columns;
    for (int column = 0; column < 2; ++column) {
      apg_total_scale[column] =
          tm_multiply(apg_event1_rays[column].pivot,
                      apg_event2_rays[column].pivot, delta);
      for (int row = 0; row < kDimension; ++row) {
        apg_reconstructed_columns[column][row] =
            tm_multiply(apg_event1_rays[column].pivot,
                        apg_event2_raw[column][row], delta);
        shared_columns[column][row] = tm_constant(interval(0.0));
        for (int inner = 0; inner < 2; ++inner) {
          shared_columns[column][row] = tm_add(
              shared_columns[column][row],
              tm_multiply(local_jacobian[row][inner],
                          event1_jacobian[inner][column], delta));
        }
      }
    }
    const TM2 apg_normalized_exterior = tm_subtract(
        tm_multiply(apg_event2_rays[0].normalized[0],
                    apg_event2_rays[1].normalized[1], delta),
        tm_multiply(apg_event2_rays[0].normalized[1],
                    apg_event2_rays[1].normalized[0], delta));
    const TM2 apg_factored_determinant = tm_multiply(
        tm_multiply(apg_total_scale[0], apg_total_scale[1], delta),
        apg_normalized_exterior, delta);
    const TM2 apg_reconstructed_determinant = tm_subtract(
        tm_multiply(apg_reconstructed_columns[0][0],
                    apg_reconstructed_columns[1][1], delta),
        tm_multiply(apg_reconstructed_columns[1][0],
                    apg_reconstructed_columns[0][1], delta));
    const TM2 shared_determinant = tm_subtract(
        tm_multiply(shared_columns[0][0], shared_columns[1][1], delta),
        tm_multiply(shared_columns[1][0], shared_columns[0][1], delta));
    const interval apg_primary = tm_range(apg_factored_determinant, delta);
    const interval apg_reconstructed =
        tm_range(apg_reconstructed_determinant, delta);
    const interval shared_composition = tm_range(shared_determinant, delta);

    const bool apg_all_finite = [&]() {
      if (!finite(affine_p1.center) || !finite(affine_p1.coefficient[0]) ||
          !finite(affine_p1.coefficient[1]) || !finite(affine_p1.residual) ||
          !finite(affine_p1.hull) || !finite(apg_local_input_hull) ||
          !finite(apg_local_full.time) || !finite(apg_local_full.image) ||
          !finite(apg_local_full.flow) ||
          !finite(apg_local_full.flow_hessian) ||
          !finite(apg_local_full.dp) || !finite(apg_local_full.d2p) ||
          !finite(apg_local_full.dtime) || !finite(apg_local_full.d2time) ||
          !finite(apg_local_full.reconstructed_dtime) ||
          !finite(apg_local_full.reconstructed_d2time) ||
          !finite(apg_local_full.postsection) ||
          !finite(apg_local_full.postsection_time) ||
          !finite(apg_local_full.normal_velocity) ||
          !finite(apg_local_center.time) || !finite(apg_local_center.image) ||
          !finite(apg_local_center.flow) ||
          !finite(apg_local_center.flow_hessian) ||
          !finite(apg_local_center.dp) || !finite(apg_local_center.d2p) ||
          !finite(apg_local_center.dtime) ||
          !finite(apg_local_center.d2time) ||
          !finite(apg_local_center.reconstructed_dtime) ||
          !finite(apg_local_center.reconstructed_d2time) ||
          !finite(apg_local_center.postsection) ||
          !finite(apg_local_center.postsection_time) ||
          !finite(apg_local_center.normal_velocity) ||
          !finite(apg_factored_determinant) ||
          !finite(apg_reconstructed_determinant) ||
          !finite(shared_determinant) || !finite(apg_primary) ||
          !finite(apg_reconstructed) || !finite(shared_composition)) {
        return false;
      }
      for (int row = 0; row < kDimension; ++row) {
        if (!finite(event1_state[row]) ||
            !finite(event1_displacement[row])) {
          return false;
        }
        for (int column = 0; column < kDimension; ++column) {
          if (!finite(event1_jacobian[row][column]) ||
              !finite(local_jacobian[row][column])) {
            return false;
          }
        }
      }
      for (int column = 0; column < 2; ++column) {
        if (!finite(apg_event1_rays[column]) ||
            !finite(apg_event2_rays[column]) ||
            !finite(apg_total_scale[column])) {
          return false;
        }
        for (int row = 0; row < kDimension; ++row) {
          if (!finite(apg_event2_raw[column][row]) ||
              !finite(apg_reconstructed_columns[column][row]) ||
              !finite(shared_columns[column][row])) {
            return false;
          }
        }
      }
    ...
    const bool all_finite =
        ...
        finite(reconstructed_determinant) && apg_all_finite;
    ...
    const bool apg_local_impact_time_crosscheck =
        overlaps(apg_local_full.dtime,
                 apg_local_full.reconstructed_dtime) &&
        overlaps(apg_local_full.d2time,
                 apg_local_full.reconstructed_d2time) &&
        overlaps(apg_local_center.dtime,
                 apg_local_center.reconstructed_dtime) &&
        overlaps(apg_local_center.d2time,
                 apg_local_center.reconstructed_d2time);
    const bool apg_local_center_full_dp_overlap =
        overlaps(apg_local_center.dp, apg_local_full.dp);
    const bool apg_local_input_containment = [&]() {
      for (int row = 0; row < kDimension; ++row) {
        if (!contains(apg_local_input_hull[row],
                      tm_range(event1_state[row], delta))) {
          return false;
        }
      }
      return true;
    }();
    const bool apg_event1_ray_reconstruction = [&]() {
      for (int column = 0; column < 2; ++column) {
        for (int row = 0; row < 2; ++row) {
          const interval reconstructed = tm_range(
              tm_multiply(apg_event1_rays[column].pivot,
                          apg_event1_rays[column].normalized[row], delta),
              delta);
          if (!contains(reconstructed,
                        tm_range(event1_jacobian[row][column], delta))) {
            return false;
          }
        }
        if (!tm_range(event1_jacobian[kSectionCoordinate][column], delta)
                 .contains(0.0) ||
            !tm_range(apg_event1_rays[column].normalized[kSectionCoordinate],
                      delta)
                 .contains(0.0)) {
          return false;
        }
    ...
    const bool apg_event2_ray_reconstruction = [&]() {
      for (int column = 0; column < 2; ++column) {
        for (int row = 0; row < 2; ++row) {
          const interval reconstructed = tm_range(
              tm_multiply(apg_event2_rays[column].pivot,
                          apg_event2_rays[column].normalized[row], delta),
              delta);
          if (!contains(reconstructed,
                        tm_range(apg_event2_raw[column][row], delta))) {
            return false;
          }
        }
        if (!tm_range(apg_event2_raw[column][kSectionCoordinate], delta)
                 .contains(0.0) ||
            !tm_range(apg_event2_rays[column].normalized[kSectionCoordinate],
                      delta)
                 .contains(0.0)) {
          return false;
        }
    ...
    const bool apg_local_event_order_certified =
        (apg_local_full.time - c2_full_p1.time).leftBound() > 0.0 &&
        (apg_local_center.time - c2_center_p1.time).leftBound() > 0.0 &&
        apg_local_full.postsection_time.leftBound() >
            apg_local_full.time.rightBound() &&
        apg_local_center.postsection_time.leftBound() >
            apg_local_center.time.rightBound();
    const bool apg_local_postsection_plus_side =
        apg_local_full.postsection[kSectionCoordinate].leftBound() > 0.0 &&
        apg_local_center.postsection[kSectionCoordinate].leftBound() > 0.0;
    const bool apg_local_transversality_certified =
        apg_local_full.normal_velocity.leftBound() > 0.0 &&
        apg_local_center.normal_velocity.leftBound() > 0.0;
    const bool apg_joint_overlap =
        std::max({apg_primary.leftBound(), apg_reconstructed.leftBound(),
                  shared_composition.leftBound(),
                  c2_hull_determinant.leftBound(),
                  affine.determinant.leftBound(),
                  liouville.determinant.leftBound()}) <=
        std::min({apg_primary.rightBound(), apg_reconstructed.rightBound(),
                  shared_composition.rightBound(),
                  c2_hull_determinant.rightBound(),
                  affine.determinant.rightBound(),
                  liouville.determinant.rightBound()});
    const bool apg_primary_positive_width = width(apg_primary) > 0.0;
    const bool apg_strictly_narrower_than_boxed =
        width(apg_primary) < width(homogeneous_determinant);
    const bool apg_strictly_narrower_than_affine =
        width(apg_primary) < width(affine.determinant);
    const bool apg_strictly_narrower_than_shared =
        width(apg_primary) < width(shared_composition);
    const bool apg_orientation_certified = sign_definite(apg_primary);
    const bool apg_negative_orientation_certified =
        apg_primary.rightBound() < 0.0;
    ...
    const bool apg_computation_valid =
        structural_pass && apg_local_impact_time_crosscheck &&
        apg_local_center_full_dp_overlap && apg_local_input_containment &&
        apg_event1_ray_reconstruction && apg_event2_ray_reconstruction &&
        apg_local_event_order_certified && apg_local_postsection_plus_side &&
        apg_local_transversality_certified && apg_joint_overlap &&
    ...
    const bool apg_certificate_pass =
        apg_computation_valid && apg_negative_orientation_certified &&
        overlaps(apg_primary, liouville.determinant) &&
        same_strict_sign(apg_primary, liouville.determinant);
    const bool apg_rescue = apg_certificate_pass && !affine_certificate_pass;
    ...
    const bool probe_pass =
        homogeneous_computation_valid && apg_computation_valid;
    ...
    std::cout
        ...
        << "TM2_MONOMIAL_ORDER=ONE_DELTA0_DELTA1_DELTA0_SQUARED_DELTA0_DELTA1_DELTA1_SQUARED\n"
    ...
    std::cout << "AFFINE_P1_CARRIER";
    write_matrix(std::cout, "M", affine_p1.center);
    write_matrix(std::cout, "A0", affine_p1.coefficient[0]);
    write_matrix(std::cout, "A1", affine_p1.coefficient[1]);
    write_matrix(std::cout, "R", affine_p1.residual);
    write_matrix(std::cout, "HULL", affine_p1.hull);
    write_interval(std::cout, "DET_POLYNOMIAL",
                   affine_p1.determinant_polynomial);
    write_interval(std::cout, "DET_REMAINDER",
                   affine_p1.determinant_residual);
    write_interval(std::cout, "DET", affine_p1.determinant);
    std::cout << '\n';
    ...
    std::cout << "APG_EVENT1_STATE_TM2";
    for (int row = 0; row < kDimension; ++row) {
      write_tm2(std::cout, "X" + std::to_string(row) + "_",
                event1_state[row], delta);
    }
    write_vector(std::cout, "INPUT_PADDING", apg_input_padding);
    write_vector(std::cout, "INPUT_RESIDUAL", apg_input_residual);
    write_vector(std::cout, "LOCAL_INPUT_HULL", apg_local_input_hull);
    std::cout << '\n';

    std::cout << "APG_LOCAL_C2_FULL";
    write_interval(std::cout, "TIME", apg_local_full.time);
    write_vector(std::cout, "X", apg_local_full.image);
    write_matrix(std::cout, "FLOW", apg_local_full.flow);
    write_hessian(std::cout, "FLOW_H", apg_local_full.flow_hessian);
    write_vector(std::cout, "DT", apg_local_full.dtime);
    write_matrix(std::cout, "D2T", apg_local_full.d2time);
    write_vector(std::cout, "D2PHIDT2",
                 apg_local_full.second_time_coefficient);
    write_vector(std::cout, "DT_RECON",
                 apg_local_full.reconstructed_dtime);
    write_matrix(std::cout, "D2T_RECON",
                 apg_local_full.reconstructed_d2time);
    write_matrix(std::cout, "DP", apg_local_full.dp);
    write_hessian(std::cout, "D2P", apg_local_full.d2p);
    write_interval(std::cout, "POST_TIME", apg_local_full.postsection_time);
    write_vector(std::cout, "POST_X", apg_local_full.postsection);
    write_interval(std::cout, "NU", apg_local_full.normal_velocity);
    std::cout << '\n';

    std::cout << "APG_LOCAL_C2_CENTER";
    write_interval(std::cout, "TIME", apg_local_center.time);
    write_vector(std::cout, "X", apg_local_center.image);
    write_matrix(std::cout, "FLOW", apg_local_center.flow);
    write_hessian(std::cout, "FLOW_H", apg_local_center.flow_hessian);
    write_vector(std::cout, "DT", apg_local_center.dtime);
    write_matrix(std::cout, "D2T", apg_local_center.d2time);
    write_vector(std::cout, "D2PHIDT2",
                 apg_local_center.second_time_coefficient);
    write_vector(std::cout, "DT_RECON",
                 apg_local_center.reconstructed_dtime);
    write_matrix(std::cout, "D2T_RECON",
                 apg_local_center.reconstructed_d2time);
    write_matrix(std::cout, "DP", apg_local_center.dp);
    write_hessian(std::cout, "D2P", apg_local_center.d2p);
    write_interval(std::cout, "POST_TIME",
                   apg_local_center.postsection_time);
    write_vector(std::cout, "POST_X", apg_local_center.postsection);
    write_interval(std::cout, "NU", apg_local_center.normal_velocity);
    std::cout << '\n';

    std::cout << "APG_LOCAL_JACOBIAN_TM2";
    for (int row = 0; row < kDimension; ++row) {
      for (int column = 0; column < kDimension; ++column) {
        write_tm2(std::cout,
                  "J" + std::to_string(row) + std::to_string(column) + "_",
                  local_jacobian[row][column], delta);
      }
    }
    std::cout << '\n';
    ...
    emit_tm_ray("APG_EVENT1_RAY0", apg_event1_rays[0], delta);
    emit_tm_ray("APG_EVENT1_RAY1", apg_event1_rays[1], delta);

    std::cout << "APG_EVENT2_RAW_TM2";
    for (int column = 0; column < 2; ++column) {
      for (int row = 0; row < kDimension; ++row) {
        write_tm2(std::cout,
                  "V" + std::to_string(row) + std::to_string(column) + "_",
                  apg_event2_raw[column][row], delta);
      }
    }
    std::cout << '\n';

    emit_tm_ray("APG_EVENT2_RAY0", apg_event2_rays[0], delta);
    emit_tm_ray("APG_EVENT2_RAY1", apg_event2_rays[1], delta);

    std::cout << "APG_SHARED_COMPOSITION_TM2";
    for (int column = 0; column < 2; ++column) {
      for (int row = 0; row < kDimension; ++row) {
        write_tm2(std::cout,
                  "W" + std::to_string(row) + std::to_string(column) + "_",
                  shared_columns[column][row], delta);
      }
    }
    write_tm2(std::cout, "DET_", shared_determinant, delta);
    std::cout << '\n';

    std::cout << "APG_RECONSTRUCTED_TM2";
    for (int column = 0; column < 2; ++column) {
      for (int row = 0; row < kDimension; ++row) {
        write_tm2(std::cout,
                  "W" + std::to_string(row) + std::to_string(column) + "_",
                  apg_reconstructed_columns[column][row], delta);
      }
    }
    write_tm2(std::cout, "DET_", apg_reconstructed_determinant, delta);
    std::cout << '\n';

    std::cout << "APG_FACTORED_EXTERIOR_TM2";
    write_tm2(std::cout, "SCALE0_", apg_total_scale[0], delta);
    write_tm2(std::cout, "SCALE1_", apg_total_scale[1], delta);
    write_tm2(std::cout, "EXTERIOR_", apg_normalized_exterior, delta);
    write_tm2(std::cout, "DET_", apg_factored_determinant, delta);
    write_interval(std::cout, "PRIMARY_DET", apg_primary);
    std::cout << '\n';
    ...
    std::cout << "LEAF_RESULT TERMINAL_CERTIFIED=" << certificate_pass
              ...
              << homogeneous_certificate_pass
              << " APG_CERTIFICATE_PASS=" << apg_certificate_pass
    ...
    std::cout
        ...
        << " APG_PRIMARY_POSITIVE_WIDTH=" << apg_primary_positive_width
        ...
        << apg_strictly_narrower_than_shared
        << " APG_ORIENTATION_CERTIFIED=" << apg_orientation_certified
        << " APG_NEGATIVE_ORIENTATION_CERTIFIED="
        << apg_negative_orientation_certified
        << " APG_COMPUTATION_VALID=" << apg_computation_valid
        << " APG_CERTIFICATE_PASS=" << apg_certificate_pass
        << " APG_RESCUE=" << apg_rescue
