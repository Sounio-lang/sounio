...
def verify_ledger(
    ...
    _, projective_x, projective_x_det = verify_projective(
        ledger.records["PROJECTIVE_X"], affine_hull_matrix, "X", (1, 0, 0, 1, 1)
    )
    _, projective_y, projective_y_det = verify_projective(
        ledger.records["PROJECTIVE_Y"], affine_hull_matrix, "Y", (0, 1, 1, 0, -1)
    )
    _, projective_plus, projective_plus_det = verify_projective(
        ledger.records["PROJECTIVE_PLUS"],
        affine_hull_matrix,
        "PLUS",
        (1, 1, -1, 1, 2),
    )
    _, projective_minus, projective_minus_det = verify_projective(
    ...
    computed["PROBE_PASS"] = computed["CERTIFICATE_PASS"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for candidate, key in (
        ...
        if computed[key]:
            method = candidate
            break
    print("VERIFICATION_SCHEMA=sounio.cs6.c1-full-source-cover-leaf-verification.v1")
