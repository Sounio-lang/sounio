#!/usr/bin/env python3
"""Exact verifier for one frozen CS6 affine-projective cocycle leaf."""
...
def frozen_apg_chart(
    coordinates: tuple[int, int, int, int], event: int, column: int
) -> str:
    if event == 2:
        return "PLUS"
    plus_second_ray = coordinates in {
        (15, 8192, 15, 24576),
        (16, 16384, 12, 3072),
    }
    return "PLUS" if column == 1 and plus_second_ray else "X"


def frozen_apg_sign(
    coordinates: tuple[int, int, int, int], event: int, column: int
) -> int:
    if event == 2 or column == 0:
        return -1
    return 1 if frozen_apg_chart(coordinates, event, column) == "PLUS" else -1
...
def verify_ledger(
    ...
    coordinates = (u_depth, u_index, s_depth, s_index)
    ...
    for column in range(2):
        ...
        pivot, _, normalized = verify_apg_ray(
            ...
            source_ray,
            frozen_apg_chart(coordinates, 1, column),
            frozen_apg_sign(coordinates, 1, column),
            delta,
    ...
    for column in range(2):
        ...
        pivot, _, normalized = verify_apg_ray(
            ...
            raw_column,
            frozen_apg_chart(coordinates, 2, column),
            frozen_apg_sign(coordinates, 2, column),
            delta,
    ...
    if computed["APG_RESCUE"]:
        ...
        for candidate, key in (
            ...
            if computed[key]:
                method = candidate
                break
    print("VERIFICATION_SCHEMA=sounio.cs6.affine-projective-cocycle-leaf-verification.v1")
