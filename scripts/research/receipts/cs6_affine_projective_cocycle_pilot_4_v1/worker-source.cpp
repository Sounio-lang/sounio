...
RayChart frozen_chart(const FrozenInput& input, int event, int column) {
  ...
  if (event != 1 || column < 0 || column > 1) {
    throw std::runtime_error("invalid frozen chart request");
  }
  const bool plus_second_ray =
      (input.u_depth == 15 && input.u_index == 8192 &&
       input.s_depth == 15 && input.s_index == 24576) ||
      (input.u_depth == 16 && input.u_index == 16384 &&
       input.s_depth == 12 && input.s_index == 3072);
  return column == 1 && plus_second_ray ? RayChart::kPlus : RayChart::kX;
}
...
int expected_pivot_sign(const FrozenInput& input, int event, int column) {
  ...
  return frozen_chart(input, event, column) == RayChart::kPlus ? 1 : -1;
}
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::cout
        << "SCHEMA=sounio.cs6.affine-projective-cocycle-leaf.v1\n"
        ...
        << "APG_CHART_POLICY=FOUR_LEAF_PREDECLARED_FIXED_CHARTS\n"
        ...
        << "EXECUTION_SCOPE=FOUR_FROZEN_DYADIC_LEAF_HAPG_CAPD_CPU_PILOT\n"
