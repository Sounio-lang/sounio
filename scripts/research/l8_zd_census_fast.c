/*
 * l8_zd_census_fast.c — fast exact level-8 (256-dim) canonical zero-divisor
 * census with exact independent verification.
 *
 * Companion to:
 *   docs/research/l8_zd_census_benchmark_spec_2026-07-26.md
 *   scripts/research/l8_zd_baseline_benchmark.py  (Python baseline to beat)
 *   scripts/ci/l8_zd_census_gate.sh               (CI gate)
 *
 * Two exact methods, sharing only the Cayley-Dickson sign table S:
 *
 *   Method 1 (census, the audited 2-cycle criterion of
 *   scripts/research/routon_zd_contract.py): for 1 <= i < j < 256,
 ...
 *
 *   Method 2 (verifier, generic exact linear algebra): build the 256x256
 ...
 *
 * Build: cc -O2 -o l8_zd_census_fast l8_zd_census_fast.c
 */
...
int main(void) {
    ...
    build_sign_table();
    double t1 = now_sec();
    printf("L8_FAST_SIGN_TABLE seconds=%.6f fnv1a=%016llx\n",
     ...
     * t odd, 1 <= t <= 2^(m-3)-1 (32 t-values max at m = 8). */
    ...
    double t2 = now_sec();
    /* census law prediction Z(8) = 4^8 - 23*2^8 + 2^7 - 4 = 59772 triples */
    uint64_t law = 59772;
    uint64_t triples = 2 * zd_pairs;
    int census_ok = (triples == law);
    printf("L8_FAST_CENSUS seconds=%.6f index_pairs=%llu triples=%llu "
           "law_Z8=%llu census_ok=%d\n",
           t2 - t1, (unsigned long long)zd_pairs, (unsigned long long)triples,
           (unsigned long long)law, census_ok);
    printf("L8_FAST_HISTOGRAM");
    for (int v = 0; v <= N / 2; v++)
        if (hist[v]) printf(" %d:%llu", v, (unsigned long long)hist[v]);
    printf("\n");

    /* Fiber laws: labels are exactly {l in [8, 256) : l not a power of 2}
     * (F(8) = 2^8 - 8 - 5 = 243); an m-born fiber has size
     * 2^8 - 2^(8-m+2) index pairs... note: fiber_size here counts index
    ...
    for (int l = 1; l < N; l++) {
        ...
        if (expect_fiber) {
            ...
            if (fiber_size[l] != want) {
                fibers_ok = 0;
                printf("L8_FAST_FIBER_MISMATCH label=%d size=%llu want=%llu\n",
                       l, (unsigned long long)fiber_size[l],
                       (unsigned long long)want);
            }
        } else if (fiber_size[l] != 0) {
            fibers_ok = 0;
            printf("L8_FAST_FIBER_UNEXPECTED label=%d size=%llu\n",
                   l, (unsigned long long)fiber_size[l]);
        }
    }
    ...
    int max_ok = (max_nullity == (N / 2) - 4); /* 2^(b-1) - 4 = 124 */
    printf("L8_FAST_FIBERS count=%llu law_F8=243 size_law_ok=%d\n",
           (unsigned long long)n_fibers, fibers_ok);
    printf("L8_FAST_NULLITY_LAW odd_part_law=%d completeness=%d "
           "max_nullity=%d want=124 max_ok=%d\n",
        ...
        for (int j = i + 1; j < N; j++) {
            for (int sgn = -1; sgn <= 1; sgn += 2) {
                ...
                if (nullity_ge != (int)nullity_tab[i][j]) {
                    if (mismatches < 10)
                        printf("L8_FAST_VERIFY_MISMATCH i=%d j=%d sgn=%d "
                               "census=%d ge=%d\n",
    ...
    double t3 = now_sec();
    printf("L8_FAST_VERIFY seconds=%.6f pair_signs_checked=%llu mismatches=%llu\n",
           t3 - t2, (unsigned long long)checked, (unsigned long long)mismatches);

    printf("L8_FAST_TOTAL seconds=%.6f\n", t3 - t0);
    int pass = census_ok && mismatches == 0 && fibers_ok && n_fibers == 243 &&
               nullity_law_ok && completeness_ok && max_ok;
    printf("L8_ZD_FAST_VERDICT %s\n", pass ? "PASS" : "FAIL");
    return pass ? 0 : 1;
}
