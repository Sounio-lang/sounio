...
WAVE6_VALIDATOR = ROOT_DIR / "scripts" / "research" / "validate_hypercomplex_wave6.py"

REQUIRED_WAVE7_TOUCHPOINTS = {
    "wave7_hyper_expr_law_profile_compile_proof_harness",
}
REQUIRED_WAVE7_AUDITS = {
    "ir.hyper_expr_law_profile_compile_proof_audit",
}
REQUIRED_WAVE7_SCAFFOLDING = {
    "wave7_hyper_expr_law_profile_compile_proof_harness",
}
...
def load_wave6_module():
    spec = importlib.util.spec_from_file_location("hypercomplex_wave6", WAVE6_VALIDATOR)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"unable to load wave6 validator from {WAVE6_VALIDATOR}")
...
def compile_fixture(souc_bin: str, fixture_rel: str) -> dict:
    ...
    out_path = Path("/tmp") / (fixture_path.stem + ".wave7.elf")
    log_path = Path("/tmp") / (fixture_path.stem + ".wave7.log")
    env = dict(os.environ)
...
def validate() -> tuple[dict, int]:
    ...
    wave6_module = load_wave6_module()
    wave6_report, wave6_code = wave6_module.validate()
    if wave6_code != 0:
        errors.append("wave6 hypercomplex validation failed")
    ...
    if errors:
        report = {
            "schema": "sounio.research.hypercomplex.validation_report.v7",
            "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat().replace("+00:00", "Z"),
            "status": "fail",
            "error_count": len(errors),
            "errors": errors,
            "wave6_status": wave6_report.get("status", "fail"),
        }
        return report, 1
    ...
    check(
        any("expression-level hyper law profiles" in item for item in prototype_safe_contract if isinstance(item, str)),
        "prototype_safe_contract must mention expression-level hyper law profiles",
        errors,
    )

    missing_touchpoints = sorted(REQUIRED_WAVE7_TOUCHPOINTS - touchpoint_ids)
    check(not missing_touchpoints, f"missing required wave7 touchpoint entries: {missing_touchpoints}", errors)

    missing_audits = sorted(REQUIRED_WAVE7_AUDITS - set(audit_by_id.keys()))
    check(not missing_audits, f"missing required wave7 compiler audit entries: {missing_audits}", errors)

    for entry_id in REQUIRED_WAVE7_TOUCHPOINTS:
    ...
    for entry_id in REQUIRED_WAVE7_AUDITS:
    ...
    for entry_id in REQUIRED_WAVE7_SCAFFOLDING:
    ...
    if isinstance(validation_contract, dict):
        ...
        check(
            isinstance(notes, str) and "Wave 7 expression-level law profiles remain advisory metadata only" in notes,
            "validation_contract notes must keep Wave 7 advisory law-profile note",
            errors,
        )

    inventory_entry = inventory_by_id.get("ir.hyper_metadata_and_opcode_surface", {})
    check(
        "hyper-expression law profiles" in inventory_entry.get("summary", ""),
        "ir.hyper_metadata_and_opcode_surface summary must mention hyper-expression law profiles",
        errors,
    )

    boundary_touchpoint = touchpoint_by_id.get("internal_ir_annotations.boundary", {})
    check(
        "per-expression law profiles" in boundary_touchpoint.get("summary", ""),
        "internal_ir_annotations.boundary summary must mention per-expression law profiles",
        errors,
    )
    ...
    check(
        any(
            isinstance(ref, dict)
            and ref.get("path") == "self-hosted/check/check.sio"
            and ref.get("symbol") == "fn record_hyper_expr("
            for ref in assoc_refs
        ),
        "associativity_sensitive_rewrites must reference fn record_hyper_expr(",
        errors,
    )
    ...
    check(
        any(
            ...
            and ref.get("symbol") == "write_hyper_expr_info"
            for ref in dist_refs
        ),
        "distributivity_sensitive_rewrites must reference write_hyper_expr_info",
        errors,
    )
        ...
        if souc_bin:
            for fixture_path in unique_fixture_paths:
                ...
                if result["status"] != "pass":
                    errors.append(f"Wave 7 compile-proof fixture failed: {fixture_path}")

    readme_text = readme_path.read_text(encoding="utf-8")
    check("hypercomplex_wave7_gate.sh" in readme_text, "README must mention hypercomplex_wave7_gate.sh", errors)
    check("hyper-expression law profile" in readme_text, "README must describe the Wave 7 hyper-expression law-profile seam", errors)
    ...
    report = {
        ...
        "counts": {
            "wave7_touchpoints": len(REQUIRED_WAVE7_TOUCHPOINTS),
            "wave7_audits": len(REQUIRED_WAVE7_AUDITS),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Wave 7 hypercomplex research scaffolding.")
    parser.add_argument(
        "--out-json",
        default=str(ROOT_DIR / "artifacts" / "research" / "hypercomplex_wave7_validation.v1.json"),
    ...
    print(
        "hypercomplex-wave7 validation passed: "
        f"{report['counts']['wave7_touchpoints']} wave7 touchpoint, "
        f"{report['counts']['wave7_audits']} wave7 audit, "
