...
WAVE7_VALIDATOR = ROOT_DIR / "scripts" / "research" / "validate_hypercomplex_wave7.py"
WAVE8_DIFF_SELFTEST = ROOT_DIR / "scripts" / "research" / "hypercomplex_wave8_diff_selftest.sh"

REQUIRED_WAVE8_TOUCHPOINTS = {
    "wave8_law_profile_diff_selftest_harness",
}
REQUIRED_WAVE8_AUDITS = {
    "ir.hyper_expr_profile_source_diff_audit",
}
REQUIRED_WAVE8_SCAFFOLDING = {
    "wave8_law_profile_diff_runtime_harness",
}
...
def load_wave7_module():
    spec = importlib.util.spec_from_file_location("hypercomplex_wave7", WAVE7_VALIDATOR)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"unable to load wave7 validator from {WAVE7_VALIDATOR}")
...
def run_baseline_comparison(script_path: Path) -> dict:
    log_path = Path("/tmp") / "hypercomplex_wave8_diff_selftest.validation.log"
    proc = subprocess.run(
...
def validate() -> tuple[dict, int]:
    ...
    wave7_module = load_wave7_module()
    wave7_report, wave7_code = wave7_module.validate()
    if wave7_code != 0:
        errors.append("wave7 hypercomplex validation failed")
    ...
    check(WAVE8_DIFF_SELFTEST.exists(), f"missing differential self-test script: {WAVE8_DIFF_SELFTEST.relative_to(ROOT_DIR)}", errors)

    if errors:
        report = {
            "schema": "sounio.research.hypercomplex.validation_report.v8",
            "generated_at_utc": dt.datetime.now(dt.timezone.utc).isoformat().replace("+00:00", "Z"),
            "status": "fail",
            "error_count": len(errors),
            "errors": errors,
            "wave7_status": wave7_report.get("status", "fail"),
        }
        return report, 1
    ...
    check(
        any("law-profile source tags" in item for item in prototype_safe_contract if isinstance(item, str)),
        "prototype_safe_contract must mention law-profile source tags",
        errors,
    )

    missing_touchpoints = sorted(REQUIRED_WAVE8_TOUCHPOINTS - touchpoint_ids)
    check(not missing_touchpoints, f"missing required wave8 touchpoint entries: {missing_touchpoints}", errors)

    missing_audits = sorted(REQUIRED_WAVE8_AUDITS - set(audit_by_id.keys()))
    check(not missing_audits, f"missing required wave8 compiler audit entries: {missing_audits}", errors)

    for entry_id in REQUIRED_WAVE8_TOUCHPOINTS:
    ...
    for entry_id in REQUIRED_WAVE8_AUDITS:
    ...
    for entry_id in REQUIRED_WAVE8_SCAFFOLDING:
    ...
    if isinstance(validation_contract, dict):
        ...
        check(
            isinstance(notes, str) and "Wave 8 law-profile source tags remain observability metadata only" in notes,
            "validation_contract notes must keep Wave 8 advisory source-tag note",
            errors,
        )

    inventory_entry = inventory_by_id.get("ir.hyper_metadata_and_opcode_surface", {})
    check(
        "law-profile source tags" in inventory_entry.get("summary", ""),
        "ir.hyper_metadata_and_opcode_surface summary must mention law-profile source tags",
        errors,
    )

    boundary_touchpoint = touchpoint_by_id.get("internal_ir_annotations.boundary", {})
    check(
        "law-profile source tags" in boundary_touchpoint.get("summary", ""),
        "internal_ir_annotations.boundary summary must mention law-profile source tags",
        errors,
    )
    ...
    check(
        any(
            isinstance(ref, dict)
            and ref.get("path") == "self-hosted/ir/ir.sio"
            and ref.get("symbol") == "IR_HYPER_LAW_PROFILE_SOURCE_REGISTRY"
            for ref in assoc_refs
        ),
        "associativity_sensitive_rewrites must reference IR_HYPER_LAW_PROFILE_SOURCE_REGISTRY",
        errors,
    )
    ...
    check(
        any(
            ...
            and ref.get("symbol") == "info.law_profile_source"
            for ref in dist_refs
        ),
        "distributivity_sensitive_rewrites must reference info.law_profile_source",
        errors,
    )

    baseline_comparison_result = {}
    if not errors:
        baseline_comparison_result = run_baseline_comparison(WAVE8_DIFF_SELFTEST)
        if baseline_comparison_result["status"] != "pass":
            errors.append("Wave 8 baseline comparison failed")

    readme_text = readme_path.read_text(encoding="utf-8")
    check("hypercomplex_wave8_gate.sh" in readme_text, "README must mention hypercomplex_wave8_gate.sh", errors)
    check("hypercomplex_wave8_diff_selftest.sh" in readme_text, "README must mention hypercomplex_wave8_diff_selftest.sh", errors)
    check("registry-vs-fallback" in readme_text, "README must describe the Wave 8 registry-vs-fallback differential path", errors)
    ...
    report = {
        ...
        "counts": {
            "wave8_touchpoints": len(REQUIRED_WAVE8_TOUCHPOINTS),
            "wave8_audits": len(REQUIRED_WAVE8_AUDITS),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Wave 8 hypercomplex research scaffolding.")
    parser.add_argument(
        "--out-json",
        default=str(ROOT_DIR / "artifacts" / "research" / "hypercomplex_wave8_validation.v1.json"),
    ...
    print(
        "hypercomplex-wave8 validation passed: "
        f"{report['counts']['wave8_touchpoints']} wave8 touchpoint, "
        f"{report['counts']['wave8_audits']} wave8 audit, "
