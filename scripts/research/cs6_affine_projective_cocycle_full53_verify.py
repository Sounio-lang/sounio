#!/usr/bin/env python3
"""Exact verifier for one manifest-bound CS6 H-APG full53 leaf."""
...
FULL53_MANIFEST_HEADERS: tuple[tuple[str, str | None], ...] = (
    ...
    ("PREDECLARED_IN_COMMIT", "9dcf1fca964d7e54e1109f9210689809666b2a54"),
    (
    ...
    (
        ...
        "21f0ca224fc79d1cfc815f203100e091bf27dbb070cb3a2c69f6056b7a6fee27",
    ),
    (
        "PARENT_FILES_INDEX_SHA256",
        "740ae39070ff90604048c37d25e2f0de954be9ea55e020a529e094699a82802d",
    ),
    (
        "PARENT_COORDINATES_SHA256",
        "6169dd7705ca4f01180f65d13d620320845684f0a9fc28411c881cfae7e4f2d8",
    ),
    (
        "PARENT_LEAVES_SHA256",
        "98c639a616ac640f1983209530f8fe30d769a0e4b0136665201c9bc57320e37f",
    ),
    (
        "PARENT_AFFINE_OBLIGATION_CANONICALIZATION",
        "SORTED_LEAF_ID_UTF8_LF_WITH_FINAL_LF",
    ),
    (
    ...
    (
        "ACCEPTANCE",
        "53_ATTEMPTED_52_PAIRED_VALID_NO_NEW_UNRESOLVED_ZERO_AFFINE_LOSS_AT_LEAST_ONE_RESCUE_EXACT_REPLAY_ALL_MUTATIONS_REJECTED",
    ),
    (
        "LEAF_CHALLENGE_BINDING",
        "PARENT_INPUT_SHA256_PARENT_RECEIPT_SHA256_CHART_SIGN_TUPLE_MANIFEST_SHA256",
    ),
...
FULL53_CHARTS = frozenset({"X", "Y", "PLUS", "MINUS"})
...
class Full53LeafContract:
    ...
    def chart_sign(self, event: int, column: int) -> tuple[str, int]:
        if event not in (1, 2) or column not in (0, 1):
            fail("invalid full53 chart/sign lookup")
        return self.chart_signs[(event - 1) * 2 + column]

    @property
    def ordered_chart_sign_bytes(self) -> bytes:
        fields: list[str] = []
        for chart, sign in self.chart_signs:
            fields.extend((chart, str(sign)))
        return ("\t".join(fields) + "\n").encode("ascii")
...
def read_full53_contract(path: Path, leaf_input: LeafInput) -> Full53LeafContract:
    try:
        raw = path.read_bytes()
        text = raw.decode("ascii")
    except UnicodeError as error:
        raise VerificationError("full53 coordinate manifest must be ASCII") from error
    except OSError as error:
        raise VerificationError(f"cannot read full53 coordinate manifest: {path}") from error
    if b"\x00" in raw or b"\r" in raw or not raw.endswith(b"\n"):
        fail("full53 coordinate manifest must use canonical LF-terminated ASCII")

    manifest_sha256 = hashlib.sha256(raw).hexdigest()
    if manifest_sha256 != FULL53_FROZEN_MANIFEST_SHA256:
        fail("full53 coordinate manifest differs from the frozen contract")
    lines = text[:-1].split("\n")
    header_count = len(FULL53_MANIFEST_HEADERS)
    if len(lines) != header_count + 1 + 53:
        fail("full53 coordinate manifest has the wrong number of lines")

    headers: dict[str, str] = {}
    for line, (expected_key, expected_value) in zip(
        lines[:header_count], FULL53_MANIFEST_HEADERS, strict=True
    ):
        if line.count("=") != 1:
            fail(f"malformed full53 manifest header: {expected_key}")
        key, value = line.split("=", 1)
        if key != expected_key or not value:
            fail(f"noncanonical full53 manifest header: {expected_key}")
        if expected_value is not None and value != expected_value:
            fail(f"full53 manifest contract mismatch: {expected_key}")
        headers[key] = value

    if COMMIT_RE.fullmatch(headers["PREDECLARED_IN_COMMIT"]) is None:
        fail("full53 predeclaration commit is not canonical")
    for key in (
        "PREDECLARATION_REPORT_SHA256",
        "PARENT_RUN_MANIFEST_SHA256",
        "PARENT_FILES_INDEX_SHA256",
        ...
        if SHA_RE.fullmatch(headers[key]) is None:
            fail(f"full53 manifest digest is not canonical: {key}")
    if lines[header_count] != "\t".join(FULL53_ROW_FIELDS):
        fail("full53 coordinate manifest row schema mismatch")

    rows: dict[str, Full53LeafContract] = {}
    ordered_leaf_ids: list[str] = []
    paired_count = 0
    affine_count = 0
    for line in lines[header_count + 1 :]:
        tokens = line.split("\t")
        if len(tokens) != len(FULL53_ROW_FIELDS) or any(not token for token in tokens):
            fail("malformed full53 coordinate manifest row")
        fields = dict(zip(FULL53_ROW_FIELDS, tokens, strict=True))
        integer_values: dict[str, int] = {}
        for key in ("U_DEPTH", "U_INDEX", "S_DEPTH", "S_INDEX"):
            if CANONICAL_INT_RE.fullmatch(fields[key]) is None:
                fail(f"noncanonical full53 manifest integer: {key}")
            integer_values[key] = int(fields[key])
        u_depth = integer_values["U_DEPTH"]
        u_index = integer_values["U_INDEX"]
        s_depth = integer_values["S_DEPTH"]
        s_index = integer_values["S_INDEX"]
        if u_depth > 30 or s_depth > 30:
            fail("full53 manifest depth exceeds worker contract")
        if not (0 <= u_index < 1 << u_depth and 0 <= s_index < 1 << s_depth):
            fail("full53 manifest index is outside its dyadic depth")
        leaf_id = canonical_leaf_id(u_depth, u_index, s_depth, s_index)
        if fields["LEAF_ID"] != leaf_id or leaf_id in rows:
            fail("noncanonical or duplicate full53 leaf ID")
        for key in ("PARENT_INPUT_SHA256", "PARENT_RECEIPT_SHA256"):
            if SHA_RE.fullmatch(fields[key]) is None:
                fail(f"full53 row digest is not canonical: {key}")

        chart_signs: list[tuple[str, int]] = []
        for prefix in ("E1_R0", "E1_R1", "E2_R0", "E2_R1"):
            sign_token = fields[f"{prefix}_SIGN"]
            if sign_token not in {"-1", "0", "1"}:
                fail(f"invalid full53 pivot sign: {prefix}")
            chart_signs.append((fields[f"{prefix}_CHART"], int(sign_token)))

        if leaf_id == "U00-0000000000_S00-0000000000":
            if not (
                fields["PARENT_STATUS"] == "COMPUTATION_UNRESOLVED_INTERVAL_DOMAIN"
                and fields["PARENT_RECEIPT_SHA256"] == EMPTY_SHA256
                and all(pair == ("NONE", 0) for pair in chart_signs)
            ):
                fail("full53 root row violates the frozen unresolved contract")
        else:
            if fields["PARENT_STATUS"] not in {
                "PROBE_VALID_CERTIFIED",
                "PROBE_VALID_UNCERTIFIED",
            }:
                fail("full53 paired row has an invalid parent status")
            if fields["PARENT_RECEIPT_SHA256"] == EMPTY_SHA256:
                fail("full53 paired row binds an empty parent receipt")
            if any(
                chart not in FULL53_CHARTS or sign not in {-1, 1}
                for chart, sign in chart_signs
            ):
                fail("full53 paired row has an invalid chart/sign tuple")
            paired_count += 1
            affine_count += fields["PARENT_STATUS"] == "PROBE_VALID_CERTIFIED"

        rows[leaf_id] = Full53LeafContract(
            leaf_id=leaf_id,
            u_depth=u_depth,
            u_index=u_index,
            s_depth=s_depth,
            s_index=s_index,
            parent_input_sha256=fields["PARENT_INPUT_SHA256"],
            parent_status=fields["PARENT_STATUS"],
            parent_receipt_sha256=fields["PARENT_RECEIPT_SHA256"],
            chart_signs=tuple(chart_signs),
            manifest_sha256=manifest_sha256,
        )
        ordered_leaf_ids.append(leaf_id)

    if ordered_leaf_ids != sorted(ordered_leaf_ids):
        fail("full53 coordinate manifest rows are not sorted by leaf ID")
    if paired_count != 52 or affine_count != 28:
        fail("full53 coordinate manifest population counts drifted")
    ...
    contract = rows.get(identity)
    if contract is None:
        fail("canonical input is absent from the full53 coordinate manifest")
    if contract.parent_input_sha256 != leaf_input.sha256:
        fail("canonical input digest differs from the full53 parent binding")
    if contract.parent_status == "COMPUTATION_UNRESOLVED_INTERVAL_DOMAIN":
        fail("the unresolved full53 root is outside the paired leaf verifier")
    return contract
...
def full53_leaf_challenge(root_challenge: str, contract: Full53LeafContract) -> str:
    if SHA_RE.fullmatch(root_challenge) is None:
        fail("root challenge must be lowercase SHA-256")
    preimage = (
        FULL53_CHALLENGE_DOMAIN
        + bytes.fromhex(root_challenge)
        + b"\0"
        + contract.leaf_id.encode("ascii")
        + b"\0"
        + bytes.fromhex(contract.parent_input_sha256)
        + b"\0"
        + bytes.fromhex(contract.parent_receipt_sha256)
        + b"\0"
        + contract.ordered_chart_sign_bytes
        + b"\0"
        + bytes.fromhex(contract.manifest_sha256)
    )
...
TM2_COEFFICIENT_NAMES = ("C00", "C10", "C01", "C20", "C11", "C02")
TM2_EXPONENTS = ((0, 0), (1, 0), (0, 1), (2, 0), (1, 1), (0, 2))


def tm2_keys(prefix: str) -> list[str]:
    return [f"{prefix}{name}" for name in TM2_COEFFICIENT_NAMES] + [
        f"{prefix}R",
        f"{prefix}POLY",
        f"{prefix}HULL",
    ]
...
FIXED_HEADERS: tuple[tuple[str, str | None], ...] = (
    ...
    ("COORDINATE_MANIFEST_SHA256", None),
    ("RUN_CHALLENGE", None),
    ...
    ("BOXED_CONTROL_TANGENT_DEPENDENCY", "INTERVAL_RAYS_AT_EVENT1"),
    ("APG_CARRIER", "SHARED_SOURCE_TM2_LOCAL_C2_PROJECTIVE_COMPOSITION"),
    (
        "TM2_MONOMIAL_ORDER",
        "ONE_DELTA0_DELTA1_DELTA0_SQUARED_DELTA0_DELTA1_DELTA1_SQUARED",
    ),
    ("TM2_MULTIPLICATION", "TOTAL_DEGREE_TWO_PLUS_INTERVAL_REMAINDER"),
    ("TM2_RECIPROCAL", "CONTRACTIVE_QUADRATIC_GEOMETRIC_IDENTITY"),
    ("APG_CHART_POLICY", "FIXED_PER_LEAF_NO_RUNTIME_RESELECTION"),
    ...
    ("APG_PRIMARY_CHANNEL", "FACTORED_SIGNED_SCALE_EXTERIOR_TM2"),
    ...
    (
        "EXECUTION_SCOPE",
        "FIFTY_TWO_PARENT_COMPUTABLE_DYADIC_LEAF_HAPG_CAPD_CPU_CORPUS",
    ),
...
APG_LOCAL_C2_KEYS = (
    C2_KEYS[:-1] + ["POST_TIME"] + vector_keys("POST_X") + ["NU"]
)
...
SUMMARY_KEYS = [
    ...
    "APG_LOCAL_IMPACT_TIME_CROSSCHECK",
    "APG_LOCAL_CENTER_FULL_DP_OVERLAP",
    "APG_LOCAL_INPUT_CONTAINMENT",
    "APG_EVENT1_RAY_RECONSTRUCTION",
    "APG_EVENT2_RAY_RECONSTRUCTION",
    "APG_LOCAL_EVENT_ORDER_CERTIFIED",
    "APG_LOCAL_POSTSECTION_PLUS_SIDE",
    "APG_LOCAL_TRANSVERSALITY_CERTIFIED",
    "APG_JOINT_OVERLAP",
    "APG_PRIMARY_POSITIVE_WIDTH",
    "APG_STRICTLY_NARROWER_THAN_BOXED",
    "APG_STRICTLY_NARROWER_THAN_AFFINE",
    "APG_STRICTLY_NARROWER_THAN_SHARED",
    "APG_ORIENTATION_CERTIFIED",
    "APG_NEGATIVE_ORIENTATION_CERTIFIED",
...
APG_RAY_KEYS = (
    ["CHART", "EXPECTED_SIGN", "PIVOT_SIGN_CERTIFIED"]
    + tm2_keys("P_")
    + tm2_keys("Q_")
    + tm2_keys("S_")
    + sum((tm2_keys(f"N{row}_") for row in range(3)), [])
)
...
RECORD_SPECS: tuple[tuple[str, Sequence[str], set[str]], ...] = (
    ...
    (
        "APG_EVENT1_STATE_TM2",
        sum((tm2_keys(f"X{row}_") for row in range(3)), [])
        + vector_keys("INPUT_PADDING")
        + vector_keys("INPUT_RESIDUAL")
        + vector_keys("LOCAL_INPUT_HULL"),
        set(),
    ),
    ("APG_LOCAL_C2_FULL", APG_LOCAL_C2_KEYS, set()),
    ("APG_LOCAL_C2_CENTER", APG_LOCAL_C2_KEYS, set()),
    (
        "APG_LOCAL_JACOBIAN_TM2",
        sum(
            (
                tm2_keys(f"J{row}{column}_")
                for row in range(3)
    ...
    *tuple(
        (marker, APG_RAY_KEYS, {"PIVOT_SIGN_CERTIFIED"})
        for marker in ("APG_EVENT1_RAY0", "APG_EVENT1_RAY1")
    ),
    (
        "APG_EVENT2_RAW_TM2",
        sum(
            (
                tm2_keys(f"V{row}{column}_")
    ...
    *tuple(
        (marker, APG_RAY_KEYS, {"PIVOT_SIGN_CERTIFIED"})
        for marker in ("APG_EVENT2_RAY0", "APG_EVENT2_RAY1")
    ),
    *tuple(
        (
            marker,
        ...
        for marker in ("APG_SHARED_COMPOSITION_TM2", "APG_RECONSTRUCTED_TM2")
    ),
    (
        "APG_FACTORED_EXTERIOR_TM2",
        tm2_keys("SCALE0_")
        + tm2_keys("SCALE1_")
        + tm2_keys("EXTERIOR_")
        + tm2_keys("DET_")
...
class Ledger:
    headers: dict[str, str]
    records: dict[str, dict[str, Interval | bool | str | int]]
    receipt_sha256: str
    physical_sha256: str
...
def parse_ledger(path: Path) -> Ledger:
    ...
    for line, (marker, expected_keys, bool_keys) in zip(lines[offset:], RECORD_SPECS):
        ...
        values: dict[str, Interval | bool | str | int] = {}
        for token, expected_key in zip(tokens[1:], expected_keys):
            ...
            if key in bool_keys:
                ...
                values[key] = value
            elif key == "EXPECTED_SIGN":
                if value not in {"-1", "1"}:
                    fail(f"invalid expected pivot sign in {marker}")
...
def string_value(record: Mapping[str, Interval | bool | str | int], key: str) -> str:
    value = record[key]
...
def integer_value(record: Mapping[str, Interval | bool | str | int], key: str) -> int:
    value = record[key]
    if not isinstance(value, int) or isinstance(value, bool):
        fail(f"expected integer: {key}")
    return value
...
@dataclass(frozen=True)
class Taylor2:
    coefficient: tuple[Interval, Interval, Interval, Interval, Interval, Interval]
    remainder: Interval


def tm2_constant(value: Interval) -> Taylor2:
    return Taylor2((value, ZERO, ZERO, ZERO, ZERO, ZERO), ZERO)


def tm2_affine(
    center: Interval, coefficient0: Interval, coefficient1: Interval, remainder: Interval
) -> Taylor2:
    return Taylor2((center, coefficient0, coefficient1, ZERO, ZERO, ZERO), remainder)


def tm2_interval_power(value: Interval, exponent: int) -> Interval:
    if exponent == 0:
        return ONE
    if exponent == 1:
        return value
    if exponent == 2:
        return centered_square(value)
    if exponent == 3:
        return centered_square(value) * value
    if exponent == 4:
        return centered_square(centered_square(value))
    fail("TM2 exponent exceeds multiplication bound")


def tm2_monomial_range(delta: Sequence[Interval], exponents: tuple[int, int]) -> Interval:
    return tm2_interval_power(delta[0], exponents[0]) * tm2_interval_power(
        delta[1], exponents[1]
    )


def tm2_polynomial_range(value: Taylor2, delta: Sequence[Interval]) -> Interval:
    return sum_intervals(
        coefficient * tm2_monomial_range(delta, exponents)
        for coefficient, exponents in zip(value.coefficient, TM2_EXPONENTS, strict=True)
    )


def tm2_range(value: Taylor2, delta: Sequence[Interval]) -> Interval:
    return tm2_polynomial_range(value, delta) + value.remainder


def tm2_add(left: Taylor2, right: Taylor2) -> Taylor2:
    return Taylor2(
        tuple(
            a + b for a, b in zip(left.coefficient, right.coefficient, strict=True)
        ),  # type: ignore[arg-type]
        left.remainder + right.remainder,
    )


def tm2_negate(value: Taylor2) -> Taylor2:
    return Taylor2(
        tuple(-coefficient for coefficient in value.coefficient),  # type: ignore[arg-type]
        -value.remainder,
    )


def tm2_subtract(left: Taylor2, right: Taylor2) -> Taylor2:
    return tm2_add(left, tm2_negate(right))


def tm2_scale(value: Taylor2, scale: Interval) -> Taylor2:
    return Taylor2(
        tuple(coefficient * scale for coefficient in value.coefficient),  # type: ignore[arg-type]
        value.remainder * scale,
    )


def tm2_multiply(left: Taylor2, right: Taylor2, delta: Sequence[Interval]) -> Taylor2:
    coefficient = [ZERO for _ in range(6)]
    discarded = ZERO
    exponent_index = {exponents: index for index, exponents in enumerate(TM2_EXPONENTS)}
    for left_index, left_exponents in enumerate(TM2_EXPONENTS):
        for right_index, right_exponents in enumerate(TM2_EXPONENTS):
            exponents = (
                left_exponents[0] + right_exponents[0],
                left_exponents[1] + right_exponents[1],
            )
            product = left.coefficient[left_index] * right.coefficient[right_index]
            target = exponent_index.get(exponents)
            if target is None:
                discarded = discarded + product * tm2_monomial_range(delta, exponents)
            else:
                coefficient[target] = coefficient[target] + product
    remainder = (
        discarded
        + tm2_polynomial_range(left, delta) * right.remainder
        + left.remainder * tm2_polynomial_range(right, delta)
        + left.remainder * right.remainder
    )
    return Taylor2(tuple(coefficient), remainder)  # type: ignore[arg-type]


def tm2_reciprocal(denominator: Taylor2, delta: Sequence[Interval]) -> Taylor2:
    denominator_range = tm2_range(denominator, delta)
    if denominator_range.contains_zero() or denominator.coefficient[0].contains_zero():
        fail("TM2 reciprocal denominator contains zero")
    center = denominator.coefficient[0]
    z = tm2_scale(
        tm2_subtract(denominator, tm2_constant(center)), ONE / center
    )
    z_range = tm2_range(z, delta)
    if max(abs(z_range.lower), abs(z_range.upper)) >= 1:
        fail("TM2 reciprocal is noncontractive")
    z_squared = tm2_multiply(z, z, delta)
    result = tm2_add(tm2_subtract(tm2_constant(ONE), z), z_squared)
    exact_tail = -(z_range * z_range * z_range) / (ONE + z_range)
    result = Taylor2(result.coefficient, result.remainder + exact_tail)
    return tm2_scale(result, ONE / center)


def tm2_divide(
    numerator: Taylor2, denominator: Taylor2, delta: Sequence[Interval]
) -> Taylor2:
    return tm2_multiply(numerator, tm2_reciprocal(denominator, delta), delta)


def tm2_from_record(
    record: Mapping[str, Interval | bool | str | int],
    prefix: str,
    delta: Sequence[Interval],
) -> Taylor2:
    value = Taylor2(
        tuple(interval(record, f"{prefix}{name}") for name in TM2_COEFFICIENT_NAMES),  # type: ignore[arg-type]
        interval(record, f"{prefix}R"),
    )
    require_tight_contains(
        interval(record, f"{prefix}POLY"),
        tm2_polynomial_range(value, delta),
        f"{prefix} polynomial range",
        32768,
    )
    require_tight_contains(
        interval(record, f"{prefix}HULL"),
        tm2_range(value, delta),
        f"{prefix} hull",
        32768,
    )
    return value
...
def require_tm2_reconstruction(
    reported: Taylor2, calculated: Taylor2, label: str, max_ulps: int = 16777216
) -> None:
    for index, (outer, inner) in enumerate(
        zip(reported.coefficient, calculated.coefficient, strict=True)
    ):
        require_tight_contains(
            outer, inner, f"{label} coefficient {index}", max_ulps
        )
    require_tight_contains(
        reported.remainder, calculated.remainder, f"{label} remainder", max_ulps
    )
...
def verify_c2(
    record: Mapping[str, Interval | bool], label: str, require_dummy_slots: bool = True
) -> None:
...
def verify_affine(
    ledger: Ledger,
    full_marker: str = "C2_FULL_P2",
    center_marker: str = "C2_CENTER_P2",
    carrier_marker: str = "AFFINE_CARRIER",
) -> dict[str, Interval | bool]:
    source = ledger.records["SOURCE_TILE"]
    full = ledger.records[full_marker]
    center_record = ledger.records[center_marker]
    carrier = ledger.records[carrier_marker]
...
def frozen_apg_chart(
    contract: Full53LeafContract, event: int, column: int
) -> str:
    return contract.chart_sign(event, column)[0]


def frozen_apg_sign(
    contract: Full53LeafContract, event: int, column: int
) -> int:
    return contract.chart_sign(event, column)[1]


def verify_apg_ray(
    record: Mapping[str, Interval | bool | str | int],
    source: Sequence[Taylor2],
    expected_chart: str,
    expected_sign: int,
    delta: Sequence[Interval],
    label: str,
) -> tuple[Taylor2, Taylor2, list[Taylor2]]:
    if string_value(record, "CHART") != expected_chart:
        fail(f"{label} violates the frozen chart")
    if integer_value(record, "EXPECTED_SIGN") != expected_sign:
        fail(f"{label} expected sign mismatch")

    if expected_chart == "X":
        expected_pivot, expected_numerator = source[0], source[1]
    elif expected_chart == "Y":
        expected_pivot, expected_numerator = source[1], source[0]
    elif expected_chart == "PLUS":
        expected_pivot = tm2_add(source[0], source[1])
        expected_numerator = tm2_subtract(source[1], source[0])
    elif expected_chart == "MINUS":
        expected_pivot = tm2_subtract(source[0], source[1])
        expected_numerator = tm2_add(source[0], source[1])
    else:
        fail(f"unsupported APG chart: {expected_chart}")

    pivot = tm2_from_record(record, "P_", delta)
    numerator = tm2_from_record(record, "Q_", delta)
    require_tm2_reconstruction(pivot, expected_pivot, f"{label} pivot")
    require_tm2_reconstruction(numerator, expected_numerator, f"{label} numerator")
    pivot_hull = tm2_range(pivot, delta)
    sign_certified = (
        pivot_hull.upper < 0 if expected_sign < 0 else pivot_hull.lower > 0
    )
    if boolean(record, "PIVOT_SIGN_CERTIFIED") != sign_certified or not sign_certified:
        fail(f"{label} pivot sign is not certified")

    slope = tm2_from_record(record, "S_", delta)
    require_tm2_reconstruction(
        slope, tm2_divide(numerator, pivot, delta), f"{label} slope"
    )
    if expected_chart == "X":
        expected_normalized = [tm2_constant(ONE), slope, tm2_constant(ZERO)]
    elif expected_chart == "Y":
        expected_normalized = [slope, tm2_constant(ONE), tm2_constant(ZERO)]
    elif expected_chart == "PLUS":
        expected_normalized = [
            tm2_scale(tm2_subtract(tm2_constant(ONE), slope), HALF),
            tm2_scale(tm2_add(tm2_constant(ONE), slope), HALF),
            tm2_constant(ZERO),
        ]
    else:
        expected_normalized = [
            tm2_scale(tm2_add(tm2_constant(ONE), slope), HALF),
            tm2_scale(tm2_subtract(slope, tm2_constant(ONE)), HALF),
            tm2_constant(ZERO),
        ]
    normalized = [tm2_from_record(record, f"N{row}_", delta) for row in range(3)]
    for row in range(3):
        require_tm2_reconstruction(
            normalized[row], expected_normalized[row], f"{label} normalized {row}"
        )
        if row < 2:
            reconstructed = tm2_multiply(pivot, normalized[row], delta)
            require_contains(
                tm2_range(reconstructed, delta),
                tm2_range(source[row], delta),
                f"{label} ray reconstruction {row}",
            )
        elif not (
            tm2_range(source[row], delta).contains_zero()
            and tm2_range(normalized[row], delta).contains_zero()
        ):
            fail(f"{label} leaves the Poincare-section tangent space")
    return pivot, slope, normalized
...
def verify_ledger(
    ...
    expected_contract: Full53LeafContract,
    expected_challenge: str,
) -> dict[str, bool]:
    for key, expected in (
        ("WORKER_SOURCE_SHA256", expected_source),
        ("INPUT_SHA256", expected_contract.parent_input_sha256),
        ("PARENT_RECEIPT_SHA256", expected_contract.parent_receipt_sha256),
        ("COORDINATE_MANIFEST_SHA256", expected_contract.manifest_sha256),
    ...
    affine_p1 = verify_affine(
        ledger, "C2_FULL_P1", "C2_CENTER_P1", "AFFINE_P1_CARRIER"
    )
    ...
    apg_state_record = ledger.records["APG_EVENT1_STATE_TM2"]
    apg_event1_state: list[Taylor2] = []
    for row in range(3):
        reported_state = tm2_from_record(apg_state_record, f"X{row}_", delta)
        expected_state = tm2_affine(
            event1_center[row],
            event1_basis[row][0],
            event1_basis[row][1],
            event1_residual[row],
        ...
        require_tm2_reconstruction(
            reported_state, expected_state, f"APG event1 state {row}"
        )
        apg_event1_state.append(reported_state)
    apg_local_input_hull = vector(apg_state_record, "LOCAL_INPUT_HULL")
    apg_input_padding = vector(apg_state_record, "INPUT_PADDING")
    apg_input_residual = vector(apg_state_record, "INPUT_RESIDUAL")
    apg_local_input_containment = True
    for row in range(3):
        if row == 2:
            expected_padding = ZERO
        else:
            magnitude = max(
                1.0,
                abs(float(event1_center[row].lower)),
                abs(float(event1_center[row].upper)),
            )
            radius = 64.0 * sys.float_info.epsilon * magnitude
            expected_padding = Interval(
                Fraction.from_float(-radius), Fraction.from_float(radius)
            )
        require_tight_contains(
            apg_input_padding[row],
            expected_padding,
            f"APG local input padding {row}",
            8,
        )
        require_tight_contains(
            apg_input_residual[row],
            event1_residual[row] + expected_padding,
            f"APG local input residual {row}",
            64,
        )
        state_hull = tm2_range(apg_event1_state[row], delta)
        require_contains(
            apg_local_input_hull[row], state_hull, f"APG local input {row}"
        )
        apg_local_input_containment = (
            apg_local_input_containment
            and apg_local_input_hull[row].contains(state_hull)
        )

    apg_local_full = ledger.records["APG_LOCAL_C2_FULL"]
    apg_local_center = ledger.records["APG_LOCAL_C2_CENTER"]
    verify_c2(apg_local_full, "APG local C2 full", require_dummy_slots=False)
    verify_c2(apg_local_center, "APG local C2 center", require_dummy_slots=False)
    apg_local_center_full_dp_overlap = matrix_overlap(
        matrix(apg_local_center, "DP"), matrix(apg_local_full, "DP")
    )
    if not apg_local_center_full_dp_overlap:
        fail("APG local center/full Jacobians do not overlap")
    apg_local_event_order_certified = (
        interval(apg_local_full, "TIME").lower > interval(full1, "TIME").upper
        and interval(apg_local_center, "TIME").lower
        > interval(center1, "TIME").upper
        and interval(apg_local_full, "POST_TIME").lower
        > interval(apg_local_full, "TIME").upper
        and interval(apg_local_center, "POST_TIME").lower
        > interval(apg_local_center, "TIME").upper
    )
    apg_local_postsection_plus_side = (
        vector(apg_local_full, "POST_X")[2].lower > 0
        and vector(apg_local_center, "POST_X")[2].lower > 0
    )
    apg_local_transversality_certified = (
        interval(apg_local_full, "NU").lower > 0
        and interval(apg_local_center, "NU").lower > 0
    )
    if not (
        apg_local_event_order_certified
        and apg_local_postsection_plus_side
        and apg_local_transversality_certified
    ):
        fail("APG local return order, side, or transversality failed")

    apg_local_jacobian_record = ledger.records["APG_LOCAL_JACOBIAN_TM2"]
    apg_local_jacobian = [[tm2_constant(ZERO) for _ in range(3)] for _ in range(3)]
    displacement = [
        tm2_subtract(apg_event1_state[row], tm2_constant(event1_center[row]))
        for row in range(3)
    ]
    local_center_dp = matrix(apg_local_center, "DP")
    for row in range(3):
        for column in range(3):
            reported_jacobian = tm2_from_record(
                apg_local_jacobian_record, f"J{row}{column}_", delta
            )
            midpoint_value = reported_jacobian.coefficient[0]
            if midpoint_value.width != 0:
                fail(f"APG local Jacobian midpoint is not point-valued: {row},{column}")
            require_contains(
                local_center_dp[row][column],
                midpoint_value,
                f"APG local Jacobian midpoint {row},{column}",
            )
            expected_jacobian = Taylor2(
                tm2_constant(midpoint_value).coefficient,
                local_center_dp[row][column] - midpoint_value,
            )
            for variable in range(2):
                derivative = hessian_derivative(
                    apg_local_full, row, column, variable
                )
                expected_jacobian = tm2_add(
                    expected_jacobian, tm2_scale(displacement[variable], derivative)
                )
            require_tm2_reconstruction(
                reported_jacobian,
                expected_jacobian,
                f"APG local Jacobian {row},{column}",
            )
            apg_local_jacobian[row][column] = reported_jacobian

    p1_carrier = ledger.records["AFFINE_P1_CARRIER"]
    p1_m = matrix(p1_carrier, "M")
    p1_a0 = matrix(p1_carrier, "A0")
    p1_a1 = matrix(p1_carrier, "A1")
    p1_r = matrix(p1_carrier, "R")
    apg_event1_jacobian = [
        [
            tm2_affine(p1_m[row][column], p1_a0[row][column], p1_a1[row][column], p1_r[row][column])
            for column in range(3)
        ]
        for row in range(3)
    ]
    apg_event1_pivots: list[Taylor2] = []
    apg_event1_normalized: list[list[Taylor2]] = []
    for column in range(2):
        source_ray = [apg_event1_jacobian[row][column] for row in range(3)]
        pivot, _, normalized = verify_apg_ray(
            ledger.records[f"APG_EVENT1_RAY{column}"],
            source_ray,
            frozen_apg_chart(expected_contract, 1, column),
            frozen_apg_sign(expected_contract, 1, column),
            delta,
            f"APG event1 ray {column}",
        )
        apg_event1_pivots.append(pivot)
        apg_event1_normalized.append(normalized)

    apg_event2_raw_record = ledger.records["APG_EVENT2_RAW_TM2"]
    apg_event2_raw: list[list[Taylor2]] = []
    apg_event2_pivots: list[Taylor2] = []
    apg_event2_normalized: list[list[Taylor2]] = []
    for column in range(2):
        raw_column: list[Taylor2] = []
        for row in range(3):
            reported_raw = tm2_from_record(
                apg_event2_raw_record, f"V{row}{column}_", delta
            )
            expected_raw = tm2_constant(ZERO)
            for inner in range(2):
                expected_raw = tm2_add(
                    expected_raw,
                    tm2_multiply(
                        apg_local_jacobian[row][inner],
                        apg_event1_normalized[column][inner],
                        delta,
                    ),
                )
            require_tm2_reconstruction(
                reported_raw, expected_raw, f"APG event2 raw {row},{column}"
            )
            raw_column.append(reported_raw)
        apg_event2_raw.append(raw_column)
        pivot, _, normalized = verify_apg_ray(
            ledger.records[f"APG_EVENT2_RAY{column}"],
            raw_column,
            frozen_apg_chart(expected_contract, 2, column),
            frozen_apg_sign(expected_contract, 2, column),
            delta,
            f"APG event2 ray {column}",
        )
        apg_event2_pivots.append(pivot)
        apg_event2_normalized.append(normalized)

    shared_record = ledger.records["APG_SHARED_COMPOSITION_TM2"]
    reconstructed_record = ledger.records["APG_RECONSTRUCTED_TM2"]
    shared_columns_tm: list[list[Taylor2]] = []
    reconstructed_columns_tm: list[list[Taylor2]] = []
    for column in range(2):
        shared_column: list[Taylor2] = []
        reconstructed_column: list[Taylor2] = []
        for row in range(3):
            expected_shared = tm2_constant(ZERO)
            for inner in range(2):
                expected_shared = tm2_add(
                    expected_shared,
                    tm2_multiply(
                        apg_local_jacobian[row][inner],
                        apg_event1_jacobian[inner][column],
                        delta,
                    ),
                )
            reported_shared = tm2_from_record(
                shared_record, f"W{row}{column}_", delta
            )
            require_tm2_reconstruction(
                reported_shared, expected_shared, f"APG shared column {row},{column}"
            )
            shared_column.append(reported_shared)

            expected_reconstructed_column = tm2_multiply(
                apg_event1_pivots[column], apg_event2_raw[column][row], delta
            )
            reported_reconstructed_column = tm2_from_record(
                reconstructed_record, f"W{row}{column}_", delta
            )
            require_tm2_reconstruction(
                reported_reconstructed_column,
                expected_reconstructed_column,
                f"APG reconstructed column {row},{column}",
            )
            reconstructed_column.append(reported_reconstructed_column)
        shared_columns_tm.append(shared_column)
        reconstructed_columns_tm.append(reconstructed_column)

    expected_shared_det = tm2_subtract(
        tm2_multiply(shared_columns_tm[0][0], shared_columns_tm[1][1], delta),
        tm2_multiply(shared_columns_tm[1][0], shared_columns_tm[0][1], delta),
    )
    shared_det_tm = tm2_from_record(shared_record, "DET_", delta)
    require_tm2_reconstruction(
        shared_det_tm, expected_shared_det, "APG shared determinant"
    )
    expected_reconstructed_det = tm2_subtract(
        tm2_multiply(
            reconstructed_columns_tm[0][0], reconstructed_columns_tm[1][1], delta
        ),
        tm2_multiply(
            reconstructed_columns_tm[1][0], reconstructed_columns_tm[0][1], delta
        ),
    )
    reconstructed_det_tm = tm2_from_record(reconstructed_record, "DET_", delta)
    require_tm2_reconstruction(
        reconstructed_det_tm,
        expected_reconstructed_det,
        "APG reconstructed determinant",
    )

    factored_record = ledger.records["APG_FACTORED_EXTERIOR_TM2"]
    total_scales: list[Taylor2] = []
    for column in range(2):
        reported_scale = tm2_from_record(
            factored_record, f"SCALE{column}_", delta
        )
        expected_scale = tm2_multiply(
            apg_event1_pivots[column], apg_event2_pivots[column], delta
        )
        require_tm2_reconstruction(
            reported_scale, expected_scale, f"APG total scale {column}"
        )
        total_scales.append(reported_scale)
    expected_exterior_tm = tm2_subtract(
        tm2_multiply(
            apg_event2_normalized[0][0], apg_event2_normalized[1][1], delta
        ),
        tm2_multiply(
            apg_event2_normalized[0][1], apg_event2_normalized[1][0], delta
        ),
    )
    exterior_tm = tm2_from_record(factored_record, "EXTERIOR_", delta)
    require_tm2_reconstruction(
        exterior_tm, expected_exterior_tm, "APG normalized exterior"
    )
    expected_factored_det = tm2_multiply(
        tm2_multiply(total_scales[0], total_scales[1], delta),
        exterior_tm,
        delta,
    )
    factored_det_tm = tm2_from_record(factored_record, "DET_", delta)
    require_tm2_reconstruction(
        factored_det_tm, expected_factored_det, "APG factored determinant"
    )
    apg_primary = interval(factored_record, "PRIMARY_DET")
    require_tight_contains(
        apg_primary,
        tm2_range(factored_det_tm, delta),
        "APG primary determinant",
        32768,
    )
    apg_reconstructed_det = tm2_range(reconstructed_det_tm, delta)
    apg_shared_det = tm2_range(shared_det_tm, delta)
    ...
    apg_joint_overlap = joint_interval(
        [
            apg_primary,
            apg_reconstructed_det,
            apg_shared_det,
            interval(full, "HULL_DET"),
            affine_det,
            liouville_det,
        ]
    )
    if not apg_joint_overlap:
        fail("APG factored, reconstructed, shared, direct, affine, and Liouville channels disagree")
    fixed_widths = [
        determinant.width
    ...
    computed = {
        ...
        "APG_LOCAL_IMPACT_TIME_CROSSCHECK": True,
        "APG_LOCAL_CENTER_FULL_DP_OVERLAP": apg_local_center_full_dp_overlap,
        "APG_LOCAL_INPUT_CONTAINMENT": apg_local_input_containment,
        "APG_EVENT1_RAY_RECONSTRUCTION": True,
        "APG_EVENT2_RAY_RECONSTRUCTION": True,
        "APG_LOCAL_EVENT_ORDER_CERTIFIED": apg_local_event_order_certified,
        "APG_LOCAL_POSTSECTION_PLUS_SIDE": apg_local_postsection_plus_side,
        "APG_LOCAL_TRANSVERSALITY_CERTIFIED": apg_local_transversality_certified,
        "APG_JOINT_OVERLAP": apg_joint_overlap,
        "APG_PRIMARY_POSITIVE_WIDTH": apg_primary.width > 0,
        "APG_STRICTLY_NARROWER_THAN_BOXED": apg_primary.width < homogeneous_det.width,
        "APG_STRICTLY_NARROWER_THAN_AFFINE": apg_primary.width < affine_det.width,
        "APG_STRICTLY_NARROWER_THAN_SHARED": apg_primary.width < apg_shared_det.width,
        "APG_ORIENTATION_CERTIFIED": not apg_primary.contains_zero(),
        "APG_NEGATIVE_ORIENTATION_CERTIFIED": apg_primary.upper < 0,
    }
    ...
    computed["APG_COMPUTATION_VALID"] = all(
        computed[key]
        for key in (
            "STRUCTURAL_PASS",
            "APG_LOCAL_IMPACT_TIME_CROSSCHECK",
    ...
    certificate_methods["APG_CERTIFICATE_PASS"] = (
        computed["APG_COMPUTATION_VALID"]
        and computed["APG_NEGATIVE_ORIENTATION_CERTIFIED"]
        and apg_primary.overlaps(liouville_det)
        and liouville_det.upper < 0
    )
    computed["APG_CERTIFICATE_PASS"] = certificate_methods[
        "APG_CERTIFICATE_PASS"
    ]
    computed["APG_RESCUE"] = (
        computed["APG_CERTIFICATE_PASS"] and not affine_certificate_pass
    )
    computed["CERTIFICATE_PASS"] = any(certificate_methods.values())
    computed["PROBE_PASS"] = (
        computed["HOMOGENEOUS_COMPUTATION_VALID"]
        and computed["APG_COMPUTATION_VALID"]
    )
    ...
    expected_leaf_result = {
        ...
        **certificate_methods,
        "APG_RESCUE": computed["APG_RESCUE"],
    }
...
def mutation_suite(
    expected_contract: Full53LeafContract, root_challenge: str
) -> list[Mutation]:
    ...
    def replacement_sha256(current: str) -> str:
        return "0" * 64 if current != "0" * 64 else "1" * 64

    def coordinated_parent_receipt_rebinding(ledger: Ledger) -> None:
        rebound = replace(
            expected_contract,
            parent_receipt_sha256=replacement_sha256(
                expected_contract.parent_receipt_sha256
            ),
        )
        ledger.headers["PARENT_RECEIPT_SHA256"] = rebound.parent_receipt_sha256
        ledger.headers["RUN_CHALLENGE"] = full53_leaf_challenge(
            root_challenge, rebound
        )

    def coordinated_manifest_rebinding(ledger: Ledger) -> None:
        rebound = replace(
            expected_contract,
            manifest_sha256=replacement_sha256(expected_contract.manifest_sha256),
        )
        ledger.headers["COORDINATE_MANIFEST_SHA256"] = rebound.manifest_sha256
        ledger.headers["RUN_CHALLENGE"] = full53_leaf_challenge(
            root_challenge, rebound
        )

    def coordinated_chart_sign_rebinding(ledger: Ledger) -> None:
        index = 3
        chart_signs = list(expected_contract.chart_signs)
        current_chart, current_sign = chart_signs[index]
        rebound_chart = "Y" if current_chart != "Y" else "X"
        chart_signs[index] = (rebound_chart, -current_sign)
        rebound = replace(expected_contract, chart_signs=tuple(chart_signs))
        record = ledger.records["APG_EVENT2_RAY1"]
        record["CHART"] = rebound_chart
        record["EXPECTED_SIGN"] = -current_sign
        ledger.headers["RUN_CHALLENGE"] = full53_leaf_challenge(
            root_challenge, rebound
        )
    ...
    def flip_record_sign(marker: str) -> Callable[[Ledger], None]:
        return lambda ledger: ledger.records[marker].__setitem__(
            "EXPECTED_SIGN", -integer_value(ledger.records[marker], "EXPECTED_SIGN")
        )
    ...
    def shrink_apg_local_input_to_state_midpoint(ledger: Ledger) -> None:
        delta = [
            interval(ledger.records["SOURCE_TILE"], "DELTA0"),
            interval(ledger.records["SOURCE_TILE"], "DELTA1"),
        ]
        state = tm2_from_record(
            ledger.records["APG_EVENT1_STATE_TM2"], "X0_", delta
        )
        set_interval(
            ledger,
            "APG_EVENT1_STATE_TM2",
            "LOCAL_INPUT_HULL0",
            interval_midpoint(tm2_range(state, delta)),
        )

    def widen_apg_reconstructed_tm2(ledger: Ledger) -> None:
        marker = "APG_RECONSTRUCTED_TM2"
        prefix = "W00_"
        ...
        set_interval(
            ...
            marker,
            f"{prefix}C00",
            Interval(Fraction(-1, 1_000_000), Fraction(1, 1_000_000)),
        )
        widened = Taylor2(
            tuple(interval(record, f"{prefix}{name}") for name in TM2_COEFFICIENT_NAMES),  # type: ignore[arg-type]
            interval(record, f"{prefix}R"),
        ...
        set_interval(
            ledger, marker, f"{prefix}POLY", tm2_polynomial_range(widened, delta)
        )
        set_interval(ledger, marker, f"{prefix}HULL", tm2_range(widened, delta))
    ...
    return [
        ...
        ("p1_affine_coefficient_replaced", lambda x: set_interval(x, "AFFINE_P1_CARRIER", "A000", ONE)),
        ("apg_state_symbol_erased", lambda x: set_interval(x, "APG_EVENT1_STATE_TM2", "X0_C10", ZERO)),
        ("apg_state_remainder_erased", lambda x: set_interval(x, "APG_EVENT1_STATE_TM2", "X0_R", ZERO)),
        ("apg_input_padding_erased", lambda x: set_interval(x, "APG_EVENT1_STATE_TM2", "INPUT_PADDING0", ZERO)),
        ("apg_local_input_shrunk_to_state_midpoint", shrink_apg_local_input_to_state_midpoint),
        ("apg_local_flow_erased", lambda x: set_interval(x, "APG_LOCAL_C2_FULL", "FLOW00", ZERO)),
        ("apg_local_hessian_replaced", lambda x: set_interval(x, "APG_LOCAL_C2_FULL", "D2P000", ONE)),
        ("apg_local_jacobian_symbol_erased", lambda x: set_interval(x, "APG_LOCAL_JACOBIAN_TM2", "J00_C10", ZERO)),
        ("apg_event1_chart_changed", change_chart("APG_EVENT1_RAY0")),
        ("apg_event1_sign_flipped", flip_record_sign("APG_EVENT1_RAY0")),
        ("apg_event1_ray1_chart_changed", change_chart("APG_EVENT1_RAY1")),
        ("apg_event1_ray1_sign_flipped", flip_record_sign("APG_EVENT1_RAY1")),
        ("apg_event1_pivot_claim_flipped", flip_record_bool("APG_EVENT1_RAY0", "PIVOT_SIGN_CERTIFIED")),
        ("apg_event1_reciprocal_erased", lambda x: set_interval(x, "APG_EVENT1_RAY0", "S_C00", ZERO)),
        ("apg_event2_raw_symbol_erased", lambda x: set_interval(x, "APG_EVENT2_RAW_TM2", "V00_C10", ZERO)),
        ("apg_event2_chart_changed", change_chart("APG_EVENT2_RAY0")),
        ("apg_event2_sign_flipped", flip_record_sign("APG_EVENT2_RAY0")),
        ("apg_event2_ray1_chart_changed", change_chart("APG_EVENT2_RAY1")),
        ("apg_event2_ray1_sign_flipped", flip_record_sign("APG_EVENT2_RAY1")),
        ("coordinated_chart_sign_rebinding", coordinated_chart_sign_rebinding),
        ("apg_shared_determinant_erased", lambda x: set_interval(x, "APG_SHARED_COMPOSITION_TM2", "DET_HULL", ZERO)),
        ("apg_reconstructed_column_erased", lambda x: set_interval(x, "APG_RECONSTRUCTED_TM2", "W00_C00", ZERO)),
        ("apg_reconstructed_column_widened", widen_apg_reconstructed_tm2),
        ("apg_factored_scale_erased", lambda x: set_interval(x, "APG_FACTORED_EXTERIOR_TM2", "SCALE0_C00", ZERO)),
        ("apg_exterior_erased", lambda x: set_interval(x, "APG_FACTORED_EXTERIOR_TM2", "EXTERIOR_C00", ZERO)),
        ("apg_factored_remainder_erased", lambda x: set_interval(x, "APG_FACTORED_EXTERIOR_TM2", "DET_R", ZERO)),
        ("apg_primary_erased", lambda x: set_interval(x, "APG_FACTORED_EXTERIOR_TM2", "PRIMARY_DET", ZERO)),
        ...
        ("leaf_apg_method_flipped", flip_record_bool("LEAF_RESULT", "APG_CERTIFICATE_PASS")),
        ("leaf_apg_rescue_flipped", flip_record_bool("LEAF_RESULT", "APG_RESCUE")),
        ("summary_apg_valid_flipped", flip_record_bool("SUMMARY", "APG_COMPUTATION_VALID")),
        ("summary_apg_width_flipped", flip_record_bool("SUMMARY", "APG_STRICTLY_NARROWER_THAN_BOXED")),
        ("summary_apg_rescue_flipped", flip_record_bool("SUMMARY", "APG_RESCUE")),
        ...
        ("parent_receipt_hash_substituted", header_mutation("PARENT_RECEIPT_SHA256")),
        ("coordinate_manifest_hash_substituted", header_mutation("COORDINATE_MANIFEST_SHA256")),
        ("coordinated_parent_receipt_rebinding", coordinated_parent_receipt_rebinding),
        ("coordinated_manifest_rebinding", coordinated_manifest_rebinding),
...
def run_mutations(
    ...
    rejected = 0
    suite = mutation_suite(expected_contract, root_challenge)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--coordinate-manifest", required=True, type=Path)
    ...
    contract = read_full53_contract(args.coordinate_manifest, leaf_input)
    parent_receipt_sha256 = digest_file(args.parent_receipt, "parent receipt")
    if parent_receipt_sha256 != contract.parent_receipt_sha256:
        fail("supplied parent receipt differs from the full53 manifest binding")
    challenge = full53_leaf_challenge(args.root_challenge, contract)
    ledger = parse_ledger(args.receipt)
    computed = verify_ledger(
        ledger, args.source_sha, leaf_input, contract, challenge
    )
    total = rejected = 0
    ...
    method = "NONE"
    if computed["APG_RESCUE"]:
        method = "APG_RESCUE"
    elif computed["APG_CERTIFICATE_PASS"]:
        method = "APG_RETAINED"
    else:
        for candidate, key in (
            ...
            if computed[key]:
                method = candidate
                break
    print("VERIFICATION_SCHEMA=sounio.cs6.affine-projective-cocycle-full53-leaf-verification.v1")
    print(f"RECEIPT_SHA256={ledger.receipt_sha256}")
    print(f"PHYSICAL_SHA256={ledger.physical_sha256}")
    print(f"COORDINATE_MANIFEST_SHA256={contract.manifest_sha256}")
    print(f"PARENT_RECEIPT_SHA256={contract.parent_receipt_sha256}")
    print(f"LEAF_CHALLENGE={challenge}")
