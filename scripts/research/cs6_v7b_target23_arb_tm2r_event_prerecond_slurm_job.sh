#!/usr/bin/bash
#SBATCH --job-name=cs6-event-prerecond
...
echo "SCHEMA=sounio.cs6.v7b-target23-arb-tm2r-event-prerecond-slurm.v1"
...
sha256sum \
  "$REPO/scripts/research/cs6_v7b_target23_arb_tm2r_event_prerecond_run.sh" \
  "$REPO/scripts/research/cs6_v7b_target23_arb_tm2r_event_prerecond_contract_v1.txt" \
  > "$PROVENANCE/control-files.sha256"

CS6_PYTHONPATH="$DEPS" CS6_OUTPUT_DIR="$RESULTS" \
  bash "$REPO/scripts/research/cs6_v7b_target23_arb_tm2r_event_prerecond_run.sh"

sha256sum "$RESULTS"/* > "$PROVENANCE/result-files.sha256"
echo "COMPLETED_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "CS6_EVENT_PRERECOND_SLURM_COMPLETE=true"
