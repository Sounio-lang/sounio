#!/usr/bin/bash
#SBATCH --job-name=cs6-event-centered
...
echo "SCHEMA=sounio.cs6.v7b-target23-arb-tm2r-event-centered-slurm.v1"
...
sha256sum \
  "$REPO/scripts/research/cs6_v7b_target23_arb_tm2r_event_centered_run.sh" \
  "$REPO/scripts/research/cs6_v7b_target23_arb_tm2r_event_centered_contract_v1.txt" \
  > "$PROVENANCE/control-files.sha256"

CS6_PYTHONPATH="$DEPS" CS6_OUTPUT_DIR="$RESULTS" \
  bash "$REPO/scripts/research/cs6_v7b_target23_arb_tm2r_event_centered_run.sh"

sha256sum "$RESULTS"/* > "$PROVENANCE/result-files.sha256"
echo "COMPLETED_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "CS6_EVENT_CENTERED_SLURM_COMPLETE=true"
