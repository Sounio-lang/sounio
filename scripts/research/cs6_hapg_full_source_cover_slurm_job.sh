...
[[ ${cfg[SCHEMA]} == sounio.cs6.hapg-full-source-cover-slurm-config.v2 ]] || \
...
for key in BASE_REPO_BUNDLE_PATH REPO_DELTA_BUNDLE_PATH PREBUILT_ARCHIVE_PATH; do
  path=${cfg[$key]}
...
contract="$work/repo/scripts/research/cs6_hapg_full_source_cover_contract_v5.txt"
...
if [[ ${cfg[MODE]} == adaptive ]]; then
  "$python_bin" -B "$aggregator" "$result" \
