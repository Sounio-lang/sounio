...
- **sort_by (2.62x / 2.15x vs stable)**: `bf_sort_by` argsorts a (key, original-row) index list with a bottom-up **stable** mergesort, then gathers each output column column-major through the sorted indices. Two optimizations took it from ~4x to 2.6x: **paired key+index buffers** (compare sequential `key[a] <= key[b]` instead of a random gather `keys[idx[a]]` that cache-thrashes the 8MB key array) and **ping-pong buffers** (no O(n) copy-back on each of the ~20 merge passes). Correct for all non-NaN f64 (direct `<=`), stable (equal keys keep input order — verified). The residual vs numpy's introsort/radix is the usual scalar-vs-SIMD gap; a radix sort on the f64 bits (with the sign-bit order transform) is the O(n) follow-up, deferred because that transform is a silent-correctness risk worth its own careful pass.
