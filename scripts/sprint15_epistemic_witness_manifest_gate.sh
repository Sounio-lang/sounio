#!/usr/bin/env bash
# sprint15_epistemic_witness_manifest_gate.sh — epistemic witness manifest schema gate
set -euo pipefail
...
OUT_JSON="${SOUNIO_SPRINT15_GATE_OUT:-$ROOT_DIR/artifacts/sprint15/epistemic_witness_manifest_gate.v1.json}"
TIMEOUT_SECS="${SOUNIO_SPRINT15_TIMEOUT_SECS:-360}"
...
contests = ep.get("contests", [])
assert isinstance(ep.get("posterior_weights", []), list)
assert isinstance(ep.get("counterexamples", []), list)
assert contests and isinstance(contests[0].get("disagreement_summary"), dict)
summary = contests[0]["disagreement_summary"]
for key in ("sign_consensus", "relative_band", "max_tail_risk", "identifiability_floor", "mechanistic_divergence"):
    assert key in summary
PY
  then
    record "$case_name" "pass" "manifest_schema_exposed"
  else
    record "$case_name" "fail" "manifest_schema_missing"
  fi
}
...
# Case 2: manifest printer contains witness schema fields
if grep -q '\\"posterior_weights\\"' self-hosted/compiler/main.sio 2>/dev/null \
   && grep -q '\\"counterexamples\\"' self-hosted/compiler/main.sio 2>/dev/null \
   && grep -q '\\"disagreement_summary\\"' self-hosted/compiler/main.sio 2>/dev/null; then
  record "manifest_printer_witness_fields_present" "pass" "found"
else
  record "manifest_printer_witness_fields_present" "fail" "missing_manifest_field"
fi

# Case 3: full audit-backed contest witness fixture emits manifest with witness schema exposed
run_manifest_case "manifest_contest_witness_basic" "tests/frontend/contest_witness_basic.sio"

# Case 4: counterfactual audit-backed witness fixture emits manifest with witness schema exposed
run_manifest_case "manifest_contest_counterfactual_witness_basic" "tests/frontend/contest_counterfactual_witness_basic.sio"

# Case 5: named-audit validate fixture emits manifest with witness schema exposed
run_manifest_case "manifest_validate_manifest_named_audit_source_basic" "tests/frontend/validate_manifest_named_audit_source_basic.sio"
...
payload = {
    "schema": "sounio.sprint15.epistemic_witness_manifest_gate.v1",
...
echo "sprint15_epistemic_witness_manifest_gate: out_json=$(to_rel "$OUT_JSON") status=$status"
[ "$status" = "pass" ] && exit 0
