...
AARCH64_COMPILE_WORK_DIR="$WORK_DIR/aarch64_compile"
...
env SOUC_NATIVE="$HEAD_BIN" WORK_DIR="$AARCH64_COMPILE_WORK_DIR" \
...
env SOUC_NATIVE="$HEAD_BIN" WORK_DIR="$AARCH64_RUNTIME_WORK_DIR" \
  bash "$ROOT_DIR/scripts/selfhost/selfhost_aarch64_runtime_gate.sh" >"$LOG_DIR/aarch64-runtime.stdout" 2>"$LOG_DIR/aarch64-runtime.stderr" || {
    echo "error: aarch64 runtime gate failed" >&2
    cat "$LOG_DIR/aarch64-runtime.stdout" >&2 || true
    cat "$LOG_DIR/aarch64-runtime.stderr" >&2 || true
    exit 1
  }

ABI_SUMMARY="$ABI_WORK_DIR/artifacts/summary.txt"
AARCH64_SUMMARY="$AARCH64_COMPILE_WORK_DIR/artifacts/summary.txt"
AARCH64_RUNTIME_SUMMARY="$AARCH64_RUNTIME_WORK_DIR/artifacts/summary.txt"
...
RUNTIME_PASS="$(sed -n 's/^summary_pass=//p' "$AARCH64_RUNTIME_SUMMARY" | head -n1)"
RUNTIME_FAIL="$(sed -n 's/^summary_fail=//p' "$AARCH64_RUNTIME_SUMMARY" | head -n1)"
RUNTIME_SKIP="$(sed -n 's/^summary_skip=//p' "$AARCH64_RUNTIME_SUMMARY" | head -n1)"

{
  ...
  echo "aarch64_runtime_summary=$AARCH64_RUNTIME_SUMMARY"
  ...
  echo "aarch64_runtime_pass=$RUNTIME_PASS"
  echo "aarch64_runtime_fail=$RUNTIME_FAIL"
  echo "aarch64_runtime_skip=$RUNTIME_SKIP"
} >"$SUMMARY_FILE"

cat >"$SUMMARY_JSON" <<EOF
{
  "schema": "sounio.selfhost_ci_abi_parity_gate.v2",
...
- aarch64_runtime: pass=$RUNTIME_PASS fail=$RUNTIME_FAIL skip=$RUNTIME_SKIP
EOF
