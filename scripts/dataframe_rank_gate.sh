...
if ! grep -qF 'Madaros v' <<<"$engine_info"; then
  echo "FAIL: dataframe-rank gate requires default Madaros" >&2
  printf '%s\n' "$engine_info" >&2
  exit 1
fi
...
echo "== run-proof: rank =="
if $SOUC compile tests/stdlib/data/test_dataframe_rank_stdlib.sio -o "$OUT/x.elf" >/dev/null 2>&1; then
