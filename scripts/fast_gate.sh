...
if [[ "$SKIP_BUILD" = "0" ]]; then
  echo "[fast-gate] 1/12 cargo preflight (no conflicting workspace cargo jobs)"
  bash "$ROOT_DIR/scripts/check_no_active_cargo_jobs.sh"
else
  echo "[fast-gate] 1/12 cargo preflight (skipped, SKIP_BUILD=1)"
fi

echo "[fast-gate] 2/12 syntax drift scan"
python3 "$ROOT_DIR/skills/sounio-language/scripts/scan_syntax_drift.py" --root "$ROOT_DIR"

echo "[fast-gate] 3/12 workflow script reference check"
bash "$ROOT_DIR/scripts/check_workflow_script_refs.sh"

echo "[fast-gate] 4/12 docs consistency check"
bash "$ROOT_DIR/scripts/check_docs_consistency.sh"

echo "[fast-gate] 5/12 issue template contract check"
bash "$ROOT_DIR/scripts/check_issue_template_contracts.sh"

echo "[fast-gate] 6/12 cultural fidelity (user-facing text leakage)"
python3 "$ROOT_DIR/scripts/cultural_fidelity_gate.py"

echo "[fast-gate] 7/12 compiler unit tests (cargo test --lib)"
(cd "$ROOT_DIR" && sounio_cargo test -p souc --lib)

echo "[fast-gate] 8/12 integration tests"
(cd "$ROOT_DIR" && sounio_cargo test -p souc --tests)

echo "[fast-gate] 9/12 check canonical example"
...
echo "[fast-gate] 10/12 stdlib science pipeline gate"
bash "$ROOT_DIR/scripts/stdlib_science_pipeline_gate.sh"

echo "[fast-gate] 11/12 stdlib reliability gate"
bash "$ROOT_DIR/scripts/stdlib_reliability_gate.sh"

echo "[fast-gate] 12/12 e2e backend gate"
