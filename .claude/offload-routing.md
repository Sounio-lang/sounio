...
| `math-review` | `xai kimi local` fan-out | `zai` (opt-in), `xai-fast`, `qwen`, `mistral` | Math / algebra / stats audit — **three-way, so one outage still leaves two independent opinions** |

> **2026-08-31 — zai → kimi.** The second frontier vendor in the `math-review` fan-out is now **Kimi K3** (`-p kimi`; Moonshot, `https://api.moonshot.ai/v1`, model `kimi-k3`, 1M context, thinking always on, temperature fixed at 1.0 by the API). Key: `MOONSHOT_API_KEY` (or `KIMI_API_KEY`) in `~/.sounio-keys.env`; the account needs a $1 top-up. Reason: Z.AI direct returned `1313` Fair-Usage lockout on every attempt that day and its OpenRouter fallback `402`; `zai` remains an explicit opt-in.
>
...
| `xai` / `grok` | Grok 4.6 | `XAI_API_KEY` | Math, blunt realist, no flattery |
