...
extern "C" void krnl_san_scan_net(hls::stream<ap_axiu<512, 0, 0, 16> > &samples_in,
                                  const flop_t *lut,
...
static int run_case(const char *name, int n_points, unsigned short q_delta,
    ...
    count_t cat_out = 0;
    flop_t flops_out = 0;
    // entrega por stream: mesmo empacotamento, so muda o transporte
    hls::stream<ap_axiu<512, 0, 0, 16> > samples_in("samples_in");
    for (int w = 0; w < n_words; w++) {
        ap_axiu<512, 0, 0, 16> t;
        t.data = beats[w];
        t.keep = -1;
        t.last = (w == n_words - 1);
        samples_in.write(t);
    }
    krnl_san_scan_net(samples_in, lut_k, q_delta, n_points, n_samples,
