...
export default function Search({ locale, strings }: Props) {
  ...
  const SearchButton = () => (
    <button
      ...
      className="flex items-center gap-2 px-3 py-1.5 text-sm text-[var(--color-ink-2)] bg-[var(--color-surface-raise)] border border-[var(--color-line)] rounded-lg hover:border-[var(--color-accent-champagne)] transition-colors"
                  ...
                  {results.map((result, index) => (
                    <li key={index}>
                      ...
                      >
                        ...
                        <p
                          className="text-sm text-[var(--color-ink-2)] line-clamp-2 [&_mark]:bg-[var(--color-accent-champagne)]/30 [&_mark]:text-[var(--color-ink-1)] [&_mark]:rounded"
