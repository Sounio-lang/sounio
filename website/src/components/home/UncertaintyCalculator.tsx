      ...
      <div className="container px-4">
          ...
          {Object.entries(PRESETS).map(([key]) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                preset === key
                  ? 'bg-[var(--color-accent-hyperlink)] text-[#10233e] shadow-lg shadow-[var(--color-accent-hyperlink)]/20'
                  : 'bg-[rgba(255,255,255,0.06)] text-[var(--color-text-secondary)] border border-[var(--glass-border)] hover:border-[var(--color-accent-hyperlink)]/40'
              }`}
            >
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-6 items-start">
          ...
          <div className="glass glass-specular rounded-2xl p-6">
            ...
            <div className="grid gap-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input type="number" value={aVal} onChange={(e) => setAVal(+e.target.value || 0)} step="0.1"
                    className="flex-1 bg-[rgba(0,0,0,0.3)] border border-[var(--glass-border)] rounded-lg px-3 py-2 text-[var(--color-text-primary)] text-sm font-mono focus:border-[var(--color-accent-hyperlink)]/60 focus:outline-none" />
                  <div className="flex items-center gap-1">
                    <span className="text-xs text-[var(--color-text-tertiary)]">\u00B1</span>
                    <input type="number" value={aUnc} onChange={(e) => setAUnc(+e.target.value || 0)} step="0.1" min="0"
                      className="w-20 bg-[rgba(0,0,0,0.3)] border border-[var(--glass-border)] rounded-lg px-3 py-2 text-[var(--color-text-primary)] text-sm font-mono focus:border-[var(--color-accent-hyperlink)]/60 focus:outline-none" />
                  </div>
                </div>
                <span className="text-xs text-[var(--color-accent-hyperlink)] mt-1 block font-mono">prov: {p.a.prov}</span>
              </div>
              ...
              <div>
                ...
                <span className="text-xs text-[var(--color-accent-hyperlink)] mt-1 block font-mono">prov: {p.b.prov}</span>
              </div>
          ...
          <div className="hidden lg:flex items-center justify-center pt-16">
            <svg className="w-10 h-10 text-[var(--color-accent-hyperlink)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              ...
              <div className="bg-[rgba(0,0,0,0.3)] rounded-xl p-5 mb-4 font-mono">
                ...
                <div className="grid gap-1 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-[var(--color-accent-hyperlink)]">{p.a.prov} \u00D7 {p.b.prov}</span>
                  </div>
                </div>
              </div>
