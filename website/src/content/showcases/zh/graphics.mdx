...
description: "通过原生执行 Sounio 示例生成的终端科学显示和光栅图像。"
domain: "graphics"
icon: "🎨"
...
本页展示 `examples/graphics/` 中的终端科学显示，以及 `examples/render/` 中的五幅光栅图像。

这些图像通过原生执行 Sounio 示例程序，并将其 PPM 输出转换为 SVG 生成。[来源记录](/assets/generated/render/provenance.json)列出了源代码提交、编译器发行包和像素比较结果。该比较验证示例输出的复现；数学精度和历史证明的验证不在其范围内。
