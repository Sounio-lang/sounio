...
- [Pharmacometrics Showcase](/science/pharma)
- [PK/PD Examples](/learn/examples)
- [Scientific Computing Showcase](/science/scientific-computing)

## Featured Safety Gate: Vancomycin Confidence Refusal

Sounio includes a clinical-safety compile-fail fixture that blocks low-confidence
prescriptions before runtime:

```bash
souc check tests/compile-fail/vancomycin_low_conf.sio
```

Use alongside the propagation demo:
...
Outcome:
- propagation example shows confidence changes through dosing steps
- refusal fixture enforces an epistemic threshold (`epsilon`) at compile time
