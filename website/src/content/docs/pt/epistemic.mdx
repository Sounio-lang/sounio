...
Se $z = x \cdot y$ ou $z = x / y$, a incerteza relativa combinada é dada por:
...
-   **Monotonicidade**: O runtime epistêmico valida se os parâmetros calculados de concentração mínima ($C_{\min}$) e área sob a curva ($AUC_{24}$) mantêm consistência física matemática honesta, impedindo modificações e tolerâncias retroajustadas artificiais.
