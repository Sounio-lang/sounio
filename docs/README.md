# Sounio Documentation

Welcome to the Sounio programming language documentation.

## For Users

### Getting Started
- [Getting Started](getting-started.md)
- [Minimum Viable Sounio](guide/MINIMUM_VIABLE_SOUNIO.md)
- [LLM Programming Guide](LLM_PROGRAMMING_GUIDE.md)

### Reference
- [Standard Library Reference](reference/STDLIB_REFERENCE.md)
- [Standard Library Organization](STDLIB_MODULE_ORGANIZATION.md)
- [Minimum Viable Sounio](guide/MINIMUM_VIABLE_SOUNIO.md) (includes fail-closed science + reliability gate commands)

## For Developers

### Compiler Internals
- [Technical Report](compiler/TECHNICAL_REPORT.md)
- [Effect Dispatch](compiler/EFFECT_DISPATCH_INTEGRATION.md)
- [Epistemic Backend](compiler/EPISTEMIC_BACKEND_GUIDE.md)
- [GPU Kernels](compiler/GPU_KERNELS.md)
- [Known Limitations](compiler/KNOWN_LIMITATIONS.md)

### Backend & Runtime
- [LLVM Codegen](compiler/LLVM_CODEGEN.md)
- [GPU Runtime](compiler/GPU_RUNTIME.md)
- [Async Runtime](compiler/ASYNC_RUNTIME.md)

### Development
- [Roadmap](development/roadmap.md)
- [Paper Artifact Packaging Spec](PAPER_ARTIFACT_PACKAGING_SPEC.md)

## Specification

- [Language Specification](../spec/LANGUAGE_SPECIFICATION.md)
