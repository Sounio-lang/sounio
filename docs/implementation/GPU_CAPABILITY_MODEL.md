...
| CPU fallback/reference execution | `tests/run-pass/gpu_kernel_basic.sio`, `tests/gpu/test_gpu_pipeline.sio`, `tests/stdlib/gpu/test_gpu.sio`, `examples/gpu.sio`, `examples/gpu_hypercomplex.sio`, `tests/run-pass/gpu_launch_multidim_surface.sio`, `tests/gpu/sim_runtime/gpu_launch_descriptor_thread_budget_reference.sio`, `tests/gpu/sim_runtime/gpu_launch_marshaled_count_reference.sio` | `gpu-sim-runtime-supported` | Deterministic repo-local runtime truth plane, including explicit multidimensional launch tuple carriage, the repo-local 1024-thread block-budget contract, and fail-closed negative-count marshaling without hardware dependency. |
