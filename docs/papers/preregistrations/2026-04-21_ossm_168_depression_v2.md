<!-- docs:meta
topic_id: repo.docs.papers.preregistrations.2026-04-21-ossm-168-depression-v2
authority: repo_only
audience: users
last_validated: 2026-03-07
validated_by: A2
source_of_truth: docs/governance/topic-registry.v1.json#repo.docs.papers.preregistrations.2026-04-21-ossm-168-depression-v2
-->

# Pre-Registration v2: O-SSM-Derived Hypotheses on LEMON + MODMA

**Supersedes:** `prereg/ossm-168-depression-v1` (tag on commit `21c1d0178e15edab0e1ddc928fc50ed74199c4d8`, SHA-256 `63c2f60223657491829c38a57a2745fb079d48259bed306538ef32223ae4c93d`). See `DEVIATIONS.md` entry 0002 for the reason for supersedence (algebraic erratum in v1's F_3, which was identically zero by Artin's theorem on alternative algebras).

**Differences from v1:** §2 H3 feature F_3 is rewritten in the sedenion algebra `S_16`; other sections are unchanged. H1 and H2 are byte-identical to v1. H3 is a coherent confirmatory hypothesis in v2 (not demoted to exploratory, because v2 is a new registration, not a within-protocol deviation from v1).
...
**Commit hash at registration (freeze commit containing the SHA-256):** `3364fbbc33a6140394a5c076447926895e9811d0`
**SHA-256 of this document at freeze (commit 12c7d934, pre-hash-insertion):** `ca458198bab2f08ec13d8edbb8b8bb1da54e2cc5f615dec9e60a3f1b2f1b82a5`
...
### H3 — Negative-valence bias ↔ Sedenion associator parity asymmetry

**Clinical substrate.** By Lemma 2 of the 168 Theorem, basis-level associator norms take only the values $\{0, 2\}$: non-associativity at the base is a $\mathbb{Z}_2$ sign flip. In the alternative octonion algebra this parity can only manifest through the sign of the associator; the norm $\|[a,b,c]\|$ is symmetric under reversal of the outer arguments because the associator is alternating (Artin 1927). The first Cayley-Dickson algebra in which the associator is **not** alternating — and in which a norm-level parity asymmetry $\|[a,b,c]\| \neq \|[c,b,a]\|$ is generic — is the sedenion algebra $\mathbb{S}_{16}$. The 168 Theorem §5 identifies 336 = 2 × 168 primitive zero-divisor pairs in $\mathbb{S}_{16}$, forming two orbits of 168 each under $\mathrm{PGL}(3,\mathbb{F}_2)$; we denote by $\widetilde{\mathcal{O}}_{168}$ one of these orbits, pre-declared as the orbit containing the representative $(e_3 + e_{10}, e_6 - e_{15})$ catalogued in `stdlib/math/sedenion.sio` and de Marrais 2000.

We hypothesise that the magnitude of this genuine sedenion parity asymmetry tracks affective polarisation.

**Feature $F_3$ (sedenion form).** Let $\tilde{h}_t \in \mathbb{S}_{16}$ be the Cayley-Dickson doubling of the octonion state $h_t \in \mathbb{O}^2$ as already defined in H2. For each epoch,
$$
F_3^{(e)} \;=\; \frac{1}{T \cdot 168} \sum_{t=1}^{T}\; \sum_{(I,J,K) \in \widetilde{\mathcal{O}}_{168}} \Big|\,\|[\tilde{h}_t \cdot e_I,\, \tilde{h}_t \cdot e_J,\, \tilde{h}_t \cdot e_K]\| \;-\; \|[\tilde{h}_t \cdot e_K,\, \tilde{h}_t \cdot e_J,\, \tilde{h}_t \cdot e_I]\|\,\Big|,
$$
where $\tilde{h}_t \cdot e_I$ denotes the sedenion product of the state with the unit $e_I$ and $(I,J,K)$ ranges over the 168 ordered representatives of $\widetilde{\mathcal{O}}_{168}$ fixed once at registration time (enumerated deterministically by `scripts/research/ossm_168_dryrun/sedenion_orbits.py` under seed `20260421`).

Subject-level feature = median of $F_3^{(e)}$.

**Algebraic rationale.** In $\mathbb{S}_{16}$ the associator $[\alpha,\beta,\gamma] = (\alpha\beta)\gamma - \alpha(\beta\gamma)$ is not alternating: $\|[\gamma,\beta,\alpha]\| \neq \|[\alpha,\beta,\gamma]\|$ generically. Zero-divisor triples, in particular, can exhibit arbitrarily large reversal asymmetry because one side of the expansion can vanish while the other does not. This makes $F_3$ structurally informative and ties H3 to the same zero-divisor geometry as H2, both inherited from Theorem T3.
