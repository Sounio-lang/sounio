...
**Supersedes:** `prereg/ossm-168-depression-v2` (SHA-256 `ca458198bab2f08ec13d8edbb8b8bb1da54e2cc5f615dec9e60a3f1b2f1b82a5`, commit `619da07c`), which itself superseded `prereg/ossm-168-depression-v1` (SHA-256 `63c2f60223657491829c38a57a2745fb079d48259bed306538ef32223ae4c93d`).

**Reason for supersedence (v2 → v3).** During implementation, the author verified numerically that the v2 F_3 formula is also identically zero, not only in octonions but throughout the entire Cayley-Dickson tower. The structural fact is: *every Cayley-Dickson algebra is flexible* ($[a,b,a] = 0$), and flexibility alone forces the associator to satisfy $[c,b,a] = -[a,b,c]$, hence $\|[c,b,a]\| = \|[a,b,c]\|$. Any feature defined as a norm asymmetry under $(a \leftrightarrow c)$ reversal is therefore zero in $\mathbb{C}, \mathbb{H}, \mathbb{O}, \mathbb{S}_{16}, \mathcal{T}_{32}, \dots$ — not only in $\mathbb{O}$. See `DEVIATIONS.md` entry 0004 for the full derivation.

**Differences from v2:** §2 H3 is renamed from "Negative-valence bias ↔ Associator parity asymmetry" to **"Negative-valence bias ↔ L¹ sedenion associator mass"**. The feature F_3 is reformulated as a componentwise-L¹ associator mass (not a norm asymmetry), which is non-trivial and well-defined throughout the CD tower. H1 and H2 are byte-identical to v1/v2. H3 in v3 is a coherent confirmatory hypothesis (not demoted to exploratory; v3 is a new registration, not a within-protocol deviation from v2).

**Registration date (frozen prior to any LEMON or MODMA signal data access):** 2026-04-21
...
**Commit hash at registration (freeze commit containing the SHA-256):** `a5762dde34caacd9d6f114deef154d3005bed82c`
**SHA-256 of this document at freeze (commit 78491ea1, pre-hash-insertion):** `c403ff55beab70273da388be5fb062006fa63a7a1f455c4a80f42d5cdf5a87c7`
...
### H3 — Negative-valence bias ↔ L¹ sedenion associator mass

**Clinical substrate.** Negative-valence processing is characterised by sustained recruitment of affect circuits whose output shows reduced symmetry in magnitude (increased intensity in emotion-laden components relative to neutral ones). Algebraically, we hypothesise that this recruitment manifests as an increased $L^1$-mass of the sedenion associator $[\,\tilde{h}_t \cdot e_I,\; \tilde{h}_t \cdot e_J,\; \tilde{h}_t \cdot e_K\,]$ when $(I,J,K)$ ranges over the 168 triples inherited from the octonion 168-orbit (DEVIATIONS entry 0003). The $L^1$ norm is chosen over $L^2$ for its robustness to outlier components in the associator expansion.

**Mathematical note on what H3 is NOT.** Earlier versions of this protocol (v1, v2) attempted to define F_3 as a norm asymmetry under argument reversal $(a,b,c) \leftrightarrow (c,b,a)$. That formulation is identically zero throughout the Cayley-Dickson tower because every CD algebra is flexible ($[x,y,x]=0$), and flexibility alone forces $[c,b,a] = -[a,b,c]$, hence $\|[c,b,a]\| = \|[a,b,c]\|$. H3 in v3 abandons the "parity asymmetry" framing entirely and is instead a dual of H1 at one level up the tower (H1 measures octonion associator mass on $\mathcal{O}_{168}$; H3 measures sedenion associator mass on the same 168-orbit lifted to $\mathbb{S}_{16}$).

**Feature $F_3$ (L¹ sedenion associator mass).** Let $\tilde{h}_t \in \mathbb{S}_{16}$ be the Cayley-Dickson doubling of the octonion state $h_t \in \mathbb{O}^2$ as already defined in H2. For each epoch,
$$
F_3^{(e)} \;=\; \frac{1}{T \cdot 168} \sum_{t=1}^{T}\; \sum_{(I,J,K) \in \widetilde{\mathcal{O}}_{168}} \; \big\|\; [\,\tilde{h}_t \cdot e_I,\, \tilde{h}_t \cdot e_J,\, \tilde{h}_t \cdot e_K\,]\;\big\|_{L^1},
$$
where $\|\cdot\|_{L^1}$ denotes the $L^1$ norm on the 16 sedenion basis components ($\|s\|_{L^1} = \sum_{k=0}^{15} |s_k|$), and $(I,J,K)$ ranges over the 168 sedenion triples fixed by the map $(i,j,k) \mapsto (i,\,8+j,\,k)$ on the octonion 168-orbit (per DEVIATIONS entry 0003). The enumeration is produced deterministically by `scripts/research/ossm_168_dryrun/sedenion.py::O_168_sedenion_triples()`.

Subject-level feature = median of $F_3^{(e)}$ across epochs.

**Algebraic rationale.** Sedenions admit zero divisors; near a zero-divisor axis, the associator $[a,b,c]$ can take large values in some components and vanish in others, making the $L^1$ mass structurally informative. F_3 is not zero on generic states; it vanishes exactly when the three arguments lie in an associative subalgebra. Tying the 168 triples to $\mathcal{O}_{168}$ keeps H3 anchored to the 168 Theorem.

**Primary endpoint.** Spearman $\rho_3 = \rho(F_3,\, \mathrm{PANAS\text{-}Negative})$.

**Sign prediction.** $\rho_3 > 0$ (higher $L^1$ sedenion associator mass → higher negative-valence bias).
...
For each subject, repeat feature extraction $K = 30$ times, each time applying one of the 30 inequivalent Fano labellings of the 7 imaginary basis elements. The 30 labellings are the lex-smallest representatives of the left cosets of $\mathrm{Aut}(\mathcal{F}) \cong \mathrm{PGL}(3,\mathbb{F}_2)$ in $S_7$ (so $|S_7|/|\mathrm{Aut}(\mathcal{F})| = 5040/168 = 30$). The canonical enumeration is produced deterministically by `scripts/research/ossm_168_dryrun/fano_orbits.py::coset_representatives_S7_mod_Aut()` under the fixed default Python ordering (no random seed required; the enumeration is purely combinatorial and verified against a Lean formalisation to be committed in a follow-up commit with back-reference to this protocol).
...
- Artin, E. (1927). *Zur Theorie der hyperkomplexen Zahlen.* Abh. Math. Sem. Univ. Hamburg 5:251–260. [Establishes alternativity of the octonions; used in §2 H3 rationale for moving F_3 into $\mathbb{S}_{16}$.]
