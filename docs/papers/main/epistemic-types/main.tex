...
\paragraph{Mechanized verification.}
All metatheoretic results---preservation, progress, the epistemic
lattice ordering, and the GEMM soundness bound---have been mechanized
in Lean~4 (731~theorems across 12~files, 5{,}539~lines, zero
\texttt{sorry}, zero Mathlib dependency).  Key files include
\texttt{SounioPreservation.lean} (type preservation under substitution),
\texttt{SounioEpistemic.lean} (lattice properties of
$\texttt{Knowledge}\langle T\rangle$), and
\texttt{EpistemicGemm.lean} (soundness of the GPU uncertainty formula
...
The \sounio compiler is self-hosted: approximately 105{,}000 lines of \sounio,
...
\paragraph{Results.}
After 10 epochs of training on 90 total samples (9 per epoch, 3 per class):
\begin{itemize}
  \item Weight uncertainty decreases monotonically from $0.1$ to
        $2.6 \times 10^{-34}$ (Bayesian concentration)
  \item \textbf{ECE = 0.004} (Expected Calibration Error; target $< 0.05$),
        demonstrating that predicted confidence closely tracks actual accuracy
  \item Sensitivity analysis: $\partial(\text{conf})/\partial(u_w) = 0.019$,
        confirming the GUM chain rule correctly connects weight uncertainty
        to prediction confidence through 28+ operations per forward pass
  \item All prediction uncertainties bounded ($< 1.0$) across 6 test samples
  \item Softmax outputs sum to 1.0 within floating-point tolerance
  \item All 10 validation checks pass (Table~\ref{tab:qnn-checks})
\end{itemize}

\begin{table}[t]
\centering
\caption{QNN epistemic benchmark: 10 validation checks.}
\label{tab:qnn-checks}
\begin{tabular}{lll}
\toprule
Check & Property & Status \\
\midrule
1 & Uncertainty decreased with training & \checkmark \\
2 & Monotonic decay (Bayesian concentration) & \checkmark \\
3 & GUM propagation active (Hamilton products) & \checkmark \\
4 & Predictive entropy positive & \checkmark \\
5 & Softmax probabilities sum to 1.0 & \checkmark \\
6 & ECE $< 0.5$ (calibration quality) & \checkmark \\
7 & Sensitivity $\partial(\text{conf})/\partial(u_w) > 0$ & \checkmark \\
8 & Entropy non-decreasing with uncertainty & \checkmark \\
9 & All prediction uncertainties bounded & \checkmark \\
10 & Weight uncertainty reduced $> 30\%$ & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
that calibration is near-perfect (ECE $= 0.004$), and that the GUM
compliance theorem (Theorem~\ref{thm:gum}) holds empirically.
...
\paragraph{Results.}
\begin{itemize}
  \item ATE $= 0.021$ with total uncertainty $\eps_{\text{total}} = 0.085$
        (decomposed: $\eps_{\text{stat}} = 0.062$, $\eps_{\text{causal}} = 0.030$,
        $\eps_{\text{model}} = 0.050$)
  \item Genotype-specific effects biologically plausible:
        poor metabolizers ($\text{effect} = 0.167$) $>$ extensive ($0.038$)
        $>$ ultra-rapid ($0.001$), reflecting slower clearance $\to$ higher exposure
  \item PK parameters within clinical ranges: AUC $= 18.1$~mg$\cdot$h/L
  \item Non-identifiable queries rejected at compile time
        by the causal type system (see \S\ref{sec:causal})
  \item All 4 validation checks pass (identifiability, PK range,
        ATE positivity, genotype ordering)
\end{itemize}
...
\paragraph{Results.}
\begin{itemize}
  \item V1 $\to$ V2 (confounded) rejected at compile time---no runtime error
  \item V2 $\to$ V4 estimated: $r = 0.45$, 95\% CI $[0.33, 0.55]$
  \item V1 $\to$ MT estimated: $r = 0.52$, 95\% CI $[0.41, 0.61]$
  \item 50-resample bootstrap provides epistemic uncertainty
        with Fisher $z$-transformed confidence intervals
  \item All 4 validation checks pass (identifiability rejection,
        backdoor verification, epistemic tracking, bootstrap CIs)
\end{itemize}
This demonstrates that \sounio prevents a class of
scientific errors---claiming causal connectivity from confounded
observations---that tools like SPM, FSL, and AFNI cannot detect.
...
The \sounio GPU backend generates PTX from the self-hosted compiler
(172~files, 105{,}K lines), achieving 5.3~TFLOPS on an NVIDIA~L4 at
$4096^3$ ($2.19\times$ vs.\ cuBLAS; see Section~\ref{sec:gpu-perf}).
Shadow register ablation decomposes the total overhead: $2.19\times$
compiler maturity gap $\times$ $3.16\times$ GUM propagation cost $=
6.91\times$ vs.\ cuBLAS.
A self-hosting cycle gate confirms deterministic compilation with
SHA-256 stage parity.
...
\begin{itemize}
  \item Decompose the $3.16\times$ shadow overhead into register
        pressure versus GUM arithmetic via Nsight Compute profiling
  \item Correlation tracking for dependent measurements
        (GUM Supplement 1~\cite{jcgm2008supplement1})
  \item Full self-hosting chain closure ($\text{souc}_2 \to \text{souc}_3$
