...
**Date:** 2026-08-13
**Status:** UNDER CORRECTION (audited 2026-08-13/14) — see §9. Do not cite the numbers in §3.1 (Dyck-1 row), §3.3 (SedenTree/Mixed rows), §3.4 (all G₂/LEMON rho/p values), §3.5 (F3/sedenion row and the "2×" claim), §4.2, or §4.3 until the retraction notices below are resolved. §1 (Artin dormancy), §3.1 (Rfam RNA row), §3.2, §3.3 (OctTree rows only), §3.5 (F1/octonion row only), and the [2,1]⊥Λ³ *theorem* (not the numbers reported to support it in §4.2/§4.3) survive independent re-audit.
...
### 4.3 The Borromean boundary — RETRACTED, resolved to "consistent with chance, underpowered" (2026-08-14, see §9)
...
>
> **Result, with all four defects fixed**: at n=15/class, [2,1]-hook AUROC=0.25 (Cohen's d=−0.69) and associator AUROC=0.31 (d=−0.54) — looked like a real effect. At n=25/class, **both moved substantially closer to chance**: hook AUROC=0.32 (d=−0.44), associator AUROC=0.34 (d=−0.31) — the direction a small-sample artifact would move. **Honest conclusion: once every identified confound and structural degeneracy is fixed, this experiment is consistent with chance but not yet powered enough to state that with confidence.** This supersedes both the original claim and this section's own earlier retraction text. A larger n and a permutation-test p-value are needed before either "chance" or "separable" can be asserted for §4.3; do not cite an AUROC in either direction from this experiment until that is done.

On Borromean paths (pairwise unlinked, globally linked), the Massey triple invariant achieves AUROC=0.998. The [2,1]-hook of sedenion states achieves AUROC≈0.50 (chance), as does the octonion associator (0.489).
...
This is an honest boundary, not a failure: the Cayley-Dickson hierarchy captures *static* non-associative structure. Temporal/dynamic non-associativity (Massey, A∞) requires a different mathematical framework. **Whether the specific §4.3 experiment can be redesigned to actually demonstrate this boundary empirically is now an open engineering question (see box above), not a settled "honest null."**
...
$SOUC run examples/cayley_dickson_borromean_rerun.sio             # §4.3 re-run — consistent with chance, underpowered, see §9
...
# LEMON real-data pilot (bridge must run first -- see cayley_dickson_lemon_g2_ffi.sio's
# footer for the confirmed system()-FFI compiler bug that prevents Sounio calling this itself):
python3 scripts/research/lemon_ffi_bridge.py 15 15 <bridge_csv_path>
$SOUC run examples/cayley_dickson_lemon_g2_ffi.sio                # §3.4 real-data pilot, n=15, see §9
```
...
| 6 | Borromean AUROC (§4.3) | `generate_borromean_path` and `generate_unlinked_path` differ ~10× in local roughness — a confound sufficient by itself to produce the reported separation, independent of any topological/associative signal. **Resolved 2026-08-14** (`examples/cayley_dickson_borromean_rerun.sio`): fixing the confound alone was not sufficient — two further structural defects (quaternion-subalgebra collapse, basis-triple alternation) also had to be fixed before either the associator or [2,1]-hook could be nonzero at all. With all four fixed, n=25/class gives AUROC 0.32–0.34 (moving toward 0.50 as n grew from 15→25) — consistent with chance, underpowered to confirm | `scripts/research/hook21_bracket.py:233-257` |
...
- `examples/cayley_dickson_borromean_rerun.sio` — §4.3 re-run; found and fixed four compounding defects in sequence (roughness confound, quaternion-subalgebra collapse, basis-triple alternation, missing 4th independent basis direction) before either the associator or [2,1]-hook could be nonzero at all. Final state (n=25/class): AUROC 0.32–0.34, moving toward chance as n grew from 15→25 — consistent with chance but underpowered to confirm. Still not a finished result (needs a larger n and a permutation-test p-value) but no longer blocked by an identified confound — see §4.3's box for the full chain.
...
- `examples/cayley_dickson_lemon_g2_ffi.sio` + `scripts/research/lemon_ffi_bridge.py` — the real-data pilot summarized in §9.5's table (n=15, real LEMON EEG + endpoints, G12 strongest at rho≈−0.53/−0.55, F1 weaker at rho≈−0.06/−0.31). Architecture: a minimal Python script (pure I/O, no science) reads the binary `.npy` epoch data Sounio cannot parse and matches subjects to `endpoints.csv`; everything else — the O-SSM forward pass, the G₂ derivation projection using the verified 14-basis above, and the Spearman correlation — runs in Sounio. Found two further genuine Madaros bugs while building this: (1) `extern "C" { fn system(...) }`, despite being declared and called in `self-hosted/compiler/pkg/registry_client.sio`, is a **confirmed non-functional no-op** — a decisive `system("touch <path>")` probe reports success (rc=0) both via `souc run` and via `souc compile`+execute, but never actually runs the command, so the Python bridge currently has to be invoked out-of-band rather than through Sounio's own FFI as originally intended; (2) a `[[f64;8];7]` nested-array **function parameter** (not element write, unlike the earlier bug) caused a silent hang (~1% CPU, never returns) — fixed by flattening to `[f64;56]`. Both flagged for forensic dispatch, not patched in `self-hosted/`.
...
- Running the actual LEMON n=204 correlation re-analysis with the now-verified 14-vector G₂ basis in place of the flawed SVD selection — needs the real cohort data (`/workspace/data/lemon/preprocessed/*.npy`), Python-side, not accessible from Sounio.
- A larger-n, statistically powered re-run of the Borromean-vs-unlinked probe (§4.3's box): with all four defects fixed (roughness, quaternion-subalgebra collapse, basis-triple alternation, and the 4th-independent-direction embedding), n=25/class already shows the effect shrinking toward chance as n grows from 15→25 — a real permutation-test p-value at a properly powered n (likely 100+/class) is needed to state "chance" with confidence rather than merely "consistent with."
...
| G₂ decomposition of psychiatric signal | **Partially resolved 2026-08-14**: the Leibniz-verified formula's true 14-dimensional basis is independently confirmed in Sounio (`examples/cayley_dickson_g2_derivation_basis.sio`). A **real-data pilot** (`examples/cayley_dickson_lemon_g2_ffi.sio`, n=15 real LEMON subjects, real epoch data via a Python I/O bridge invoked — per the FFI compiler bug noted there — out-of-band rather than through Sounio's own `system()`) finds G12 (D₄,₆) strongest (rho(rumination)=−0.531, rho(neuroticism)=−0.549) and the F1 aggregate markedly weaker (rho(rumination)=−0.062) — qualitatively consistent with the original hypothesis (G12 was one of the two generators the retracted analysis highlighted; the aggregate genuinely dilutes signal) but **not statistically validated**: n=15 is small (G12's rho is right at the uncorrected p≈0.05 boundary, not clearly significant), no FDR correction, downsampled to 15/1000 timesteps, mean not median, independently-seeded O-SSM weights. The full n=204/84-test analysis with this basis is **still open**. |
