...
\item \textbf{The bootstrapping protocol} (\S\ref{sec:bootstrap}):
  a 14-generation self-hosting experiment in which \lean (the \sounio
...
\centering
\caption{%
  Bootstrap convergence across 14 generations.
...
\end{tikzpicture}
\caption{%
  Confidence convergence across 14 bootstrap generations.
...
For programs using the Knowledge\textless T\textgreater{} bridge, GOLD also includes
call sites to \texttt{measured()} (EXPR\_CONF $= 990$) and \texttt{asserted()}
(EXPR\_CONF $= 970$), making the tier distribution a combined indicator of both
epistemic origin and unification depth.
