...
Across 10 self-hosted generations, confidence rises from 26\% to 98\%
monotonically, with bit-identical fixed points at every generation.
At Generation~10, all 8,875 call sites emit direct code
(100\% call-site certainty) with zero guard overhead---the 2-byte NOP
guard never fires.
...
\item \textbf{The bootstrapping protocol} (\S\ref{sec:bootstrap}):
  a 10-generation self-hosting experiment in which \lean (the \sounio
  self-hosted compiler, 16,365 lines) applies its own epistemic engine
  to its own source.
  Confidence rises from 26\% at Generation~0 to 98\% at Generation~10,
...
\item \textbf{Codegen gates} (\S\ref{sec:gates}): a practical
  ...
  At Generation~10, 100\% of call sites are direct; overhead reaches
  zero---the NOP guard never fires.
...
\centering
\caption{%
  Bootstrap convergence across 10 generations.
...
\node[font=\tiny, red!60!black] at (axis cs: 9.5, 96.8) {$\theta{=}95\%$};

% Annotation: Gen 5→6 jump
\draw[->, gray!60, thin] (axis cs: 0.5, 43) -- (axis cs: 0.05, 28)
  node[above left, font=\tiny, gray!60] {};
% Annotation: Gen 10 final value (100% call-site certainty)
\node[font=\tiny, blue!70!black] at (axis cs: 9.3, 96.5) {98\%};
\node[font=\tiny, green!50!black] at (axis cs: 10, 100) {$\star$};
\node[font=\tiny, green!50!black, align=center] at (axis cs: 10, 101.5) {};
\end{axis}
\end{tikzpicture}
\caption{%
  Confidence convergence across 10 bootstrap generations.
  ...
  Generation~10 (marked $\star$) adds generic call recognition, achieving
  100\% call-site certainty (0 guarded sites) at 98\% overall expression
  certainty.
}
