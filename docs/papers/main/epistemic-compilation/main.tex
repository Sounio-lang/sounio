...
Across 14 self-hosted generations, confidence rises from 26\% to 100\%
...
(100\% expression certainty) with zero guard overhead.
Generation~12 adds the \emph{Epistemic Inspector}: per-file confidence
breakdowns, token-level JSON export (\texttt{--dump-conf-json}), and
the first compile-time bridge between the confidence lattice and runtime
\texttt{Knowledge\textless T\textgreater} measurement types.
Generation~13 extends the bridge with a dual-channel system: a parallel
\textsc{MEAS\_CONF} array assigns GUM measurement quality (990 for instrument
data, 970 for literature assertions) independently of type-system certainty,
propagating through arithmetic via the GUM min-rule.
Generation~14 adds a four-tier confidence stratification---PLATINUM
($c=1000$), GOLD ($c \in [950,999]$), SILVER, and BRONZE---exposing the
fine structure of certainty within the 100\% threshold.
Generation~15 closes the precision gap: a special case in type unification
preserves the PLATINUM ceiling exactly ($\texttt{ety\_unify}(1000,1000)=1000$),
lifting all tokens to PLATINUM on the self-compile; cross-module epistemic
certificates (\texttt{.econf}) propagate verified function confidence across
module boundaries; and three core properties---MEAS\_CONF soundness,
PLATINUM preservation, and product monotonicity---are proved in Lean~4.
...
Generation~12 introduces the first connection between the compile-time
