...
\subsection{Static Prevention of Uncertainty Loss}

The static guarantee is that \texttt{Knowledge<T>} cannot implicitly
coerce to~\texttt{T}: any code path that discards uncertainty must call
an explicit \texttt{extract} operation, which the compiler flags with a
...
definitions/theorems, zero \texttt{sorry}), available in the source
repository and buildable via \texttt{lake build}.
...
(2$\times$64-bit unsigned integers, approximating the software \texttt{f64}
representation in fixed-point). Each transaction atomically increments $\alpha$
...
netlists (pre-place-and-route; no specific FPGA target). The complete
epistemic slave (variance propagation, confidence update, Merkle fold)
synthesizes to approximately 50K~equivalent gates in a single
combinatorial stage. The epistemic MAC (fused multiply-accumulate with
delta-method variance) requires two 64$\times$64 signed multipliers and
synthesizes to approximately 200K~gates.
...
\midrule
GUM Chain (1K ops) & 14 & 151$^{\dagger}$ & \textbf{3} \\
GUM Chain (10K ops) & 137 & DNF$^{\dagger}$ & \textbf{29} \\
GUM Chain (100K ops) & 1\,804 & DNF$^{\dagger}$ & \textbf{237} \\
Independent ops (10K)$^{\S}$ & 652 & \textbf{5} & 51 \\
Independent ops (100K)$^{\S}$ & 6\,496 & \textbf{49} & 455 \\
...
Independent-per-iteration workloads remain $O(n)$.

$^{\S}$Each iteration creates fresh \texttt{Knowledge} values (no chain
accumulation). This tests per-operation overhead, not Monte Carlo sampling.
