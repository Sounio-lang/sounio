...
| C22 | **A green gate that certifies a string.** `last_validated` in every docs:meta header is a **constant** hardcoded at two generator sites and uniform across the governed corpus. The CI docs-registry gate enforces the constant, so a document carrying a topic-specific date recorded in Git turns the gate red. This does not establish when human validation occurred; it establishes that the field cannot encode that answer. The family's inverse: R1 a claim bound to no gate, R5 a gate nobody ran, R20 an oracle never committed; C22 a gate that runs, is green, and guards no topic-specific validation information. | R22 | `VALIDATION_DATE_IS_A_LITERAL__GATE_REJECTS_RECORDED_DATE` |
...
| C26 | **The dangling dependency closed by reconstruction.** The oracle C20 found never committed to any branch — loaded by the orbit theorem's verifier with no fallback, so that verifier could not run in any checkout — is reconstructed from the verifier's recorded block-form construction and validated (**all 168 GL(3,2) maps are permutation parts admitting a sign completion, though only 21 preserve the signed table**). The bounded verifier now runs in every checkout: orbits 2^(n-4)×[7]+(2^(n-4)-1)×[1], stab 24, n=4..7. This rung does not independently re-prove the recorded all-n argument. | R26 | `ORACLE_RECONSTRUCTED__ORBIT_VERIFIER_RUNS_IN_TREE` |
| C25 | **Third docs:meta field: path-default historical.** Research `authority` is `ACTIVE_RESEARCH_DOCS.has(path) ? repo_only : historical`, and the whitelist is **four** explicit path literals. Of **350** research topics, **346** are historical with the auto lineage note; the four whitelisted topics are three `repo_only` and one website-backed `dual`. Claiming `repo_only` without whitelist membership turns the docs gate red. EXECUTABLE findings of this line are green only as lineage. | R25 | `RESEARCH_AUTHORITY_IS_PATH_DEFAULT_HISTORICAL__GATE_REJECTS_CURRENT` |
| C23 | **The sibling field: path ownership under a validation name.** `validated_by` is filled from `topic.owner_agent`; every governed path under `docs/research/` receives the literal **A6**. The checker enforces equality — a document naming a different validator turns the gate red. Variation by directory is not evidence of validation. Sibling of C22 in the same docs:meta block. | R23 | `VALIDATED_BY_IS_PATH_OWNERSHIP__GATE_REJECTS_TRUE_VALIDATOR` |
