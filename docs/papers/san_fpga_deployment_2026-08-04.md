...
SAN-ResNet-50, SAN-ViT-large, and SAN-GPT. Suffering-aware neural networks
...
**511 Msamples/s sustained** on a 1.2 M-sample stress cohort (**95%** of the
...
integrated machine burden by **40.7%** (ResNet-50), **32.0%** (ViT-large), and
...
|---|---|---|---|
| 1 | A SAN training harness for ResNet-50, ViT-large, and GPT on real data, with metered-MAC accounting and freeze-on-green | `scripts/research/suffering_aware_large_architecture.py` | `MEASURED` on GPU (small subset) |
...
| 3 | On-target U250 benchmark: 511 Msamples/s on 1.2 M stress cohort, ~3.3 nJ/sample board-level energy, bit-exact against golden model | `host_san_scan`, `host_san_scan_bench` | `MEASURED` |
...
**SAN-ViT-large.** Patch 4×4 → 64 tokens + CLS, d=384, 12 blocks, 6 heads, MLP
ratio 4. One CLS exit head per block. The real-scale ViT-L/16 ledger uses the
published 61.55 GMAC figure.
...
cohort (5 000 samples, 7 exit points), and a 1.2 M-sample stress cohort (5 exit
points, ImageNet-completo-sized). The host program `host_san_scan` compares
...
The 1.2 M stress cohort reaches **511 Msamples/s sustained**, **95%** of the
...
1.2 M cohort: 26.153 W. Incremental draw ΔP ≈ 1.7 W. The bench processed
...
| ViT-large | 4 | 0.262 | 183.3 TMAC | 369.3 TMAC | **32.0%** | 0.310 ms | 0.308 ms | 0.99x | L1–L4, L7–L8 PASS; L5 PASS, L6 exit-frac 0.03 |
...
ViT-large and GPT are essentially tied with Dense in wall time because the
...
**Patient channel.** ResNet-50 and ViT-large show the disclosed tradeoff the
...
### 4.3 Threshold ablation (in progress)

The Δ ablation is running on the Slurm `gpu-orangefs` partition (jobs
8599–8607): Δ ∈ {0.35, 0.45, 0.55, 0.65, 0.75} for ResNet-50 and Δ ∈ {0.25,
0.35, 0.45, 0.55} for ViT on CIFAR-10. Results will be reported here when the
jobs complete.

### 4.4 Larger proxy: CIFAR-100 / ImageNette-320 (in progress)

CIFAR-100 is being downloaded and the harness has been extended to support it.
Once the dataset is staged on OrangeFS, we will train SAN-ResNet-50 and
SAN-ViT-large with the same metered-MAC accounting. ImageNette-320/full remains
a candidate real-image proxy if bandwidth and storage permit. Results will be
reported here.
...
real architecture FLOP constants, (b) the 1.2 M-sample stress cohort, or (c)
...
**Attention family shows a patient-channel tradeoff.** SAN-ViT-large's
...
**The kernel is pre-silicon in spirit.** Although synthesized and run on real
hardware, the work did not include placement/routing exploration, multi-bitstream
campaigns, or comparison with HLS alternatives. The 135.2 MHz achieved clock is
the honest result of a single Vitis build.
...
architecture families. The U250 kernel runs at 511 Msamples/s on a 1.2 M-sample
