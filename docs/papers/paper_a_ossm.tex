...
Every modern state space model assumes associativity.
S4~\cite{gu2022efficiently} and Mamba~\cite{gu2024mamba} depend on it for efficient parallel scans: $(A_1 A_2) A_3 = A_1 (A_2 A_3)$ must hold to split work across processors.
Quaternion networks~\cite{parcollet2019quaternion} gave up commutativity for richer states, but kept associativity intact.
The field treats non-associativity as a bug to be avoided.

We ask: what if non-associativity is a feature?
...
This path dependence is not a defect---it is precisely what allows O-SSM to distinguish sequences that differ only in order.
...
\paragraph{Two models, one tower.}
O-SSM and diagonal SSM are not competitors---they occupy different rungs of the Cayley--Dickson tower.
O-SSM wins the 12 tasks where input order carries meaning (sorting, Morse, ListOps, sMNIST); diagonal wins the 2 tasks where each state dimension should gate independently (Adding Problem, long brackets).
A practitioner should choose their algebra the way a physicist chooses coordinates: match the symmetry to the problem.
...
\paragraph{The parallelism question.}
O-SSM cannot use associative parallel scans.
This is the price of non-associativity, and we do not pretend otherwise.
Mamba chose parallelism over expressiveness; we made the opposite bet.
Whether the 7 associative Fano sub-scans can be composed into a useful Moufang-aware partial scan remains open.
...
\paragraph{What we have not shown.}
Our largest model has 640 parameters and 32-dimensional hidden state.
This is proof-of-concept, not production.
The Adding Problem result (diagonal wins) is a genuine limitation, not an anomaly: when the task does not require order sensitivity, the extra coupling is wasted.
External replication in PyTorch or JAX would strengthen the case considerably.
...
SSMs have treated associativity as a requirement.
We showed it is a choice---and sometimes the wrong one.
O-SSM uses the 168 non-associative directions of the octonion algebra to create hidden states that remember not just \emph{what} they saw, but \emph{in what order}.
This is not a theoretical curiosity: across 15 benchmarks, it wins 12.
