...
We evaluate O-SSM against a diagonal (associative) SSM baseline across 11 tasks spanning order-dependent, symmetry, hierarchical, and gating benchmarks.
Both models use 8-dimensional hidden states with identical training protocols (SGD with momentum, 0.9).

\begin{table}[h]
\centering
\caption{Benchmark results. O-SSM wins 9/11 tasks. All tasks use 8-dim hidden state. Random baselines shown for reference. $\star$ = kill shot results.}
...
O-SSM dominates on \emph{order-dependent} tasks (sorting 69.5\% vs 35\%, Morse 44.5\% vs 14\%, sMNIST 24\% vs 10\%).
...
An extended O-SSM could use 27-dimensional Albert-valued states, with the cubic determinant as an $E_6$-invariant norm---a direction connecting neural architecture design to the exceptional mathematics of M-theory.
...
The magic square constrains the landscape of hypercomplex SSMs: the $(\R, \Oct)$ entry yields $F_4$, the symmetry group of the Albert algebra; the $(\Oct, \Oct)$ corner yields $E_8$, the largest exceptional Lie algebra.
No SSM built from an associative algebra can access this exceptional structure.
...
O-SSM wins 9/11 tasks where sequential order, symmetry, or hierarchical structure matters.
...
The sMNIST result (24\% vs 10\% on real MNIST data with only 5 epochs) and sorting result (69.5\% vs 35\%) provide evidence that the advantage transfers to real data and real sequential tasks.
