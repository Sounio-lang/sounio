...
- **The target is a constraint, not a summand.** The model *must* reach target performance $\tau$; models below target are infeasible, not merely expensive. This is the anti-Goodhart constraint, and §2.3 proves it cannot be moved into the objective without reintroducing the failure it prevents.
...
2. **The anti-Goodhart theorems** (§2.3–2.4): penalty formulations fail at a computable crossover weight $\lambda^*$ (Theorem 2.1, with a numerical certificate matching the closed form to within grid resolution); the abstention trap is universal across ethical weights (Theorem 2.2).
...
5. **A synthetic benchmark** (§5) with an eight-clause executable contract, all green: standard ML over-treats (3.2× necessary suffering), naive mercy abstains (the Goodhart pathology, made quantitative), mercyful training reaches target within 0.9% of necessary suffering at 13% of the training compute, the Theorem 2.1 crossover is realized dynamically, and the stability bound is never violated in 20 random field perturbations.
...
The theorem is constructive: $\lambda^*$ is *computable from declared quantities*, and §5.3 verifies the closed form against a grid measurement on the synthetic benchmark (3.0814 vs 3.0750) and shows gradient descent dynamically realizing the switch. Its content is ethical as much as mathematical: **any Lagrangian or penalty relaxation of the target constraint reintroduces, at a computable weight, exactly the trade the constraint exists to block.** An optimizer of the penalized objective treats the target as a price; mercyful training treats it as a boundary. This is the training-form analogue of the Lean-proved `goodhart_trap` of the companion formalization [11] and of the "categorical means non-priced" doctrine of the expanded-ethics core [9].
...
Theorem 2.2 is the expanded-ethics abstention trap of [9, Thm. T8] restated for training, and it is the sense in which *expanding the moral circle creates a new Goodhart failure*: the paradigm's own ethics, unconstrained, turn against it. The deontic component is the fence.
...
| Mercyful training reaches target at near-minimal suffering on the benchmark | **Measured** (synthetic; P1–P3, P6, P7) |
...
**Reinforcement learning and its failures.** RL maximizes expected cumulative reward [1]; Goodhart's law [2, 3] and the concrete-problems agenda [4] document the consequences. Mercyful Learning is the antithesis at the functional level (§2.6), not an RL variant.

**Safe and constrained RL.** Constrained MDPs optimize one expected cost subject to constraints on others [12]; CPO enforces near-constraint-satisfaction during policy learning [13]; safety-gym benchmarks measure violation rates [14]. The paradigm shares the hard-constraint instinct but differs in object and direction: constraints there bound *auxiliary expected costs*; here the constraint is the *target* (competence), and the *objective* is the suffering. The nonlinear-programming literature offers middle paths — exact penalties, exact when the weight exceeds a dual bound [24], and filter/restoration methods [23] — but Theorem 2.1 adds the mercyful reading: in this setting the suffering weight is itself the dangerous penalty parameter, chosen for ethical reasons, and above a computable value it purchases mercy with the target. Penalty and Lagrangian versions of either approach trade away their constraint at such a weight.
