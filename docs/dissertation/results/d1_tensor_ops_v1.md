...
```text
cd "$(git rev-parse --show-toplevel)" && bin/souc-lean-single-x86_64 tests/stdlib/tensor/test_tensor_ops_d1.sio /tmp/d1_tensor_ops.elf && chmod +x /tmp/d1_tensor_ops.elf && /tmp/d1_tensor_ops.elf
...
- `tests/stdlib/tensor/test_tensor_ops_d1.sio` (ELF `/tmp/d1_tensor_ops.elf`): stdout contained `D1_TENSOR_OPS_PASS`
