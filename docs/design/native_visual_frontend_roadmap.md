...
- `scripts/ci/native_visual_frontend_completion_audit.py` is the final completion contract: it groups the original plan into Visual IR, renderer, controls, physchem, authoring/persistence, replay/export, demos/CI, and evidence-chain categories, then fails if either coverage requirements or gate labels stop proving those categories.
