...
| A11 | **Finalize-path `IrModule` by-value copies → in-place `&! IrModule` (DONE, but memory-neutral; wall is elsewhere).** Branch `fix/madaros-irmodule-byref` (PR base main). Rewrote `ir_module_finalize_merged_calls` / `promote_canonical_into_stub_slots` / `compact_duplicate_fn_refs` / `restore_user_main_calls` + their 2 call sites to thread the merged module through `&! IrModule` (Box pointee) instead of 5+ whole-module by-value copies. **Slurm base↔branch differential (/proc VmHWM/VmPeak):** memory UNCHANGED — sret_fwd RSS 0.93→0.94 GB / VM 7.65→8.01 GB; cd_exact RSS 1.45→1.45 GB / VM 22.0→22.4 GB (noise). The by-value copies were never the wall (a `[IrFunction;2048]` copy does not fault in a 500 MB memcpy in the Madaros native runtime); the 22 GB VM is address-space reservation from per-module `Box<IrModule>` allocations in summary+body lowering, upstream of finalize. cd_exact still SIGSEGVs at `dep_begin 2 → bodies_begin` (module-2 **body** lowering), trace byte-identical base↔branch. W3: 12-test multi-module battery EXACT rc/stdout parity, zero green→red. Correct + zero-regression refactor; does NOT reduce memory, does NOT unblock cd_exact. Real wall needs `IR_MAX_FUNCS`/`IR_MAX_INSTRS` shrink or Box reuse across a module's summary/body lowering. | MEM base↔branch above; W3 parity `a11_after_full.log`↔`a11_base_full.log`. |
