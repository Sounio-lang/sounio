...
| A5 | Convergence + phase-2 PR | Opus+Haiku | A1–A4 | BLOCKED(2 Madaros native bugs: runtime forwarding miscompile + compile-time dep_begin crash) | cd_exact on Madaros: ZD PROVED + SQ PASS + NONZERO PASS + 16×COMP 0 |
| A8 | Cross-module forwarding miscompile (FINAL A5 blocker) | Opus | A5 | BLOCKED(fix/madaros-crossmod-sret-lowering, PR TBD) | ROOT-CAUSED (not the prompt's compile-time hypothesis — a RUNTIME miscompile). `var r = <inner struct-returning call>` in an imported dep fn lowers to corrupt IR: call `dst=3 arg=[2]` while the local `r`=vreg2 (the uninitialized arg slot) — result never bound to `r`, arg never = the param → field access derefs a garbage handle → SIGSEGV. SIZE-INDEPENDENT (`Small{[i64;4]}` repro `a8_diag_fwd.sio` rc=139; single-module identical shape `a8_diag_single.sio` rc=0). Disasm+IR evidence + fix entry points in `docs/handoff/continuity/A8_CROSSMOD_FORWARDING_MISCOMPILE.md`. Defect-2 (independent): compaction collapses distinct `*_zero` call targets onto one fn. Regression guards green: sret_8_field_return/generic_struct_return rc=0. |
...
| A8 | Madaros imported-lane RUNTIME miscompile of `var r = <inner same-module struct-returning call>` forwarding. Body lowered via the summary+bodies (imported) path binds `r` to the uninitialized call-argument vreg instead of the call result, and the param arg is never passed → garbage-handle deref → SIGSEGV. Single-module (single-pass) lowering of the identical source is correct. SIZE-INDEPENDENT. Compile succeeds; the produced ELF crashes at runtime. Distinct from the A6 compile-time `dep_begin 1` crash. | `a8_diag_fwd.sio` rc=139 vs `a8_diag_single.sio` rc=0; IR `call dst=3 arg=[2]`/disasm in docs/handoff/continuity/A8_CROSSMOD_FORWARDING_MISCOMPILE.md |
| A8 | Madaros merge compaction (`ir_module_compact_duplicate_fn_refs`) collapses distinct intra-module call targets: all `*_zero` callees (`small_zero`/`m16_zero`/`m64_zero`/`big_zero`) resolve to a single `fn_id` post-finalize. Independent of the crash; a correctness bug. | post_finalize IR: every `*_basis_fwd` calls `fn=5`(big_zero) |
