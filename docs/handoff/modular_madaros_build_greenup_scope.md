...
| **W5 kaxi buf API drift** | ✅ done | `compiler/main.sio` + `gpu/kaxi_backend.sio` | `kaxi.data` on heap-ptr `KaxiAsmBuf` — migrated to `kaxi_asm_write_file` (see W5 section) |
...
**CONFIRMED (2026-07-11): full stack → 0.** With **W5 fixed and committed** (`4855ff50e`) and W1+W3 re-stacked locally, `build_modular_madaros.sh` reports `typecheck: OK`, **0 errors**, and emits a runnable `madaros` compiler binary (`Madaros ready`, ~96.8 MB — consistent with the modular build's large module-scope BSS staging arrays). So the EISA modular typecheck is **fully clean once W1+W3 land** (via PRs #771/#773) — W2/W4/W5 are already committed to `gpu/epistemic-tensor-core-next`. (Runtime maturity — the emitted compiler correctly building non-trivial programs — is the separate native-v2 backend track, still open.)
...
### W5 — KaxiAsmBuf API drift in the K-AXI write path (✅ DONE, **EISA-specific**)
- **Resolved** (commit `4855ff50e`): added `kaxi_asm_write_file(path, buf)` in `gpu/kaxi_backend.sio` — copies `buf.len` bytes via the pub `kaxi_asm_get` into a module-scope BSS staging array (`KAXI_WRITE_STAGING: [i8; 16777216]`, demand-paged, not a stack local), then the `write_file` builtin. Co-located with the struct so it reads the private `len`. Repointed both main.sio call sites; dropped the stale `io_write_kaxi_binary`. **Verified:** the 2 `<main>` errors cleared, no new errors; with W1+W3 stacked the modular build is `typecheck: OK` / 0. Modular-main-only (`lean_single.sio` untouched → gen2==gen3 unaffected).
