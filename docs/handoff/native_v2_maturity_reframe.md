...
### Lane 3 — generic `<F>` with `[F; N]` field monomorphizer residual (✅ FIXED, commit `db3403e82`)

> **RESOLVED 2026-07-11:** `infer_bind_type_param`/`_list` added — struct-literal type-param inference now recurses into array/ref/tuple fields (both the active `check_struct_lit` `*mut`-spine site ~5374 and the by-value method ~22300). Verified: `spike_generic_struct_return.sio` E009→PASS (prints `1` + `spike PASS`). Regression: 24-test generics suite identical pre/post (15 PASS both, **zero regressions**). Front-end/modular only. Details below.

- **Symptom:** `spike_generic_struct_return.sio` → `error[E009] expected CDExact__i64 / found CDExact__T`, then *"Type check failed during IR lowering"* — it **fails at typecheck, never reaches codegen**.
- **ROOT CAUSE (pinned 2026-07-11):** `infer_type_params_from_fields` at **`check/check.sio:22300`** binds a struct's type param **only when the field type is directly `TyNamed(F)`** (`if tmpl_field_ty.kind == TypeKind::TyNamed`). For a field `c: [F; N]` (kind `TyArray`, inner `TyNamed(F)`) the guard is false → `F` is never bound from the field → the struct-literal type-arg stays unresolved → `mangle_append_type_name` emits the `__T` fallback → the arg type `CDExact__T` ≠ the param type `CDExact__i64`. **Discriminators:** scalar field `Box<F> { v: F }` → binds `F=i64`, compiles & runs (prints 7); array field `CDExact<F> { c: [F;4] }` → `__T`, even with an explicit `let a: CDExact<i64>` annotation. `substitute_type_param` (`check.sio:646`) already recurses into `ty.inner` — the 2026-07-06 diagnosis's point #4 is done; the residual is on the **inference** side, not substitution.
- **Fix:** at `check.sio:22300`, when `tmpl_field_ty` is a compound (`TyArray`/`TyRef`/`TyRefMut`/`TySlice`/`TyTuple`) whose inner references a type param, recurse and match it against the corresponding inner of `init_ty` to bind the param (`[F;4]` vs `[i64;4]` → `F=i64`). Front-end only; rebuild the modular madaros locally; verify with `madaros run spike`.
- **NOT #651 (verified):** #651 is a **codegen/runtime** `[struct;N]` aggregate-loop miscompile that its own filing says is **"NOT generics"** (repros with a plain non-generic `struct SmallR { c: [Rational;16] }`; garbage@16, SIGSEGV@2048; same family as #643/D6). Lane 3 is a **typecheck** monomorphizer-inference gap. Distinct bugs, distinct stages. Exact-algebra `<F>` (feature ④) is walled by **both**: Lane 3 at typecheck, #651 at codegen for `F=Rational`.
- **Where the fix lives (CONFIRMED — modular, not lean_single):** `bin/souc`'s default engine is the modular **Madaros** (`artifacts/self-hosted/madaros`). Its checker/monomorphizer is `main.sio`'s imports — `use check::mod`, `use check::*`, and `check/specializer.sio` (the monomorphizer) + `module_frontend.sio`. **`main.sio` does not import `lean_single`** — that is a separate self-contained bundle. So the fix is edit `check/specializer.sio`/`check.sio`/`module_frontend.sio` → **rebuild the modular madaros locally** (`build_modular_madaros.sh`, the exact W2/W4/W5 flow) → verify with `madaros run spike`. No `lean_single` edit, no Foundry.
- **Risk (CORRECTED — was overstated):** moderate and **local**. The 2026-07-06 diagnosis's "fix both engines + `lean_single.sio` + Foundry rebuild" mandate is **not required** to make the *shipping* compiler compile generic-`<F>`: `lean_single` is the bootstrap **seed / self-hosting-proof** anchor, not the day-to-day compiler, and no gate forces it to compile the exact-algebra consumer (it only self-compiles to prove gen2==gen3). Touching `lean_single` is **optional** — only if you also want the self-hosting *proof* to cover the feature. The real risk is monomorphizer subtlety (the `__T` leak on the constructed-value side), not proven-compiler exposure.
...
If a lane is picked: **Lane 2** (driver by-value-param) is the most tractable and lowest-risk — local, verifiable, no proven-compiler exposure — and is a prerequisite for exercising the driver-path gates anyway. **Lane 3** (generic-`<F>`) is real consumer value and — corrected above — is **modular-only + locally rebuildable** (not a `lean_single`/Foundry rabbit hole); its risk is monomorphizer subtlety, not proven-compiler exposure. **Lane 1** is highest-leverage but gated on a Foundry-vs-local feasibility probe.

> **Framing correction (2026-07-11):** the shipping compiler is the modular **Madaros** (`main.sio`), not `lean_single`. `lean_single` is only the bootstrap **seed source** (its derived seed compiles `main.sio`) and the **self-hosting proof** anchor (gen2==gen3). Capability work on this track = edit the modular sources + local rebuild; `lean_single` stays untouched unless you deliberately want the *proof* to grow to cover the new capability. This lowers the effective risk of Lanes 2 and 3.
