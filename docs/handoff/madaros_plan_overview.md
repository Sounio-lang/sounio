...
- The **native-v2 reframe** (#789, [[native_v2_maturity_reframe]]) is exactly the "engine split hides science" problem the plan names. Open hypothesis under investigation: whether the **generic-`<F>` `[F;N]` residual (Lane 3)** is a front-end monomorphizer/mangle issue (`__T` leak) or the back-end `[struct;N]` codegen bug **#651** that also walls exact-algebra with `F=Rational` — i.e. whether fixing it unblocks feature ④ generically.
