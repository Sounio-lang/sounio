...
validated_by: Codex coordinated compiler lane
source_of_truth: docs/internal/concepts/rebracketing-authority.md
-->
...
Status: hypothesis. The scalar protocol is executable locally; all references
to production source anchors below are static-only and establish no production
compiler behavior. Production compiler execution remains blocked.
...
- The production probe adds call-bearing, packed-register, uncertified
  ...
  eleven unchanged refusals when it becomes executable.
...
declarations; an executable cross-module production check remains part of the
strict blocked gate. Likewise, the scalar hash-collision case demonstrates only
that a diagnostic hash cannot replace structural identity. The production
transaction does not use that hash as function scope.
...
The strict acceptance path must check and execute the production runner with a
...
The strict form must be run in the Sounio Compiler Foundry or the approved
Slurm path, not as a full stress build in `/workspace/sounio`.
...
- Merge readiness while the strict gate reports a blocked production runner.

## Current Blocker

```text
Blocker-ID: BLK-20260715-REBRACKET-MODULAR-SOURCE
Status: classified
Severity: B1
Class: compiler-semantics
Owner: Codex rebracketing-authority coordination lane, with dependencies on the issue #854 and IR-capacity owners
...
Observed: the focused production import parses, then current Madaros reports the known contextual E175 failures and the existing IrFunction [2048] versus [1024] initializer mismatch
Expected: focused production runner checks and prints its exact PASS receipt
Acceptance-Gate: SOUNIO_REBRACKET_COMPILER_BIN=<current-source-foundry-madaros> SOUNIO_REBRACKET_REQUIRE_COMPILER=1 bash scripts/ci/exact_bitwise_rebracket_authority_gate.sh
Evidence-Level: E2
Fallback-Path: none
Legacy-Kept: yes; legacy Add/Mul rewriting remains explicitly outside this authority claim
Next-Action: validate after the issue #854 contextual-binding and IrFunction-capacity stack lands, then prove default native-v2 optimizer reachability
```
...
Write-Set: self-hosted/ir/opt_cleanup.sio; self-hosted/ir/rebracket_authority_self_test_runner.sio; tests/compiler/rebracket_authority_*; scripts/ci/exact_bitwise_rebracket_authority_gate.sh; docs/internal/concepts/rebracketing-authority.md; docs/internal/concepts/registry.tsv
Read-Set: self-hosted/ir/ir.sio; self-hosted/compiler/main.sio; scripts/ci/no_false_float_axioms.sh; tests/compiler/madaros_visibility_context/*
...
Authoritative-Only-If: strict production runner executes, no fallback is used, Foundry build is green, and the default compiler path is shown to reach the guarded mutation
```

## Integration Receipt

```text
Semantic-Outcome: implementation-shaped hypothesis with executable scalar protocol and static-only production source anchors
...
Evidence-Run: local 11-case kernel; exact E175/E176 controls; no-false-float guard; production blocker classifier
Fallback-Path: none
Legacy-Kept: legacy Add/Mul Block L path retained outside the new claim
Conflicting-Lanes: issue #854 contextual binding and pending IrFunction-capacity/source-build stack
