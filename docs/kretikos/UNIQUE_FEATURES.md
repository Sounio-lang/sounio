...
| Cross-backend semantic triangulation | The documented GPU pipeline targets PTX, Metal, and SPIR-V from the same GPU IR surface. Kretikos now has a structural triangulation gate for `vec_add_f32` and `epistemic_dual_output_f32`. The gate emits PTX, Metal, and SPIR-V artifacts, records hashes, checks backend-specific value/uncertainty/validity markers, and binds them to scalar oracles. This is not yet runtime equivalence across all backends. | `docs/architecture/GPU_PROGRAMMING_GUIDE.md`; `self-hosted/gpu/ptx.sio`; `self-hosted/gpu/metal.sio`; `self-hosted/gpu/kretikos_emit_spirv.sio`; `self-hosted/gpu/kaxi_to_ptx.sio`; `self-hosted/gpu/kaxi_to_c.sio`; `self-hosted/gpu/metal_render.sio`; `self-hosted/gpu/spirv_render.sio`; `self-hosted/gpu/wgsl_render.sio`; `self-hosted/gpu/runtime/cpu_ir_interpreter.sio`; `scripts/ci/kretikos_cross_backend_semantic_gate.sh` | `bash scripts/ci/kretikos_cross_backend_semantic_gate.sh` writes `artifacts/omega/kretikos_cross_backend_semantic/kretikos_cross_backend_semantic.v1.json` with PTX, Metal, SPIR-V hashes and marker checks for value add, epsilon square-sum, and validity AND. Existing related check: `scripts/ci/kaxi_ptx_golden_gate.sh`. | Infrastructure present | Extend the gate from structural markers to runtime comparison where backend runtimes are available, then add provenance-lane output digests. |
