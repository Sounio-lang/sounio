...
| Cross-backend semantic triangulation | The documented GPU pipeline targets PTX, Metal, and SPIR-V from the same GPU IR surface. Kretikos now has a structural `vec_add_f32` triangulation gate that emits PTX, Metal, and SPIR-V artifacts, records hashes, and checks backend-specific add markers against a scalar add oracle. This is not yet runtime equivalence across all backends. | `docs/architecture/GPU_PROGRAMMING_GUIDE.md`; `self-hosted/gpu/kaxi_to_ptx.sio`; `self-hosted/gpu/kaxi_to_c.sio`; `self-hosted/gpu/metal_render.sio`; `self-hosted/gpu/spirv_render.sio`; `self-hosted/gpu/wgsl_render.sio`; `self-hosted/gpu/runtime/cpu_ir_interpreter.sio`; `scripts/ci/kretikos_cross_backend_semantic_gate.sh` | `bash scripts/ci/kretikos_cross_backend_semantic_gate.sh` writes `artifacts/omega/kretikos_cross_backend_semantic/kretikos_cross_backend_semantic.v1.json` with PTX, Metal, SPIR-V hashes and add-marker checks. Existing related check: `scripts/ci/kaxi_ptx_golden_gate.sh`. | Infrastructure present | Extend the gate from structural add markers to runtime comparison where backend runtimes are available, then add one epistemic kernel with value and uncertainty output digests. |
...
6. Extend cross-backend semantic triangulation from structural `vec_add_f32`
   markers to backend runtime outputs where the backend runtime is available.
