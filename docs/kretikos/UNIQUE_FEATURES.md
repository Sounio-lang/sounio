...
| Epistemic tensor fragments | The GPU guide documents `Knowledge<T>` lowering as value, epsilon, validity, and provenance lanes; tensor-core-oriented epistemic modules already exist. The missing feature is a first-class fragment abstraction that carries these lanes through tensor operations instead of treating epistemic state as side-channel code. | `docs/architecture/GPU_PROGRAMMING_GUIDE.md`; `self-hosted/gpu/tensor_epistemic.sio`; `self-hosted/gpu/epistemic_tensor_core.sio`; `self-hosted/gpu/epistemic_tensor_core_optimized.sio`; `self-hosted/gpu/epistemic_mma_reference.ptx` | Existing related gates: `scripts/ci/native_v2_epistemic_accel_spine_gate.sh`; `scripts/ci/epistemic_witness_gate.sh`. Needed: a tensor-fragment gate that validates value, epsilon, valid, and provenance outputs against a CPU oracle. | Infrastructure present | Define `EpistemicFragment` at the GPU IR/K-AXI boundary and lower one 16x16 tensor kernel with value plus uncertainty/provenance validation. |
...
| Cross-backend semantic triangulation | The documented GPU pipeline targets PTX, Metal, and SPIR-V from the same GPU IR surface. Kretikos now has a structural triangulation gate for `vec_add_f32` and `epistemic_dual_output_f32`. The gate emits PTX, Metal, and SPIR-V artifacts, compiles the PTX artifacts with `ptxas`, records hashes, checks backend-specific value/uncertainty/validity markers, and binds them to scalar oracles. This is not yet runtime equivalence across all backends. | `docs/architecture/GPU_PROGRAMMING_GUIDE.md`; `self-hosted/gpu/ptx.sio`; `self-hosted/gpu/metal.sio`; `self-hosted/gpu/kretikos_emit_spirv.sio`; `self-hosted/gpu/kaxi_to_ptx.sio`; `self-hosted/gpu/kaxi_to_c.sio`; `self-hosted/gpu/metal_render.sio`; `self-hosted/gpu/spirv_render.sio`; `self-hosted/gpu/wgsl_render.sio`; `self-hosted/gpu/runtime/cpu_ir_interpreter.sio`; `scripts/ci/kretikos_cross_backend_semantic_gate.sh` | `bash scripts/ci/kretikos_cross_backend_semantic_gate.sh` writes `artifacts/omega/kretikos_cross_backend_semantic/kretikos_cross_backend_semantic.v1.json` with PTX, PTXAS CUBIN, Metal, and SPIR-V hashes plus marker checks for value add, epsilon square-sum, and validity AND. Existing related check: `scripts/ci/kaxi_ptx_golden_gate.sh`. | Infrastructure present | Extend the gate from structural markers to runtime comparison where backend runtimes are available, then add provenance-lane output digests. |
...
6. Extend cross-backend semantic triangulation from structural `vec_add_f32`
   and `epistemic_dual_output_f32` markers to backend runtime outputs where the
   backend runtime is available.
