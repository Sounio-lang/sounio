...
| Cross-backend semantic triangulation | The documented GPU pipeline targets PTX, Metal, and SPIR-V from the same GPU IR surface. Kretikos also has renderers/lowerers for PTX, Metal, SPIR-V, WGSL, and C-style paths. The unique feature is not just multi-backend emission; it is comparing backend outputs against a CPU oracle and preserving digests as evidence. | `docs/architecture/GPU_PROGRAMMING_GUIDE.md`; `self-hosted/gpu/kaxi_to_ptx.sio`; `self-hosted/gpu/kaxi_to_c.sio`; `self-hosted/gpu/metal_render.sio`; `self-hosted/gpu/spirv_render.sio`; `self-hosted/gpu/wgsl_render.sio`; `self-hosted/gpu/runtime/cpu_ir_interpreter.sio` | Existing related check: `scripts/ci/kaxi_ptx_golden_gate.sh`. Needed: a cross-backend oracle gate that records output digests and max error per backend. | Infrastructure present | Pick one small numerical kernel and one epistemic kernel, lower each to PTX, Metal, SPIR-V or CPU-interpreter where available, and store comparable validation digests. |
...
1. Extend evidence-carrying CUBIN from the five f32 CUDA rungs to f64 and
   epistemic runtime-backed rungs.
...
6. Build cross-backend semantic triangulation only after each backend has a
   clear capability envelope for the selected kernel.
