<!-- docs:meta
topic_id: repo.docs.decisions.adr-009-verified-foreign-reference
authority: repo_only
audience: users
last_validated: 2026-09-04
validated_by: Claude Sonnet 5
source_of_truth: docs/governance/topic-registry.v1.json#repo.docs.decisions.adr-009-verified-foreign-reference
-->
...
Python/mpmath/SciPy/Rust to claim authority)
...
**Hard exclusion (non-negotiable):** **Python** and **Rust** do not qualify
and are not reclassified by this ADR. Their role (measurement,
corroboration, bug-hunting, research harnesses, or incidental tooling) is
unchanged from ADR-008 / bootstrap practice. Static typing alone is not
enough; the admitted languages below are chosen for purity, proof-carrying
power, effect-system authority, or existing bootstrap SOTA — not for
general systems convenience.
...
above for the claim at hand. Another **SOTA++++** language may be admitted
only if it meets all four criteria **and** is not Python or Rust.
...
- Python/mpmath/SciPy **and Rust** remain `external_corroboration_only` /
  `research_harness` (or tooling). This ADR does not restore their
  authority as claim clocks.
- Ladder stages (e.g. f128/f256 softfloat) that today use Python limb
  oracles must classify that path as corroboration only; claim pass/fail
  is `sounio_native_expected` / `sounio_closed_form_twin`, or a future
  `verified_foreign_reference` (F\*/Futhark/…) that meets §1.
