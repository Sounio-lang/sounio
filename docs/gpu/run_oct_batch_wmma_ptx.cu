...
int main(int c,char**v){const char*p=c>1?v[1]:"/tmp/obw.ptx";
 double a[8]={0.5,-0.2,0.3,0.0,0.1,0.0,-0.4,0.2};
 double H[128]; for(int b=0;b<16;b++)for(int r=0;r<8;r++)H[b*8+r]=((b+1)*0.15+r*0.07)*((r%2)?-1:1);
 // reference D[k*16+b] = (a ⊗ H_b)[k]
 double ref[256]={0};
 ...
 CUdeviceptr pa,pH,po;CK(cuMemAlloc(&pa,8*8));CK(cuMemAlloc(&pH,128*8));CK(cuMemAlloc(&po,256*4));
 CK(cuMemcpyHtoD(pa,a,8*8));CK(cuMemcpyHtoD(pH,H,128*8));float z[256]={0};CK(cuMemcpyHtoD(po,z,256*4));
 ...
 printf("Sounio source-level BATCHED octonion `oct_batch_mul` tensor-core GB10: D=L(a)*H  mismatch %d/128 maxerr=%.4f\n",fails,mx);
 if(!fails){printf("PASS: compiler-emitted tensor-core L(a)*H tile from source matches scalar reference on GB10\n");return 0;}
 printf("FAIL\n");return 1;}
