...
- **D3 — multi-module native path residuals (partial closeout).** Historically: segfault in `lower_array` dep-lowering, or thin-link `rc=12` on combined imports. Extends `docs/audit/MADAROS_MULTIMODULE_FALLBACK_SEGFAULT_2026-06-30.md`, `docs/audit/MADAROS_NATIVE_MULTIMODULE_SCALE_2026-07-14.md`. Issues #901, #921.
  ...
  - **Remaining D3 surface:** concrete shipped exclusive-ref science witnesses are green (unsplit `oct_mul`, imported `associator_field`); gate `scripts/ci/madaros_d3_exclref_shipped_gate.sh` → `MADAROS_D3_EXCLREF_SHIPPED_GATE_OK`. ~~stats OLS E019~~ / ~~open-slice `.len()`~~ closed earlier. **Still open:** (1) trait-impl methods on primitive `i64` SIGSEGV at Madaros lower (`BLK-20260806-madaros-trait-i64-method-lower`); (2) `cayley_dickson_exact` / `cd_exact_generic_i64` Madaros preflight E035/E011 blocks memory-wall re-measure (`BLK-20260806-madaros-d3-cd-exact-e035-preflight`). Handoff: `docs/handoff/MADAROS_D3_EXCLREF_MEMWALL_2026-08-06.md`. Do not over-claim “all multi-module is green.”
...
Workarounds: use free functions / inherent struct methods instead of trait methods on
primitives under Madaros; run `cd_exact_generic_i64` under `lean_single` until the
E035/E011 preflight is repaired. Recommended residual order: **trait-for-i64 method
lowering → cd_exact Madaros check → then IrModule memory-wall re-measure**.
