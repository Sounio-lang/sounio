...
Several active bugs were found 2026-08-22 while building `stdlib/net/socket.sio`, `stdlib/net/dns.sio`, and `stdlib/net/http_client.sio` (real TCP sockets, hostname resolution, and a plain HTTP/1.1 client via raw syscalls). All have working stdlib-level workarounds documented in those files' top-of-file comment blocks; none have been fixed in the compiler itself.

**`extern "C"` silently no-ops for any name outside a fixed allow-list**: `append_extern_c_stubs()` in `self-hosted/compiler/lean_single.sio` (~line 29575) only recognizes the `getpid` family, a dozen libm functions, and `malloc`/`free`. Any other extern name (e.g. `socket`, `connect`, `bind`) compiles without error but is silently replaced with a generated stub that guesses a signature from arity and panics at runtime on every call. This means `stdlib/os/process.sio` and `stdlib/mem/box.sio` — commonly cited as "proof `extern \"C\"` FFI works" — only ever exercise the allow-listed names, not general C FFI. **Workaround**: use the `syscall6(nr, a1..a6)` builtin directly (the same intrinsic `getpid` etc. are implemented on top of) instead of declaring new `extern "C"` functions for anything not already allow-listed.

**Tuple-destructured structs cannot be passed as call arguments**: `let (x, err) = f()` followed by passing `x` itself (by value or by `&`/`&!` reference) to another function fails with `error[E001]: Type mismatch in call argument`, even though `x`'s type is correct and reading a scalar field off it (`x.fd`) works fine. Reproduced with both plain and `linear` structs, same-module and cross-module. **Workaround**: rebuild via a fresh struct literal before passing (`let x2 = TcpSocket { fd: x.fd }`).

**`u16` arithmetic is broken for most operators**: `as i32`/`as i64` casts on a `u16` return garbage (the bit pattern looks like an IEEE-754 double encoding, suggesting the cast routes through a float-conversion path). `&`, `|`, `>>`, `<<`, `%` between two `u16` operands silently return the left operand unchanged rather than computing anything. `/` is inconsistent depending on whether operands are inline literals or pass through a `let` binding first. Only `+`, `-`, `*`, `==`, `<`, `<=`, `>`, `>=` compute correctly; a `while` loop re-checking a condition on a mutated `var: u16` evaluates the condition wrong after the first iteration (the equivalent straight-line `if`-sequence, no back-edge, works). **Workaround**: avoid the broken operators on `u16` entirely; `stdlib/net/socket.sio`'s `port_hi_lo` isolates a `u16` port's two bytes via an unrolled binary-search-style sequence of `if`/subtraction statements using only `+`/`-`/comparisons, then casts the resulting sub-256 values to `u8`.

**`[u8; N]` (and other narrow-element) arrays use an 8-byte stride internally instead of 1-byte — a layout-compatibility bug, not a memory-corruption bug**: the compiler allocates `[u8; N]` arrays with an internal 8-byte-per-element stride regardless of the element's real 1-byte size — `arr[0]` lands at the real base address, `arr[1]` lands 8 bytes further on, `arr[2]` 16 bytes further, etc. Both indexed reads and indexed writes use this same stride consistently, so **ordinary in-language use is memory-safe**: this was directly tested against the committed compiler by placing guard i64 values immediately before and after a `[u8; 16]` field (plus a second adjacent `[u8; 16]` field) and confirming both stayed intact across many index positions 0-15, and by round-tripping a full `[u8; 4096]` write-then-verify correctly across all 4096 bytes. An earlier version of this entry claimed this was "a real stack-buffer-overflow risk" (a naively-indexed 16-byte buffer supposedly corrupting 104 bytes of adjacent stack memory) and that `Buf4096` was safe only because it is "oversized enough to never overflow" — that claim was empirically FALSE and has been corrected here. The real, narrower problem is a **layout-compatibility bug**: because of the 8-byte internal stride, `[u8; N]` has no C-compatible 1-byte-packed layout, so a raw pointer taken into an indexed `[u8; N]` array cannot be handed to a syscall (or any other consumer) expecting packed bytes at real 1-byte offsets — the syscall has no knowledge of Sounio's internal stride and would read/write at up to 8x the intended offsets in the *external* memory it is given, which is a real problem specifically for syscall-facing buffers. **Workaround** (unchanged — still correct regardless of the corrected justification above): never take a raw pointer into a `[u8; N]`/other-narrow-type array indexed beyond element 0 for anything handed to a syscall; stage data through an `i64`-word buffer instead (correctly-strided for 8-byte elements) and transcode byte-level reads/writes against that, as `stdlib/net/socket.sio`'s `pack_bytes`/`unpack_bytes`/`SockAddrWords`/`WordBuf512` do. Ordinary in-language indexed use of `Buf4096`/similar structs (`buf.data[i] = x` at any in-bounds `i`, never taking a raw pointer into it) is safe as-is.

**Module-path resolution for sibling stdlib modules is asymmetric between a top-level program and a file already inside that module's own directory**: `stdlib/net/dns.sio` (itself already inside `net/`) fails to typecheck with `use net::socket::Buf4096`, `use net::socket::*`, and `pub use net::socket::*` — every later `Buf4096 { data: [...] }` struct-literal use then fails with `` error[E200] `Buf4096` at line ... `` — even though `tests/run-pass/net_socket_loopback.sio` (a top-level program outside `net/`) successfully imports the identical struct via that same fully-qualified `use net::socket::*` form. From inside `dns.sio`, only the sibling-relative `use socket::*` (no `net::` prefix) typechecks; `pub use socket::Buf4096` (a single named re-export instead of a glob) failed the same way as the fully-qualified form, so only the unqualified glob form (`use socket::*`) was confirmed to work for re-exporting. Reproduce: from a file inside `net/`, try `use net::socket::*` — it fails with `E200` on the imported struct's first use; the identical `use net::socket::*` from a top-level program file compiles cleanly. **Workaround**: a file inside `net/` importing a sibling module in the same directory must use the short sibling-relative form (`use socket::*`, `use dns::*`), never the fully-qualified `net::socket::*`/`net::dns::*` form; a top-level program outside `net/` must use the fully-qualified form instead. See `stdlib/net/dns.sio`'s header comment (SIXTH COMPILER LIMITATION) for the full writeup.

**Linear-type enforcement does not cross module boundaries**: a `linear struct` declared and consumed within the same file is correctly rejected by the compiler when left unconsumed (`tests/compile-fail/linear_not_consumed.sio` still fails to compile with `linear value not consumed`, confirmed 2026-08-22). The identical unconsumed-value pattern against a `linear struct` *imported* via `use` from another module (e.g. `TcpSocket` from `stdlib/net/socket.sio`) compiles successfully with no error — the linearity check is silently skipped once the type crosses a module boundary. This significantly weakens the resource-safety guarantee `linear` is meant to provide, since real code almost always consumes a library's linear types from a different module than the one that declares them. No workaround restores the compile-time guarantee; callers must rely on manual discipline (always pass the value to a consuming function) until this is fixed. Documented via `//@ known-failure` in `tests/compile-fail/net_socket_linear_not_consumed.sio`.

**`str::lib`'s `str_from_i64`/`str_to_string` segfault when called cross-module**: calling `str_from_i64` (which internally calls its own file-private helpers `str_i64_to_buf`/`str_buf64_rev`/`str_from_buf_i64`) from a different module or a standalone program segfaults at runtime despite typechecking cleanly. `souc check` independently surfaces real `E200`/warning-level diagnostics inside those private helpers when `str::lib` is imported into another file — corroborating the chain is genuinely broken cross-module, not merely fragile. **Workaround**: treat `str::lib::Str` as a plain struct shape only (build/read its fields directly); never call `str_from_i64`/`str_to_string` (or anything that transitively calls them) from outside `stdlib/str/lib.sio` itself.

**The scalar half of a `(linear struct, i32)` tuple return can misread as `0`, masking a real nonzero error**: functions with the shape `fn f(...) -> (SomeLinearStruct, i32)` (e.g. `stdlib/net/socket.sio`'s `tcp_connect`/`tcp_listen`/`tcp_accept`, which return `(TcpSocket, i32)`) were observed, via direct instrumentation with zero intervening calls between the tuple-destructure and the first field read, returning a scalar error code of `0` in the same observation where the struct's own field (`fd == -1`) correctly showed failure — a combination the producing function's own source cannot legitimately produce (every return path pairs `fd == -1` with a nonzero error code). Not fully root-caused; the leading hypothesis is a register/stack-slot allocation bug specific to `(linear struct, scalar)` tuple returns, not confirmed. **This affects any existing caller that trusts the paired scalar instead of the struct's own sentinel field** — including `tests/run-pass/net_socket_loopback.sio` and `tests/run-pass/net_socket_highport.sio`'s `if listen_err != 0`/`if connect_err != 0` checks, both committed before this bug was found. **Workaround**: treat the struct's own field (e.g. `sock.fd < 0`) as the sole authoritative success/failure signal for any function returning `(linear struct, scalar)`; never trust the paired scalar alone. The already-committed socket tests (`net_socket_loopback.sio`, `net_socket_highport.sio`) were checked against this bug during final review and found NOT exposed to a silent false-negative from it: both assert exact payload/byte-count values that cannot spuriously pass on a failed operation, so a scalar-half misread would surface as an assertion failure rather than a silently-accepted wrong result. This is a closed assessment — no hardening of those tests is required.

All previously listed compiler bugs (prior to 2026-08-22) have been fixed in `self-hosted/compiler/lean_single.sio` and are live in the current `bin/souc-native` binary (rebuilt 2026-04-20) — see the sections below.
