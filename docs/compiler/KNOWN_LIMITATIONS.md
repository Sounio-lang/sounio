...
Last reconciled: 2026-08-24 against `origin/main` at `7ecec1088158db3a92983cb3c96f9fc52f5ed19e`. Engine- and artifact-specific limits below name their scope; an unqualified claim must hold for both Madaros and `lean_single`.
...
|-----------|--------------|---------------|
| **Standard library support surface** | `stdlib.surface = validated_research` | Claim only the bounded support contract checked by `scripts/ci/sounio_stdlib_surface_support_gate.sh`: current inventory has 1604 `.sio` files, 0 disabled files, 0 stub-only `mod.sio` files, and 178 active module entrypoints; package-backed epistemic/GUM, units, formats, io-primitives, canonical PETAB, and PBPK/GUM workflows pass through `scripts/ci/package_pbpk_gum_gate.sh`. **NOT PROVED:** broad all-file stdlib callability, `scripts/ci/stdlib_evolution_gate.sh`, hyper native lanes, fMRI/PBPK science pipeline, external runtime dependencies, cryptographic security, clinical/regulatory validity, or API stability beyond the checked gate. |
...
## Hessian AD is engine- and artifact-specific (β⁷)

### Shipped Madaros artifact

The committed Madaros ELF was last refreshed at `842195928ae5`, before
#2115/#2119. `bin/souc --version` explicitly reports that it was not built from
the reconciliation tree. Compiling and running
`tests/run-pass/madaros_hessian_transcendental.sio` with that ELF prints zeros
for the four transcendental rows and `MADAROS_HESSIAN_TRANSCENDENTAL_FAIL`.
Therefore the distributed default does **not** carry the current-source Hessian
improvement; rebuild Madaros before citing it.
...
by the merged #2119 source/witness pair; it is not a receipt for the stale
committed ELF described above.
