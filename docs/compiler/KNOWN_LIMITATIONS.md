...
| **combined zero-provenance (sedenion+eisa)** | **fail-closed** on stock Madaros (`rc=12` thin-link); green under `lean_single` | `tests/known_failures/zero_provenance_native_v2_probe.sio` — do not claim Madaros-green |
...
Do not promote zero-provenance or receipt constructors to Madaros `run-pass`
until default Madaros prints their PASS markers without `SOUNIO_SOUC_ENGINE=lean_single`.
