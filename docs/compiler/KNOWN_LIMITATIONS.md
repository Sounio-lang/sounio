...
**WP-A11 — finalize-path `IrModule` by-value copies converted to in-place `&! IrModule` (DONE; memory-neutral; the wall is elsewhere).** The multi-module compile-to-file path (`module_frontend_compile_imported_to_file` / `module_frontend_merge_imported_into`) finalized the merged module by value: `var merged_module = ir_module_finalize_merged_calls(*module_box)` deref-copied the ~250–500 MB `IrModule` out of its `Box`, and `ir_module_finalize_merged_calls` itself threaded `var out = module; out = promote(out); out = compact(out); out = promote(out)` — 5+ whole-module by-value copies — before a second by-value `ir_module_restore_user_main_calls(merged_module, …)`. WP-A11 rewrote `ir_module_finalize_merged_calls`, `ir_module_promote_canonical_into_stub_slots`, `ir_module_compact_duplicate_fn_refs`, and `ir_module_restore_user_main_calls` to operate through `&! IrModule` (the `Box` pointee), extracting each function slot to a 256 KB local, mutating it, and writing the whole slot back (the proven 2-level `(*module).functions[fi] = f` idiom; no miscompiled 3-level `(*m).functions[fi].instrs[ii] = …` store). Both call sites now run `ir_module_finalize_merged_calls_ip(&! (*module_box))` + `ir_module_restore_user_main_calls_ip(&! (*module_box), …)` in place. **Empirical result (Slurm, base↔branch differential, `/proc` VmHWM/VmPeak high-water):** peak memory is UNCHANGED — a finalize-reaching 2-module build (`sret_forwarding_cross_module_min`) peaks RSS 0.93 GB→0.94 GB / VM 7.65 GB→8.01 GB, and `cd_exact` peaks RSS 1.45 GB→1.45 GB / VM 22.0 GB→22.4 GB (both within run-to-run noise). The by-value copies were never the wall: in the Madaros native runtime a `[IrFunction; 2048]` whole-struct copy does not fault in a full 500 MB memcpy (RSS never exceeds ~1 GB even with base's 5 finalize copies live). The real multi-module footprint is **address-space reservation from the per-module `Box<IrModule>` allocations during summary+body lowering** (≈15 IrModule-sized reservations for 2 modules → 7.65 GB VM; ≈45 for 4-module cd_exact → 22 GB VM) — which the finalize refactor does not touch, and which is upstream of where `cd_exact` faults. Regressions: 12-test multi-module battery EXACT rc/stdout parity base↔branch (sret_forwarding_cross_module_min, sret_8_field_return, generic_struct_return "6"/"spike PASS", turbofish 3/3, turbofish_native, generic_struct_return_structf E035, cd_exact_generic_vs_concrete/associator_field_octonion/algebra_g2_invariants_import/cardinality_erdos rc139, audit_trail E137). The change is correct and zero-regression but does NOT reduce memory and does NOT unblock `cd_exact` (still SIGSEGVs at module-2 body lowering, byte-identical trace base↔branch). Reducing the wall needs shrinking `IR_MAX_FUNCS`/`IR_MAX_INSTRS` or reusing one `Box<IrModule>` across the summary/body lowering of each module — both larger, higher-risk than this refactor.
