...
- **D3 — multi-module native path residuals (partial closeout).** Historically: segfault in `lower_array` dep-lowering, or thin-link `rc=12` on combined imports. Extends `docs/audit/MADAROS_MULTIMODULE_FALLBACK_SEGFAULT_2026-06-30.md`, `docs/audit/MADAROS_NATIVE_MULTIMODULE_SCALE_2026-07-14.md`. Issues #901, #921.
  ...
  - **Remaining D3 surface:** exclusive-ref / memory-wall fragile chains; ~~stats OLS multi-mod still red with **`E019` method calls**~~ **closed 2026-08-04:** fixed-array OLS (`stats::ols_fixed` + cooks + shapiro) and `stats::validation` (`[f64; 256]`+`n`) green under Madaros. ~~Open-slice `&[f64]` + imported `.len()` lower segfault~~ **CLOSED 2026-08-06:** intrinsic `IrArrayLen` lowering + println scalar_kind=1; gate `scripts/ci/madaros_d3_openslice_len_gate.sh` → `MADAROS_D3_OPENSLICE_LEN_GATE_OK`. Audit: `docs/audit/MADAROS_D3_OPENSLICE_LEN_2026-08-06.md`. Do not over-claim “all multi-module is green.”
...
residual order after D1/D2/#921/#901-scale/**D4**/validation-E019/**D6**/open-slice-`.len()`
closeout: **D3 exclusive-ref / memory-wall chains** (IrModule footprint / fragile
`&!` paths). Fixed-buffer `stats::validation` and open-slice `.len()` are gated green.
