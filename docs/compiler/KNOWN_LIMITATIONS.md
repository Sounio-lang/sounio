...
Four active bugs were found 2026-08-22 while building `stdlib/net/socket.sio` (real TCP sockets via raw syscalls). All four have working stdlib-level workarounds documented in `stdlib/net/socket.sio`'s top-of-file comment block; none have been fixed in the compiler itself.
...
**`[u8; N]` (and other narrow-element) indexed writes use an 8-byte stride instead of 1-byte**: `arr[i] = x` for a `[u8; N]` array computes the write address as if the element were 8 bytes wide, regardless of its real 1-byte size — `arr[0]` lands correctly, `arr[1]` lands 8 bytes further on, `arr[2]` 16 bytes further, etc. Reads via the same indexing are "wrong" in the same consistent way, so purely-logical code that only ever reads what it wrote through the array (never takes a raw pointer to it) looks completely correct and self-consistent while being actually broken underneath — this was only caught by dumping raw memory via `write()`/`od -c` rather than trusting in-language reads. **This is a real stack-buffer-overflow risk**: a naively-written 16-byte buffer indexed up to byte 15 actually writes up to offset 120, corrupting 104 bytes of adjacent stack memory. Safe only when a byte array is never indexed anywhere near its declared bound (e.g. `Buf4096` in `stdlib/net/socket.sio`, which is 4096 bytes but only ever touches the first few dozen). **Workaround**: never take a raw pointer into a `[u8; N]`/other-narrow-type array indexed beyond element 0 for anything performance/address-sensitive; stage data through an `i64`-word buffer instead (correctly-strided for 8-byte elements) and transcode byte-level reads/writes against that, as `stdlib/net/socket.sio`'s `pack_bytes`/`unpack_bytes`/`SockAddrWords`/`WordBuf512` do.
