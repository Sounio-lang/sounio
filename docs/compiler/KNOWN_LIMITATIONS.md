...
- **D2 — `&local_array` passed to a builtin receives a wrong base pointer.** `read_file`, `write_file`, `str_from_bytes` all SIGSEGV or write garbage. A user-defined fn receives the identical `&[i8; N]` correctly, so the mis-lowering is in builtin-call argument marshalling. Blocks all Data I/O readers + the file sink (stdout writers `stdlib/data/{csv,json}.sio` ship around it). Dispatch: `docs/audit/DATA_IO_TRILHA_B_BUILTIN_BUFPTR_DISPATCH_2026-07-14.md`. Issue #933.
