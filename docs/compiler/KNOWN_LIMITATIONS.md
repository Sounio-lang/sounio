...
**Imported-module native path (D1–D4) — OPEN (2026-07-14).** On the default **native** engine, composing real modules fails or *silently miscompiles* in four distinct ways. Full catalogue + minimal repros + priority: [`docs/audit/MADAROS_IMPORTED_MODULE_NATIVE_PATH_ESCALATION_2026-07-14.md`](../audit/MADAROS_IMPORTED_MODULE_NATIVE_PATH_ESCALATION_2026-07-14.md). Which stdlib results still survive native import: [`docs/audit/EPISTEMIC_TRUST_MAP_2026-07-14.md`](../audit/EPISTEMIC_TRUST_MAP_2026-07-14.md). Live trip-wire gate: `scripts/epistemic_trust_gate.sh`.

- **D1 — `f64 → i64/i32` cast in an imported-module body is a bit reinterpretation, not a truncating convert.** *Silent wrong numbers*: `4.172 as i64` returns the IEEE-754 payload `4616383272838735331`. So `stdlib/epistemic/gum.sio`'s `dof_to_i64(nu_eff)` makes `gum_k95` return 1.960 for **all** effective dof (correct is `t95(nu_eff)`), silently under-covering every finite-sample `U95`/`U99`. The same cast in the importer's own `main()` is correct. Dispatch: `docs/audit/MADAROS_IMPORTED_MODULE_F64_CAST_BITCAST_2026-07-14.md`. Issue #932.
...
Workarounds today: inline logic into `main()`, keep modules self-contained (no stdlib `use` deps), or run under `lean_single`. Recommended fix order: **D3 → D1 → D2 → D4**. Note D1–D2 do **not** reproduce single-file — the same `f64 as i64` and `&buf`→builtin are correct in a non-importing `main()`.
