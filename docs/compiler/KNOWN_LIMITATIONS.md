...
**RESIDUAL COMPILER BUG — transitive cross-module call drop (OPEN).** The underlying defect above is NOT fixed. Isolation (all Slurm, actual rc; repros in `docs/handoff/repros/a13_crossmod_*.sio`): a generic/cross-module call from a **non-first** user function is dropped (put `main` second → its `cd_basis_exact` call vanishes → SIGSEGV), while the identical call from the **first** function works. Only *different-module* targets drop; same-module resolve. This is adjacent to the A8 fix (`ir_module_finalize_merged_calls`, which covered only the same-module `var = <inner struct-returning call>` finalize scramble) — likely an fn_id-resolution hole for transitively-imported bodies in the `module_frontend.sio` merge. A real fix belongs there; any program calling a transitively-imported function from a non-entry function will silently miscompile until then.
