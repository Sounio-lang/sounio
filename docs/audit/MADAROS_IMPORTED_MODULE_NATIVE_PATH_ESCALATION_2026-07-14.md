...
also reproduces single-file, so it is out of scope here -- **fixed 2026-08-28**,
issue #2244: 3-argument call sites are now retagged onto their own builtin so the
range copy is emitted, and the 2-argument suffix form keeps its own stub). They are not four
