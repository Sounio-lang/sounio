...
|---|---|
| **Madaros** (default `bin/souc`) | Same-format source arithmetic typechecks (V0-E.2). Casts/implicit still refused. V0-E.5 runs IEEE `F128Bits` add/sub (anti-f64). V0-E.5.1–V0-E.5.8 Madaros-run language `f128` `+` `−` `*` `/` unary `-` and comparisons via softfloat limbs, f128 params/returns across user fns (V0-E.5.4), f128 struct fields/assignment (V0-E.5.5), `[f128; N]` fixed arrays — elements, stores, `&[f128; N]` params, array copies (V0-E.5.7), and exact textual output via stdlib `print_f128` / `f128_to_string` (36 significant digits, round-half-even) and `f128_to_hex_string` (V0-E.5.8, `stdlib/math/softfloat_f128_fmt.sio`, no f64 on the path); f256 and builtin `println(x: f128)` remain deferred / fail-closed. |
