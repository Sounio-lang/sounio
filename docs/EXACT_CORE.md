...
|---|---|
| **Madaros** (default `bin/souc`) | Same-format source arithmetic typechecks (V0-E.2). Casts/implicit still refused. V0-E.5 runs IEEE `F128Bits` add/sub (anti-f64). V0-E.5.1/V0-E.5.2 Madaros-run language `f128` `+` `−` `*` unary `-` and comparisons via softfloat limbs; `/`, f256, params ABI, and builtin `print_f128` remain deferred. |
