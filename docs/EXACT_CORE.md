...
|---|---|
| **Madaros** (default `bin/souc`) | Same-format source arithmetic typechecks (V0-E.2). Casts/implicit still refused. V0-E.5 runs IEEE `F128Bits` add/sub (anti-f64). V0-E.5.1–V0-E.5.5 Madaros-run language `f128` `+` `−` `*` `/` unary `-` and comparisons via softfloat limbs, f128 params/returns across user fns (V0-E.5.4), and f128 struct fields/assignment (V0-E.5.5); f256 and builtin `print_f128` remain deferred. |
