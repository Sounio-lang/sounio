...
|---|---|
| **Madaros** (default `bin/souc`) | Same-format source arithmetic typechecks (V0-E.2). Casts/implicit still refused. V0-E.5 runs IEEE `F128Bits` add/sub (anti-f64). V0-E.5.1 Madaros-runs language `f128` `+`/`−` via softfloat limbs; mul/div, f256, params ABI, and builtin `print_f128` remain deferred. |
