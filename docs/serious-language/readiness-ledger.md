...
| qd128_core / qd_mul native-v2 | `validated research` | `scripts/ci/madaros_qd128_core_native_v2_gate.sh`; `scripts/ci/madaros_qd128_mul_native_v2_gate.sh`; #1652 | "Madaros executes qd128_core constructors and full qd_mul after the [f64;9] nine-sum pack. Combined sedenion+eisa zero-provenance is Madaros-green after KL-12." |
| Combined zero-provenance (sedenion+eisa) | `validated research` | `scripts/ci/madaros_zero_provenance_failclosed_gate.sh`; `tests/run-pass/zero_provenance_native_v2_combined.sio`; `docs/handoff/BLK-20260805-p0b-zero-provenance.md` | "Madaros executes combined sedenion+eisa::core_v2 zero-provenance (ZERO_PROVENANCE PASS). Closed by KL-12 bool-field layout honesty." |
| Compact zero-provenance (sedenion+local) | `validated research` | `scripts/ci/madaros_zero_provenance_native_v2_gate.sh`; `tests/run-pass/zero_provenance_native_v2_smoke.sio` | "Madaros executes a compact zero-provenance smoke (sedenion annihilated zero + local f64 kinds). Combined eisa import is a separate, now-green claim." |
