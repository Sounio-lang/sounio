...
| Native-v2 | `validated research` | `scripts/ci/native_v2_epistemic_science_spine_gate.sh`; `docs/guide/MINIMUM_VIABLE_SOUNIO.md` | "native-v2 is a preview research lane for scalar-core and science-spine contracts; do not claim full zero-event or qd128 native closure." |
| Sedenion native-v2 import smoke | `validated research` | `scripts/ci/madaros_sedenion_native_v2_gate.sh`; `tests/run-pass/sedenion_import_native_v2_smoke.sio`; `docs/compiler/KNOWN_LIMITATIONS.md` zero-event section | "Madaros native-v2 executes the sedenion zero-divisor import smoke after array-ref CD helpers. qd128, combined eisa provenance, and zero_event constructors remain fail-closed." |
| Fixed-array OLS under Madaros | `validated research` | `scripts/ci/madaros_ols_fixed_e2e_gate.sh`; `stdlib/stats/ols_fixed.sio` | "Fixed-array OLS (`stats::ols_fixed` + cooks + shapiro) is green under Madaros. Do not claim `stats::validation` slice methods; E019 residual remains." |
| `uncertain_eq` Madaros trust | `validated research` | `scripts/ci/madaros_uncertain_eq_trust_gate.sh`; `scripts/epistemic_trust_gate.sh`; `docs/audit/EPISTEMIC_TRUST_MAP_2026-07-14.md` | "`uncertain_eq` is TRUSTWORTHY under multi-module Madaros native import via the named trust gate. Historical 'blocked' notes that omit the IO-driver E035 fix are stale." |
