...
| v2 bridge + gate v2 lanes | W4 in v2-arch §7 | Landed after `5cf18e8af`; reconciled and pushed at `059062c2c` with 33 lanes green | `docs/handoff/eisa_w4_v2_bridge_continuation.md` |
...
### 5.4 v2 cross-reference — what v2 supersedes
...
- **Rump showcase:** moved from planned V1e to v2 (`W-H` on EVM; bridge v2 + gate
  v2 lanes landed as **W4** and were reconciled/pushed at `059062c2c`).
- **Mandatory dd64-failure lane** alongside qd128 success (decisions §15; planned
  as `v2-rump-dd` in v2-arch §6; the handoff records `v1-rump-dd` as the shipped
  paired failure witness in the 33-lane gate).
...
Imported into Madaros v2 from pushed ref `059062c2c`; older bridge-highreg-only
counts are superseded by the W4/W5 handoff where noted above.*
