...
- **Contribution 3 — Bootstrap convergence as empirical validation.** We validate the system by applying it to its own implementation. Sounio's compiler (`lean_single.sio`) is self-hosted; across seven generations of self-application, compile-time epistemic confidence rose from 26% to 97%. Every expression the compiler cannot statically discharge emits a one-byte guard marker (`66 90` NOP) in the output ELF; the percentage of guarded sites is a quality metric for the compiler's own type inference. This "epistemic gradual compilation" is a novel mechanism: the compiler *measures* its own epistemic state.
...
Across seven generations of self-application we observed the following trajectory:
...
| gen6 | 97% | Fixed point; further generations do not improve |

The limit at 97% is informative: the remaining 3% are sites where confidence degradation under composition (§8) prevents a static guarantee. This is a concrete measurement of the deep open problem.
...
**(i) Self-hosted compiler.** As described in §6, Sounio's compiler self-compiles at 97% compile-time epistemic confidence. The bit-identical fixed point gen2 == gen3 is established by MD5 hash (`7b91e249`, 230 KB ELF). The compiler applies its own epistemic type discipline: `Knowledge<Token>`, `Knowledge<Type>`, `Knowledge<AST>` flow through the pipeline, with confidence degrading at each stage that adds inference uncertainty.
...
**From 97% to 99%.** The path from current compiler-confidence 97% to 99% requires resolving three of the remaining guarded-site classes: (a) composition-correlation sites (the deep problem above), (b) SMT timeouts on large refinement contexts (addressable by CVC5 with portfolio tactics), (c) non-polynomial body Jacobians (requiring user-supplied Lipschitz constants). None are fundamentally blocked; each is engineering.
