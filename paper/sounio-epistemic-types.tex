...
Future native code generation will enable \textbf{SIMD vectorization} for
parallel value and variance computation.
...
The current v1.0 release provides full epistemic type support in the
\textbf{interpreter}. Native code generation for \texttt{Knowledge<T>}
is planned for v1.1 and will enable the SIMD optimizations described below.
...
The planned native representation of \texttt{Knowledge<f64>} will occupy
40 bytes, stack-allocated:
\begin{center}
\begin{tabular}{lr}
\toprule
Field & Size \\
...
The native backend will compute value and variance in parallel using SIMD
(AVX2 on x86-64), enabling 2-wide parallelism for epistemic arithmetic.
