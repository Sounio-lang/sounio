...
% Title
\title{Sounio: A Systems Programming Language for Epistemic Computing with Hardware-Aware Uncertainty Propagation}
...
in Measurement (GUM, JCGM 100:2008). We introduce \textbf{Sounio}, a
systems programming language for \emph{epistemic computing} in which
uncertainty, confidence, and provenance are explicit in the type system.
The \texttt{Knowledge<T>} type encapsulates a value, a GUM variance term,
confidence metadata, and a provenance chain for traceability. An algebraic
effect system tracks epistemic operations and emits diagnostics when
uncertainty may be discarded. Beyond the software type system, Sounio
introduces \textbf{K-AXI}, a hardware bus extension that carries epistemic
metadata alongside data in AXI4-Stream transactions, enabling
uncertainty-aware computation in FPGA and GPU accelerator fabrics.
Our contributions are: (1)~a type-and-effect discipline for epistemic
values; (2)~a Scientific IR that represents uncertainty propagation as
compiler-visible nodes; (3)~provenance tracking with Merkle-hashed audit
chains; (4)~K-AXI, a hardware bus protocol for streaming epistemic metadata
through accelerator pipelines; and (5)~numerical cross-validation against
analytical pharmacokinetic benchmarks, showing first-order convergence of
Euler integration under step-size refinement. The formal and numerical
results reported here are for first-order GUM propagation with uncorrelated
inputs. Sounio is open source under the Apache License~2.0.
\end{abstract}
...
\begin{itemize}
  \item A type system for epistemic values with compile-time effect
        tracking and explicit uncertainty-loss diagnostics
        (Section~\ref{sec:type-system})
  \item GUM-compliant uncertainty propagation integrated into the
        compiler's intermediate representation
        (Section~\ref{sec:propagation})
  \item K-AXI, a hardware bus extension that streams epistemic metadata
        (variance, confidence, provenance) through accelerator fabrics,
        with a hardware epistemic power accumulator for live scoring
  ...
  \item Case studies in pharmacokinetics, climate science, and
...
\begin{table}[!t]
\caption{Representative uncertainty systems and static guarantees.}
\label{tab:approaches}
\centering
\footnotesize
\begin{tabular}{llllll}
\toprule
Tool & Lang. & Static & GUM & Prov. & Scope \\
\midrule
uncertainties & Python & No & Yes & No & AD wrappers \\
Measurements.jl & Julia & No & Yes & No & Dual-number UQ \\
IntervalArithmetic.jl & Julia & No & Interval & No & Verified bounds \\
UQTK & C++ & No & PCE/MC & No & UQ toolkit \\
\textbf{Sounio} & \textbf{Sounio} & \textbf{Yes} & \textbf{Yes} & \textbf{Yes} & \textbf{Language} \\
\bottomrule
\end{tabular}
\end{table}
...
We formalize the epistemic type system to ensure uncertainty is tracked
and preserved. The typing judgment
$\Gamma \vdash e : \texttt{Knowledge}\langle T \rangle$ asserts that
expression $e$ computes an epistemic value.
...
\begin{definition}[No Implicit Epistemic Erasure]
\[
\texttt{Knowledge}\langle T \rangle \nleq T
\]
Extraction is represented as an explicit typing rule:
\[
\frac{\Gamma \vdash e : \texttt{Knowledge}\langle T \rangle}
     {\Gamma \vdash \texttt{extract}(e) : T \dashv \texttt{warn(uncertainty\_loss)}}
\]
\end{definition}
...
is either a numeric constant $n$ or a knowledge tuple
$(v, \sigma^2, \beta, \pi)$ where $v$ is the point estimate,
$\sigma^2$ is variance, $\beta$ is BetaConfidence, and $\pi$ is
provenance.

\begin{definition}[Epistemic Evaluation]
The evaluation rules for arithmetic on knowledge values:
\begin{align}
  \text{E-ADD:} \; & (v_1, \sigma_1^2, \beta_1, \pi_1) + (v_2, \sigma_2^2, \beta_2, \pi_2) \nonumber \\
    & \to (v_1{+}v_2, \sigma_1^2{+}\sigma_2^2, \text{comb}(\beta_1,\beta_2), \text{merge}(\pi_1,\pi_2)) \\[0.3em]
  \text{E-MUL:} \; & (v_1, \sigma_1^2, \beta_1, \pi_1) \times (v_2, \sigma_2^2, \beta_2, \pi_2) \nonumber \\
    & \to (v_1 v_2, v_2^2\sigma_1^2 {+} v_1^2\sigma_2^2, \ldots) \\[0.3em]
  \text{E-DIV:} \; & (v_1, \sigma_1^2, \beta_1, \pi_1) / (v_2, \sigma_2^2, \beta_2, \pi_2) \nonumber \\
    & \to (v_1/v_2, \sigma_1^2/v_2^2 {+} v_1^2\sigma_2^2/v_2^4, \ldots)
\end{align}
\end{definition}

\begin{theorem}[Type Safety for the Core Epistemic Calculus]
If $\emptyset \vdash e : T$, then either $e$ diverges or reduces to a
value $v : T$ under the evaluation rules of Definition~3. If
$T = \texttt{Knowledge}\langle S \rangle$, then $v$ preserves all
...
acknowledgment. A mechanized artifact covering progress/preservation lemmas
for the core calculus is provided in \texttt{formal/lean4}.
\end{proof}
...
\begin{table}[!t]
\caption{Built-in first-order uncertainty propagation for common functions.}
...
\midrule
\texttt{sqrt} & $\sqrt{x}$ & $u^2(x)/4x$ \\
\texttt{exp} & $e^x$ & $e^{2x} u^2(x)$ \\
\texttt{ln} & $\ln x$ & $u^2(x)/x^2$ \\
\texttt{log10} & $\log_{10}(x)$ & $u^2(x)/(x^2\ln^2 10)$ \\
\texttt{sin} & $\sin x$ & $\cos^2(x)\, u^2(x)$ \\
\texttt{cos} & $\cos x$ & $\sin^2(x)\, u^2(x)$ \\
\texttt{tan} & $\tan x$ & $\sec^4(x)\, u^2(x)$ \\
\texttt{asin} & $\arcsin x$ & $u^2(x)/(1-x^2)$ \\
\texttt{acos} & $\arccos x$ & $u^2(x)/(1-x^2)$ \\
\texttt{atan} & $\arctan x$ & $u^2(x)/(1+x^2)^2$ \\
...
All entries in Table~\ref{tab:transcendental} are delta-method formulas
for differentiable domains; handling of singularities and branch cuts is
implemented with runtime domain checks.
...
A self-hosted compiler written in Sounio implements a pipeline from lexer
through native x86-64 ELF generation via a three-stage
...
verifies stage equivalence. A reproducible line-count snapshot is generated
by \texttt{paper/reproduce.sh}; for the submission snapshot we report
89{,}771 LOC in \texttt{self-hosted/}, 265{,}288 LOC in
\texttt{stdlib/}, and 646{,}316 Rust LOC in \texttt{crates/} (including
tests and experimental modules).
...
\begin{equation}
  \mathcal{E}_{\text{hw}} = \frac{F}{8} + \frac{P}{16} + \frac{Q}{32}
  \label{eq:epistemic-power}
\end{equation}

\noindent where $F$ is the accumulated fidelity (GUM-compliant transaction
count), $P$ is the provenance depth, and $Q$ combines quantum measurement
fidelity and quantum-classical fusion metrics. The weighting reflects
the relative contribution of each epistemic channel: runtime fidelity
dominates ($-3$\,dB), provenance provides secondary assurance
($-4.8$\,dB), and quantum calibration contributes tertiary refinement
($-5.9$\,dB). Division is implemented as right-shifts, avoiding
floating-point logic entirely.

\subsection{GPU Integration}

On the GPU side, K-AXI is realized through a 64-entry atomic ring buffer
in device memory. The compiler emits calls to a publish intrinsic:

\begin{lstlisting}[caption={GPU-side K-AXI publish (CUDA intrinsic)}]
...
Five testbenches validate the hardware implementation:
(1)~master-slave handshake with three operation profiles (ADD, FMA, Merkle);
(2)~bidirectional FIFO overflow and drain;
(3)~epistemic power accumulation with clear-and-resume;
(4)~delta-method MAC with signed operands; and
(5)~quantum-classical controller integration with the accumulator.
...
Sounio timings are from the tree-walking interpreter (v0.99.0);
JIT and native backends would reduce these further.
Table~\ref{tab:uncertainty-benchmarks} reports median wall-clock times
over 5--10 runs after warmup. Platform: Xeon Gold 6148 @ 2.40\,GHz.
...
confidence metadata in $O(1)$ per operation regardless of chain length,
...
scientific domains. The pharmacokinetics example is validated by the ODE
cross-validation tests described above; the climate and neuroimaging
examples demonstrate language expressiveness.

\subsection{Pharmacokinetics}

The Cockcroft--Gault equation~\cite{cockcroft1976prediction} for
...
automatically propagates these:
...
CMIP6 climate projections~\cite{ipcc2021ar6} involve multiple sources of
uncertainty. Sounio's epistemic types enable rigorous ensemble analysis:

\begin{lstlisting}[caption={Climate ensemble with structured uncertainty}]
fn sea_level_rise_projection(
    ...
    var within = 0.0
    for m in models {
        within = within + m.variance
    }
    within = within / dof
    let mean_val = Knowledge::mean_value(models)
    var between = 0.0
    for m in models {
        let d = m.value - mean_val
        between = between + d * d
    }
    between = between / dof
    Knowledge::new(
        ...
        confidence: BetaConfidence
            ::from_ensemble_size(models.len()),
        provenance: Provenance::cmip6(scenario)
    )
}
\end{lstlisting}

This structures uncertainty compatibly with IPCC conventions for
reporting ``likely'' (66\%) and ``very likely'' (90\%) ranges.

\subsection{Neuroimaging}

fMRI statistical analysis requires careful propagation of scanner noise
through preprocessing pipelines~\cite{eklund2016cluster}. Sounio tracks
uncertainty through motion correction, smoothing, and GLM estimation:

\begin{lstlisting}[caption={fMRI preprocessing with uncertainty}]
fn preprocess_fmri(
    raw: &[Knowledge<VoxelIntensity>],
    motion_params: &MotionEstimate
) -> Vec<Knowledge<VoxelIntensity>>
   with IO {
    let corrected = motion_correct(
        raw, motion_params)
    gaussian_smooth(corrected, fwhm: 6.0)
}
\end{lstlisting}

Scanner measurement uncertainty is tracked through to final statistical
maps, with provenance metadata supporting reproducibility.
...
We formalize a core epistemic calculus and establish three key properties:

\begin{enumerate}
  \item \textbf{Progress:} If $\emptyset \vdash e : \tau$, then $e$ is
        a value or $e \longrightarrow e'$.
  \item \textbf{Preservation:} If $\Gamma \vdash e : \tau$ and
        $e \longrightarrow e'$, then $\Gamma \vdash e' : \tau$.
  \item \textbf{First-Order GUM Soundness:} For any well-typed expression
        $e : \texttt{Knowledge}\langle T, \epsilon \rangle$ that reduces
        to value $(v, u)$, the uncertainty $u$ equals the GUM combined
        standard uncertainty $u_c$ (Eq.~\ref{eq:gum-propagation}) for
        uncorrelated inputs.
\end{enumerate}

The Lean 4 mechanization for core typing and operational lemmas is
available in \texttt{formal/lean4/}. We also use an epistemic degradation
model for ODE integration:
\[
  \epsilon(t + \Delta t) = \epsilon(t) \cdot (1 + \alpha \cdot \Delta t)
\]
with $\alpha$ depending on the solver method. The cross-validation
results in Section~\ref{sec:evaluation} validate this model empirically.
Covariance-preserving metatheory is not yet mechanized.
...
DOI: \href{https://doi.org/10.5281/zenodo.18726647}{10.5281/zenodo.18726647}.
