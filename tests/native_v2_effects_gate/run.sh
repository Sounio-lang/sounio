...
#
# Each witness is a cardinal pair: every effectful-but-undeclared case is paired with
# an ACCEPT that differs ONLY in the effect clause (`with IO` / `with Div` / `with Alloc`).
#
...
#
# Flipping effects_enforcement_mode() to 2 (error) promotes every ACCEPT_WARN row into a
# hard error[E035] REJECT; update EXPECTED.txt accordingly to re-verify under error mode.
#
...
while read -r relpath verdict phase _rest; do
    ...
    out="$("$CHECK_BIN" --check "$witness" 2>&1)"
