...
#
# THE BUG: float arithmetic between TWO memory-loaded operands (two f64 params, two
...
#
# KNOWN RESIDUAL (DEFER): f32 arithmetic is still not covered by this gate in the current
# compiler line; explicit f32 casts compile but do not yet produce the expected f64-style
# result. Struct FIELD float arithmetic (p.x + p.y) is also not fixed here — the
