...
use sounio_compiler::hlir::HlirModule;
use sounio_compiler::mir::{
    analysis::ssa_validator::SSAValidator,
    builder::{FunctionBuilder, ModuleBuilder},
    optimization::*,
    BlockId, MirConstant, MirType, ValueId,
};
...
mod constant_propagation_tests {
    ...
    fn test_constant_propagation_simple() {
        ...
        let z = func.build_add(x, y, MirType::I64);
    ...
    fn test_constant_propagation_with_loops() {
        ...
        let x_plus_one = func.build_add(x, x, MirType::I64);
    ...
    fn test_constant_propagation_eliminates_dead_code() {
        ...
        let y = func.build_add(x, func.build_i64(1), MirType::I64);
...
mod dead_code_elimination_tests {
    ...
    fn test_unreachable_block_elimination() {
        ...
        let unreachable_block = func.create_block("unreachable".to_string());
...
mod common_subexpression_tests {
    ...
    fn test_cse_basic() {
        ...
        let sum1 = func.build_add(x, y, MirType::I64);
        let sum2 = func.build_add(x, y, MirType::I64);
        let result = func.build_add(sum1, sum2, MirType::I64);
        func.build_return(Some(result));
    ...
    fn test_cse_with_memory_accesses() {
        ...
        let ptr = func.build_alloca(MirType::I64);
        let load1 = func.build_load(ptr, MirType::I64);
        let load2 = func.build_load(ptr, MirType::I64);
        let result = func.build_add(load1, load2, MirType::I64);
        func.build_return(Some(result));
...
mod strength_reduction_tests {
    ...
    fn test_strength_reduction_with_constants() {
        ...
        let result = func.build_div(value, divisor, MirType::I64);
        func.build_return(Some(result));
...
mod function_inlining_tests {
    ...
    fn test_simple_inlining_candidate() {
        // Test detection of inlining candidates
...
mod ssa_validation_tests {
    ...
    fn test_ssa_validator_detects_issues() {
        // Test that SSA validator can detect SSA violations
        ...
        // Create a function that violates SSA (same value defined twice)
        let val1 = func.build_i64(10);
        let val2 = func.build_i64(20); // This would be a second definition of the same value
        ...
        // The validator should detect potential issues
...
mod integration_tests {
    ...
    fn test_full_optimization_pipeline() {
        ...
        let x = func.build_i64(10);
        let y = func.build_i64(20);
        let sum = func.build_add(x, y, MirType::I64);
        let unused = func.build_i64(100); // Dead code
        let result = func.build_add(sum, sum, MirType::I64);
        func.build_return(Some(result));
    ...
    fn test_optimization_improves_performance() {
        ...
        let y = func.build_i64(20);
        let sum1 = func.build_add(x, y, MirType::I64);
        let sum2 = func.build_add(x, y, MirType::I64); // Common subexpression
        let result = func.build_add(sum1, sum2, MirType::I64);
        func.build_return(Some(result));
...
mod performance_tests {
    ...
    fn test_constant_propagation_performance() {
        ...
        let result = func.build_add(constants[0], constants[1], MirType::I64);
        func.build_return(Some(result));
