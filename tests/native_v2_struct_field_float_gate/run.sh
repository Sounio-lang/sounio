...
#
# KNOWN RESIDUAL (DEFER, scoped out by design): f32 struct-field arithmetic is not covered
# by this port because current Madaros rejects implicit f64 literals in f32 struct fields,
# and explicit f32 paths remain a separate lowering/backend issue. Also scoped out:
