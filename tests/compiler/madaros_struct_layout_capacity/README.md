...
Madaros ELF through `MADAROS_RAW_BIN`. The gate invokes that ELF with
`--native-v2-compile` directly, so wrapper artifact routing cannot stand in
for a runtime result; this focused gate does not make a separate
native-dispatch fallback claim.
