...
| `genshadow`  | the same shape as `skip`, but the parameter is spelled like the **generated** name (`helper__m1`) rather than the original -- pre-fix this was renamed anyway and crashed during lowering instead of being refused; `a=101 b=20` |
...
| `restricted` | two `pub(crate)` fns of one name: both are exported, so neither can be renamed -- **refused**, with the "it is exported" reason, not a shadowing one |
| `restrictedmix` | a `pub(crate)` fn met first and a private one of the same name: the private one is renamed away from it, the exported one is then unique, and it compiles |
