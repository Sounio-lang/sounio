...
Wide ints (i128, Zig-style N-limb of u64) — 18..24:
  ...
  Supported: inline-CAST form (X as i128) for + - * / %, limb-aligned >> (multiple of
  64), and unsigned compare ==/!=/</<=/>/>= between two equal-width wide operands.
  Division/remainder require a SINGLE-LIMB divisor (a `X as i128` cast, high limbs 0).
  HONEST REJECTS (no ELF, never a silent wrong answer — via IrWideReject): a multi-limb
  divisor, a non-literal or non-limb-aligned shift, a mixed-width compare, and any other
  wide op (shl, bitwise). These reach the lowerer's reject sentinel, which has no codegen
  arm, so the backend returns false and emits no binary.
  NOT yet: i128 vars (need lowerer ident-type tracking) [Increment C]; non-aligned funnel
  shift [Increment B]; full N/N division (Knuth Algorithm D) + multi-limb mod [Increment D].
